...
namespace DTO
{
        ...
        private static readonly Dictionary<string, string> _map = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        public static string FormattedSize(long? args)
        {
            var sb = new StringBuilder();
            if (args != null)
            {
                if (args > 1000000000)
                    sb.AppendFormat("{0:N1} Gb ", args / 1000000000);
                else if (args > 1000000)
                    sb.AppendFormat("{0:N1} Mb ", args / 1000000);
                else if (args > 1000)
                    sb.AppendFormat("{0:N0} Kb ", args / 1000);
                else
                    sb.AppendFormat("{0:N0} Bytes ", args);
            }
            return sb.ToString();
        }
