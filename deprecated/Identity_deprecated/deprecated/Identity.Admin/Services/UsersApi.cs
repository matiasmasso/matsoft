...
public class UsersApi : _BaseApiClient
{
    ...
    public Task<List<string>?> UpdateUserAsync(UserDto args)
    {
        return CallAsync(() =>
            _http.PostAsJsonAsync($"users", args)
        );
    }
    ...
    // DELETE /users/{id}
    // ------------------------------------------------------------
    public Task<List<string>?> DeleteUserAsync(UserDto args)
    {
        return CallAsync(() =>
            _http.DeleteAsync($"users/{args.Id}")
        );
    }
