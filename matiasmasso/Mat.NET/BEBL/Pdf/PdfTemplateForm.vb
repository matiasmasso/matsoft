Public Class PdfTemplateForm
    ...
    Protected Sub PrintDoc(ByVal oCurrentDoc As DTODoc, Optional iCopias As Integer = 0)
        ...
        mBackBrush = New Pen(mCurrentDoc.BackColor).Brush
    ...
    Private Sub DrawRowEnd(ByVal oQuota As DTOTaxBaseQuota)
        Select Case oQuota.Tax.Codi
            Case DTOTax.Codis.Iva_Standard
                DrawRowEnd(EndRows.IVAstd, oQuota.Tax.Tipus, oQuota.Base.Eur)
                If mCurrentDoc.RecarrecEquivalencia Then
                    ...
                    DrawRowEnd(EndRows.REQstd, oReq.Tipus, oQuota.Base.Eur)
                End If
            Case DTOTax.Codis.Iva_Reduit
                DrawRowEnd(EndRows.IVAred, oQuota.Tax.Tipus, oQuota.Base.Eur)
                If mCurrentDoc.RecarrecEquivalencia Then
                    Dim oReq As DTOTax = DTOTax.Closest(DTOTax.Codis.Recarrec_Equivalencia_Reduit, oQuota.Tax.Fch)
                    DrawRowEnd(EndRows.REQred, oReq.Tipus, oQuota.Base.Eur)
                End If
            Case DTOTax.Codis.Iva_SuperReduit
                DrawRowEnd(EndRows.IVAsuperRed, oQuota.Tax.Tipus, oQuota.Base.Eur)
                If mCurrentDoc.RecarrecEquivalencia Then
                    Dim oReq As DTOTax = DTOTax.Closest(DTOTax.Codis.Recarrec_Equivalencia_SuperReduit, oQuota.Tax.Fch)
                    DrawRowEnd(EndRows.REQsuperRed, oReq.Tipus, oQuota.Base.Eur)
                End If
        End Select
    End Sub
        ...
        Select Case oEndRow
            ...
            Case EndRows.Total
                If mCurrentDoc.Valorat Then
                    Select Case mCurrentDoc.Incoterm
                        Case DTOProveidor.Incoterms.CIF
                            sText = "CIF"
                        Case DTOProveidor.Incoterms.EXW
                            sText = "ex-works"
                        Case DTOProveidor.Incoterms.FOB
                            sText = "FOB"
                        Case Else
                            sText = "Total"
                    End Select
    ...
    Shared Function PlataformaPage(ByVal oDeliveries As List(Of DTODelivery)) As DTODoc
        Dim oFirstAlb As DTODelivery = oDeliveries.First
        Dim oClient As DTOCustomer = oFirstAlb.Customer
        Dim oPlataforma As DTOCustomerPlatform = oClient.DeliveryPlatform
        ...
        With retval
            ContactLoader.Load(oPlataforma)
            .Dest.Add(oPlataforma.Nom)
