...
@Code
    ...
    ViewBag.Title = ContextHelper.Tradueix("Inscripción en sorteo", "Inscripció en sorteig", "Raffle subscription", "Inscrição no sorteio") & " | MATIAS MASSO, S.A."
...
<fieldset id="Raffle_Duplicated">
    <legend>@ContextHelper.Tradueix("Inscripción en sorteo", "Inscripció en sorteig", "Raffle subscription", "Inscrição no sorteio")</legend>
    <div>
        <p>
            @ContextHelper.Tradueix("Confirmamos que ya estas participando en este sorteo desde el " & Format(Model.Fch, "dd/MM/yy") & " a las " & Format(Model.Fch, "HH:mm") & " con el número " & Model.Num, "Confirmem que ja estas participant en aquest sorteig desde el " & Format(Model.Fch, "dd/MM/yy") & " a les " & Format(Model.Fch, "HH:mm") & " amb el número " & Model.Num, "We are glad to confirm you are already participating on this raffle since " & Format(Model.Fch, "dd/MM/yy") & " at " & Format(Model.Fch, "HH:mm") & " with ticket number " & Model.Num)
        </p>
        <p>
            @ContextHelper.Tradueix("Una vez celebrado el sorteo, publicaremos el ganador en", "Un cop celebrat el sorteig, publicarem el guanyador a", "Once the raffle is over, we will publish the winner at", "Uma vez se celebre o sorteio, publicaremos o nome do ganhador em")
            &nbsp;<a href='@FEB.Raffles.Url(ContextHelper.lang())'>@FEB.Raffles.Url(ContextHelper.lang(), True)</a>
        </p>
        <p>
            @ContextHelper.Tradueix("Si te toca el premio, tienes 30 días para ponerte en contacto con nosotros en ", "Si et toca el premi, tens 30 dies per posar-te en contacte amb nosaltres a ", "If your name becomes published, you have 30 days to confirm your acceptance at ", "Se te toca o prémio, tens 30 dias para pôr-te em contacto connosco em ")
            <a href="mailto:marketing@matiasmasso.es">marketing@matiasmasso.es</a>
        </p>
        <p class="FbWarn">
                <span>
                    @Html.Raw(ContextHelper.Tradueix("Recuerda que este es un sorteo exclusivo para seguidores de nuestros perfiles en Facebook y/o Instagram. Si aun no lo eres puedes seguirnos haciendo clic en los botones <b>Me gusta</b> (Facebook) o <b>Seguir</b> (Instagram)  en las siguientes páginas:",
        ...
        <p>
            @If ContextHelper.Lang.IsPor() Then
            ...
            End If
            <br /><a href="@FEB.Raffle.FacebookPage(Model.Raffle, ContextHelper.Lang())" target="_blank">@FEB.Raffle.FacebookPageLabel(Model.Raffle, ContextHelper.Lang())</a>
        ...
        <p>
            @ContextHelper.Tradueix("Gracias por participar", "Gracies per participar", "Thanks for participating", "Obrigada por participar.")
        </p>
    </div>
