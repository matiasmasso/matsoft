@Code
    ...
    Dim oRep = FEB.User.GetRepSync(oUser, exs)
    Dim oProductDistributors As List(Of DTOProductDistributor) = FEB.ProductDistributors.FromRepSync(exs, oRep)
    ...
    If Model.contact IsNot Nothing Then
        ...
        oCountry = New DTOArea(DTOAddress.Country(oCustomer.address).Guid, DTOAddress.Country(oCustomer.address).LangNom.Tradueix(ContextHelper.lang()))
...
<div id="CustomerSelection">
    <div>
        <select>
            <option value="">@ContextHelper.Tradueix("(seleccionar pais)", "(sel.leccionar pais)", "(country selection)")</option>
    ...
    <div>
        <select>
            <option value="">@ContextHelper.Tradueix("(seleccionar zona)", "(sel.leccionar zona)", "(area selection)")</option>
    ...
    <div>
        <select>
            <option value="">@ContextHelper.Tradueix("(seleccionar población)", "(sel.leccionar població)", "(location selection)")</option>
            @If oCustomer IsNot Nothing Then
    ...
    <div>
        <select>
            <option value="">@ContextHelper.Tradueix("(seleccionar cliente)", "(sel.leccionar client)", "(customer selection)")</option>
            @If oCustomer IsNot Nothing Then
...
<div Class="Fch">
    ...
    <input type = "date" value="@DTO.GlobalVariables.Today().ToString("yyyy-MM-dd")" @IIf(Model.IsNew, "", "disabled = 'disabled'") />
...
<div Class="Submit" >
    <input type = "button" id="SubmitButton" @IIf(Model.IsNew, "", "hidden = 'hidden'") value='@ContextHelper.Tradueix("Enviar", "Enviar", "Submit")' />
</div>
