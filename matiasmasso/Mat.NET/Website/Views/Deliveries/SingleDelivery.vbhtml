...
@Code
    ...
    ViewBag.Title = ContextHelper.Tradueix("Albarán de entrega", "Albará d'entrega", "Delivery note")
...
<div class="pagewrapper">
    ...
    <section class="PageTitle">
        <a href='@FEB.Delivery.PdfUrl(Model)' title='@(ContextHelper.Tradueix("Descargar fichero Pdf", "Descarregar fitxer Pdf", "Download Pdf file")) '>
            <img class="Download" src="~/Media/Img/48x48/Pdf48.png" />
        </a>

        @DTODelivery.Caption(Model, ContextHelper.lang())
    </section>

    <a href="@FEB.Contact.Url(Model.Customer)">@Model.Nom</a>
    <br />
    @Model.Address.Text
    <br />
    ...
    @If Model.Invoice Is Nothing Then
        If Model.Facturable Then
            Html.Raw(ContextHelper.lang().Tradueix("pendiente de facturar", "pendent de facturar", "invoice pending"))
        Else
            Html.Raw(ContextHelper.lang().Tradueix("no facturable", "no facturable", "not invoiceable"))
        End If
    Else
        @<a href="@FEB.Invoice.Url(Model.Invoice)">
            @(ContextHelper.lang().Tradueix("factura ", "factura ", "invoice ") & Model.Invoice.Num & ContextHelper.lang().Tradueix(" del ", " del ", " from ") & String.Format(Model.Invoice.Fch, "dd/MM/yy"))
        </a>
    End If
    ...
    <div class="Grid">
        ...
        @If Model.Items IsNot Nothing Then
            For Each item As DTODeliveryItem In Model.Items
                    ...
                    @<div class="Row Order">
                        <div class="CellEpigraf">
                            <a href="@FEB.PurchaseOrder.Url(oOrder)">
                                @oOrder.Caption(ContextHelper.lang())
                            </a>
                        </div>
                ...
                @<div class="Row Item">
                    <div class="CellTxt">
                        <a href="@item.purchaseOrderItem.sku.GetUrl(ContextHelper.Lang))" target="_blank">
