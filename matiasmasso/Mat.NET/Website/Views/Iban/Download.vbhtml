@Code
    ...
    For Each oCustomer As DTOCustomer In FEB.User.CustomersRaonsSocialsSync(exs, oUser)
    ...
    If oContacts.Count = 1 Then
        Dim oIbans = FEB.Ibans.PendingUploadsSync(exs, oUser)
...
End Code
    ...
    <div id="Iban" @IIf(oContacts.Count > 1, "hidden='hidden'", "")>
        <p>
            @ContextHelper.Tradueix("Por favor rellene los datos de la persona con poderes que va a firmar el mandato, los dígitos (IBAN) de su cuenta bancaria y pulse el botón 'verificar'",
        ...
        <div class="Query">
            @ContextHelper.Tradueix("Nombre de la persona que firma", "Nom de la persona que firma", "Signature name")
            <div c><input id="PersonNom" type="text" maxlength="50" value="@Html.Raw(sPersonNom)"/></div>
        </div>
        <div class="Query">
            @ContextHelper.Tradueix("DNI", "DNI", "Id document number")
            <div><input id="PersonDni" type="text" maxlength="12"  value="@Html.Raw(sPersonDni)"/></div>
        </div>
        <div class="Query">
            @ContextHelper.Tradueix("dígitos del IBAN", "digits del IBAN", "IBAN digits")
            <div id="IbanDigits">
        ...
        <div class="Query">
            <input type="button" value='@ContextHelper.Tradueix("verificar", "verificar", "validate")' />
        </div>
    </div>
    ...
    <div id = "Download" hidden="hidden">
        ...
        <div>
            <a href = "#" >
                @ContextHelper.Tradueix("Descargar mandato para su firma",
    ...
    <div id = "Upload" hidden="hidden">
        <p>
            @ContextHelper.Tradueix("Gracias por su descarga.",
            ...
            @ContextHelper.Tradueix("Cuando esté listo puede subirlo mediante el siguiente enlace:  ", _
                                  "", _
                                  "")
        </p>
        <p>
            <a href = "#" >
                @ContextHelper.Tradueix("Subir mandato firmado",
                                  "Pujar mandat signat",
    ...
    <div id = "Thanks" hidden="hidden">
        @ContextHelper.Tradueix("Gracias por subir el documento.", _
                                  "Gracies per pujar el document", _
                                  "Thanks for loading the document")
        <br/>
        @ContextHelper.Tradueix("El mandato ha quedado a la espera de su validación.", _
