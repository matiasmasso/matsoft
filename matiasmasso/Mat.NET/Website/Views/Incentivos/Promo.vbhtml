...
@Code
    ...
    ViewBag.Title = ContextHelper.Tradueix("incentivo", "incentiu", "Incentive")
End Code
...
<div class="pagewrapper">
    ...
    <section class="Header">
        <div class="Thumbnail">
            <img src="@FEB.Incentiu.ThumbnailUrl(Model)" />
        </div>
        <div class="Fchs">
            <div class="FchFrom">
                <div class = "Label">
    ...
    @If oUser.Rol.id = DTORol.Ids.Manufacturer Or oUser.Rol.IsStaff Or oUser.Rol.IsRep Then
        Dim oOrders = FEB.Incentiu.PurchaseOrdersSync(exs, Model, oUser)
        @<section id = "Orders" >
             @If oOrders.Count = 0 Then
                 @<div>
                    @ContextHelper.lang().Tradueix("Ningún pedido registrado aún", "Encara no s'ha registrat ninguna comanda", "No orders logged under this promo yet")
                </div>
             Else
                @<div>
                    @String.Format(ContextHelper.lang().Tradueix("Registrados {0} pedidos dentro de esta promoción:", "Registrades {0} comandes dins aquesta promoció:", "{0} orders already logged under this promotion:"), oOrders.Count)
                </div>
