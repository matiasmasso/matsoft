...
<div class="PageWrapper">
    ...
    <a class="Epigraf Baskets" href="#" >Conversiones</a>
</div>
...
@section Scripts
    ...
    <script>
        ...
        $(document).on('click', '.Baskets', function (e) {
            var url = "/wtbol/baskets/" + model.Site.Guid;
            window.location = url;
        })
        ...
        function LoadConfig() {
            $("#CustomerNom").val(model.Site.Customer.Nom);
            $("#MerchantId").val(model.Site.MerchantId);
            $("#SiteGuid").val(model.Site.Guid.toString());
            ...
            if (model.Site.LandingPages.length == 0) {
                $("#LandingPagesCount").val('Aun no se ha registrado ninguna landing page');
                $("#LandingPagesCount").addClass('Warning');
            } else {
                $("#LandingPagesCount").val(model.Site.LandingPages.length);
                $("#LandingPagesCount").removeClass('Warning');
            }
            if (model.Site.Stocks.length == 0) {
                if (model.Site.LastStkFch == null) {

                    $("#LastStkFch").val('Este comercio aun no ha publicado sus existencias');
                } else {
                    $("#LastStkFch").val('Este comercio no ha publicado sus existencias en las últimas 24 horas. (ultima vez ' + formattedFch(model.Site.LastStkFch) + ' ' + formattedTime(model.Site.LastStkFch) + ')');

                }
                $("#LastStkFch").addClass('Warning');
            } else {
                $("#LastStkFch").val(formattedFch(model.Site.LastStkFch) + ' ' + formattedTime(model.Site.LastStkFch));
                $("#LastStkFch").removeClass('Warning');
            }
        }
        ...
        $(document).on('click', '.Task', function (e) {
            ...
            function UpdateSettings() {
                ...
                button.val(spinner);
                var url = '@MmoUrl.ApiUrl("WtbolSite/model")';
                ...
                data.UsrGuid = '@ContextHelper.GetUser().Guid.ToString';
            ...
            var jqxhr = $.post(url, data, function (result) {})
                .done(function () {
                    ...
                    if (stockModel == undefined) {
                        if (stock > 0)
                            model.Site.Stocks.push({ 'ProductGuid': productGuid, 'Qty': stock });
                    } else {
                        if (stock <= 0)
                            model.Site.Stocks.splice(model.Site.Stocks.findIndex(e => e.ProductGuid === productGuid), 1);
                        else
        ...
        function LoadData() {
            var url = '@MmoUrl.ApiUrl("wtbol/model", ContextHelper.GetUser().Guid.ToString())';
...
@Section Styles
    <style scoped>
        ...
        input.Warning {
            background: url(/Media/Img/Ico/warn.gif) no-repeat 4px center;
            padding-left: 24px;
        }
