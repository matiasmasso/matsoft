...
@Code
    ...
    If FEB.SellOut.FilterValues(Model, DTOSellOut.Filter.Cods.Customer).Count > 0 Then
        HideIfCustomerFilter = "hidden='hidden'"
        CustomerGuid = FEB.SellOut.FilterValues(Model, DTOSellOut.Filter.Cods.Customer).First().Guid.ToString
    End If

    Dim iYears As List(Of Integer) = FEB.SellOut.YearsSync(exs, Model)
    Dim oProductsFilter = FEB.SellOut.FilterValues(Model, DTOSellOut.Filter.Cods.Product)
    Dim oChannelsFilter = FEB.SellOut.FilterValues(Model, DTOSellOut.Filter.Cods.Channel)
    Dim oAreasFilter = FEB.SellOut.FilterValues(Model, DTOSellOut.Filter.Cods.Atlas)


    Dim oBrands = FEB.SellOut.BrandsSync(exs, Model)
    ...
    If oProductsFilter.Count > 0 Then
        Dim oProduct = FEB.Product.FindSync(exs, oProductsFilter.First.Guid)
        If oProduct.SourceCod = DTOProduct.SourceCods.Brand Then
            oBrandFilter = oProduct
            oCategories = FEB.ProductCategories.AllSync(exs, oProduct)
        End If
    End If

    Dim oCountries As List(Of DTOCountry) = FEB.SellOut.CountriesSync(exs, Model)
    Dim oZonas As New List(Of DTOZona)
    If oCountries.Count = 1 Then
        oZonas = oCountries.First().Zonas
    End If

    Dim oChannels = FEB.SellOut.ChannelsSync(exs, Model)
End Code

<section class="PageTitle ToolStrip">
            <a id="ExcelFullDownload2" title='@(ContextHelper.Tradueix("Descargar todos los datos del ejercicio", "Descarregar totes les dades de l'exercici", "Download full year data ")) '>
                <img class="Download" src="~/Media/Img/48x48/Excel48.png" />
            </a>

            @ContextHelper.Tradueix("Pedidos de clientes", "Comandes de clients", "Sell-out data")
        </section>



        <p class="miscellanea">
            @Html.Raw(ContextHelper.Tradueix("Las cifras se refieren a unidades vendidas en el mes en que el que el cliente cursó el pedido, tanto si se le ha servido como si no.<br/>Estas cifras pueden verse afectadas por potenciales devoluciones, en cuyo caso se restan del pedido original.",
        ...
        <div class="divselectors">
            ...
            <select id="conceptType" @HideIfCustomerFilter>
                @If Model.conceptType = DTOSellOut.ConceptTypes.product Then
                    @<option value="@CInt(DTOSellOut.ConceptTypes.product)" selected='selected'>@ContextHelper.Tradueix("productos", "productes", "products", "produtos")</option>
                Else
                    @<option value="@CInt(DTOSellOut.ConceptTypes.product)">@ContextHelper.Tradueix("productos", "productes", "products", "produtos")</option>
                End If

                @If Model.conceptType = DTOSellOut.ConceptTypes.geo Then
                    @<option value="@CInt(DTOSellOut.ConceptTypes.geo)" selected='selected'>@ContextHelper.Tradueix("destinos", "destinacions", "destinations")</option>
                Else
                    @<option value="@CInt(DTOSellOut.ConceptTypes.geo)">@ContextHelper.Tradueix("destinos", "destinacions", "destinations")</option>
                End If

                @If Model.conceptType = DTOSellOut.ConceptTypes.channels Then
                    @<option value="@CInt(DTOSellOut.ConceptTypes.channels)" selected='selected'>@ContextHelper.Tradueix("canales", "canals", "channels")</option>
                Else
                    @<option value="@CInt(DTOSellOut.ConceptTypes.channels)">@ContextHelper.Tradueix("canales", "canals", "channels")</option>
                End If
            </select>


            <select id="format">
                <option value="@CInt(DTOSellOut.Formats.units)" @IIf(Model.format = DTOSellOut.Formats.units, "selected='selected'", "")> @ContextHelper.Tradueix("unidades", "unitats", "units")</option>
                <option value="@CInt(DTOSellOut.Formats.amounts)" @IIf(Model.format = DTOSellOut.Formats.amounts, "selected='selected'", "")>@ContextHelper.Tradueix("importes", "imports", "amounts")</option>
            </select>



            <select id="brand" @HideIfSingleBrand @HideIfCustomerFilter>
                <option value="">@ContextHelper.Tradueix("(todas las marcas)", "(totes les marques)", "(all brands)")</option>
                @If oBrands IsNot Nothing Then
                    ...
                    For Each oBrand As DTOProductBrand In oBrands
                        ...
                        End If
                        If match Then
                            @<option value="@oBrand.Guid.ToString" selected='selected'>@oBrand.Nom.Tradueix(ContextHelper.Lang)</option>
                        Else
                            @<option value="@oBrand.Guid.ToString">@oBrand.Nom.Tradueix(ContextHelper.Lang)</option>
                        End If
                    Next
                End If
            </select>
            ...
            <select id="category" @HideIfCustomerFilter>
                <option value="" selected="selected">@ContextHelper.Tradueix("(todas las categorias)", "(totes les categories)", "(all categories)")</option>
                ...
                @For Each oCategory As DTOProductCategory In oCategories
                    ...
                    End If
                    If match Then
                        @<option value="@oCategory.Guid.ToString" selected='selected'>@oCategory.Nom.Tradueix(ContextHelper.Lang)</option>
                    Else
                        @<option value="@oCategory.Guid.ToString">@oCategory.Nom.Tradueix(ContextHelper.Lang)</option>
                    End If
            ...
            <select id="channel" @HideIfCustomerFilter>
                <option value="" selected="selected">@ContextHelper.Tradueix("(todos los canales)", "(tots els canals)", "(all channels)")</option>
                @For Each oChannel As DTODistributionChannel In oChannels
                    ...
                    If match Then
                        @<option value="@oChannel.Guid.ToString" selected='selected'>@oChannel.langText.Tradueix(ContextHelper.lang())</option>
                    Else
                        @<option value="@oChannel.Guid.ToString">@oChannel.langText.Tradueix(ContextHelper.lang())</option>
                    End If
            ...
            <select id="country" @HideIfCustomerFilter>
                <option value="" selected="selected">@ContextHelper.Tradueix("(todos los paises)", "(tots els paisos)", "(all countries)")</option>
                @For Each oCountry As DTOCountry In oCountries
                    ...
                    If match Then
                        @<option value="@oCountry.Guid.ToString" selected='selected'>@DTOCountry.NomTraduit(oCountry, ContextHelper.lang())</option>
                    Else
            ...
            <select id="zona" @HideIfCustomerFilter>
                <option value="" selected="selected">@ContextHelper.Tradueix("(todas las zonas)", "(totes les zones)", "(all zones)")</option>
            ...
            <select id="location" @HideIfCustomerFilter>
                <option value="" selected="selected">@ContextHelper.Tradueix("(todas las poblaciones)", "(totes les poblacions)", "(all locations)")</option>
            ...
            <select id="contact" hidden="hidden" @HideIfCustomerFilter>
                <option value="" selected="selected">@ContextHelper.Tradueix("(todos los clientes)", "(tots els clients)", "(all customers)")</option>
