...
@Code
    ...
    Dim lang As DTOLang = If(ViewBag.Lang Is Nothing, ContextHelper.Lang, ViewBag.Lang)
End Code
...
<div id="Filters" hidden>
    <select id="Year">
        <option selected>@DTO.GlobalVariables.Today().Year</option>
        @For year As Integer = DTO.GlobalVariables.Today().Year - 1 To DTO.GlobalVariables.Today().Year - 10 Step -1
            @<option>@year</option>
        Next
    </select>

    <a href="#" id="DownloadExcel2" title='@lang.Tradueix("descargar todos los datos del año en Excel", "descarregar totes les dades de l'any en Excel", "download Excel full year raw data")'>
...
<input type="hidden" id="RawDataUrl" value="@MmoUrl.ApiUrl("sellout/data", ContextHelper.GetUser().Guid.ToString())" />
