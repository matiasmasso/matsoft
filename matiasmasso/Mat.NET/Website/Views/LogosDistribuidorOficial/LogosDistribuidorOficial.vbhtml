@Code
    ...
    If (oUser.Rol.Id = DTORol.Ids.CliFull Or oUser.Rol.Id = DTORol.Ids.CliLite) Then
        oCustomers = FEB.User.GetCustomersSync(oUser, exs)
    End If
    Dim oBrands As List(Of DTOProductBrand) = FEB.ProductBrands.AllSync(exs, oUser)
End Code
    ...
    @<div class="pagewrapper">
        ...
        <div class="logos">
            @For Each item As DTOProductBrand In oBrands
                    ...
                    @<a href="@FEB.ProductBrand.LogoDistribuidorOficialAnchor(item, oCustomers(0))" target="_blank">
                        <img src="@FEB.ProductBrand.LogoDistribuidorOficialUrl(item)" title="@item.Nom" />
                    </a>
            End If
        Next
        </div>
