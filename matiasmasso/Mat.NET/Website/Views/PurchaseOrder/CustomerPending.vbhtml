...
<div class="title">
        <h3>@ContextHelper.Tradueix("Pendiente de entrega", "Pendent d'entrega", "Open Orders")</h3>
        @If Model.Count > 0 And Not ShowCustomers Then
            @<a href='@FEB.PurchaseOrders.ExcelUrl(Model(0).PurchaseOrder.Contact)' title='@(ContextHelper.Tradueix("Descargar fichero Excel", "Descarregar fitxer Excel", "Download Excel file")) '>
    ...
    <div class="Grid">
            ...
            <span class="CellTxt">@ContextHelper.Tradueix("Concepto", "Concepte", "Concept")</span>
            <span class="CellQty">@ContextHelper.Tradueix("Cant", "Quant", "Qty")</span>
        ...
        @For Each item As DTOPurchaseOrderItem In Model
                ...
                If ShowCustomers Then
                    ...
                    End If
                        @<span class="CellEpigraf">
                            <a href="@FEB.Contact.Url(item.PurchaseOrder.Contact)" title='@ContextHelper.Tradueix("ver perfil de cliente", "veure perfil de client", "see customer profile")'>
            ...
            If Not item.PurchaseOrder.Guid.Equals(oOrderGuid) Then
                oOrderGuid = item.PurchaseOrder.Guid
                    ...
                    @<span Class="CellPO">
                        <a href = "@FEB.PurchaseOrder.Url(item.purchaseOrder)" title='@ContextHelper.Tradueix("ver pedido original", "veure comanda original", "see original order")'>
                            @item.purchaseOrder.caption(ContextHelper.Lang())
                        </a>
                    </span>
                    @<span class="CellNum"></span>
                    @<span class="CellIco"></span>
            End If
                ...
                @<span class="CellSku">
                    <a href="@item.sku.GetUrl(ContextHelper.Lang)" title='@ContextHelper.Tradueix("ver ficha de producto", "veure fitxa del producte", "see product information")' target="_blank">
            ...
            If item.PurchaseOrder.FchDeliveryMin > DTO.GlobalVariables.Today() Then
                    @<span class="CellWait" title='@String.Format("{0} {1:dd/MM/yy}", ContextHelper.Tradueix("programado para", "programat per", "scheduled for"), item.PurchaseOrder.FchDeliveryMin)'>&nbsp</span>
            ElseIf item.Sku.IsAvailable Then
                    @<span class="CellOk" title='@ContextHelper.Tradueix("en stock", "en stock", "in stock")'>&nbsp;</span>
            Else
                    @<span class="CellKo" title='@ContextHelper.Tradueix("agotado", "exhaurit", "out of stock")'>&nbsp;</span>
                 End If
