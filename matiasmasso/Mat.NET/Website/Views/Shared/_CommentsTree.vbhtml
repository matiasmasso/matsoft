...
<div class="CommentsWrapper">
            ...
            @If Model.Items.Count = 0 Then
                @<div>
                    <label for="CommentFormTextArea">
                        @ContextHelper.Tradueix("No hay comentarios aún.", "Encara no hi ha cap comentari.", "No comments yet.", "Não há comentários.")
                        @ContextHelper.Tradueix("¡Sé el primero en colgar uno!", "Sigues el primer a deixar-ne!", "Be the first to post one!", "Sê o primeiro, a enviar-nos o teu!")
                    </label>
                </div>
            Else
                @<div>
                    <label for="CommentFormTextArea">
                        @If Model.Items.Count = 1 Then
                            @Html.Raw(ContextHelper.Tradueix("Un comentario publicado hasta ahora.", "Un comentari publicat fins ara.", "One comment so far."))
                        Else
                            @Html.Raw(String.Format("{0:N0} {1}.", Model.RootItemsCount, ContextHelper.Tradueix("comentarios publicados hasta ahora.", "comentaris publicats fins ara.", "comments so far.")))
                        End If
                        @Html.Raw(ContextHelper.Tradueix("¡Deja el tuyo a continuación!", "Deixa el teu a continuació!", "Leave yours here!"))
                    </label>
                </div>
            End If
    ...
    @If Model.MoreItems() Then
        @<a href="#"
            ...
            @Html.Raw(ContextHelper.Tradueix("comentarios más", "comentaris més", "more comments", "comentários"))
