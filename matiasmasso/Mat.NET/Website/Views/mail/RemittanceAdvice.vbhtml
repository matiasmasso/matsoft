...
@Code
    ...
    FEB.Contact.Load(Model.Proveidor, exs)
    ...
    @For Each item As DTORemittanceAdviceItem In Model.Items
        @<tr>
            <td width="30px">
                ...
                @<a href="@FEB.DocFile.DownloadUrl(item.DocFile, True)" title='@oLang.Tradueix("descargar copia de factura", "descarregar copia de la factura", "download invoice copy")'>
...
<table style="font-family: Helvetica Neue, Helvetica, Arial, sans-serif; font-size: 1em;width:100%">
    ...
    <tr>
        <td width="100%" valign="top" style="padding:0;vertical-align:top;">
            ...
            <p>
                @Html.Raw(FEB.Iban.ToHtml(Model.Iban, True))
            </p>
        </td>
    </tr>
    ...
    <tr>
        <td align="center">
            @If Model.DocFile IsNot Nothing Then
                @<a href="@FEB.DocFile.DownloadUrl(Model.DocFile, False)" title='@oLang.Tradueix("descargar justificante bancario", "descarregar justificant bancari", "download bank receipt")'>
                    <img src="@FEB.DocFile.ThumbnailUrl(Model.DocFile)" width='150px' height='auto' alt='@oLang.Tradueix("descargar justificante bancario", "descarregar justificant bancari", "download bank receipt")' />
                </a>
            End If
        </td>
    </tr>
    ...
    @If Model.DocFile IsNot Nothing Then
        @<tr>
            <td style="background-color: #cc0000; text-align: center; vertical-align: middle; height: 20px;font-family: Helvetica Neue, Helvetica, Arial, sans-serif;font-size:14px;">
                <a href="@FEB.DocFile.DownloadUrl(Model.DocFile, False)" title='@oLang.Tradueix("descargar justificante bancario", "descarregar justificant bancari", "download bank receipt")' style="color: white;text-decoration:none;">
