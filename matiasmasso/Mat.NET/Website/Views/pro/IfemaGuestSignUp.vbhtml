...
@Code
    ...
    For Each itm As DTOIfemaGuest.Activitats In [Enum].GetValues(GetType(DTOIfemaGuest.Activitats))
        If itm = DTOIfemaGuest.Activitats.NotSet Then
            oActivitats.Add(New SelectListItem() With {.Value = 0, .Text = ContextHelper.Tradueix("(seleccionar una actividad)", "(triar una activitat)", "(pick your main activity)")})
        Else
...
@Using (Html.BeginForm()) ' "Edit", "State", FormMethod.Post,  Nothing))
    ...
    @<fieldset class="mat_form" id="mat_form">
        ...
        <ul>
            ...
            <li>
                <label for='TextboxNom'>@ContextHelper.Tradueix("Nombre", "Nom", "First Name")</label>
                @Html.TextBoxFor(Function(Model) Model.Nom)
            </li>
            <li>
                <label for='TextboxCognoms'>@ContextHelper.Tradueix("Apellidos", "Cognoms", "Surname")</label>
                @Html.TextBoxFor(Function(Model) Model.Cognoms)
            </li>
            <li>
                <label for="TextboxCargo">@ContextHelper.Tradueix("Cargo", "Carrec", "Position")</label>
                @Html.TextBoxFor(Function(Model) Model.Cargo)
            </li>
            <li>
                <label for="TextboxNIF">@ContextHelper.Tradueix("NIF", "NIF", "VAT number")</label>
                @Html.TextBoxFor(Function(Model) Model.Nif)
            </li>
            <li>
                <label for="TextboxEmpresa">@ContextHelper.Tradueix("Empresa", "Empresa", "Business name")</label>
                @Html.TextBoxFor(Function(Model) Model.Empresa)
            </li>
            <li>
                <label for="DropdownlistActividad">@ContextHelper.Tradueix("Actividad", "Activitat", "Activity")</label>
                @Html.DropDownListFor(Function(Model) Model.Actividad, oActivitats, New With {.selectedValue = Model.Actividad})
            </li>
            <li>
                <label for="TextboxDireccion">@ContextHelper.Tradueix("Dirección", "Adreça", "Address")</label>
                @Html.TextBoxFor(Function(Model) Model.Direccion)
            </li>
            <li>
                <label for="TextboxZip">@ContextHelper.Tradueix("Código postal", "Codi postal", "Zip code")</label>
                @Html.TextBoxFor(Function(Model) Model.Zip)
            </li>
            <li>
                <label for="TextboxPoblacion">@ContextHelper.Tradueix("Población", "Població", "Location")</label>
                @Html.TextBoxFor(Function(Model) Model.Poblacion)
            </li>
            <li>
                <label for="TextboxPais">@ContextHelper.Tradueix("Pais", "Pais", "Country")</label>
                @Html.DropDownListFor(Function(Model) Model.CountryCod, oCountryListItems, New With {.selectedValue = Model.CountryCod})
            </li>
            <li>
                <label for="TextboxTel">@ContextHelper.Tradueix("Teléfono", "Telefon", "Phone number")</label>
                @Html.TextBoxFor(Function(Model) Model.tel)
            </li>
            <li>
                <label for="TextboxMovil">@ContextHelper.Tradueix("Movil", "Mobil", "Cell phone")</label>
            ...
            <li>
                <label>&nbsp;</label>
                <input type="submit" id="signup" class="submit" value='@ContextHelper.Tradueix("Aceptar","Acceptar","Submit","Aceitar")' />
            </li>
        </ul>
    </fieldset>
End Using
