Public Class DoxController
    ...
    Public Async Function Index(sBase64Params As String) As Threading.Tasks.Task(Of ActionResult)
        ...
        Dim oParams As New Dictionary(Of String, String)
        If Not String.IsNullOrEmpty(sBase64Params) Then oParams = CryptoHelper.FromUrlFriendlyBase64Json(sBase64Params)
        ...
        Select Case oCod
            ...
            Case DTODocFile.Cods.StocksExcel
                Dim oExcelSheet = Await FEB.SkuStocks.Excel(exs, Website.GlobalVariables.Emp, oParams)
            ...
            Case DTODocFile.Cods.rawdatalast12monthscsv
                ...
                If oAuthResult = AuthResults.success Then
                    ...
                    If oProveidor.Equals(DTOProveidor.Wellknown(DTOProveidor.Wellknowns.Mayborn)) Then
                        Dim items = Await FEB.SellOut.RawDataLast12Months(exs, oProveidor)
            ...
            Case DTODocFile.Cods.MaybornSalesExcel
                ...
                If oAuthResult = AuthResults.success Then
                    ...
                    Dim oProveidor = Await FEB.User.GetProveidor(oUser, exs)
                    If oProveidor.Equals(DTOProveidor.Wellknown(DTOProveidor.Wellknowns.Mayborn)) Then
                        Dim oCsv As DTOCsv = Await FEB.Mayborn.SalesCsv(exs)
                        retval = MyBase.CsvResult(oCsv)
                    Else
                        retval = MyBase.UnauthorizedView
                    End If
            ...
            Case DTODocFile.Cods.IbanMandato
                If oParams.Count > 1 Then
                    ...
                    If GuidHelper.IsGuid(sGuid) Then
                        ...
                        Dim oIban = Await FEB.Iban.Find(oGuid, exs)
                        Dim oDocFile As DTODocFile = oIban.DocFile
                        If oDocFile Is Nothing Then
                            If FEB.Contact.Load(oIban.titular, exs) Then
                                Dim sSwift = FEB.Iban.Swift(oIban)
                                Dim oSepaTexts = Await FEB.SepaTexts.All(exs)
                                oDocFile = LegacyHelper.LegacyDivers.SepaMandato(exs, Website.GlobalVariables.Emp, oIban, sSwift, oSepaTexts, oIban.titular.lang)
                            End If
                        Else
                            FEB.DocFile.Load(exs, oDocFile, LoadStream:=True)
                        End If
            ...
            Case DTODocFile.Cods.zipgallerydownloads
                If oParams.Count > 1 Then
                    Dim oProduct = Await FEB.Product.Find(exs, New Guid(oParams("product")))
            ...
            Case DTODocFile.Cods.SellOutPerChannel
                If oParams.Count > 1 Then
                    ...
                    Dim oExcelBook As MatHelper.Excel.Book = Await FEB.ProductDistributors.PerChannel(exs, user, year)
                    retval = Await LoginOrFile(oExcelBook)
                End If
            Case DTODocFile.Cods.ExcelSalePoints
                If oParams.Count > 1 Then
                    Dim oProveidor = Await FEB.Proveidor.Find(New Guid(oParams("proveidor")), exs)
                    If exs.Count = 0 Then
                        Dim items As List(Of DTOProductDistributor) = Await FEB.StoreLocator.Distributors(exs, oProveidor)
                        If exs.Count = 0 Then
                            Dim oExcelSheet As MatHelper.Excel.Sheet = DTOProductDistributor.Excel(items)
                            retval = Await ExcelResult(oExcelSheet)
                        End If
    ...
    Private Async Function JSONSalePoints(oParams As Dictionary(Of String, String)) As Threading.Tasks.Task(Of JsonResult)
        ...
        Dim oDistributors = Await FEB.ProductDistributors.FromBrand(exs, oBrand)
