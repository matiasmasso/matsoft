Public Class PurchaseOrderController
    ...
    Async Function SingleOrder(guid As Guid) As Threading.Tasks.Task(Of ActionResult)
        ...
        Dim oOrder = Await FEB.PurchaseOrder.Find(guid, exs)
        If exs.Count = 0 Then
            Return View(oOrder)
        Else
            Return View("Error")
    ...
    Public Async Function PurchaseOrderItem(guid As Guid, lin As Integer) As Threading.Tasks.Task(Of ActionResult)
        ...
        If oOrder IsNot Nothing Then
            ...
            If item IsNot Nothing Then
                item.deliveries = Await FEB.PurchaseOrderItem.DeliveryItems(exs, item)
                retval = View(item)
            End If
        End If
        Return retval
    ...
    Public Async Function RepSortides(guid As Guid) As Threading.Tasks.Task(Of ActionResult)
        ...
        Dim oOrder = Await FEB.PurchaseOrder.OrderWithDeliveryItems(exs, guid)
        If oOrder IsNot Nothing Then
        ...
        If oUser IsNot Nothing AndAlso oUser.Rol.isAuthenticated Then
            Select Case oUser.Rol.id
                Case DTORol.Ids.manufacturer
                    ...
                    If exs.Count = 0 Then
                        Model = Await FEB.PurchaseOrders.Headers(exs, Website.GlobalVariables.Emp, DTOPurchaseOrder.Codis.proveidor, oContact, Year:=DTO.GlobalVariables.Today().Year)
                ...
                Case DTORol.Ids.cliFull, DTORol.Ids.cliLite
                    If oContact Is Nothing Then oContact = Await ContextHelper.Contact(exs)
                    If exs.Count = 0 Then
                        Model = Await FEB.PurchaseOrders.All(exs, Website.GlobalVariables.Emp, DTOPurchaseOrder.Codis.client, 0, oContact)
                        If exs.Count = 0 Then
                            retval = View("PurchaseOrders", Model)
                        Else
                            Return View("Error")
                ...
                Case DTORol.Ids.comercial, DTORol.Ids.rep, DTORol.Ids.salesManager, DTORol.Ids.admin, DTORol.Ids.superUser
                    ...
                    Select Case oContact.Rol.id
                        Case DTORol.Ids.cliFull, DTORol.Ids.cliLite
                            ...
                            End If
                        Case DTORol.Ids.manufacturer
                            Model = Await FEB.PurchaseOrders.All(exs, Website.GlobalVariables.Emp, DTOPurchaseOrder.Codis.proveidor, 0, oContact)
                            If exs.Count = 0 Then
                                retval = View("PurchaseOrders", Model)
                            Else
                                Return View("Error")
    ...
    Async Function pageindexchanged(pageindex As Integer, pagesize As Integer, guid As Guid) As Threading.Tasks.Task(Of PartialViewResult)
        ...
        Select Case oContact.Rol.Id
            Case DTORol.Ids.Manufacturer
                oItems = Await FEB.PurchaseOrders.All(exs, Website.GlobalVariables.Emp, DTOPurchaseOrder.Codis.Proveidor, 0, oContact)
            Case DTORol.Ids.CliFull, DTORol.Ids.CliLite
                oItems = Await FEB.PurchaseOrders.All(exs, Website.GlobalVariables.Emp, DTOPurchaseOrder.Codis.Client, 0, oContact)
        End Select
    ...
    Async Function CustomerPending(Optional customer As Nullable(Of Guid) = Nothing) As Threading.Tasks.Task(Of ActionResult)
        ...
        If customer Is Nothing Then
            ...
            Else
                Select Case oUser.Rol.Id
                    Case DTORol.Ids.CliFull, DTORol.Ids.CliLite
                        Model = Await FEB.PurchaseOrderItems.Pending(exs, oUser, DTOPurchaseOrder.Codis.Client, Website.GlobalVariables.Emp.Mgz)
                    Case DTORol.Ids.Manufacturer
                        Model = Await FEB.PurchaseOrderItems.Pending(exs, oUser, DTOPurchaseOrder.Codis.Proveidor, Website.GlobalVariables.Emp.Mgz)
                End Select
            End If
        Else
            ...
            Select Case oCustomer.Rol.Id
                Case DTORol.Ids.CliFull, DTORol.Ids.CliLite
                    Model = Await FEB.PurchaseOrderItems.Pending(exs, oCustomer, DTOPurchaseOrder.Codis.Client, Website.GlobalVariables.Emp.Mgz)
                Case DTORol.Ids.Manufacturer
                    Model = Await FEB.PurchaseOrderItems.Pending(exs, oCustomer, DTOPurchaseOrder.Codis.Proveidor, Website.GlobalVariables.Emp.Mgz)
            End Select
        End If
