Public Class CommentsController
    ...
    Async Function Update(model As CommentViewModel) As Threading.Tasks.Task(Of CommentViewModel.Results)
        ...
        ElseIf ContextHelper.GetUser IsNot Nothing AndAlso ContextHelper.GetUser.EmailAddress = model.EmailAddress Then
            ...
            If Await FEB.PostComment.Update(exs, oPostComment) Then
                Await FEB.PostComment.EmailPendingModeration(exs, Website.GlobalVariables.Emp, oPostComment)
        ...
        ElseIf Not String.IsNullOrEmpty(model.Password) AndAlso Not String.IsNullOrEmpty(model.EmailAddress) Then
            'verificar email
            Dim oPasswordValidationModel = Await FEB.User.ValidatePassword(exs, Website.GlobalVariables.Emp, model.EmailAddress, model.Password)
            ...
            ElseIf oPwdResult = DTOUser.ValidationResults.newValidatedUser Then
                ...
                Else
                    ...
                    oUser = DTOUser.Factory(Website.GlobalVariables.Emp,, model.EmailAddress)
                    With oUser
                        ...
                        .FchActivated = DTO.GlobalVariables.Now()
        ...
        Else
            ...
            Dim oUser = Await FEB.User.FromEmail(exs, Website.GlobalVariables.Emp, model.EmailAddress)
            If exs.Count = 0 Then
    ...
    Async Function PartialTree(target As Guid, targetSrc As DTOPostComment.ParentSources, take As Integer, from As Integer) As Threading.Tasks.Task(Of PartialViewResult)
        ...
        Dim model = Await FEB.PostComments.Tree(exs, oTarget, targetSrc, oLang, take, from)
        If exs.Count = 0 Then
            If from = 0 Then
