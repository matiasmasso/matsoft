...
Public Class DescatalogadosController
    ...
    Async Function Index() As Threading.Tasks.Task(Of ActionResult)
        ...
        Select Case MyBase.Authorize({DTORol.Ids.cliFull, DTORol.Ids.cliLite, DTORol.Ids.manufacturer, DTORol.Ids.superUser, DTORol.Ids.marketing})
            Case AuthResults.success
                Dim fchFrom = DTO.GlobalVariables.Now().AddMonths(-12)
                Dim oUser = ContextHelper.GetUser()
                Dim oSheet = Await FEB.ProductSkus.Obsolets(exs, oUser, ContextHelper.Lang, fchFrom)
                If exs.Count = 0 Then
                    ...
                    With Model
                        .FchFrom = fchFrom
                        .User = oUser
                        .IsSubscribed = Await FEB.Subscriptor.IsSubscribed(exs, DTOSubscription.Wellknowns.AvisDescatalogats, .User)
    ...
    Async Function Reload(fchFrom As Date) As Threading.Tasks.Task(Of PartialViewResult)
        ...
        With Model
            .FchFrom = fchFrom
            .User = oUser
            .Sheet = Await FEB.ProductSkus.Obsolets(exs, oUser, ContextHelper.Lang, fchFrom)
        End With
    ...
    Async Function Subscribe(value As Boolean) As Threading.Tasks.Task(Of Boolean)
        ...
        If value Then
            retval = Await FEB.Subscription.Subscribe(exs, Website.GlobalVariables.Emp, oSubscription, oUser)
        Else
            retval = Await FEB.Subscription.UnSubscribe(exs, Website.GlobalVariables.Emp, oSubscription, oUser)
        End If
        Return retval
    End Function
    ...
    Async Function Excel(Optional fchFrom As Nullable(Of Date) = Nothing) As Threading.Tasks.Task(Of ActionResult)
        Dim exs As New List(Of Exception)
        Dim retval As ActionResult = Nothing
        If fchFrom Is Nothing Then fchFrom = DTO.GlobalVariables.Today().AddYears(-1)
        Dim Model As New DescatalogadosModel
        With Model
            .FchFrom = fchFrom
            .User = ContextHelper.GetUser()
            .Sheet = Await FEB.ProductSkus.Obsolets(exs, Model.User, ContextHelper.Lang, Model.FchFrom)
        End With

        If exs.Count = 0 Then
            retval = Await MyBase.ExcelResult(Model.Sheet)
        Else
            retval = PartialView("Error")
        End If
...
Public Class DescatalogadosModel
    ...
    Property Sheet As MatHelper.Excel.Sheet
