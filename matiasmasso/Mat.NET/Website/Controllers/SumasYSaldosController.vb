Public Class SumasYSaldosController
    ...
    Function Summary(oEmp As DTOEmp) As ActionResult
    ...
    Async Function FromYear(year As Integer) As Threading.Tasks.Task(Of PartialViewResult)
        Dim oExercici As DTOExercici = DTOExercici.FromYear(Website.GlobalVariables.Emp, year)
        Dim DtFch As New Date(oExercici.Year, 12, 31)
        Dim exs As New List(Of Exception)
        Dim Model As List(Of DTOBalanceSaldo) = Await FEB.Balance.SumasySaldos(exs, oExercici, DtFch)
        ViewBag.fch = DtFch
    ...
    Async Function FromContact(guid As Guid) As Threading.Tasks.Task(Of ActionResult)
        Dim exs As New List(Of Exception)
        Dim oExercici As DTOExercici = DTOExercici.Current(Website.GlobalVariables.Emp)
        Dim Model = Await FEB.Contact.Find(guid, exs)
    ...
    Async Function FromYearContact(guid As Guid, year As Integer) As Threading.Tasks.Task(Of PartialViewResult)
        ...
        If guid = Nothing Then
            Model = Await FEB.SumasYSaldos.Summary(exs, oExercici)
        Else
            Dim oContact As New DTOContact(guid)
            Model = Await FEB.SumasYSaldos.All(exs, oExercici, False, DTO.Defaults.ContactRange.OnlyThisContact, oContact)
        End If
    ...
    Async Function SubComptes(guid As Guid, year As Integer) As Threading.Tasks.Task(Of ActionResult)
        ...
        Dim exs As New List(Of Exception)
        Dim oExercici As DTOExercici = DTOExercici.FromYear(Website.GlobalVariables.Emp, year)
        Dim oCta = Await FEB.PgcCta.Find(guid, exs)
        Dim oSubComptes = Await FEB.SumasYSaldos.SubComptes(exs, oExercici, oCta)
