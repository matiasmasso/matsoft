...
Public Class IbanController
    ...
    Function Index() As ActionResult
        ...
        Select Case MyBase.Authorize({DTORol.Ids.cliFull, DTORol.Ids.cliLite})
            Case AuthResults.success
                ViewBag.Title = ContextHelper.Tradueix("Mandato de Domiciliación Bancaria",
    ...
    Async Function Validate(src As String) As Threading.Tasks.Task(Of JsonResult)
        ...
        If src.Length >= 8 Then
            Dim oBranch = Await FEB.IbanStructure.GetBankBranch(src, exs)
            If oBranch Is Nothing Then
                Dim oBank = Await FEB.IbanStructure.GetBank(src, exs)
    ...
    Async Function Save(digits As String, titular As Guid, personNom As String, personDni As String) As Threading.Tasks.Task(Of JsonResult)
        ...
        ElseIf DTOIban.ValidateDigits(cleanDigits) Then
            Dim oPendingIbans = Await FEB.Ibans.PendingUploads(exs, oUser)
            ...
            If oIban Is Nothing Then
                ...
                'Else
                oIban = Await FEB.Iban.NewForDownload(cleanDigits, titular, oUser)
                oIban.PersonNom = personNom
                oIban.PersonDni = personDni

                If Await FEB.Iban.Update(oIban, exs) Then
                    oData = New With {.status = 1, .url = FEB.Iban.DownloadUrl(oIban, False), .guid = oIban.Guid.ToString, .iban = FEB.Iban.ToHtml(oIban)}
                Else
                    ...
                    FEB.WinBug.UpdateSync(exs, oWinbug)
                End If
                'End If
            Else
                oData = New With {.status = 1, .url = FEB.Iban.DownloadUrl(oIban, False), .guid = oIban.Guid.ToString}
            End If
        Else
            oData = New With {.status = 2,
    ...
    Private Async Function CheckIsAlreadyActive(cleanDigits As String, oTitular As DTOContact) As Threading.Tasks.Task(Of Boolean)
        Dim exs As New List(Of Exception)
        Dim oIbans As List(Of DTOIban) = Await FEB.Ibans.FromContact(exs, oTitular, OnlyVigent:=True, oCod:=DTOIban.Cods.client)
    ...
    Async Function Upload() As Threading.Tasks.Task(Of JsonResult)
        ...
        If oFile IsNot Nothing Then
            ...
            Else
                ...
                If Await FEB.Iban.Upload(oIban, oDocfile, oUser, exs) Then
                    ...
                    If oDocfile.Mime = MimeCods.NotSet Then
                        With oMailMessage
                            ...
                            .Body = "Fitxer amb format desconegut " & FEB.DocFile.DownloadUrl(oDocfile, True)
                        End With
                    Else
                        With oMailMessage
