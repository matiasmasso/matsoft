Public Class SearchController
    ...
    Async Function SearchAction(SearchBox As String) As Threading.Tasks.Task(Of ActionResult)
        ...
        With oRequest
            ...
            .SearchKey = SearchBox
        End With
        Dim oResult = Await FEB.LangText.Search(exs, oRequest)
        If exs.Count = 0 Then
    ...
    Async Function Index(SearchKey As String) As Threading.Tasks.Task(Of ActionResult)
        ...
        Dim oSearchRequest = DTOSearchRequest.Factory(Website.GlobalVariables.Emp, oUser, ContextHelper.Lang, SearchKey)
        Dim model As DTOSearchRequest = Await FEB.SearchRequest.Load(oSearchRequest, exs)
        If exs.Count = 0 Then
    ...
    Async Function SearchRequest(SearchKey As String) As Threading.Tasks.Task(Of PartialViewResult)
        ...
        Dim oUser = ContextHelper.GetUser()
        Dim oSearchRequest = DTOSearchRequest.Factory(Website.GlobalVariables.Emp, oUser, ContextHelper.Lang, SearchKey)
        Dim model = Await FEB.SearchRequest.Load(oSearchRequest, exs)
