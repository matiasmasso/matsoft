...
Public Class ImgController
    ...
    Async Function Index(cod As String, id As String) As Threading.Tasks.Task(Of ActionResult)
        ...
        Try
            If id > "" Then
                If id.EndsWith(".jpg") Then id = id.Substring(0, id.Length - 4)
                Dim oCod As DTO.Defaults.ImgTypes = cod
                Dim oImg As Byte() = Await GetImageFromCod(oCod, id, exs)
                If oImg IsNot Nothing Then
                    ...
                    Dim oStream As New System.IO.MemoryStream(oImg)
                    oStream.Position = 0
        ...
        Catch ex As Exception
            ...
            FEB.MailMessage.MailAdminSync("Error a ImgController oCod=" & cod & " Id=" & id, sb.ToString, exs)
        End Try
        Return retval
    End Function

    <OutputCache(Duration:=600, VaryByParam:="none")>
    Private Async Function GetImageFromCod(oCod As DTO.Defaults.ImgTypes, Id As String, exs As List(Of Exception)) As Threading.Tasks.Task(Of Byte())
        Dim retval As Byte() = Nothing

        Select Case oCod
            Case DTO.Defaults.ImgTypes.tpalogo
                Dim oGuid As New Guid(Id)
                retval = Await FEB.ProductBrand.Logo(exs, oGuid)
            Case DTO.Defaults.ImgTypes.news265x150
                Dim oGuid As New Guid(Id)
                retval = Await FEB.Noticia.Image265x150(exs, oGuid)
            Case DTO.Defaults.ImgTypes.stpwebthumbnail
                Dim oGuid As New Guid(Id)
                Dim oProductCategory As New DTOProductCategory(oGuid)
                retval = If(Await FEB.ProductCategory.Image(exs, oProductCategory), Await FEB.DefaultImage.Image(DTO.Defaults.ImgTypes.stpwebthumbnail, exs))
            Case DTO.Defaults.ImgTypes.art150
                Dim oGuid As New Guid(Id)
                Dim oSku As New DTOProductSku(oGuid)
                retval = Await FEB.ProductSku.Thumbnail(exs, oSku)
            Case DTO.Defaults.ImgTypes.art
                Dim oGuid As New Guid(Id)
                Dim oSku As New DTOProductSku(oGuid)
                Dim oImage = Await FEB.ProductSku.Image(exs, oSku)
                If oImage Is Nothing Then
                    retval = Await FEB.DefaultImage.Image(DTO.Defaults.ImgTypes.art150, exs)
                Else
                    retval = Await FEB.ProductSku.Image(exs, oSku)
                End If
            Case DTO.Defaults.ImgTypes.art150
                Dim oGuid As New Guid(Id)
                Dim oSku As New DTOProductSku(oGuid)
            ...
            Case DTO.Defaults.ImgTypes.salesgrafic
                ...
                retval = Await FEB.GrfMesValue.Image(exs, oUser, 500, 200)

            Case DTO.Defaults.ImgTypes.galleryItem
                Dim oGuid As New Guid(Id)
                retval = Await FEB.GalleryItem.Image(oGuid, exs)
            Case DTO.Defaults.ImgTypes.clilogo
                Dim oGuid As New Guid(Id)
                Dim oContact As New DTOContact(oGuid)
                retval = Await FEB.Contact.Logo(exs, oContact)
            Case DTO.Defaults.ImgTypes.download150
                Dim sHash As String = CryptoHelper.HexToString(Id)
                retval = Await FEB.DocFile.Thumbnail(exs, sHash, 150)
            Case DTO.Defaults.ImgTypes.download
                Dim sHash As String = CryptoHelper.HexToString(Id)
                retval = Await FEB.DocFile.Thumbnail(exs, sHash)
            Case DTO.Defaults.ImgTypes.banner
                Dim oGuid As New Guid(Id)
                Dim oBanner As DTOBanner = Await FEB.Banner.Find(oGuid, exs)
            ...
            Case DTO.Defaults.ImgTypes.iban
                retval = Await FEB.Iban.Img(exs, Id)
            Case DTO.Defaults.ImgTypes.sorteofbfeatured200
                ...
                End If
                retval = Await FEB.Raffle.ImageFbFeatured(exs, oGuid)
            Case DTO.Defaults.ImgTypes.sorteobanner600
                Dim oGuid As New Guid(Id)
                retval = Await FEB.Raffle.ImageBanner600(exs, oGuid)
            Case DTO.Defaults.ImgTypes.sorteocallaction500
                Dim oGuid As New Guid(Id)
                retval = Await FEB.Raffle.ImageCallToAction500(exs, oGuid)
            Case DTO.Defaults.ImgTypes.sorteowinner
                Dim oGuid As New Guid(Id)
                retval = Await FEB.Raffle.ImageWinner(exs, oGuid)
            Case DTO.Defaults.ImgTypes.bloggerlogo
                Dim oGuid As New Guid(Id)
                retval = Await FEB.Blogger.Logo(oGuid, exs)
            ...
            Case DTO.Defaults.ImgTypes.tpalogodistribuidoroficial
                Dim oGuid As New Guid(Id)
                retval = Await FEB.ProductBrand.LogoDistribuidorOficial(exs, oGuid)
            Case DTO.Defaults.ImgTypes.mayborn
                Dim items = Await FEB.Mayborn.Sales(exs)
            ...
            Case DTO.Defaults.ImgTypes.mediaresourcethumbnail
                ...
                'retval = Await FEB.MediaResource.Thumbnail(exs, oMediaResource)
            Case DTO.Defaults.ImgTypes.staff
                Dim oGuid As New Guid(Id)
                retval = Await FEB.Staff.Avatar(exs, oGuid)
            Case DTO.Defaults.ImgTypes.incentiu
                Dim oGuid As New Guid(Id)
                Dim oIncentiu = Await FEB.Incentiu.Find(exs, oGuid)
                If oIncentiu Is Nothing Then
                    retval = Await FEB.DefaultImage.Image(DTO.Defaults.ImgTypes.incentiu, exs)
                Else
            ...
            Case DTO.Defaults.ImgTypes.wtbolsitelogo
                ...
                retval = Await FEB.WtbolSite.Logo(exs, oSite)
    ...
    Function renderPdf(maxWidth As Integer, maxHeight As Integer) As JsonResult
        ...
        If oFile IsNot Nothing Then
            Dim oImgBytes As Byte() = LegacyHelper.DocfileHelper.GetPdfThumbnail(oFile, maxWidth, maxHeight)

            Dim oDictionary As Dictionary(Of Guid, Byte()) = Me.HttpContext.Application("tmpImages")
            If oDictionary Is Nothing Then
                oDictionary = New Dictionary(Of Guid, Byte())
                Me.HttpContext.Application("tmpImages") = oDictionary
            End If
            Dim oGuid As Guid = Guid.NewGuid
            oDictionary.Add(oGuid, oImgBytes)
    ...
    Function Pdf2Img() As JsonResult
        Dim retval As JsonResult = Nothing
        Dim oFile As Byte() = MyBase.PostedFile()
        If oFile IsNot Nothing Then
            ...
            Dim oPdf = LegacyHelper.GhostScriptHelper.Pdf2Img(oFile)
            Dim oGuid = System.Guid.NewGuid()
            oDictionary.Add(oGuid, oPdf.Portrait)

            Dim myData As Object = New With {
                .PortraitUrl = "https://www.matiasmasso.es/img/" & CInt(DTO.Defaults.ImgTypes.pdfpreview).ToString & "/" & oGuid.ToString,
                .Length = oFile.Length,
                .Width = oPdf.Width,
                .Height = oPdf.Height,
                .Mime = System.Net.Mime.MediaTypeNames.Application.Pdf,
                .Pags = oPdf.PageCount
            }
            retval = Json(myData, JsonRequestBehavior.AllowGet)
        End If
