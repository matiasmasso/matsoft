Public Class SkuStocksController
    ...
    Async Function Index() As Threading.Tasks.Task(Of ActionResult)
        ...
        If oUser IsNot Nothing AndAlso oUser.Rol.isAuthenticated Then
            Dim oCatalog As DTOCatalog = Await FEB.SkuStocks.ForWeb(oUser, exs)
            If exs.Count = 0 Then
                Dim Model As New StocksModel
                Model.Lang = Request.Lang
                Model.User = oUser
                Model.IsSubscribed = Await FEB.Subscriptor.IsSubscribed(exs, DTOSubscription.Wellknowns.Stocks, oUser)
                Model.Brands = oCatalog
                Select Case oUser.Rol.id
                    Case DTORol.Ids.manufacturer
                        retval = View("ManufacturerStocks", Model)
                    Case Else
                        Dim oArtCustRefs = Await FEB.ArtCustRefs.FromUser(exs, oUser)
                        If exs.Count = 0 Then
    ...
    Async Function Csv() As Threading.Tasks.Task(Of ActionResult)
        ...
        Dim oUser = ContextHelper.GetUser()
        If oUser IsNot Nothing AndAlso oUser.Rol.isAuthenticated Then
            Dim oCatalog As DTOProductCatalog = Await FEB.Stocks.ForWeb(oUser, exs)
            Dim oCsv As DTOCsv = DTOProductCatalog.CsvStocks(oCatalog, oUser.Rol) ' FEB.ProductStocks.Csv(oUser)
