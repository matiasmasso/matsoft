...
Public Class ProductController
    ...
    Function Index(Brand As String, Catchall As String, Optional filters As String = "") As ActionResult
        ...
        '    ViewBag.Lang = oLang
        '    Dim oProductAndTab = Await FEB.ProductUrl.Search(exs, Website.GlobalVariables.Emp, sFriendlySegment)
        '    If exs.Count = 0 Then
    ...
    Public Function ProductGuid(guid As Guid) As ActionResult
        'Public Async Function ProductGuid(guid As Guid) As Threading.Tasks.Task(Of ActionResult)
        Dim exs As New List(Of Exception)
        Dim retval As ActionResult = Redirect("/")
        'Dim retval As ActionResult = Nothing
        'Dim oProduct As DTOProduct = Await FEB.Product.Find(exs, guid)
        'If exs.Count = 0 Then
    ...
    Private Async Function Product(oProduct As DTOProduct, oTab As DTOProduct.Tabs, oLang As DTOLang, Optional filters As String = "") As Threading.Tasks.Task(Of ActionResult)
        ...
        Select Case oProduct.sourceCod
            ...
            Case DTOProduct.SourceCods.Category
                retval = Await CategoryPage(oProduct.ToDerivedClass(), oTab, oLang)
            Case DTOProduct.SourceCods.Sku
                Dim oSku As DTOProductSku = oProduct.ToDerivedClass()
                Select Case oTab
                    Case DTOProduct.Tabs.imagen
                        Dim oImgBytes = Await FEB.ProductSku.Image(exs, oSku)
                        Dim oMimeCod As MimeCods = LegacyHelper.ImageHelper.GuessMime(oImgBytes)
                    ...
                    Case Else
                        retval = Await SkuPage(oProduct.ToDerivedClass(), oTab, oLang)
                End Select
        End Select
        Return retval
    End Function
    ...
    Private Async Function BrandPage(oBrand As DTOProductBrand, oTab As DTOProduct.Tabs, oLang As DTOLang) As Threading.Tasks.Task(Of ActionResult)
        ...
        Select Case oTab
            Case DTOProduct.Tabs.distribuidores
                ViewBag.Title = String.Format("{0} {1}", ContextHelper.Tradueix("Distribuidores Oficiales", "Distribuidors Oficials", "Official Dealers"), oBrand.Nom.Tradueix(oLang))
            ...
            Case Else
                ...
                'next line is commented to avoid a roundtrip to the server since brand content does not currently use plugins
                'oDept.Content = Await FEB.ProductPlugins.Deploy(exs, oDept.Content)
                oModel.Text = oBrand.Content.Html(oLang)

                Dim oDepts = Await FEB.Depts.All(exs, oBrand)
                If oDepts.Count = 0 Then
                    Dim oCategories = Await FEB.ProductCategories.All(exs, oBrand)
                    If exs.Count = 0 Then
                        oModel.GalleryMode = ProductModel.GalleryModes.Categories
                        For Each oCategory In oCategories.Where(Function(x) x.Codi = DTOProductCategory.Codis.standard And x.EnabledxConsumer)
        ...
        If oTab <> DTOProduct.Tabs.general Then
            ViewBag.Canonical = oBrand.UrlCanonicas.WithSufix(DTOProduct.TabUrlSegment(oTab).Tradueix(oLang))
        End If
    ...
    Private Async Function DeptPage(oDept As DTODept, oTab As DTOProduct.Tabs, oLang As DTOLang, Optional filters As String = "") As Threading.Tasks.Task(Of ActionResult)
        ...
        Dim exs As New List(Of Exception)
        Dim oModel As New ProductModel
        FEB.Dept.Load(oDept, False, exs)

        'get canonical url of brand for breadcrumb links
        Dim oCache = Await FEB.Cache.Fetch(exs, Website.GlobalVariables.Emp)

        'next line is commented to avoid a roundtrip to the server since dept content does not currently use plugins
        'oDept.Content = Await FEB.ProductPlugins.Deploy(exs, oDept.Content)
        ...
        'FEB.ProductBrand.Load(oDept.Brand, exs)

        If exs.Count = 0 Then
            'Dim oDepts = Await FEB.Depts.AllWithFilters(exs, oDept.Brand)
            'oDept = oDepts.FirstOrDefault(Function(x) x.Guid.Equals(oDept.Guid))
            oModel = ProductModel.Factory(oDept, oLang, oTab)
            oModel.Text = oDept.Content.Html(oLang)
            ...
            If oDept.Brand.Equals(DTOProductBrand.Wellknown(DTOProductBrand.Wellknowns.Romer)) Then
                ...
                Dim oFilters = Await FEB.Filters.All(exs)
            ...
            ViewBag.Title = oDept.SeoTitleOrDefault.Tradueix(oLang)
        ...
        If oTab <> DTOProduct.Tabs.general Then
            ViewBag.Canonical = oDept.UrlCanonicas.WithSufix(DTOProduct.TabUrlSegment(oTab).Tradueix(oLang))
        End If

        Return retval
    End Function

    Private Async Function CategoryPage(oCategory As DTOProductCategory, oTab As DTOProduct.Tabs, oLang As DTOLang) As Threading.Tasks.Task(Of ActionResult)
        Dim retval As ActionResult = Nothing
        Dim exs As New List(Of Exception)
        Dim oModel As New ProductModel
        ...
        oCategory.Content = Await FEB.ProductPlugins.Deploy(exs, oCategory.Content)
        ...
        oModel.Title = oCategory.SeoTitleOrDefault.Tradueix(oLang)
        ...
        ViewBag.Canonical = oCategory.UrlCanonicas
        If oTab <> DTOProduct.Tabs.general Then
            ViewBag.Canonical = oCategory.UrlCanonicas.WithSufix(DTOProduct.TabUrlSegment(oTab).Tradueix(oLang))
        End If

        Return retval
    End Function

    Private Async Function SkuPage(oSku As DTOProductSku, oTab As DTOProduct.Tabs, oLang As DTOLang) As Threading.Tasks.Task(Of ActionResult)
        Dim retval As ActionResult = Nothing
        Dim exs As New List(Of Exception)
        Dim oModel As New ProductModel
        ...
        'get canonical url of parents for breadcrumb links
        Dim oCache = Await FEB.Cache.Fetch(exs, Website.GlobalVariables.Emp)
        oSku.Category = oCache.Categories.FirstOrDefault(Function(x) x.Guid.Equals(oSku.Category.Guid))

        oModel.Product = oSku
        oModel.Title = oSku.SeoTitleOrDefault.Tradueix(oLang)
        ViewBag.Title = oModel.Title
        ...
        If oSku.Hereda Then
            FEB.ProductCategory.Load(oSku.Category, exs)
            oSku.Category.Content = Await FEB.ProductPlugins.Deploy(exs, oSku.Category.Content)
            oModel.Text = oSku.Category.Content.Html(oLang)
        Else
            oSku.Content = Await FEB.ProductPlugins.Deploy(exs, oSku.Content)
        ...
        ViewBag.Canonical = IIf(oSku.Hereda, oSku.Category.UrlCanonicas, oSku.UrlCanonicas)
        If oTab <> DTOProduct.Tabs.general Then
            ViewBag.Canonical = oSku.UrlCanonicas.WithSufix(DTOProduct.TabUrlSegment(oTab).Tradueix(oLang))
        End If
        ...
        If retval Is Nothing Then
            If oSku.Hereda Then
                ...
                If oTab <> DTOProduct.Tabs.general Then
                    ViewBag.Canonical = oSku.Category.UrlCanonicas.WithSufix(DTOProduct.TabUrlSegment(oTab).Tradueix(oLang))
                End If
            End If
    ...
    Private Function TabResult(oProduct As DTOProduct, oTab As DTOProduct.Tabs, oModel As ProductModel, oLang As DTOLang) As ActionResult
        ...
        Select Case oTab
            ...
            Case DTOProduct.Tabs.distribuidores
                ViewBag.Title = String.Format("{0} {1}", ContextHelper.Tradueix("Distribuidores Oficiales", "Distribuidors Oficials", "Official Dealers"), oModel.Title)
                retval = View("StoreLocator", oModel)
            Case DTOProduct.Tabs.galeria
                ViewBag.Title = String.Format("{0} {1}", ContextHelper.Tradueix("Galería de imágenes", "Galeria d'imatges", "Image gallery", "Galeria de Imagens"), oModel.Title)
                retval = View("Galeria", oModel.Product)
            Case DTOProduct.Tabs.descargas
                ViewBag.Title = String.Format("{0} {1}", ContextHelper.Tradueix("Descargas", "Descarregues", "Downloads", "Descargas"), oModel.Title)
            ...
            Case DTOProduct.Tabs.videos
                ViewBag.Title = String.Format("{0} {1}", ContextHelper.Tradueix("Vídeos"), oModel.Title)
                retval = View("Videos", oModel.Product)
            Case DTOProduct.Tabs.bloggerposts
                ViewBag.Title = String.Format("{0} {1}", ContextHelper.Tradueix("Publicaciones", "Publicacions", "Posts"), oModel.Title)
    ...
    Public Async Function Cnap(cnapKey As String) As Threading.Tasks.Task(Of ActionResult)
        Dim exs As New List(Of Exception)
        Dim oModel As List(Of DTOProduct) = Await FEB.Products.FromCnap(exs, cnapKey)
        Return View(oModel)
    End Function
    ...
    Public Async Function Ref(id As String) As Threading.Tasks.Task(Of ActionResult)
        ...
        If IsNumeric(id) Then
            Dim oSku = Await FEB.ProductSku.FromId(exs, Website.GlobalVariables.Emp, id)
            If oSku IsNot Nothing Then
                FEB.Product.Load(oSku, exs)
                retval = New RedirectResult(oSku.GetUrl(ContextHelper.Lang))
            End If
        End If
        Return retval
    End Function
    ...
    Async Function General(productGuid As Guid) As Threading.Tasks.Task(Of PartialViewResult)
        ...
        If GuidHelper.IsGuid(productGuid) Then
            ...
            Dim oProduct = Await FEB.Product.Find(exs, productGuid)
            Select Case oProduct.sourceCod
                Case DTOProduct.SourceCods.Brand
    ...
    Async Function PartialCollection(guid As Guid) As Threading.Tasks.Task(Of PartialViewResult)
        Dim exs As New List(Of Exception)
        Dim oCategory = Cache.Categories.FirstOrDefault(Function(x) x.Guid.Equals(guid))
        If oCategory Is Nothing Then oCategory = New DTOProductCategory(guid)
        Dim oSkus = Await FEB.ProductSkus.All(exs, oCategory, MyBase.Lang, Website.GlobalVariables.Emp.Mgz, False)
        Dim model = oSkus.FindAll(Function(x) x.NoWeb = False And DTOProductSku.IsHidden(x) = False And x.obsoleto = False).ToList
    ...
    Async Function PartialDownloads(guid As Guid) As Threading.Tasks.Task(Of PartialViewResult)
        ...
        Dim model = Await FEB.Downloads.FromProductOrParent(exs, oProduct, False, True, ContextHelper.Lang())
        If exs.Count = 0 Then
    ...
    Async Function PartialVideos(guid As Guid) As Threading.Tasks.Task(Of PartialViewResult)
        ...
        Dim oLang = ContextHelper.Lang
        Dim model = Await FEB.YouTubeMovies.All(exs, oUser, oProduct, oLang)
        If exs.Count = 0 Then
    ...
    Async Function PartialImgGallery(guid As Guid) As Threading.Tasks.Task(Of PartialViewResult)
        Dim exs As New List(Of Exception)
        Dim oProduct As New DTOProduct(guid)
        Dim items = Await FEB.MediaResources.All(exs, oProduct)
        If exs.Count = 0 Then
            Dim filteredItems = items.Where(Function(x) x.Obsolet = False And x.LangSet.HasLang(ContextHelper.Lang())).ToList
    ...
    Async Function PartialAccessories(guid As Guid) As Threading.Tasks.Task(Of PartialViewResult)
        Dim exs As New List(Of Exception)
        Dim oProduct As New DTOProduct(guid)
        Dim model = Await FEB.Product.Relateds(exs, DTOProduct.Relateds.Accessories, oProduct)
        If exs.Count = 0 Then
    ...
    Async Function Accesorios(product As Guid) As Threading.Tasks.Task(Of PartialViewResult)
        Dim exs As New List(Of Exception)
        Dim oProduct = Await FEB.Product.Find(exs, product)
    ...
    Async Function PartialBloggerPosts(guid As Guid) As Threading.Tasks.Task(Of PartialViewResult)
        Dim exs As New List(Of Exception)
        Dim oProduct As New DTOProduct(guid)
        Dim model = Await FEB.BloggerPosts.FromProductOrParent(oProduct, exs)
        If exs.Count = 0 Then
...
#Region "Videos"
    Async Function AllVideos() As Threading.Tasks.Task(Of ActionResult)
        Dim exs As New List(Of Exception)
        Dim retval As ActionResult = Nothing
        Dim model As DTOYouTubeMovie.ProductModel = Await FEB.YouTubeMovies.Model(exs, Website.GlobalVariables.Emp, ContextHelper.Lang, ContextHelper.GetUser())
        If exs.Count = 0 Then
...
#Region "SalePoints"
    ...
    Async Function Paisos(productGuid As Guid) As Threading.Tasks.Task(Of JsonResult)
        ...
        Dim oCountries = Await FEB.StoreLocator.Countries(exs, oProduct, oLang)
    ...
    Async Function Zonas(productGuid As Guid, countryGuid As Guid) As Threading.Tasks.Task(Of JsonResult)
        ...
        Dim oLang = ContextHelper.Lang()
        Dim oZonas = Await FEB.StoreLocator.Zonas(exs, oProduct, oCountry, oLang)
    ...
    Async Function Locations(productGuid As Guid, zonaGuid As Guid) As Threading.Tasks.Task(Of JsonResult)
        ...
        Dim oLang = ContextHelper.Lang()
        Dim oLocations = Await FEB.StoreLocator.Locations(exs, oProduct, oZona, oLang)
    ...
    Async Function StoreLocatorTest(product As Guid, location As Guid) As Threading.Tasks.Task(Of PartialViewResult)
        ...
        If oProduct.sourceCod = DTOProduct.SourceCods.Dept Then
            ...
            Dim oDept = Await FEB.Dept.Find(oProduct.Guid, exs)
            oProduct = oDept.Brand
        End If
        Dim Model As New DTOProductPageQuery
        With Model
            .Product = oProduct
            If location <> Guid.Empty Then .Location = Await FEB.Location.Find(location, exs)
    ...
    Async Function StoreLocator(product As Guid, location As Guid) As Threading.Tasks.Task(Of PartialViewResult) 'JScript web mobile
        ...
        With Model
            .Product = oProduct
            .Location = Await FEB.Location.Find(location, exs)
            .Tab = DTOProduct.Tabs.distribuidores
        End With
    ...
    Async Function Distributors(productGuid As Guid, locationGuid As Guid) As Threading.Tasks.Task(Of PartialViewResult)
        ...
        'Dim oLocation = Await FEB.Location.Find(locationGuid, exs)
        'Dim oProduct = Await FEB.Product.Find(exs, productGuid)
        Dim oDistributors = Await FEB.StoreLocator.Distributors(exs, oProduct, oLocation, ContextHelper.Lang())
    ...
    Async Function FromGeoLocation(product As Guid, latitud As String, longitud As String) As Threading.Tasks.Task(Of PartialViewResult)
        ...
        If Decimal.TryParse(latitud, Globalization.NumberStyles.Float, culture, dcLatitud) Then
            If Decimal.TryParse(longitud, Globalization.NumberStyles.Float, culture, dcLongitud) Then
                ...
                Dim oProduct = Await FEB.Product.Find(exs, product)
                Dim Model As List(Of DTONeighbour) = Await FEB.StoreLocator.NearestNeighbours(exs, oProduct, oCoordenadas, ContextHelper.GetLang)
    ...
    Async Function FromLocation(product As Guid, location As Guid) As Threading.Tasks.Task(Of PartialViewResult)
        ...
        oProductArea.Product = Await FEB.Product.Find(exs, product)
        oProductArea.Area = Await FEB.Location.Find(location, exs)
    ...
    Async Function BestZona(productGuid As Guid, countryGuid As Guid) As Threading.Tasks.Task(Of JsonResult)
        ...
        Dim oProduct = Await FEB.Product.Find(exs, productGuid)
        Dim oCountry As New DTOCountry(countryGuid)
        Dim oDistributors = Await FEB.StoreLocator.Distributors(exs, oProduct, oCountry, ContextHelper.Lang())
    ...
    Async Function BestLocation(productGuid As Guid, zonaGuid As Guid) As Threading.Tasks.Task(Of JsonResult)
        ...
        Dim oDistributors = Await FEB.StoreLocator.Distributors(exs, oProduct, oZona, ContextHelper.Lang())
...
#End Region
    ...
    Async Function videospageindexchanged(pageindex As Integer, pagesize As Integer, guid As Guid) As Threading.Tasks.Task(Of PartialViewResult)
        ...
        Dim exs As New List(Of Exception)
        Dim oMovies = Await FEB.YouTubeMovies.FromProductGuid(guid, ContextHelper.Lang(), exs)
        If exs.Count = 0 Then
