...
Public Class _MatController
    ...
    Protected Function Cache() As DTO.Models.ClientCache
        Return FEB.GlobalVariables.Cache(Website.GlobalVariables.Emp)
    End Function
    ...
    Protected Async Function LogOff() As Threading.Tasks.Task
        ...
        Await ContextHelper.SetNavViewModel()
    End Function
    ...
    Protected Async Function ExcelResult(oBook As MatHelper.Excel.Book) As Threading.Tasks.Task(Of ActionResult)
        Dim retval As ActionResult = Nothing
        Dim exs As New List(Of Exception)
        Try
            Dim bytes = MatHelper.Excel.ClosedXml.Bytes(oBook)
            retval = New FileContentResult(bytes, "application/vnd.openxmlformats-officedocument." + "spreadsheetml.sheet")
        ...
        If exs.Count > 0 Then retval = Await ErrorResult(exs)
        Return retval
    End Function

    Protected Async Function ExcelResult(oSheet As MatHelper.Excel.Sheet) As Threading.Tasks.Task(Of FileResult)
        Dim retval As ActionResult = Nothing
        Dim exs As New List(Of Exception)
        Try
            ...
            If oSheet.Filename > "" Then
                ...
                HttpContext.Response.AddHeader("content-disposition", "attachment; filename=" & oSheet.Filename & "")
            End If
    ...
    Protected Async Function LoginOrFile(oBook As MatHelper.Excel.Book) As Threading.Tasks.Task(Of ActionResult)
        Dim retval As ActionResult = Nothing
        Dim exs As New List(Of Exception)
        Try
            Dim bytes = MatHelper.Excel.ClosedXml.Bytes(oBook)
    ...
    Protected Async Function LoginOrFile(oSheet As MatHelper.Excel.Sheet) As Threading.Tasks.Task(Of ActionResult)
        Dim retval As ActionResult = Nothing
        Dim exs As New List(Of Exception)
        Try
            Dim bytes = MatHelper.Excel.ClosedXml.Bytes(oSheet)
    ...
    Protected Async Function LogWebErr(cod As DTOWebErr.Cods, msg As String) As Threading.Tasks.Task(Of DTOWebErr)
        ...
        Await FEB.WebErr.Update(exs, oWebErr)
        Return oWebErr
    End Function
    ...
    Function UrlSplit(oUri As Uri, oMainSegment As DTOLangText, sFileExtension As String, ByRef oDomain As DTOWebDomain, ByRef oLang As DTOLang, ByRef sFriendlySegment As String) As Boolean
        ...
        sFriendlySegment = Server.UrlDecode(sFriendlySegment) 'resolves url encoded special characters as ç
