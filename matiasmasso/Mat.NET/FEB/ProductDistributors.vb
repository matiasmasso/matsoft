Public Class ProductDistributors
    ...
    Shared Async Function PerChannel(exs As List(Of Exception), oUser As DTOUser, Optional iYear As Integer = 0) As Task(Of MatHelper.Excel.Book)
        Return Await Api.Fetch(Of MatHelper.Excel.Book)(exs, "ProductDistributors/PerChannel", oUser.Guid.ToString, iYear)
    End Function

    Shared Async Function ManufacturerCsv(oProveidor As DTOProveidor) As Task(Of String)
        Dim exs As New List(Of Exception)
        Dim sb As New System.Text.StringBuilder
        Dim oDistributors = Await ProductDistributors.FromManufacturer(exs, oProveidor)
    ...
    Shared Function StoreLocatorAttachments(oProveidor As DTOProveidor, exs As List(Of Exception)) As ArrayList
        ...
        If FileSystemHelper.DownloadStream(sUrl, oMemoryStream, exs) Then
            Dim sfilename As String = String.Format("M+O Store locator {0:yyyy.MM.dd.HH.mm}", DTO.GlobalVariables.Now())
