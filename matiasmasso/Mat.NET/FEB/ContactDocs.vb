Public Class ContactDoc
    ...
    Shared Function Url(oContactDoc As DTOContactDoc, AbsoluteUrl As Boolean) As String
        ...
        If oContactDoc IsNot Nothing Then
            retval = UrlHelper.Factory(AbsoluteUrl, DTODocFile.Cods.Download, CryptoHelper.StringToHexadecimal(oContactDoc.DocFile.Hash))
        End If
        Return retval
    End Function
End Class
...
Public Class ContactDocs
    ...
    Shared Async Function ExcelMod145s(oExercici As DTOExercici, exs As List(Of Exception)) As Task(Of MatHelper.Excel.Sheet)
        Return Await Api.Fetch(Of MatHelper.Excel.Sheet)(exs, "ContactDocs/Mod145s/Excel", oExercici.Emp.Id, oExercici.Year)
    End Function
