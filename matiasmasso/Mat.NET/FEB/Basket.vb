Public Class Basket
    ...
    Shared Function Factory(oOrder As DTOPurchaseOrder) As DTOBasket
        ...
        With retval
            ...
            .customerUrl = Contact.Url(oOrder.Contact)
    ...
    Shared Async Function Brands(exs As List(Of Exception), basket As DTOBasket) As Task(Of List(Of DTOGuidNom))
        Dim oCustomer = Await Customer.Find(exs, basket.customer)
        Dim oCcx As DTOCustomer = Customer.CcxOrMe(exs, oCustomer)
        Dim oBrands = Await ProductBrands.All(exs, oCcx)
    ...
    Shared Async Function Categories(exs As List(Of Exception), basket As String, brand As Guid, stockOnly As Boolean, Optional includeHidden As Boolean = False) As Task(Of List(Of DTOGuidNom))
        ...
        Dim oCustomer = Await Customer.Find(exs, oBasket.customer)
        Dim oCcx = Customer.CcxOrMe(exs, oCustomer)
        Dim oCategories = Await ProductCategories.All(exs, oCcx)
        ...
        For Each item As DTOProductCategory In oCategories
            If includeHidden Or item.HideUntil < DTO.GlobalVariables.Today() Then
                retval.Add(New DTOGuidNom(item.Guid, item.Nom.Esp))
            End If
        Next
        Return retval
    ...
    Shared Async Function Skus(exs As List(Of Exception), oEmp As DTOEmp, basket As String, category As Guid, stockOnly As Boolean, Optional includeHidden As Boolean = False) As Task(Of List(Of DTOGuidNom))
        ...
        Dim retval As List(Of DTOGuidNom) = Await ProductSkus.GuidNoms(exs, oCategory, oCcx, oEmp.Mgz, stockOnly, includeHidden)
        Return retval
    End Function
    ...
    Shared Async Function GetPurchaseOrder(oBasket As DTOBasket, oUser As DTOUser, oSrc As DTOPurchaseOrder.Sources) As Task(Of DTOPurchaseOrder)
        ...
        Dim oCustomCosts = Await PriceListItemsCustomer.Active(exs, oCcx, DTO.GlobalVariables.Today())
        Dim oTarifaDtos = Await CustomerTarifaDtos.Active(exs, oCcx)
        Dim oCliProductDtos = Await CliProductDtos.All(oCcx, exs)

        User.Load(exs, oUser)
        Select Case oUser.Rol.id
            Case DTORol.Ids.Comercial, DTORol.Ids.Rep
                oRep = Await User.GetRep(oUser, exs)
                oRepProducts = Await RepProducts.All(exs, oUser.Emp, oRep, False)
                oRepCliComs = Await RepCliComs.All(exs, oUser.Emp)
        End Select

        Dim retval = DTOPurchaseOrder.Factory(oCustomer, oUser, DTO.GlobalVariables.Today(), oSrc)
        With retval
            ...
            If Date.TryParseExact(oBasket.fchmin, "yyyy-MM-dd", System.Globalization.CultureInfo.InvariantCulture, Globalization.DateTimeStyles.None, DtFchMin) Then
                If DtFchMin > DTO.GlobalVariables.Today() Then
        ...
        For Each oBasketLine As DTOBasketLine In oBasket.lines
            ...
            With item
                ...
                .Qty = oBasketLine.qty
                .Pending = .qty
                .sku = Await ProductSku.Find(exs, oBasketLine.sku)
                .price = DTOAmt.Factory(oBasketLine.price)
                .Dto = oBasketLine.dto
                .RepCom = Await PurchaseOrderItem.SuggestedRepCom(oUser.Emp, item, oRepProducts, oRepCliComs, exs)
            End With
            retval.items.Add(item)

            If item.sku.isBundle Then
                item.bundle = Await PurchaseOrderItem.BundleItemsFactory(exs, item, oUser.Emp, oCustomCosts, oTarifaDtos, oCliProductDtos, oRepProducts)
            End If
    ...
    Shared Async Function Deliver(oOrders As List(Of DTOPurchaseOrder), oUser As DTOUser, oMgz As DTOMgz) As Task(Of DTODelivery)
        ...
        Dim retval = Delivery.Factory(exs, oCustomer, oUser, oMgz)
        With retval
            .Nom = oCustomer.NomComercialOrDefault()
            .Address = Customer.ShippingAddressOrDefault(oCustomer)
            .Tel = Await Contact.Tel(exs, oCustomer)
