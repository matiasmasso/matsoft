Public Class ElCorteIngles
    ...
    End Function
    Shared Async Function PlantillaDescatalogats(exs As List(Of Exception)) As Threading.Tasks.Task(Of MatHelper.Excel.Sheet)
        Return Await Api.Fetch(Of MatHelper.Excel.Sheet)(exs, "ElCorteIngles/plantillas/descatalogats")
    End Function

    Shared Async Function PlantillaExhaurits(exs As List(Of Exception)) As Threading.Tasks.Task(Of MatHelper.Excel.Sheet)
        Return Await Api.Fetch(Of MatHelper.Excel.Sheet)(exs, "ElCorteIngles/plantillas/exhaurits")
    End Function
    ...
    Shared Async Function Platforms(oContact As DTOContact) As Task(Of List(Of DTOCustomerPlatform))
        If _platforms Is Nothing Or oContact.UnEquals(_lastContact) Then
            ...
            _platforms = Await CustomerPlatforms.All(oContact, exs)
        End If
        Return _platforms
    End Function
    ...
    Shared Function Central() As DTOCustomer
        ...
        If retval Is Nothing Then
            ...
            ' retval = Customer.Find(exs,oGuid) 'implementar
        End If
        Return retval
    End Function
    ...
    Shared Async Function PurchaseOrderYears(exs As List(Of Exception), oEmp As DTOEmp) As Task(Of List(Of Integer))
        ...
        Return Await FEB.PurchaseOrders.Years(exs, oEmp, DTOPurchaseOrder.Codis.client, oCentral, True)
    End Function
    ...
    Shared Async Function PurchaseOrders(exs As List(Of Exception), oEmp As DTOEmp, iYear As Integer) As Task(Of List(Of DTOPurchaseOrder))
        Dim oCentral = DTOCustomer.Wellknown(DTOCustomer.Wellknowns.ElCorteIngles)
        Return Await FEB.PurchaseOrders.All(exs, oEmp, DTOPurchaseOrder.Codis.client, iYear, oCentral, True)
    End Function
                    ...
                    With item
                        ...
                        If oSku IsNot Nothing Then
                            If oSku.Category IsNot Nothing Then
                                If oSku.Category.Brand IsNot Nothing AndAlso oSku.Category.Brand.Nom IsNot Nothing Then
    ...
    Shared Async Function AppendToCataleg(exs As List(Of Exception), items As List(Of DTO.Integracions.ElCorteIngles.Cataleg)) As Task(Of Boolean)
        Return Await Api.Execute(Of List(Of Integracions.ElCorteIngles.Cataleg), Boolean)(items, exs, "ArtCustRefs/ElCorteIngles")
    End Function

    Shared Async Function ExcelTransmisions(exs As List(Of Exception), oTransms As List(Of DTOTransmisio)) As Task(Of MatHelper.Excel.Sheet)

        Dim retval As New MatHelper.Excel.Sheet
        retval.AddRowWithCells("Transmision", "Plataforma", "Pedido", "Centro", "Albaran")

        Dim items = Await ElCorteIngles.ComandesDeTransmisions(exs, oTransms)
    ...
    Shared Async Function Descataloga(exs As List(Of Exception), oGuids As List(Of Guid)) As Task(Of Boolean)
        Return Await Api.Execute(Of List(Of Guid))(oGuids, exs, "ElCorteIngles/descataloga")
    End Function

    Shared Async Function Recataloga(exs As List(Of Exception), oGuids As List(Of Guid)) As Task(Of Boolean)
        Return Await Api.Execute(Of List(Of Guid))(oGuids, exs, "ElCorteIngles/recataloga")
    End Function
End Class
