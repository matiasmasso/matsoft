Public Class Incidencia
    ...
    Shared Async Function FromNum(exs As List(Of Exception), oEmp As DTOEmp, id As String) As Task(Of DTOIncidencia)
    ...
    Shared Function Summary(oIncidencia As DTOIncidencia, oLang As DTOLang) As Dictionary(Of String, String)
        ...
        With oIncidencia
            If Not .IsNew Then
                retval.Add(oLang.Tradueix("Incidencia", "Incidencia", "Id"), .AsinOrNum())
                retval.Add(oLang.Tradueix("Fecha", "Data", "Date"), TextHelper.VbFormat(.fch, "dd/MM/yy HH:mm"))
            End If

            If oIncidencia.ContactType = DTOIncidencia.ContactTypes.professional Then
            ...
            If .DocFileImages.Count > 0 Then
                ...
                For Each oDocfile As DTODocFile In .DocFileImages
                    sb.Append("<td>")
                    sb.Append("<a href='" & oDocfile.DownloadUrl(True) & "'><img src='" & oDocfile.ThumbnailUrl(True) & "' width='150' height='auto' style='padding:5px;width:150px;height:auto;'/></a>")
                    sb.AppendLine("</td>")
                Next
                sb.AppendLine("</tr>")
            End If
            ...
            If .PurchaseTickets.Count > 0 Then
                For Each oDocfile As DTODocFile In .PurchaseTickets
                    sb.AppendLine("<a href='" & oDocfile.DownloadUrl(True) & "'><img src='" & oDocfile.ThumbnailUrl(True) & "' width='150' height='auto'/></a>")
                Next
            End If
    ...
    Shared Async Function MailMessage(exs As List(Of Exception), oIncidencia As DTOIncidencia, oLang As DTOLang) As Task(Of DTOMailMessage)
        ...
        Dim oSubscriptors = Await Subscription.Subscriptors(oSsc, exs)
        Dim retval = DTOMailMessage.Factory(oIncidencia.EmailAdr)
        With retval
            .Bcc = DTOSubscriptor.Recipients(oSubscriptors)
            .Subject = String.Format("M+O: {0} {1} {2}", oLang.Tradueix("Registro de Incidencia Postventa", "Registre de Incidencia Postvenda", "Service incident", "Registo de Incidência Pós-venda"), oIncidencia.AsinOrNum, oIncidencia.NomRemitent())
            .BodyUrl = Mailing.BodyUrl(DTODefault.MailingTemplates.Incidencia, oIncidencia.Guid.ToString())
        End With
        Return retval
    End Function
    ...
    Shared Async Function SendMail(exs As List(Of Exception), oUser As DTOUser, oIncidencia As DTOIncidencia, oLang As DTOLang) As Task(Of Boolean)
        Dim oMailMessage = Await Incidencia.MailMessage(exs, oIncidencia, oLang)
        If exs.Count = 0 Then
...
Public Class Incidencias
    ...
    Shared Async Function Headers(exs As List(Of Exception), oUser As DTOUser, Optional oContact As DTOContact = Nothing) As Task(Of List(Of DTOIncidencia))
        ...
        oQuery.Year = DTO.GlobalVariables.Today().Year
