...
Public Class ProductPlugin
    ...
    Public Shared Function Html(oCache As Models.ClientCache, oPlugin As DTOProductPlugin, ByVal lang As DTOLang) As String
        ...
        For Each item In oPlugin.Items.Where(Function(x) x.Product.obsoleto = False).ToList()
...
Public Class ProductPlugins
    ...
    Public Shared Async Function Deploy(exs As List(Of Exception), ByVal src As DTOLangText) As Task(Of DTOLangText)
        ...
        Dim oCache = Await Cache.Fetch(exs, New DTOEmp(DTOEmp.Ids.MatiasMasso))
        If exs.Count = 0 Then
    ...
    Public Shared Function Deploy(oCache As Models.ClientCache, ByVal oLangText As DTOLangText, ByVal lang As DTOLang) As String
        ...
        Dim pattern = DTOProductPlugin.SnippetRegex()
        ...
        For Each match As Match In matches
            ...
            Dim plugin As DTOProductPlugin
            If oCache.Categories.Any(Function(x) x.Guid.Equals(guid)) Then
                ...
                Dim snippet = match.Value
                Dim modePattern = "ProductPluginMode=(\'.*?\')"
                Dim modeMatches = Regex.Matches(snippet, modePattern)
                If (modeMatches.Count = 0) Then
                    plugin = oCategory.PluginCollection(oCache) 'plugin anterior a posar-hi el mode, eren tots de colecció
                Else
                    Dim sMode = modeMatches.Item(0).Value.Replace("ProductPluginMode=", "").Replace("'", "")
                    Select Case sMode
                        Case DTOProductPlugin.Modes.Collection.ToString()
                            'build a new plugin with all active skus in category collection
                            plugin = oCategory.PluginCollection(oCache)
                        Case DTOProductPlugin.Modes.Accessories.ToString()
                            plugin = oCategory.PluginAccessories(oCache)
                        Case Else
                            plugin = New DTOProductPlugin(guid)
                    End Select
                End If
                input = input.Replace(match.Value, ProductPlugin.Html(oCache, plugin, lang))
            Else
                'look for plugin on plugins collection
                plugin = oCache.ProductPlugins.FirstOrDefault(Function(x) x.Guid.Equals(guid))
            End If
