Public Class Cca
    ...
    Shared Async Function UpdateOnlyDocfile(exs As List(Of Exception), value As DTOCca) As Task(Of Boolean)
        ...
        If exs.Count = 0 Then
            ...
            If value.DocFile IsNot Nothing Then
                ...
                oMultipart.AddFileContent("docfile_stream", value.DocFile.Stream)
            End If
            retval = Await Api.Upload(Of Boolean)(oMultipart, exs, "Cca/Docfile")
        End If
        Return retval
    End Function
    ...
    Shared Async Function Delete(exs As List(Of Exception), oCca As DTOCca) As Task(Of Boolean)
        ...
        Select Case oCca.Ccd
            Case DTOCca.CcdEnum.TransferNorma34
                Dim oTransfer = Await BancTransferPool.FromCca(oCca, exs)
                If oTransfer Is Nothing Then
                    oTransfer = New DTOBancTransferPool()
                    oTransfer.Cca = oCca
                End If

                If Await BancTransferPool.Delete(oTransfer, exs) Then
                    retval = True
                Else
    ...
    Shared Function DownloadUrl(oCca As DTOCca, Optional AbsoluteUrl As Boolean = False) As String
        ...
        If oCca IsNot Nothing Then
            retval = DocFile.DownloadUrl(oCca.DocFile, AbsoluteUrl)
        End If
        Return retval
    End Function

    Shared Async Function LastBlockedYear(oEmp As DTOEmp, exs As List(Of Exception)) As Task(Of Integer)
        Dim iLastBlockedYear = Await [Default].EmpInteger(oEmp, DTODefault.Codis.lastBlockedCcaYea, exs)
    ...
    Shared Async Function IsBlockedYear(oEmp As DTOEmp, ByVal iYea As Integer, exs As List(Of Exception)) As Task(Of Boolean)
        Dim retval As Boolean
        Dim iLastBlockedYear = Await Cca.LastBlockedYear(oEmp, exs)
        If exs.Count = 0 Then
                ...
                If Cca.Load(oCca, exs) Then
                    For Each oCcb As DTOCcb In oCca.Items
                        If oCcb.Contact IsNot Nothing Then
                            ...
                            Else
                                If Not User.IsAllowedToRead(oBrowser, oCcb.Contact) Then
