Public Class BookFra
    ...
    Shared Async Function FindOrNew(exs As List(Of Exception), oCca As DTOCca) As Task(Of DTOBookFra)
        Dim retval = Await BookFra.Find(exs, oCca)
        If Cca.Load(oCca, exs) Then
...
Public Class Bookfras
    ...
    Shared Function Excel(oBookFras As List(Of DTOBookFra), sSheetName As String, sFilename As String) As MatHelper.Excel.Sheet
        Dim retval As New MatHelper.Excel.Sheet(sSheetName, sFilename)
        Bookfras.LoadExcel(retval, oBookFras)
        Return retval
    End Function

    Shared Async Function Excel(exs As List(Of Exception), oExercici As DTOExercici, Optional FchTo As Date = Nothing, Optional ShowProgress As ProgressBarHandler = Nothing) As Task(Of MatHelper.Excel.Sheet)
        Dim retval As MatHelper.Excel.Sheet = Nothing
        Dim CancelRequest As Boolean = False

        Dim items = Await Bookfras.All(exs, DTOBookFra.Modes.All, oExercici)
        If exs.Count = 0 Then
            Dim sFilename As String = DTOBookFra.Filename(oExercici, 0, FchTo)
            retval = New MatHelper.Excel.Sheet(oExercici.Year, sFilename)
            If FchTo <> Nothing Then
                items = items.Where(Function(x) x.Cca.Fch <= FchTo).ToList
            End If
            Bookfras.LoadExcel(retval, items, ShowProgress)
        End If
        Return retval
    End Function

    Shared Sub LoadExcel(ByRef oExcelSheet As MatHelper.Excel.Sheet, items As List(Of DTOBookFra), Optional ShowProgress As ProgressBarHandler = Nothing)
        ...
        With oExcelSheet
            ...
            .AddColumn("proveidor", MatHelper.Excel.Cell.NumberFormats.PlainText)
            ...
            .AddColumn("Base sujeta", MatHelper.Excel.Cell.NumberFormats.Euro)
            .AddColumn("Tipo Iva", MatHelper.Excel.Cell.NumberFormats.Decimal2Digits)
            .AddColumn("Quota Iva", MatHelper.Excel.Cell.NumberFormats.Euro)
            .AddColumn("Base exenta", MatHelper.Excel.Cell.NumberFormats.Euro)
            .AddColumn("Clau", MatHelper.Excel.Cell.NumberFormats.W50)
            .AddColumn("Iva Comunitari", MatHelper.Excel.Cell.NumberFormats.Euro)
            .AddColumn("Iva Extracomunitari", MatHelper.Excel.Cell.NumberFormats.Euro)
            .AddColumn("Total devengado", MatHelper.Excel.Cell.NumberFormats.Euro)
            .AddColumn("Base Irpf", MatHelper.Excel.Cell.NumberFormats.Euro)
            .AddColumn("Tipo Irpf", MatHelper.Excel.Cell.NumberFormats.Decimal2Digits)
            .AddColumn("Quota Irpf", MatHelper.Excel.Cell.NumberFormats.Euro)
        End With
    ...
    Shared Async Function SendToSii(oEmp As DTOEmp, entorno As DTO.Defaults.Entornos, oBookFras As List(Of DTOBookFra), ShowProgress As ProgressBarHandler, exs As List(Of Exception)) As Task(Of DTOTaskResult)
        ...
        Dim oX509Cert = Await Cert.X509Certificate2(oEmp.Org, exs)
        If exs.Count = 0 Then
            Dim BlCancelRequest As Boolean
            Dim sLabel As String = ""
            For Each oBookFra As DTOBookFra In oBookFras
                If BookFra.Load(exs, oBookFra) Then
            ...
            If AeatSii.FacturasRecibidas.Send(entorno, oBookFras, DTOTax.closest, oX509Cert, exs) Then
                ...
                For Each oBookFra As DTOBookFra In oBookFras
                    Await BookFra.LogSii(exs, oBookFra)
