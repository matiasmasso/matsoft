Public Class Csa
    ...
    Shared Async Function SaveRemesaCobrament(ByVal oCsa As DTOCsa, oUser As DTOUser, exs As List(Of Exception)) As Task(Of Boolean)
        ...
        Dim pCsa = Await Execute(Of DTOCsa, DTOCsa)(oCsa, exs, "Csas/SaveRemesaCobrament", oUser.Guid.ToString())
        If exs.Count = 0 Then
            DTOBaseGuid.CopyPropertyValues(Of DTOCsa)(pCsa, oCsa, exs)
            retval = True
        End If
...
Public Class Csas
    ...
    Shared Function Excel(oCsas As List(Of DTOCsa)) As MatHelper.Excel.Sheet
        ...
        Dim retval As New MatHelper.Excel.Sheet(sCaption, sCaption & ".xlsx")

        retval.AddColumn("remesa", MatHelper.Excel.Cell.NumberFormats.PlainText)
        retval.AddColumn("data", MatHelper.Excel.Cell.NumberFormats.DDMMYY)
        retval.AddColumn("banc", MatHelper.Excel.Cell.NumberFormats.PlainText)
        retval.AddColumn("efecte", MatHelper.Excel.Cell.NumberFormats.Integer)
        retval.AddColumn("client", MatHelper.Excel.Cell.NumberFormats.PlainText)
        retval.AddColumn("import", MatHelper.Excel.Cell.NumberFormats.Euro)
        retval.AddColumn("venciment", MatHelper.Excel.Cell.NumberFormats.DDMMYY)
