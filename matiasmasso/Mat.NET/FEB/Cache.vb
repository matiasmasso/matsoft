        ...
        For Each tableToUpdate In oCacheResponse.LastUpdates
            Select Case tableToUpdate.Table
                ...
                Case Models.ServerCache.Tables.DeptCategories
                    oCurrentCache.DeptCategories = oCacheResponse.DeptCategories
                    hasToRefreshTree = True
                Case Models.ServerCache.Tables.ProductSpares
                    oCurrentCache.ProductSpares = oCacheResponse.ProductSpares
                    hasToRefreshTree = True
    ...
    Private Shared Sub SetProductUrls(oCache As Models.ClientCache)
        ...
        For Each oBrand In oCache.Brands
            ...
            For Each oCategory In oBrand.Categories
                ...
                Dim deptSegment As DTOLangText = Nothing
                Dim oCategoryDept = oCache.DeptCategories.FirstOrDefault(Function(x) x.Category = oCategory.Guid)
                If oCategoryDept IsNot Nothing Then
                    Dim oDept = oBrand.Depts.FirstOrDefault(Function(x) x.Guid = oCategoryDept.Dept)
                    deptSegment = oCanonicalUrlSegments.FirstOrDefault(Function(x) x.Guid.Equals(oDept.Guid))
                End If

                If deptSegment Is Nothing Then
                    oCategory.UrlCanonicas = DTOUrl.Factory(brandSegment, categorySegment)
                Else
                    oCategory.UrlCanonicas = DTOUrl.Factory(brandSegment, deptSegment, categorySegment)
                End If
