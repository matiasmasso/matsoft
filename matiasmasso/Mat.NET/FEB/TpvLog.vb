Public Class TpvLog
    ...
    Shared Async Function CcaFactory(oEmp As DTOEmp, oLog As DTOTpvLog, exs As List(Of Exception)) As Task(Of DTOCca)
        ...
        Try
            ...
            Dim oTitular As DTOContact = Await TpvLog.Titular(oLog, exs)
            ...
            Dim oDeutor As DTOBanc = TpvLog.Banc(oEmp, exs)
            Dim sConcept As String = TpvLog.CcaConcepte(exs, oDeutor, oTitular, oLog)
            ...
            retval = DTOCca.Factory(DTO.GlobalVariables.Today(), oLog.User, DTOCca.CcdEnum.VisaCobros, iCdn)
            With retval
                .Ref = oLog
                .Concept = sConcept
        ...
        If retval Is Nothing Then
            Select Case oLog.Mode
                Case DTOTpvRequest.Modes.Free
                    Dim oDeutors As List(Of DTOCustomer) = Await User.GetCustomers(oLog.User, exs)
                ...
                Case DTOTpvRequest.Modes.Pdc
                    ...
                    retval = Customer.CcxOrMe(exs, oPurchaseOrder.Contact)
                Case DTOTpvRequest.Modes.Alb
                    Dim oDelivery As DTODelivery = oLog.Request
                    Delivery.Load(oDelivery, exs)
                    retval = Customer.CcxOrMe(exs, oDelivery.Customer)
    ...
    Shared Function Banc(oEmp As DTOEmp, exs As List(Of Exception)) As DTOBanc
        ...
        Dim oGuid As Guid = [Default].EmpGuidSync(oEmp, DTODefault.Codis.bancTpv, exs)
        If oGuid.Equals(Guid.Empty) Then
            exs.Add(New Exception("Falta configurar el banc del Tpv"))
        Else
            retval = FEB.Banc.FindSync(oGuid, exs)
        End If
        Return retval
    End Function
    ...
    Shared Function Purpose(exs As List(Of Exception), oLog As DTOTpvLog) As String
        ...
        If retval = "" Then
            Select Case oLog.Mode
                ...
                Case DTOTpvRequest.Modes.Pdc
                    Dim oPurchaseOrder As DTOPurchaseOrder = oLog.Request
                    PurchaseOrder.Load(oPurchaseOrder, exs)
