...
Public Class Csbs
    ...
    Shared Async Function IndexImpagats(exs As List(Of Exception), oEmp As DTOEmp, oCustomer As DTOContact, Optional ByVal iDesdeElsDarrersMesos As Integer = 6) As Task(Of Decimal)
        Dim fchFrom As Date = DTO.GlobalVariables.Today().AddMonths(-iDesdeElsDarrersMesos)
        Dim oCsbs = Await Csbs.All(exs, oEmp, oCustomer)
    ...
    Shared Async Function ExcelPendentsDeVto(oExercici As DTOExercici, exs As List(Of Exception)) As Task(Of MatHelper.Excel.Sheet)
        Dim retval As MatHelper.Excel.Sheet = Nothing
        Dim FchTo As Date = oExercici.LastDayOrToday
        Dim items = Await Csbs.PendentsDeVto(exs, oExercici.Emp, FchTo)
        If exs.Count = 0 Then
            ...
            With retval
                .AddColumn("banc", MatHelper.Excel.Cell.NumberFormats.PlainText)
                .AddColumn("efecte", MatHelper.Excel.Cell.NumberFormats.PlainText)
                .AddColumn("venciment", MatHelper.Excel.Cell.NumberFormats.DDMMYY)
                .AddColumn("import", MatHelper.Excel.Cell.NumberFormats.Euro)
                .AddColumn("lliurat", MatHelper.Excel.Cell.NumberFormats.PlainText)
            End With
