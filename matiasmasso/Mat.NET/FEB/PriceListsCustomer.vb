...
Public Class PriceListsCustomer
    ...
    Shared Async Function ExcelCompareSheet(exs As List(Of Exception), value As DTOPricelistCustomer, Optional oLang As DTOLang = Nothing) As Task(Of MatHelper.Excel.Sheet)
        Dim retval As New MatHelper.Excel.Sheet
        Dim oDomain = DTOWebDomain.Factory(oLang)
        If PriceListCustomer.Load(exs, value) Then

            Dim currents = Await PriceListItemsCustomer.Vigent(exs, DtFch:=value.Fch)
            currents = currents.OrderBy(Function(x) x.sku.id).ToList

            Dim previous = Await PriceListItemsCustomer.Vigent(exs, DtFch:=value.Fch.AddDays(-1))
            ...
            With retval
                .addColumn("id", MatHelper.Excel.Cell.NumberFormats.W50)
                .addColumn("marca", MatHelper.Excel.Cell.NumberFormats.W50)
                .addColumn("categoria", MatHelper.Excel.Cell.NumberFormats.W50)
                .addColumn("producte", MatHelper.Excel.Cell.NumberFormats.W50)
                .addColumn("Pvp", MatHelper.Excel.Cell.NumberFormats.Euro)
                .addColumn("Pvp anterior", MatHelper.Excel.Cell.NumberFormats.Euro)
                .addColumn("diferencia", MatHelper.Excel.Cell.NumberFormats.Percent)
            End With
