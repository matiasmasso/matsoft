Public Class Delivery
    ...
    Shared Function Factory(exs As List(Of Exception), oCustomer As DTOCustomer, oUser As DTOUser, oMgz As DTOMgz) As DTODelivery
        Dim retval As DTODelivery = Nothing
        If Customer.Load(oCustomer, exs) Then
            Dim oCcx = Customer.CcxOrMe(exs, oCustomer)
            ...
            With retval
                ...
                Else
                    If Contact.Load(oCustomer.DeliveryPlatform, exs) Then
                        ...
                        .ExportCod = DTOAddress.ExportCod(Customer.ShippingAddressOrDefault(oCustomer))
                        .Tel = .Platform.Telefon
                    End If
                End If
    ...
    Shared Async Function DeliveryDate(exs As List(Of Exception), oDelivery As DTODelivery) As Task(Of Date)
        ...
        If Delivery.Load(oDelivery, exs) Then
            If oDelivery.Transmisio Is Nothing Then
                Dim oDateTimeOffset = Await Transmisio.NextTime(exs)
    ...
    Shared Async Function OrdrSps(exs As List(Of Exception), oDelivery As DTODelivery, oSender As DTOContact) As Task(Of List(Of DTOEdiOrdrsp))
        ...
        Dim fchDelivery = Await Delivery.DeliveryDate(exs, oDelivery)
    ...
    Shared Function LoadCustSkuRefs(ByRef oDelivery As DTODelivery, exs As List(Of Exception)) As Boolean
        ...
        Dim oCustomerProducts As List(Of DTOCustomerProduct) = CustomerProducts.AllSync(exs, oCustomer)
        For Each item As DTODeliveryItem In oDelivery.Items
            ...
            If oCustomerProduct Is Nothing Then
                exs.Add(New Exception("falta ref.client de " & item.Sku.RefYNomLlarg.Esp))
                retval = False
            Else
    ...
    Shared Async Function LoadEdiSkus(exs As List(Of Exception), oPncGuids As List(Of Guid)) As Task(Of List(Of DTOEdiSku))
        Dim retval = Await Api.Execute(Of List(Of Guid), List(Of DTOEdiSku))(oPncGuids, exs, "purchaseOrderItems/PncCustomSkuEans")
        Return retval
    End Function
    ...
    Shared Function DeliveryAdr(oDelivery As DTODelivery) As String
        ...
        Else
            If oDelivery.Platform.Address Is Nothing Then
                Contact.Load(oDelivery.Platform, exs)
            End If
    ...
    Shared Function PdfUrl(oDelivery As DTODelivery, Optional AbsoluteUrl As Boolean = False) As String
        Dim retval = UrlHelper.Factory(AbsoluteUrl, "doc", CInt(DTODocFile.Cods.alb), oDelivery.Guid.ToString())
        Return retval
    End Function
    ...
    Shared Function ExcelTraspas(oDelivery As DTODelivery, Optional oLang As DTOLang = Nothing) As MatHelper.Excel.Sheet
        ...
        Dim exs As New List(Of Exception)
        Contact.Load(oMgzFrom, exs)
        Contact.Load(oMgzTo, exs)
        ...
        With retval
            ...
            .AddColumn("ref M+O")
            .AddColumn("producte")
            .AddColumn(oMgzFrom.Nom, MatHelper.Excel.Cell.NumberFormats.Integer)
            .AddColumn("preu", MatHelper.Excel.Cell.NumberFormats.Euro)
            .AddColumn(oMgzTo.Nom, MatHelper.Excel.Cell.NumberFormats.Integer)
            .AddColumn("preu", MatHelper.Excel.Cell.NumberFormats.Euro)
        End With
        For Each item In oSortides
            ...
            With item
                ...
                oRow.AddCell(.Sku.Id)
                oRow.AddCell(.Sku.RefYNomLlarg.Esp, url)
                oRow.AddCell(.Qty)
    ...
    Shared Function Excel(exs As List(Of Exception), oDelivery As DTODelivery) As MatHelper.Excel.Book
        Dim retval As New MatHelper.Excel.Book(String.Format("Albarà {0}.xlsx", oDelivery.Formatted()))
        If Load(oDelivery, exs) Then
            retval.Sheets.Add(Delivery.ExcelMercancia(oDelivery))
            retval.Sheets.Add(Delivery.ExcelBultos(exs, oDelivery))
        End If
        Return retval
    End Function

    Shared Function ExcelMercancia(oDelivery As DTODelivery, Optional oLang As DTOLang = Nothing) As MatHelper.Excel.Sheet
        Dim retval As MatHelper.Excel.Sheet
        Select Case oDelivery.Cod
            Case DTOPurchaseOrder.Codis.traspas
                retval = Delivery.ExcelTraspas(oDelivery, oLang)
            Case Else
                retval = Delivery.ExcelRaw(oDelivery, oLang)
        End Select
        Return retval
    End Function

    Shared Function ExcelBultos(exs As List(Of Exception), oDelivery As DTODelivery) As MatHelper.Excel.Sheet
        ...
        If Delivery.Load(oDelivery, exs) Then
            ...
            retval = New MatHelper.Excel.Sheet("packing list", sTitle)
            With retval
                .AddColumn("pallet")
                .AddColumn(oLang.Tradueix("bulto", "bulto", "package"), MatHelper.Excel.Cell.NumberFormats.Integer)
                .AddColumn(oLang.Tradueix("matrícula", "matricula", "SSCC plate"))
                .AddColumn(oLang.Tradueix("producto", "producte", "product"))
                .AddColumn(oLang.Tradueix("unidades", "unitats", "units"), MatHelper.Excel.Cell.NumberFormats.Integer)
                .AddColumn(oLang.Tradueix("precio", "preu", "price"), MatHelper.Excel.Cell.NumberFormats.Euro)
                .AddColumn(oLang.Tradueix("descuento", "descompte", "discount"), MatHelper.Excel.Cell.NumberFormats.Percent)
                .AddColumn(oLang.Tradueix("importe", "import", "amount"), MatHelper.Excel.Cell.NumberFormats.Euro)
                .AddColumn(oLang.Tradueix("volumen unitario", "volum unitari", "unit volume"), MatHelper.Excel.Cell.NumberFormats.m3)
                .AddColumn(oLang.Tradueix("volumen", "volum", "volume"), MatHelper.Excel.Cell.NumberFormats.m3)
                .AddColumn(oLang.Tradueix("linea", "linia", "line"), MatHelper.Excel.Cell.NumberFormats.Integer)
            End With
        ...
        For Each oPallet In oDelivery.Pallets
            For Each oPackage In oPallet.Packages.OrderBy(Function(x) x.Num).ToList()
                ...
                For Each oItem In oPackage.Items
                    oRow.AddCell(oItem.DeliveryItem.Sku.RefYNomLlarg.Tradueix(oDelivery.Customer.Lang))
        ...
        For Each oPackage In oDelivery.Packages.OrderBy(Function(x) x.Num).ToList()
            For Each oItem In oPackage.Items
                ...
                oRow.AddCell(oPackage.SSCC)
                With oItem.DeliveryItem
                    oRow.AddCell(.Sku.RefYNomLlarg.Tradueix(oDelivery.Customer.Lang))
                    oRow.AddCell(oItem.QtyInPackage)
                    oRow.AddCellAmt(.Price)
                    oRow.AddCell(.Dto)
                    oRow.AddFormula("RC[-3]*RC[-2]*(100-RC[-1])/100")
    ...
    Shared Function ExcelRaw(oDelivery As DTODelivery, Optional oLang As DTOLang = Nothing, Optional valorat As Boolean = False) As MatHelper.Excel.Sheet
        ...
        With retval
            ...
            End If
            .AddColumn(oLang.Tradueix("producto", "producte", "product"))
            .AddColumn(oLang.Tradueix("cant.", "quant", "units"), MatHelper.Excel.Cell.NumberFormats.Integer)
            ...
            .AddColumn(oLang.Tradueix("uds./embalaje", "uts./embalatge", "units/package"), MatHelper.Excel.Cell.NumberFormats.Integer)
            .AddColumn(oLang.Tradueix("long.embalaje", "llong.embalatge", "package length"), MatHelper.Excel.Cell.NumberFormats.mm)
            .AddColumn(oLang.Tradueix("ancho embalaje", "ampl.embalatge", "package width"), MatHelper.Excel.Cell.NumberFormats.mm)
            .AddColumn(oLang.Tradueix("alt.embalaje", "alç.embalatge", "package height"), MatHelper.Excel.Cell.NumberFormats.mm)
            .AddColumn(oLang.Tradueix("volumen por embalaje", "volum embalatge", "package volume"), MatHelper.Excel.Cell.NumberFormats.m3)
            .AddColumn(oLang.Tradueix("volumen total", "volum total", "total volume"), MatHelper.Excel.Cell.NumberFormats.m3D2)
            .AddColumn(oLang.Tradueix("peso unitario", "pes unitari", "unit weight"), MatHelper.Excel.Cell.NumberFormats.Kg)
            .AddColumn(oLang.Tradueix("peso", "pes", "weight"), MatHelper.Excel.Cell.NumberFormats.KgD1)
            .AddColumn("made in")
        ...
        For Each oLine As DTODeliveryItem In oDelivery.Items
            ...
            With oLine
                ...
                If oDelivery.Cod = DTOPurchaseOrder.Codis.traspas Then
                    oRow.AddCell()
                Else
                    url = PurchaseOrder.Url(.PurchaseOrderItem.PurchaseOrder, True)
                    If url > "" Then
    ...
    Shared Function FullNameAddress(oDelivery As DTODelivery) As String
        Dim exs As New List(Of Exception)
        If oDelivery.Platform IsNot Nothing Then FEB.Contact.Load(oDelivery.Platform, exs)
        Dim sb As New System.Text.StringBuilder
        sb.AppendLine(DeliveryNom(oDelivery))
        sb.AppendLine(DeliveryAdr(oDelivery))
        sb.AppendLine(DTOZip.FullNom(DeliveryZip(oDelivery))) 'oDelivery.Address.Zip))
    ...
    Shared Function TotalsText(exs As List(Of Exception), oDelivery As DTODelivery) As String
        ...
        Else
            ...
            Dim Req As Boolean
            If oDelivery.Cod = DTOPurchaseOrder.Codis.client Then
                Req = Customer.CcxOrMe(exs, oDelivery.Customer).Req
            End If
            Dim oTotal = oDelivery.baseImponible.Clone
            sb.Append(DTOAmt.CurFormatted(oTotal))
    ...
    Shared Function TotalLiquid(exs As List(Of Exception), oDelivery As DTODelivery) As DTOAmt
        ...
        Select Case oDelivery.Cod
            Case DTOPurchaseOrder.Codis.proveidor
                ...
                Req = False
            Case Else
                Dim oCcx = Customer.CcxOrMe(exs, oDelivery.Customer)
                Iva = oCcx.Iva
                Req = oCcx.Req
        End Select
    ...
    Shared Function PickUpFchFrom(oDelivery As DTODelivery) As DateTimeOffset
        ...
        If Delivery.Load(oDelivery, exs) Then
            ...
            If oDelivery.Transmisio Is Nothing Then
                Dim oTask = Task.FindSync(DTOTask.Cods.VivaceTransmisio, exs)
                Task.Load(oTask, exs)
    ...
    Shared Function Abonar(exs As List(Of Exception), oDelivery As DTODelivery, oUser As DTOUser, oMgz As DTOMgz) As DTODelivery
        Dim retval As DTODelivery = Nothing
        If Delivery.Load(oDelivery, exs) Then
            retval = Delivery.Factory(exs, oDelivery.Customer, oUser, oMgz)
            With retval
    ...
    Shared Async Function CheckValidationErrors(exs As List(Of Exception), oEmp As DTOEmp, oDelivery As DTODelivery) As Task(Of Boolean)
        ...
        If oDelivery.Cod = DTOPurchaseOrder.Codis.client Then
            ...
            If oDelivery.IsNew() Then
                ...
                Dim oCcx = Customer.CcxOrMe(exs, oDelivery.Customer)
                Dim oLiquidAlbara As DTOAmt = Delivery.TotalLiquid(exs, oDelivery)
                ...
                If oDelivery.CashCod = DTOCustomer.CashCodes.credit Then
                    Dim oCreditDisponible = Await Risc.CreditDisponible(oCcx, exs)
                ...
                Dim PendentsDePagament = Await Pnds.All(exs, oEmp, oCcx, DTOPnd.Codis.Deutor, onlyPendents:=True)
                PendentsDePagament = PendentsDePagament.Where(Function(x) x.Vto < DTO.GlobalVariables.Today() And (x.Cfp = DTOPnd.FormasDePagament.reposicioFons Or x.Cfp = DTOPnd.FormasDePagament.transferencia)).ToList
    ...
    Shared Function CheckPorts(oPortsCondicio As DTOPortsCondicio, ByVal oItms As List(Of DTODeliveryItem), ByVal DcDto As Decimal, ByVal DcDpp As Decimal) As Boolean
        Dim exs As New List(Of Exception)
        PortsCondicio.Load(exs, oPortsCondicio)
...
Public Class Deliveries
    ...
    Shared Async Function Update(exs As List(Of Exception), oDeliveries As List(Of DTODelivery)) As Task(Of List(Of DTODelivery))
        For Each oDelivery In oDeliveries
            ...
            'Dim pDelivery = Await Delivery.Update(exs, oDelivery)
        Next
    ...
    Shared Async Function PendentsDeFacturar(exs As List(Of Exception), oEmp As DTOEmp, Optional oCustomer As DTOCustomer = Nothing) As Task(Of List(Of DTODelivery))
        Dim retval = Await Api.Fetch(Of List(Of DTODelivery))(exs, "deliveries/pendentsDeFacturar", oEmp.Id, OpcionalGuid(oCustomer))
        If exs.Count = 0 Then
            'salva un bug per el cual arriben els Nifs duplicats encara que de la Api surten be:
            For Each oDelivery In retval
                Dim oNifs = oDelivery.Customer?.Ccx?.Nifs
                If oNifs IsNot Nothing AndAlso oNifs.Count > 1 Then
                    If (oNifs(0).Cod = oNifs(1).Cod AndAlso oNifs(0).Value = oNifs(1).Value) Then
                        oNifs.RemoveAt(1)
                    End If
                End If
            Next
    ...
    Shared Function HaveSameCcx(exs As List(Of Exception), ByVal oDeliveries As List(Of DTODelivery)) As Boolean
        ...
        If oDeliveries.Count = 1 Then
            retval = True
        Else
            Dim oCcxs = oDeliveries.Select(Function(x) Customer.CcxOrMe(exs, x.Customer))
    ...
    Shared Async Function LlibreDeAlbarans(exs As List(Of Exception), oExercici As DTOExercici) As Task(Of MatHelper.Excel.Sheet)
        ...
        With retval
            .AddColumn("Albará", MatHelper.Excel.Cell.NumberFormats.W50)
        ...
        Dim items As List(Of DTODelivery) = Await Deliveries.All(exs, oExercici)
        If exs.Count = 0 Then
            For Each item As DTODelivery In items
                Dim oRow As MatHelper.Excel.Row = retval.AddRow
                With item
                    oRow.AddCell(.Id, Delivery.Url(item, True))
                    oRow.AddCell(.Fch)
                    oRow.AddCell(.Nom)
                    ...
                    If .Invoice Is Nothing Then
                        oRow.AddCell()
                    Else
                        oRow.AddCell(.Invoice.NumeroYSerie(), Invoice.PdfUrl(.Invoice, True))
                    End If
                End With
            Next
        End If
    ...
    Shared Async Function Delete(exs As List(Of Exception), oDeliveries As List(Of DTODelivery), Optional ShowWarnings As Boolean = True) As Task(Of Boolean)
        ...
        For Each oDelivery As DTODelivery In oDeliveries
            ...
            If Not Await Delivery.Delete(ex2, oDelivery, ShowWarnings) Then
