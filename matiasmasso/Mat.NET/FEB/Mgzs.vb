Public Class Mgz
    ...
    Shared Async Function ExcelStocks(oUser As DTOUser, oMgz As DTOMgz, exs As List(Of Exception)) As Task(Of MatHelper.Excel.Sheet)
        ...
        Else
            ...
            Dim items As List(Of DTOProductSku) = Await Mgzs.Stocks(oUser, oMgz, exs)
            If exs.Count = 0 Then
    ...
    Shared Async Function ExcelInventari(exs As List(Of Exception), oMgz As DTOMgz, oUser As DTOUser, Optional DtFch As Date = Nothing) As Task(Of MatHelper.Excel.Sheet)
        Dim retval As MatHelper.Excel.Sheet = Nothing
        If DtFch = Nothing Then DtFch = DTO.GlobalVariables.Now()
        Dim oLang As DTOLang = oUser.lang
        ...
        With retval
            ...
            .AddColumn(oLang.Tradueix("Producto", "Producte", "Product"), MatHelper.Excel.Cell.NumberFormats.W50)
            .AddColumn("Stock", MatHelper.Excel.Cell.NumberFormats.Integer)
            .AddColumn("Valor", MatHelper.Excel.Cell.NumberFormats.Euro)
            .AddColumn("Import", MatHelper.Excel.Cell.NumberFormats.Euro)
        End With

        Dim items As List(Of DTOProductSku) = Await Mgz.Inventory(exs, oMgz, oUser, DtFch)
        If exs.Count = 0 Then
            For Each item In items
                Dim oRow = retval.AddRow
                oRow.AddCell(item.Id)
