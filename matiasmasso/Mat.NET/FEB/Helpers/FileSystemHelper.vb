...
Public Class FileSystemHelper
    ...
    Shared Function GetFirstLineFromFile(oDocFile As DTODocFile) As String
    ...
    Shared Function GetLinesFromFile(oDocFile As DTODocFile) As List(Of String)
    ...
    Shared Function GetFileText(oDocFile As DTODocFile) As String
        Dim oMemStream As New IO.MemoryStream(oDocFile.Stream)
        Dim sr As IO.StreamReader = New IO.StreamReader(oMemStream)
        Dim retval As String = ""
        Try
    ...
    Shared Function GetPdfTmpFileName(Optional ByVal sFileName As String = "", Optional exs As List(Of Exception) = Nothing) As String
        ...
        Dim sTempPath As String = MatHelperStd.FileSystemHelper.TmpFolder
    ...
    Shared Sub CleanFolder(ByVal oFolder As IO.DirectoryInfo)
        ...
        For Each oFile In oFolder.GetFiles()
            If DTO.GlobalVariables.Now().Subtract(oFile.CreationTime).TotalMinutes > 15 Then
                Try
    ...
    Shared Function GetStreamFromFile(ByVal sFileName As String, ByRef oByteArray As Byte(), ByRef exs As List(Of Exception)) As Boolean
        ...
        If sFileName.EndsWith(".csv") Then
            Dim src = System.IO.File.ReadAllText(sFileName)
            oByteArray = System.Text.Encoding.UTF8.GetBytes(src)
        Else
    ...
    Shared Sub SaveZipFromString(ByVal sSrc As String, ByVal sZipFileName As String, Optional ByVal sTxtFileName As String = "")
        Dim oZip As New Ionic.Zip.ZipFile()
        oZip.AddEntry(sTxtFileName, sSrc)
        oZip.Save(sZipFileName)
    End Sub

    Shared Function SaveStringToZippedByteArray(ByVal sSrc As String, Optional ByVal sInnerTxtFileName As String = "") As System.IO.MemoryStream
        Dim oZip As New Ionic.Zip.ZipFile()
        oZip.AddEntry(sInnerTxtFileName, sSrc)
        Dim oMemStream As New System.IO.MemoryStream
        oZip.Save(oMemStream)
        Return oMemStream
    End Function
        ...
        If iPos >= 0 Then
            ...
            Select Case sUcaseExtension
                ...
                Case ".MP4"
                    retval = MimeCods.Mp4
                Case Else
                    retval = MimeHelper.GetMimeFromContent(sFileName)
            End Select
        End If
