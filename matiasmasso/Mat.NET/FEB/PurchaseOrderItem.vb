Public Class PurchaseOrderItem
    ...
    Shared Function Factory(basketline As String) As DTOPurchaseOrderItem
        ...
        Dim oBasketLine As DTOBasketLine = Basket.GetModelBasketLine(exs, basketline)
    ...
    Shared Async Function SuggestedRepCom(oEmp As DTOEmp, value As DTOPurchaseOrderItem, oRepProducts As List(Of DTORepProduct), oRepCliComs As List(Of DTORepCliCom), exs As List(Of Exception)) As Task(Of DTORepCom)
        ...
        Dim retval As DTORepCom = Await RepCom.GetRepCom(oEmp, oCustomer, oProduct, DtFch, oRepProducts, oRepCliComs, exs)
        Return retval
    End Function
    ...
    Shared Async Function SuggestedRepCom(exs As List(Of Exception), oEmp As DTOEmp, value As DTOPurchaseOrderItem, oRepProducts As List(Of DTORepProduct), oRepCliComs As List(Of DTORepCliCom)) As Task(Of DTORepCom)
        ...
        Else
            ...
            Dim oSuggestedRepCom = Await PurchaseOrderItem.SuggestedRepCom(oEmp, value, oRepProducts, oRepCliComs, exs)
    ...
    Shared Function GetDiscount(oSku As DTOProductSku, oCustomer As DTOCustomer) As Decimal
        Dim exs As New List(Of Exception)
        Dim oCliProductDtos = CliProductDtos.AllSync(oCustomer, exs)
    ...
    Shared Async Function BundleItemsFactory(exs As List(Of Exception), item As DTOPurchaseOrderItem, oEmp As DTOEmp, oCustomCosts As List(Of DTOPricelistItemCustomer), oTarifaDtos As List(Of DTOCustomerTarifaDto), oCliProductDtos As List(Of DTOCliProductDto), Optional oRepProducts As List(Of DTORepProduct) = Nothing) As Task(Of List(Of DTOPurchaseOrderItem))
        ...
        item.sku.bundleSkus = Await ProductSku.Bundles(exs, item.sku)
        If exs.Count = 0 Then
            ...
            For Each oSkuBundle As DTOSkuBundle In oSku.bundleSkus
                ...
                If oOrder.cod = DTOPurchaseOrder.Codis.client Then
                    oRepCom = Await RepCom.GetRepCom(oEmp, oOrder.customer, oSkuBundle.Sku, oOrder.fch, oRepProducts, exs:=exs)
                End If
    ...
    Shared Async Function ReasignaComisions(oEmp As DTOEmp, items As List(Of DTOPurchaseOrderItem)) As Threading.Tasks.Task
        For Each item As DTOPurchaseOrderItem In items
            If item.PurchaseOrder Is Nothing Then
                item.RepCom = Nothing
            Else
                item.RepCom = Await RepCom.GetRepCom(oEmp, item.PurchaseOrder.Contact, item.Sku, item.PurchaseOrder.Fch)
            End If
        Next
    End Function

    Shared Function Excel(items As List(Of DTOPurchaseOrderItem), Optional sCaption As String = "M+O Pendiente de entrega", Optional sFilename As String = "") As MatHelper.Excel.Sheet
        Dim exs As New List(Of Exception)
        Dim oFirstOrder As DTOPurchaseOrder = items.First.PurchaseOrder
        Dim oContact As DTOContact = oFirstOrder.Contact
        Contact.Load(oContact, exs)
        Dim oLang As DTOLang = oContact.lang
        Dim oDomain = DTOWebDomain.Factory(oLang, True)

        Dim retval As New MatHelper.Excel.Sheet(sCaption, sFilename)

        With retval
            .DisplayTotals = True
            .AddColumn(oLang.Tradueix("Pedido", "Comanda", "Order"), MatHelper.Excel.Cell.NumberFormats.PlainText)
            .AddColumn(oLang.Tradueix("Fecha", "Data", "Date"), MatHelper.Excel.Cell.NumberFormats.PlainText)
            .AddColumn(oLang.Tradueix("Concepto", "Concepte", "Subject"), MatHelper.Excel.Cell.NumberFormats.PlainText)
            .AddColumn("Sku", MatHelper.Excel.Cell.NumberFormats.PlainText)
            .AddColumn(oLang.Tradueix("Producto", "Producte", "Product"), MatHelper.Excel.Cell.NumberFormats.PlainText)
            .AddColumn(oLang.Tradueix("Cantidad", "Quantitat", "Quantity"), MatHelper.Excel.Cell.NumberFormats.Integer)
        ...
        Dim oRow As MatHelper.Excel.Row = Nothing
        If items.Count > 0 Then

            Dim oOrder As New DTOPurchaseOrder
            Dim oCell As MatHelper.Excel.Cell
            For Each item As DTOPurchaseOrderItem In items
                oRow = retval.AddRow()
                ...
                Else
                    oCell = oRow.AddCell(item.Sku.RefYNomLlarg.Esp)
                    oCell.Indent = 1
                End If
