Public Class CredencialController
    ...
    Public Function CredencialsPost1(user As DTOBaseGuid) As List(Of DUI.Credencial)
        Dim retval As List(Of DUI.Credencial) = GetCredencials1(user)
        Return retval
    End Function

    <HttpPost>
    <Route("api/credencials2")>
    Public Function CredencialsPost2(<FromBody> ByVal user As DUI.User) As List(Of DTOCompactCredencial)
        Dim retval As List(Of DTOCompactCredencial) = GetCredencials2(user)
        Return retval
    End Function

    <HttpGet>
    <Route("api/credencials2/{guid}")>
    Public Function CredencialsGet2(guid As Guid) As List(Of DTOCompactCredencial)
        Dim user As New DUI.User
        user.guid = guid
        Dim retval As New List(Of DTOCompactCredencial)
        Try
            retval = GetCredencials2(user)
    ...
    Private Function GetCredencials1(Optional user As DTOBaseGuid = Nothing) As List(Of DUI.Credencial)
    ...
    Private Function GetCredencials2(Optional user As DUI.User = Nothing) As List(Of DTOCompactCredencial)
        Dim retval As New List(Of DTOCompactCredencial)
        Dim oUser As DTOUser = BLLUser.Find(user.Guid)

        Dim oCredencials As List(Of DTOCredencial) = BLLCredencials.Headers(oUser)
        For Each oCredencial As DTOCredencial In oCredencials
            Dim item As New DTOCompactCredencial
            With item
                .guid = oCredencial.Guid
                .referencia = oCredencial.Referencia
            End With
            retval.Add(item)
        Next
