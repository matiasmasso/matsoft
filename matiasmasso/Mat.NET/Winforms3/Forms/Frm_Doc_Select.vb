...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Me.ButtonCopyLink.Image = Global.Mat.Net.My.Resources.Resources.Copy
...
#End Region
    ...
    Private Async Function Incidencias() As Task(Of List(Of DTOIncidencia))
        ...
        For Each itm In NumList()
            Dim oIncidencia = Await FEB.Incidencia.FromNum(exs, GlobalVariables.Emp, itm)
    ...
    Private Async Function PurchaseOrders(exs As List(Of Exception)) As Task(Of List(Of DTOPurchaseOrder))
        ...
        For Each itm In NumList()
            Dim oOrder = Await FEB.PurchaseOrder.FromNum(Current.Session.Emp, iYea, itm, exs)
                ...
                Select Case IntGuion
                    Case Is > 0
                        ...
                        For tmp = FirstItm To LastItm
                            Dim oDelivery = Await FEB.Delivery.FromNum(Current.Session.Emp, iYea, tmp, exs)
                        ...
                        Next
                    Case Else
                        Dim oDelivery = Await FEB.Delivery.FromNum(Current.Session.Emp, iYea, itm, exs)
                        If oDelivery Is Nothing Then
        ...
        For Each itm In NumList()
            ...
            Select Case IntGuion
                Case Is > 0
                    ...
                    For tmp = FirstItm To LastItm
                        Dim oInvoice = Await FEB.Invoice.FromNum(exs, Current.Session.Emp, iYea, tmp)
                        If exs.Count = 0 Then
        ...
        For Each itm In NumList()
            ...
            Select Case IntGuion
                Case Is > 0
                    ...
                    LastItm = itm.Substring(IntGuion + 1)
                    For tmp = FirstItm To LastItm
                        oCca = Await FEB.Cca.FromNum(Current.Session.Emp, ComboBoxYea.Text, tmp, exs)
                    ...
                    Next
                Case Else
                    oCca = Await FEB.Cca.FromNum(Current.Session.Emp, ComboBoxYea.Text, itm, exs)
                    If oCca Is Nothing Then
    ...
    Private Function NumList() As List(Of String)
                    ...
                    If oDeliveries.Count > 0 Then
                        ...
                        Select Case oDelivery.Cod
                            ...
                            Case DTOPurchaseOrder.Codis.Proveidor
                                ...
                                If Await FEB.AlbBloqueig.BloqueigStart(Current.Session.User, oProveidor, DTOAlbBloqueig.Codis.ALB, exs) Then
                                    Dim oFrm As New Frm_AlbNew2(oDelivery)
                                    oFrm.Show()
                                Else
                                    UIHelper.WarnError(exs)
        ...
        Select Case mStyle
            ...
            Case Styles.Factura
                ...
                For Each oInvoice As DTOInvoice In Await Invoices(exs)
                    FEB.Invoice.Load(oInvoice, exs)
                    Dim sDisplayText As String = "factura " & DTOInvoice.FormattedId(oInvoice)
                    Dim sUrl = FEB.Invoice.PdfUrl(oInvoice, True)
                ...
                If exs.Count = 0 Then
                    ...
                    For Each oCca As DTOCca In oCcas
                        If FEB.Cca.Load(oCca, exs) Then
                            ...
                            Dim sUrl As String = FEB.DocFile.DownloadUrl(oCca.DocFile, True)
                            Dim oRow As MatHelper.Excel.Row = oSheet.AddRow()
                            oRow.AddCell(sDisplayText, sUrl)
                        Else
                            UIHelper.WarnError(exs)
