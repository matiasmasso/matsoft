Public Class Frm_Auditoria
    ...
    Private Sub LoadDefaultYear()
        ...
        For i As Integer = DTO.GlobalVariables.Today().Year To 1985 Step -1
            iYears.Add(i)
        Next
        Xl_Years1.Load(iYears, DTO.GlobalVariables.Today().Year - 1)
    End Sub
    ...
    Private Async Function DoTask() As Task
        ...
        Select Case oTask
            Case tasks.llibreDiariExcel
                ...
                Dim oSheet = Await FEB.LlibreDiari.Excel(exs, oExercici, Current.Session.Lang)
            ...
            Case tasks.llibreMajorDeComptesExcel
                ProgressBar1.Visible = True
                Dim oSheet = Await FEB.LlibreMajor.Excel(exs, oExercici, Current.Session.Lang)
                If UIHelper.ShowExcel(oSheet, exs) Then
                    ProgressBar1.Visible = False
                Else
                    ProgressBar1.Visible = False
            ...
            Case tasks.llibreMajorDeComptesExcelNextYear1T
                Dim oSheet = Await FEB.LlibreMajor.Excel(exs, oExercici.Next, Current.Session.Lang)
                If UIHelper.ShowExcel(oSheet, exs) Then
                    ProgressBar1.Visible = False
                Else
                    ProgressBar1.Visible = False
            ...
            Case tasks.llibreFacturesRebudes
                ProgressBar1.Visible = True
                Dim oSheet = Await FEB.Bookfras.Excel(exs, oExercici)
                If exs.Count = 0 Then
                    If UIHelper.ShowExcel(oSheet, exs) Then
                        ProgressBar1.Visible = False
                    Else
            ...
            Case tasks.llibreFacturesEmesesNextYearFirstQ
                ...
                Dim oSheet = Await FEB.Invoices.LlibreDeFactures(exs, oNextExercici, DtFchTo)
                If exs.Count = 0 Then
                    If UIHelper.ShowExcel(oSheet, exs) Then
                        ProgressBar1.Visible = False
                    Else
            ...
            Case tasks.llibreFacturesRebudesNextYearFirstQ
                ...
                Dim oNextExercici As DTOExercici = oExercici.Next
                Dim DtFchTo As Date = oExercici.LastFch1stQuarterNextYear
                Dim oSheet = Await FEB.Bookfras.Excel(exs, oNextExercici, DtFchTo)
                If exs.Count = 0 Then
                    If UIHelper.ShowExcel(oSheet, exs) Then
                        ProgressBar1.Visible = False
                    Else
            ...
            Case tasks.llibreFacturesIntracomunitaries
                ProgressBar1.Visible = True
                Dim values = Await FEB.Bookfras.All(exs, DTOBookFra.Modes.All, oExercici)
                If exs.Count = 0 Then
                    values = values.Where(Function(x) x.ClaveExenta = "E5").ToList
                    Dim oSheet = FEB.Bookfras.Excel(values, oExercici.Year, "M+O facturas intracomunitarias")
                    ProgressBar1.Visible = False
                    If UIHelper.ShowExcel(oSheet, exs) Then
                        ProgressBar1.Visible = False
                    Else
            ...
            Case tasks.sumesIsaldos4digits
                ProgressBar1.Visible = True
                Dim value = Await FEB.SumasYSaldos.All(exs, oExercici.Emp, oExercici.LastDayOrToday)
                If exs.Count = 0 Then
            ...
            Case tasks.sumesISaldos4digitsNextYear1T
                ...
                Dim value = Await FEB.SumasYSaldos.All(exs, oExercici.Emp, FchTo)
                If exs.Count = 0 Then
                    Dim oSheet = DTOSumasYSaldos.Excel(Current.Session.Emp, value, DTOPgcCta.Digits.Digits4, Nothing)
                    If UIHelper.ShowExcel(oSheet, exs) Then
                        ProgressBar1.Visible = False
            ...
            Case tasks.llibreAlbarans
                ProgressBar1.Visible = True
                Dim oSheet = Await FEB.Deliveries.LlibreDeAlbarans(exs, oExercici)
                If exs.Count = 0 Then
                    If UIHelper.ShowExcel(oSheet, exs) Then
                        ProgressBar1.Visible = False
                    Else
            ...
            Case tasks.Garanties
                ProgressBar1.Visible = True
                Dim oSheet As MatHelper.Excel.Sheet = Await FEB.Garantias.Excel(exs, oExercici)
                If UIHelper.ShowExcel(oSheet, exs) Then
                    ProgressBar1.Visible = False
                Else
                    ProgressBar1.Visible = False
            ...
            Case tasks.efectesEnCirculacio
                ...
                Dim oSheet = Await FEB.Csbs.ExcelPendentsDeVto(oExercici, exs)
                If UIHelper.ShowExcel(oSheet, exs) Then
                    ProgressBar1.Visible = False
                Else
                    ProgressBar1.Visible = False
            ...
            Case tasks.StaffCategoriaSex
                ...
                If exs.Count = 0 Then
                    Dim oSheet As MatHelper.Excel.Sheet = DTOStaff.Excel(oStaffs, Current.Session.Lang, oExercici)
                    If UIHelper.ShowExcel(oSheet, exs) Then
                        ProgressBar1.Visible = False
                    Else
                        ProgressBar1.Visible = False
            ...
            Case tasks.LlibreNomines
                ProgressBar1.Visible = True
                Dim oSheet As MatHelper.Excel.Sheet = Await FEB.Nominas.Llibre(oExercici, exs)
                If UIHelper.ShowExcel(oSheet, exs) Then
                    ProgressBar1.Visible = False
                Else
                    ProgressBar1.Visible = False
            ...
            Case tasks.Models145
                ProgressBar1.Visible = True
                Dim oSheet = Await FEB.ContactDocs.ExcelMod145s(oExercici, exs)
                If exs.Count = 0 Then
                    If UIHelper.ShowExcel(oSheet, exs) Then
                        ProgressBar1.Visible = False
                    Else
            ...
            Case tasks.llibreContractes
                ProgressBar1.Visible = True
                Dim oContracts As List(Of DTOContract) = Await FEB.Contracts.All(exs, Current.Session.User)
    ...
    Private Async Function Get10UltimsAlbs(exs As List(Of Exception), oExercici As DTOExercici, oCod As DTOPurchaseOrder.Codis) As Task(Of List(Of DTODelivery))
        ...
        Dim oDeliveries = Await FEB.Deliveries.All(exs, oExercici)
        If exs.Count = 0 Then
    ...
    Private Sub ShowExcelFromAlbs(oDeliveries As List(Of DTODelivery))
        ...
        With oSheet
            .AddColumn("albarà", MatHelper.Excel.Cell.NumberFormats.W50)
            ...
            If oDelivery.Cod = DTOPurchaseOrder.Codis.client Then
                ...
                If oInvoice IsNot Nothing Then
                    ...
                    If oCca Is Nothing Then
                        ...
                        oRow.AddCell()
                    Else
                        oRow.AddCell(oInvoice.NumeroYSerie(), FEB.DocFile.DownloadUrl(oCca.DocFile, True))
                        oRow.AddCell(oInvoice.Fch)
    ...
    Private Sub CarteraDeDeutorsYCreditors2()
        Dim oSheet As MatHelper.Excel.Sheet = Nothing ' DTOPnd.ExcelCarteraDeDeutorsYCreditors(CurrentExercici)
    ...
    Private Async Function CarteraDeDeutorsYCreditors() As Task
        ...
        Dim oBook As New MatHelper.Excel.Book("M+O Cartera de deutors i creditors " & oExercici.Year - 1)
        Dim exs As New List(Of Exception)
        oBook.Sheets.Add(Await FEB.Pnds.CarteraDeDeutorsYCreditors(exs, oExercici.Emp, DTOPnd.Codis.Creditor, DtFch))
        oBook.Sheets.Add(Await FEB.Pnds.CarteraDeDeutorsYCreditors(exs, oExercici.Emp, DTOPnd.Codis.Deutor, DtFch))
        ProgressBar1.Visible = False
