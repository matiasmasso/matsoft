...
Public Class Frm_Cca
    ...
    Private Async Sub Frm_Cca_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        _Ctas = Await FEB.PgcCtas.All(exs, _Cca.Fch.Year)
        If exs.Count = 0 Then
            If FEB.Cca.Load(_Cca, exs) Then
                mFchOriginal = _Cca.Fch
                mLastBlockedCcaYea = Await FEB.Default.EmpInteger(Current.Session.Emp, DTODefault.Codis.LastBlockedCcaYea, exs)
                If exs.Count = 0 Then
                    _AllowEvents = True
                    If AllowBrowse(_Cca) Then
                        ...
                        End If
                    Else
                        Await FEB.MailMessage.MailAdmin(My.User.Name & " BROWSE ASSENTAMENT SENSIBLE " & Year(_Cca.fch) & "/" & _Cca.id)
    ...
    Private Async Function Display(oCca As DTOCca, exs As List(Of Exception)) As Task(Of Boolean)
        With oCca
            ...
            Else
                ...
                Dim isBlockedYear As Boolean = Await FEB.Cca.IsBlockedYear(GlobalVariables.Emp, .Fch.Year, exs)
            ...
            If .docFile IsNot Nothing Then
                .DocFile.Nom = .Concept
                .DocFile.Filename = "assentament " & oCca.formattedId & MimeHelper.GetExtensionFromMime(.DocFile.Mime)
            End If
    ...
    Private Async Function CheckIvaFch() As Task(Of Boolean)
        ...
        Dim fchUltimaDeclaracio = Await FEB.Cca.IvaFchUltimaDeclaracio(exs, Current.Session.Emp)
        If exs.Count = 0 Then
            If _Cca.IsNew Then
    ...
    Private Function GetContactFromRow(ByVal oRow As DataGridViewRow) As DTOContact
        ...
        If Not IsDBNull(oRow.Cells(Cols.ContactGuid).Value) And Not IsDBNull(oRow.Cells(Cols.Clx).Value) Then
    ...
    Private Async Sub ButtonDel_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles ButtonDel.Click
        ...
        If rc = MsgBoxResult.Ok Then
            Dim exs As New List(Of Exception)
            If Await FEB.Cca.Delete(exs, _Cca) Then
                RaiseEvent AfterUpdate(Me, New MatEventArgs(_Cca))
    ...
    Private Sub ToolStripButtonExport_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles ToolStripButtonExport.Click
        Dim oSheet = MatHelper.Excel.Sheet.Factory(mDs)
        Dim exs As New List(Of Exception)
