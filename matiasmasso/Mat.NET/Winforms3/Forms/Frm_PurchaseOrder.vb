Public Class Frm_PurchaseOrder
    ...
    Private Async Sub Frm_PurchaseOrder_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If _PurchaseOrder.IsNew Then
            If Not FEB.Customer.ValidateCustomer(_PurchaseOrder.Customer, exs) Then
                UIHelper.WarnError(exs, "Error o Fitxa de client incomplerta:")
                Me.Close()
            End If
        Else
            If Not FEB.PurchaseOrder.Load(exs, _PurchaseOrder, GlobalVariables.Emp.Mgz) Then
                UIHelper.WarnError(exs)
            End If
        End If

        If _PurchaseOrder.Cod <> DTOPurchaseOrder.Codis.proveidor Then

            _Ccx = _PurchaseOrder.Customer.CcxOrMe
            Xl_PurchaseOrderItems1.CustomerTarifaDtos = Await FEB.CustomerTarifaDtos.Active(exs, _Ccx)
            Xl_PurchaseOrderItems1.CliProductDtos = Await FEB.CliProductDtos.All(_Ccx, exs)
            Xl_PurchaseOrderItems1.CustomCosts = Await FEB.PriceListItemsCustomer.Active(exs, _Ccx, DateTimePicker1.Value)
        End If
        ...
        Dim oMenuItem As New ToolStripMenuItem("Conceptes frequents", Nothing, AddressOf ShowShortcut)
        ArxiuToolStripMenuItem.DropDownItems.Add(oMenuItem)
    ...
    Private Sub ShowShortcut()
        ...
        If _PurchaseOrder.Contact IsNot Nothing AndAlso _PurchaseOrder.Contact.Lang IsNot Nothing Then
            oLang = _PurchaseOrder.Contact.Lang
        End If
        Dim oFrm As New Frm_PurchaseOrderConceptShortcuts(oLang, , Defaults.SelectionModes.selection)
        AddHandler oFrm.itemSelected, AddressOf ConceptShortcutSelected
        oFrm.Show()
    End Sub

    Private Sub ConceptShortcutSelected(sender As Object, e As MatEventArgs)
        Dim oShortcut As DTOPurchaseOrder.ConceptShortcut = e.Argument
        Dim oLang = DTOLang.ESP
        If _PurchaseOrder.Contact IsNot Nothing AndAlso _PurchaseOrder.Contact.Lang IsNot Nothing Then
            oLang = _PurchaseOrder.Contact.Lang
        End If
        TextBoxConcept.Text = oShortcut.Concept.Tradueix(oLang)
    End Sub
    ...
    Private Async Function refresca() As Task
        ...
        With _PurchaseOrder
            ...
            Dim oContactMenu = Await FEB.ContactMenu.Find(exs, .contact)
            TextBoxConcept.BackColor = oContactMenu.BackColor()
            ...
            If Not .IsNew Then
                Dim oDeliveryItems = Await FEB.DeliveryItems.All(exs, _PurchaseOrder)
                If exs.Count = 0 Then
                    ButtonDel.Enabled = True ' oDeliveryItems.Count > 0
                Else
                    UIHelper.WarnError(exs)
                End If
            End If
            ...
            RefrescaFchMin()
            _FchMax = .FchDeliveryMax
            RefrescaFchMax()

            RefrescaStatusBar()

            'With _Ccx
            '.TarifaDtos = Await FEB.CustomerTarifaDtos.Active(exs, _Ccx)
            '.ProductDtos = Await FEB.CliProductDtos.All(_Ccx, exs)
            'End With
            ...
            Dim oMenuClient As New Menu_Contact(.contact, oContactMenu)
    ...
    Private Sub RefrescaFchMax()
        If _FchMax = Date.MinValue Then
            ToolStripButtonFchMax.Text = "sense data tope"
            ToolStripButtonFchMax.Checked = False
        Else
            ToolStripButtonFchMax.Text = "data tope " & _FchMax.ToShortDateString
            ToolStripButtonFchMax.Checked = True
        End If
    End Sub
    ...
    Private Async Function ValidateCreditEmail(exs As List(Of Exception)) As Task(Of Boolean)
        If _Ccx.CashCod = DTOCustomer.CashCodes.credit Then
            If _Ccx.FraPrintMode = DTOCustomer.FraPrintModes.Email Then
                If Not Await FEB.Customer.EFrasEnabled(exs, _Ccx) Then
    ...
    Private Sub Frm_PurchaseOrder_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
        Dim exs As New List(Of Exception)
        If Not FEB.AlbBloqueig.BloqueigEnd(Current.Session.User, _PurchaseOrder.Contact, DTOAlbBloqueig.Codis.PDC, exs) Then
            e.Cancel = True
            UIHelper.WarnError(exs)
        End If
    End Sub
    ...
    Private Async Function Save(oPurchaseOrder As DTOPurchaseOrder, exs As List(Of Exception)) As Task(Of Boolean)
        If ReadFromForm(exs) Then
            If exs.Count = 0 Then
                Dim pPurchaseOrder = Await FEB.PurchaseOrder.Update(exs, oPurchaseOrder)
                If exs.Count = 0 Then
    ...
    Private Async Sub Do_NewAlb()
        ...
        If Await FEB.AlbBloqueig.BloqueigStart(Current.Session.User, oCustomer, DTOAlbBloqueig.Codis.ALB, exs) Then
            ...
            Dim oDelivery As DTODelivery = FEB.Delivery.Factory(exs, _PurchaseOrder.Customer, Current.Session.User, GlobalVariables.Emp.Mgz)
            If exs.Count = 0 Then
    ...
    Private Async Sub ButtonDel_Click(sender As Object, e As EventArgs) Handles ButtonDel.Click
        ...
        If rc = MsgBoxResult.Ok Then
            Dim exs As New List(Of Exception)
            If Await FEB.PurchaseOrder.Delete(exs, _PurchaseOrder) Then
        ...
        If _DirtyPdd Then
            ...
            Select Case e.KeyCode
                Case Keys.Return, Keys.Tab
                    Dim oPdd = Await FEB.PurchaseOrder.SearchConcepte(exs, s)
                    If exs.Count = 0 Then
    ...
    Private Sub TextBoxStatus_DoubleClick(sender As Object, e As EventArgs) Handles TextBoxStatus.DoubleClick
        ...
        If FEB.User.IsAllowedToRead(Current.Session.User, oUser) Then
            ...
            Select Case .Checked
                ...
                Case True
                    ...
                    If _ViewSortides Is Nothing Then
                        Dim items = Await FEB.DeliveryItems.All(exs, _PurchaseOrder)
                        If exs.Count = 0 Then
    ...
    Private Sub ToolStripButtonFchMax_Click(sender As Object, e As EventArgs) Handles ToolStripButtonFchMax.Click
        Dim oFrm As New Frm_Date(_FchMax)
        AddHandler oFrm.AfterUpdate, AddressOf onFchMaxChanged
        oFrm.Show()
    End Sub
    ...
    Private Sub onFchMaxChanged(sender As Object, e As MatEventArgs)
        _FchMax = e.Argument
        RefrescaFchMax()
        If Not _PurchaseOrder.IsNew Then SetDirty()
    End Sub
    ...
    Private Sub ButtonExcel_Click(sender As Object, e As EventArgs) Handles ButtonExcel.Click
        Dim exs As New List(Of Exception)
        If ReadFromForm(exs) Then
            Dim oSheet = FEB.PurchaseOrder.Excel(_PurchaseOrder)
