Public Class Frm_PdcFchs
    ...
    Private Async Sub Frm_PdcFchs_Load(sender As Object, e As EventArgs) Handles Me.Load
        DateTimePickerTo.Value = DTO.GlobalVariables.Today()
        DateTimePickerFrom.Value = DTO.GlobalVariables.Today().AddMonths(-3)
    ...
    Private Async Function refrescaSources() As Task
        ...
        _allOrders = Await FEB.PurchaseOrders.Headers(exs, Current.Session.Emp, Cod:=DTOPurchaseOrder.Codis.Client, FchCreatedFrom:=DateTimePickerFrom.Value, FchCreatedTo:=DateTimePickerTo.Value)
        If exs.Count = 0 Then
