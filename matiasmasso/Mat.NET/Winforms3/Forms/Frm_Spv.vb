...
Public Class Frm_Spv
    ...
    Private Sub Frm_Spv_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If _Spv.IsNew Then
            ...
            If _Spv.Incidencia IsNot Nothing Then
                Me.Text = Me.Text & " per la incidencia " & _Spv.incidencia.AsinOrNum()
            End If
        Else
            If FEB.Spv.Load(_Spv, exs) Then
                Me.Text = "Full de reparació num." & _Spv.Id.ToString
                If _Spv.Incidencia IsNot Nothing Then
                    Me.Text = Me.Text & " (incidencia " & _Spv.incidencia.AsinOrNum() & ")"
                End If
    ...
    Private Async Sub Refresca()
        ...
        With _Spv
            ...
            Else
                ...
                Xl_Adr1.Load(FEB.Customer.ShippingAddressOrDefault(_Spv.Customer))
            End If
            ...
            Else
                ...
                Dim oSpvs = Await FEB.Spvs.All(exs, .SpvIn)
                If exs.Count = 0 Then
                    Xl_Spvs1.Load(oSpvs)
    ...
    Private Async Function LoadEmailsForLabel() As Task
        Dim exs As New List(Of Exception)
        Dim oUsers = Await FEB.Users.All(exs, _Spv.Customer)
        If exs.Count = 0 Then
    ...
    Private Async Sub ButtonOk_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonOk.Click
        ...
        If FEB.Contact.Load(Xl_Contact1.Customer, exs) Then
            With _Spv
                .Nom = TextBoxNom.Text
                If CheckBoxAdr.Checked Then
                    .Address = Xl_Adr1.Address
                Else
                    .Address = FEB.Customer.ShippingAddressOrDefault(Xl_Contact1.Customer)
                End If
                ...
                If CheckBoxRead.Checked Then
                    If .FchRead > Date.MinValue Then
                    Else
                        .fchRead = DTO.GlobalVariables.Today()
                    End If
                Else
                ...
                If mDirtyOutSpvIn Then
                    ...
                    .fchOutOfSpvIn = DTO.GlobalVariables.Now()
                    If CheckBoxOutSpvIn.Checked Then
                        .SpvIn = Nothing
                        .ObsOutOfSpvIn = TextBoxObsOutOfSpvIn.Text
                    Else
                        .usrOutOfSpvIn = Nothing
                        .fchOutOfSpvIn = Nothing
                        .obsOutOfSpvIn = ""
                ...
                If mDirtyOutSpvOut Then
                    If CheckBoxOutSpvOut.Checked Then
                        ...
                        .fchOutOfSpvOut = DTO.GlobalVariables.Now()
                    ...
                    If .Delivery IsNot Nothing Then
                        With .Delivery
                            ...
                            If CheckBoxFacturable.Checked Then
                                .facturable = True
                                .cashCod = FEB.Customer.CcxOrMe(exs, .customer).cashCod
                            End If
                            SetAmtsToAlb()

                        End With
                        exs = New List(Of Exception)
                        Dim id As Integer = Await FEB.Delivery.Update(exs, .Delivery)
                        If exs.Count = 0 Then
                            .Delivery.id = id
                        Else
                ...
                exs = New List(Of Exception)
                Dim oSpv = Await FEB.Spv.Update(_Spv, exs)
                If exs.Count = 0 Then
                    _Spv = oSpv
                    If _Spv.Incidencia IsNot Nothing Then
                        Dim oIncidencia = _Spv.Incidencia
                        If FEB.Incidencia.Load(exs, oIncidencia) Then
                            With oIncidencia
                                .Description = .Description & vbCrLf & "reparacion " & _Spv.Id
                                .Spv = _Spv
                                .UsrLog.usrLastEdited = Current.Session.User
                            End With
                            oIncidencia = Await FEB.Incidencia.Update(exs, oIncidencia)
                            If exs.Count = 0 Then
                            Else
    ...
    Private Async Sub ButtonDel_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles ButtonDel.Click
        ...
        If rc = MsgBoxResult.Ok Then
            Dim exs As New List(Of Exception)
            If Await FEB.Spv.Delete(_Spv, exs) Then
