Public Class Frm_CreateContact
    ...
    Private Async Sub Frm__Wizard_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        _Incoterms = Await FEB.Incoterms.All(exs)
        ...
        Select Case oCallingStep.WizardStep
            Case WizardSteps.Channel
                ...
                If oContactClass.DistributionChannel Is Nothing Then
                    ...
                    End If
                Else
                    _Contact = FEB.Customer.Factory(exs, _Contact)
                    If exs.Count = 0 Then
                        LoadStepCustomer()
                    Else
    ...
    Private Async Sub Finish() Handles MyBase.requestToFinish
        ...
        Select Case _WizardMode
            Case WizardSteps.Standard
                ...
                Dim id As Integer = Await FEB.Contact.Update(exs, _Contact)
            ...
            Case WizardSteps.Customer
                ...
                With oCustomer
                    ...
                    .TarifaDtos = Await FEB.CustomerTarifaDtos.Active(exs, oCustomer.ContactClass.DistributionChannel, DTO.GlobalVariables.Today())
                    ...
                    If oCustomer.Address.Zip IsNot Nothing Then
                        .PortsCondicions = DTOPortsCondicio.Default(oCustomer)
                    End If
                End With

                UIHelper.ToggleProggressBar(Panel1, True)
                Dim id = Await FEB.Contact.Update(exs, oCustomer)
                UIHelper.ToggleProggressBar(Panel1, False)
                If exs.Count = 0 Then
                    _Contact.id = id
                    Dim success = Await FEB.Customer.Update(exs, oCustomer)
                    onUpdate(success)
                Else
                    UIHelper.WarnError(exs)
                End If
            ...
            Case WizardSteps.Proveidor
                ...
                UIHelper.ToggleProggressBar(Panel1, True)
                Dim id = Await FEB.Contact.Update(exs, oProveidor)
                UIHelper.ToggleProggressBar(Panel1, False)
                If exs.Count = 0 Then
                    _Contact.id = id
                    onUpdate(Await FEB.Proveidor.Update(oProveidor, exs))
                Else
                    UIHelper.WarnError(exs)
                End If
            ...
            Case WizardSteps.Staff
                ...
                UIHelper.ToggleProggressBar(Panel1, True)
                Dim id = Await FEB.Contact.Update(exs, oStaff)
                UIHelper.ToggleProggressBar(Panel1, False)
                If exs.Count Then
                    _Contact.id = id
                    onUpdate(Await FEB.Staff.Update(exs, oStaff))
                Else
                    UIHelper.WarnError(exs)
                End If
            ...
            Case WizardSteps.Rep
                ...
                UIHelper.ToggleProggressBar(Panel1, True)
                Dim id = Await FEB.Contact.Update(exs, oRep)
                UIHelper.ToggleProggressBar(Panel1, False)
                If exs.Count = 0 Then
                    _Contact.id = id
                    onUpdate(Await FEB.Rep.Update(exs, oRep))
                Else
                    UIHelper.WarnError(exs)
                End If
    ...
    Private Async Sub LoadStepChannel()
        Dim exs As New List(Of Exception)
        Dim oValues = Await FEB.ContactClasses.All(exs)
        If exs.Count = 0 Then
    ...
    Public Async Function NotifySubscriptors(exs As List(Of Exception)) As Task(Of Boolean)
        ...
        Dim oRecipients = Await FEB.Subscriptors.Recipients(exs, GlobalVariables.Emp, DTOSubscription.Wellknowns.AltaClient)
        If exs.Count = 0 AndAlso oRecipients.Count > 0 Then
            ...
            retval = Await FEB.MailMessage.Send(exs, Current.Session.User, oMailMessage)
        End If
        Return retval
    End Function
