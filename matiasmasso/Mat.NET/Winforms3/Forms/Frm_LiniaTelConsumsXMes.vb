...
Public Class Frm_LiniaTelConsumsXMes
    ...
    Private Async Sub Frm_LiniaTelConsumsXMes_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        _Consums = Await FEB.LiniaTelefonConsums.All(exs, _LiniaTelefon)
        If exs.Count = 0 Then
            SetFch()
            LoadView()
        Else
    ...
    Private Sub SetFch()
        Dim LastMonth As Date = DTO.GlobalVariables.Today().AddMonths(-1)
    ...
    Private Async Sub UpdateConsum(oLinia As DTOLiniaTelefon, oDocFile As DTODocFile)
        ...
        Dim exs As New List(Of Exception)
        If Await FEB.LiniaTelefonConsum.Update(oConsum, exs) Then
    ...
    Private Async Sub Delete()
        ...
        If rc = MsgBoxResult.Ok Then
            Dim exs As New List(Of Exception)
            If Await FEB.LiniaTelefonConsum.Delete(oConsum, exs) Then
