    ...
    Private Async Function refresca() As Task
        ...
        Select Case _Src
            Case DTOContent.Srcs.Blog
                Dim oPosts = Await FEB.BlogPosts.All(exs)
                If exs.Count = 0 Then
            ...
            Case DTOContent.Srcs.Content
                Dim oPosts = Await FEB.Contents.All(exs)
                If exs.Count = 0 Then
                    Xl_Noticias1.Load(oPosts, _Src, _SelectionMode)
                Else
                    UIHelper.WarnError(exs)
                End If
            Case Else
                Dim oNoticias = Await FEB.Noticias.Headers(exs, _Src)
                If exs.Count = 0 Then
    ...
    Private Sub Xl_Noticias1_RequestToAddNew(sender As Object, e As MatEventArgs) Handles Xl_Noticias1.RequestToAddNew
        Select Case _Src
            ...
            Case DTOContent.Srcs.SabiasQue
                ...
                oFrm.Show()
            Case DTOContent.Srcs.Blog
                Dim oBlogPost = DTOBlogPost.Factory(Current.Session.User)
