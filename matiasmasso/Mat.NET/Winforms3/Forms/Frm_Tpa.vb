Public Class Frm_Tpa
    ...
    Private Async Sub Frm_Tpa_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If FEB.ProductBrand.Load(_Brand, exs, IncludeImages:=True) Then
            Await refresca()
    ...
    Private Async Function refresca() As Task
        With _Brand
            ...
            If .obsoleto Then
                RadioButtonObsoleto.Checked = True
            ElseIf .EnLiquidacio Then
                RadioButtonEnLiquidacio.Checked = True
            Else
                RadioButtonActiva.Checked = True
            End If
    ...
    Private Async Sub ButtonOk_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles ButtonOk.Click
        With _Brand
            ...
            .EnLiquidacio = RadioButtonEnLiquidacio.Checked
            .obsoleto = RadioButtonObsoleto.Checked
        ...
        If Await FEB.ProductBrand.Update(_Brand, exs) Then
    ...
    Private Async Sub ButtonDel_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles ButtonDel.Click
        ...
        If rc = MsgBoxResult.Ok Then
            Dim exs As New List(Of Exception)
            If Await FEB.ProductBrand.Delete(_Brand, exs) Then
    ...
    Private Sub Xl_Image_Logo_AfterUpdate(ByVal sender As Object, ByVal e As System.EventArgs) Handles Xl_Image_Logo.AfterUpdate
        ...
        If BlEnableButtons Then
            _Brand.Logo = Xl_Image_Logo.Bitmap.Bytes
            Xl_Image_Logo.Bitmap = LegacyHelper.ImageHelper.GetThumbnailToFit(Xl_Image_Logo.Bitmap, Xl_Image_Logo.Width, Xl_Image_Logo.Height)
            SetDirty()
        End If
    End Sub
    ...
    Private Sub ControlChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles _
     RadioButtonActiva.CheckedChanged,
     RadioButtonEnLiquidacio.CheckedChanged,
     RadioButtonObsoleto.CheckedChanged,
...
#Region "Reps"
    ...
    Private Async Function LoadGridReps() As Task
        Dim exs As New List(Of Exception)
        Dim oRepProducts = Await FEB.RepProducts.All(exs, _Brand, True)
        If exs.Count = 0 Then
    ...
    Private Async Sub ButtonMailTpaReps_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonMailTpaReps.Click
        Dim exs As New List(Of Exception)
        Dim oEmails = Await FEB.Reps.Emails(exs, _Brand)
        If exs.Count = 0 Then
...
#End Region
    ...
    Private Async Sub LoadStps(sender As Object, e As MatEventArgs)
        Dim exs As New List(Of Exception)
        Dim oCategories = Await FEB.ProductCategories.All(exs, _Brand, IncludeObsolets:=True)
        If exs.Count = 0 Then
    ...
    Private Async Function LoadPlugins() As Task
        Dim exs As New List(Of Exception)
        Dim oPlugins = Await FEB.ProductPlugins.All(exs, _Brand)
        If exs.Count = 0 Then
    ...
    Private Sub Xl_ProductPlugins_RequestToAddNewSku(sender As Object, e As MatEventArgs) Handles Xl_ProductPlugins1.RequestToAddNew
        Dim oProductPlugin = DTOProductPlugin.Factory(_Brand, Current.Session.User)
    ...
    Private Async Sub LoadProductDownloads()
        Dim exs As New List(Of Exception)
        Dim oDownloads As List(Of DTOProductDownload) = Await FEB.Downloads.FromProductOrParent(exs, _Brand)
        If exs.Count = 0 Then
    ...
    Private Async Function LoadMediaResources() As Task
        ...
        Dim items = Await FEB.MediaResources.AllWithThumbnails(exs, _Brand)
    ...
    Private Async Function LoadGridZonas() As Task
        Dim exs As New List(Of Exception)
        Dim oBrandAreas = Await FEB.BrandAreas.All(_Brand, exs)
        If exs.Count = 0 Then
    ...
    Private Async Function RefrescaChannels() As Task
        Dim exs As New List(Of Exception)
        Dim oProductChannels = Await FEB.ProductChannels.All(exs, _Brand)
        If exs.Count = 0 Then
