            ...
            If exs.Count = 0 Then
                If oContact.rol IsNot Nothing Then
                    If oContact.rol.IsStaff Then
                        oIban = Await FEB.Iban.FromContact(exs, oContact, DTOIban.Cods.staff)
                        oCtaCod = DTOPgcPlan.Ctas.PagasTreballadors
                    Else
                        Select Case oContact.rol.id
                            Case DTORol.Ids.Banc
                                oIban = Await FEB.Iban.FromContact(exs, oContact, DTOIban.Cods.banc)
                                oCtaCod = DTOPgcPlan.Ctas.bancs
                            Case DTORol.Ids.CliFull, DTORol.Ids.CliLite
                                oIban = Await FEB.Iban.FromContact(exs, oContact, DTOIban.Cods.client)
                                oCtaCod = DTOPgcPlan.Ctas.Clients
                            Case Else
                                oIban = Await FEB.Iban.FromContact(exs, oContact, DTOIban.Cods.proveidor)
