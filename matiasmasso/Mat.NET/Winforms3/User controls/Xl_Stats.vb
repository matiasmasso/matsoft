Public Class Xl_Stats
    ...
    Public Shadows Sub Load(value As DTOStat)
        If value IsNot Nothing Then
            ...
            If _Stat.Items IsNot Nothing Then
                Dim oControlTotal As ControlItem = Nothing

                Dim dcTotals As Decimal = _Stat.Tot.Tot()

                If _Stat.ConceptType <> DTOStat.ConceptTypes.Yeas Then
                    oControlTotal = New ControlItem(_Stat.Tot, dcTotals)
                ...
                Dim items As ControlItems = _ControlItems '.ToList.Where(Function(x) x.Source.Level = 2)
                With oControlTotal
                    .tot = items.Sum(Function(x) x.Source.Tot)
                    For j = 0 To 11
                        .Source.Values(0) = items.Sum(Function(x) x.Source.Values(0))
                    Next
                End With
        ...
        With DataGridView1
            ...
            For iMes As Integer = 1 To 12
                ...
                With .Columns(Cols.share + iMes)
                    .HeaderText = Current.Session.Lang.MesAbr(iMes)
    ...
    Private Async Function SetContextMenu() As Task
        ...
        If oControlItem IsNot Nothing Then
            Dim oStatItem As DTOStatItem = oControlItem.Source
            If TypeOf oStatItem.Tag Is DTOContact Then
                Dim oContactMenu = Await FEB.ContactMenu.Find(exs, oStatItem.Tag)
                oMenu = New Menu_Contact(oStatItem.Tag, oContactMenu)
                AddHandler oMenu.AfterUpdate, AddressOf RefreshRequest
                oContextMenu.Items.AddRange(oMenu.Range)
            ElseIf TypeOf oStatItem.Tag Is DTOLocation Then
                oMenu = New Menu_Location(oStatItem.Tag)
                AddHandler oMenu.AfterUpdate, AddressOf RefreshRequest
                oContextMenu.Items.AddRange(oMenu.Range)
            ElseIf TypeOf oStatItem.Tag Is DTOZona Then
                oMenu = New Menu_Zona(oStatItem.Tag)
                AddHandler oMenu.AfterUpdate, AddressOf RefreshRequest
                oContextMenu.Items.AddRange(oMenu.Range)
            ElseIf TypeOf oStatItem.Tag Is DTOCountry Then
                oMenu = New Menu_Country(oStatItem.Tag)
                AddHandler oMenu.AfterUpdate, AddressOf RefreshRequest
                oContextMenu.Items.AddRange(oMenu.Range)
            ElseIf TypeOf oStatItem.Tag Is DTOProductBrand Then
                oMenu = New Menu_ProductBrand(oStatItem.Tag)
                AddHandler oMenu.AfterUpdate, AddressOf RefreshRequest
                oContextMenu.Items.AddRange(oMenu.Range)
            ElseIf TypeOf oStatItem.Tag Is DTOProductCategory Then
                oMenu = New Menu_ProductCategory(oStatItem.Tag)
                AddHandler oMenu.AfterUpdate, AddressOf RefreshRequest
                oContextMenu.Items.AddRange(oMenu.Range)
            ElseIf TypeOf oStatItem.Tag Is DTOProductSku Then
                oMenu = New Menu_ProductSku(oStatItem.Tag)
                AddHandler oMenu.AfterUpdate, AddressOf RefreshRequest
                oContextMenu.Items.AddRange(oMenu.Range)
            End If
