Public Class Xl_EdiversaFiles
    ...
    Public Shadows Sub Load(values As List(Of DTOEdiversaFile), includeClosed As Boolean)
        _Values = values
    ...
    Public Function FilteredValues() As List(Of DTOEdiversaFile)
        ...
        If _Filter = "" Then
            retval = _Values
        Else
            retval = _Values.FindAll(Function(x) x.Docnum.ToLower.Contains(_Filter.ToLower) Or x.FileName.ToLower.Contains(_Filter.ToLower)).ToList()
        End If

        If Not _IncludeClosed Then
            retval = retval.Where(Function(x) x.Result = DTOEdiversaFile.Results.pending).ToList()
        End If
        Return retval
    End Function
