Public Class Xl_BancsComboBox
    ...
    Public Async Function LoadDefaultsFor(oCod As DTODefault.Codis, exs As List(Of Exception)) As Task(Of Boolean)
        Dim oBancs As List(Of DTOBanc) = Await FEB.Bancs.AllActive(Current.Session.Emp, exs)
        If exs.Count = 0 Then
            Dim oGuid As Guid = Await FEB.Default.EmpGuid(Current.Session.Emp, oCod, exs)
            If exs.Count = 0 Then
