Public Class Xl_ImportacioDocs
    ...
    Private Sub refresca()
        ...
        If _Importacio.Items IsNot Nothing Then
            For Each oItem As DTOImportacioItem In _Importacio.items.OrderBy(Function(x) DTOImportacioItem.GetConcept(x, Current.Session.Lang)).OrderBy(Function(y) y.srcCod)
            ...
            If oControlItem.LinCod = ControlItem.LinCods.item AndAlso oControlItem.Source IsNot Nothing Then
                ...
                Select Case oControlItem.Source.srcCod
                    Case DTOImportacioItem.SourceCodes.alb
                        Dim oDelivery As New DTODelivery(oControlItem.Source.Guid)
                        Dim oDeliveries = {oDelivery}.ToList()
                        Dim oMenu_Delivery As New Menu_Delivery(oDeliveries)
                        Dim oMenuAlb As ToolStripMenuItem = oContextMenu.Items.Add("Albarà...")
                        oMenuAlb.DropDownItems.AddRange(oMenu_Delivery.Range)
                        oContextMenu.Items.Add("-")
                    Case DTOImportacioItem.SourceCodes.fra
                        Dim oCca As New DTOCca(oControlItem.Source.Guid)
                        Dim oBookFra As New DTOBookFra(oCca)
                        Dim oMenu_BookFra As New Menu_BookFra(oBookFra)
                        Dim oMenuFra As ToolStripMenuItem = oContextMenu.Items.Add("Factura...")
                        oMenuFra.DropDownItems.AddRange(oMenu_BookFra.Range)
                        oContextMenu.Items.Add("-")
                End Select
            End If
        End If
    ...
    Private Async Sub DataGridViewDocs_DragDrop(ByVal sender As Object, ByVal e As System.Windows.Forms.DragEventArgs) Handles DataGridView1.DragDrop
        ...
        ElseIf (e.Data.GetDataPresent(GetType(DTODelivery))) Then
            ...
            If FEB.Delivery.Load(oDelivery, exs) Then
                ...
                With oItem
                    .Amt = Await FEB.Delivery.Total(exs, oDelivery)
            ...
            For Each oDelivery As DTODelivery In oDeliveries
                If FEB.Delivery.Load(oDelivery, exs) Then
                    ...
                    With oItem
                        ...
                        .descripcio = Current.Session.Tradueix("Albarán ", "Albarà ", "Delivery note ") & oDelivery.Id & " del " & oDelivery.Fch.ToShortDateString
                    End With
                    _Importacio.Items.Add(oItem)
                Else
                    UIHelper.WarnError(exs)
        ...
        Public Sub New(oImportacioItem As DTOImportacioItem)
            ...
            With oImportacioItem
                ...
                If .Descripcio > "" Then
                    _Nom = .Descripcio
                Else
                    _Nom = DTOImportacioItem.GetConcept(oImportacioItem, Current.Session.Lang)
                End If
