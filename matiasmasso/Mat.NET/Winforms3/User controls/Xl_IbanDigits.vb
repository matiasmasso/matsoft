Public Class Xl_IbanDigits
    ...
    Public Shadows Async Function Load(oIban As DTOIban) As Task
        If oIban IsNot Nothing Then
            ...
            If oIban.BankBranch Is Nothing Then
                oIban.BankBranch = Await FEB.Iban.GetBankBranchFromDigits(oIban, exs)
    ...
    Private Async Sub TextBox1_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles TextBox1.TextChanged
        If _AllowEvents Then
            Dim exs As New List(Of Exception)
            _BankBranch = Await FEB.IbanStructure.GetBankBranch(TextBox1.Text, exs)
            If exs.Count = 0 Then
                Await RefreshRequest()
    ...
    Private Async Function Do_Bank() As Task
        Dim exs As New List(Of Exception)
        Dim oBank = Await FEB.IbanStructure.GetBank(TextBox1.Text, exs)
        If exs.Count = 0 Then
    ...
    Private Async Function RefreshRequest() As Task
        ...
        Else
            Dim bytes = Await FEB.Iban.Img(exs, TextBox1.Text, Current.Session.Lang)
            PictureBox1.Image = LegacyHelper.ImageHelper.FromBytes(bytes)
            If exs.Count = 0 Then
                SetContextMenu()
            Else
                UIHelper.WarnError(exs)
