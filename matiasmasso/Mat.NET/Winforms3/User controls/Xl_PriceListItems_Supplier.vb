Public Class Xl_PriceListItems_Supplier
    ...
    Private Async Sub AfterImportFromExcel(sender As Object, e As MatEventArgs)
        ...
        Dim oItemsToAdd = Await FEB.PriceListItemsSupplier.FromExcelSheet(exs, _PriceList, oSheet)
    ...
    Private Sub Do_ExportToExcel()
        Dim exs As New List(Of Exception)
        If FEB.PriceListSupplier.Load(exs, _PriceList) Then
    ...
    Private Async Sub Do_Delete()
        ...
        If rc = MsgBoxResult.Ok Then
            Dim exs As New List(Of Exception)
            If Await FEB.PriceListItemsSupplier.Delete(exs, oSelectedItems) Then
                RefreshRequest(Me, MatEventArgs.Empty)
            Else
                UIHelper.WarnError(exs)
            End If
