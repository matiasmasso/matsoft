Public Class Xl_GalleryItems
    ...
    Private Async Sub DataGridView1_CellFormatting(sender As Object, e As DataGridViewCellFormattingEventArgs) Handles MyBase.CellFormatting
        Select Case e.ColumnIndex
            Case Cols.Image
                Dim oRow As DataGridViewRow = DirectCast(sender, DataGridView).Rows(e.RowIndex)
                Dim oControlItem As ControlItem = oRow.DataBoundItem
                Select Case oControlItem.Status
                    Case ControlItem.LoadStatus.NotSet
                        oControlItem.Status = ControlItem.LoadStatus.Loading
                        Dim exs As New List(Of Exception)
                        Dim oGalleryItem As DTOGalleryItem = oControlItem.Source
                        Dim bytes = Await FEB.GalleryItem.Thumbnail(exs, oGalleryItem.Guid)
                        oControlItem.Thumbnail = LegacyHelper.ImageHelper.FromBytes(bytes)
                        oControlItem.Status = ControlItem.LoadStatus.Loaded
                        MyBase.Refresh()
                    Case ControlItem.LoadStatus.Loaded
                        e.Value = oControlItem.Thumbnail
                End Select
        End Select
    End Sub
    ...
    Protected Class ControlItem
        ...
        Property Status As LoadStatus = LoadStatus.NotSet
