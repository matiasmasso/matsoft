...
Public Class Menu_Contact
    ...
    Public Sub New(ByVal oContact As DTOContact, oContactMenu As DTOContactMenu)
        MyBase.New()
        _Contact = oContact
    ...
    Private Sub AddMenuItems()
        ...
        If _ContactMenu IsNot Nothing Then
            If _ContactMenu.IsClient Then MyBase.AddMenuItem(MenuItem_Client)
            If _ContactMenu.IsProveidor Then MyBase.AddMenuItem(MenuItem_Proveidor)
            If _ContactMenu.IsRep Then MyBase.AddMenuItem(MenuItem_Rep)
        End If
    ...
    Private Function MenuItem_Email() As ToolStripMenuItem
        ...
        If oMenuItem.DropDownItems.Count = 0 Then
            If _ContactMenu IsNot Nothing Then
                For Each oUser In _ContactMenu.Emails
    ...
    Private Function MenuItem_Tels() As ToolStripMenuItem
        ...
        If oMenuItem.DropDownItems.Count = 0 Then
            If _ContactMenu IsNot Nothing Then
                For Each item In _ContactMenu.Tels
                    Select Case item.Cod
                        Case Else 'DTOContactTel.Cods.tel, DTOContactTel.Cods.movil
    ...
    Private Function MenuItem_Rep() As ToolStripMenuItem
        ...
        oMenuItem.DropDownItems.AddRange(New Menu_Rep(oRep, _ContactMenu).Range)
        oMenuItem.Text = "rep..."
        Return oMenuItem
    End Function
    ...
    Private Async Sub Do_FtpBrowse(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim exs As New List(Of Exception)
        Dim oFtp = Await FEB.Ftpserver.Find(exs, _Contact)
        If exs.Count = 0 Then
    ...
    Private Sub Do_LastIncidencies()
        Dim oFrm As New Frm_Last_Incidencies(DTOCustomer.FromContact(_Contact))
        oFrm.Show()
    End Sub
    ...
    Private Async Sub Do_AmazonCostAndInventoryEdiQueue()
        ...
        If Await FEB.EdiversaInvRpt.Send(exs, GlobalVariables.Emp, oCustomer) Then
    ...
    Private Async Sub Do_AmazonCostAndInventoryFeedFile()
        Dim exs As New List(Of Exception)
        Dim oCustomer = DTOCustomer.Wellknown(DTOCustomer.Wellknowns.Amazon)
        Dim src As String = Await FEB.EdiversaInvRpt.Src(exs, Current.Session.Emp, oCustomer)
        If exs.Count = 0 Then
            Dim sFilename = String.Format("Amazon Cost & Inventory feed {0:yyyyMMddHHmmss}", DTO.GlobalVariables.Now())
    ...
    Private Async Sub Do_AmazonCostAndInventoryExcel()
        Dim exs As New List(Of Exception)
        Dim oCustomer = DTOCustomer.Wellknown(DTOCustomer.Wellknowns.Amazon)
        Dim oSheet = Await FEB.EdiversaInvRpt.Excel(exs, GlobalVariables.Emp, oCustomer)
        If exs.Count = 0 Then
    ...
    Private Async Sub Do_SetGln()
        ...
        If FEB.Contact.Load(_Contact, exs) Then
            If _Contact.GLN Is Nothing Then
                If Await FEB.Aecoc.NextEanToContact(GlobalVariables.Emp, _Contact, exs) Then
