Public Class Menu_Raffle
    ...
    Private Function MenuItem_Advanced() As ToolStripMenuItem
        Dim oMenuItem As New ToolStripMenuItem
        oMenuItem.Text = Current.Session.Tradueix("Avanzado", "Avançat", "Advanced")
        oMenuItem.DropDownItems.Add(MenuItem_BrowseWinnerCongrats)
        Return oMenuItem
    End Function


    Private Function MenuItem_BrowseWinnerCongrats() As ToolStripMenuItem
        Dim oMenuItem As New ToolStripMenuItem
        oMenuItem.Text = Current.Session.Tradueix("Texto del mensaje al ganador", "Text del missatge al guanyador", "Winner message text")
        AddHandler oMenuItem.Click, AddressOf Do_BrowseWinnerCongrats
        Return oMenuItem
    End Function
    ...
    Private Async Sub Do_SetRandomWinner()
        Dim exs As New List(Of Exception)
        Dim oTaskResult = Await FEB.Raffle.SetRandomWinner(_Raffle, exs)
    ...
    Private Async Sub Do_RemoveWinner()
        Dim exs As New List(Of Exception)
        If Await FEB.Raffle.RemoveWinner(exs, _Raffle) Then
    ...
    Private Async Sub Do_SetAllPendingWinners()
        Dim exs As New List(Of Exception)
        Dim oTaskResult = Await FEB.Raffles.SetWinners(Current.Session.User, exs)
        If oTaskResult.Success Then
    ...
    Private Async Sub Do_ExcelParticipantsList()
        Dim exs As New List(Of Exception)
        Dim oSheet = Await FEB.RaffleParticipants.ExcelParticipantsList(exs, _Raffle)
        If exs.Count = 0 Then
    ...
    Private Async Sub Do_MailWinnerCongrats_Outlook()
        ...
        Else
            If FEB.RaffleParticipant.Load(exs, oWinner) Then
                Dim oLang As DTOLang = oWinner.User.Lang
                If FEB.Raffle.Load(oWinner.Raffle, exs) Then
                    Dim sSubject As String = "M+O: ¡Enhorabuena, has ganado el sorteo de " & oWinner.Raffle.Brand.Nom.Tradueix(Current.Session.Lang) & "!"
                    Dim sBodyUrl As String = FEB.Mailing.BodyUrl(DTODefault.MailingTemplates.raffleWinnerCongrats, oWinner.Guid.ToString())
    ...
    Private Sub Do_BrowseWinnerCongrats()
        Dim exs As New List(Of Exception)
    ...
    Private Async Sub Do_RaffleDistributorNotification_Outlook()
        ...
        If FEB.Raffle.Load(oRaffle, exs) Then
            ...
            If oDistribuidor IsNot Nothing Then
                Dim oUser As DTOUser = Await FEB.Contact.DefaultUser(oDistribuidor, exs)
                If oUser IsNot Nothing Then
                    sRecipient = oUser.EmailAddress
                End If
            End If

            Dim sBodyUrl As String = FEB.Mailing.BodyUrl(DTODefault.MailingTemplates.raffleDistributorNotification, oRaffle.Guid.ToString())
    ...
    Private Async Sub Do_MailWinnerCongrats_Exchange()
        Dim exs As New List(Of Exception)
        Dim oWinner As DTORaffleParticipant = _Raffle.Winner
        Dim oMailMessage = Await FEB.RaffleParticipant.RaffleWinnerCongrats(exs, GlobalVariables.Emp, oWinner)
    ...
    Private Async Sub Do_DeliverPrize()
        ...
        If FEB.RaffleParticipant.Load(exs, oWinner) Then
            If FEB.Raffle.Load(_Raffle, exs) Then
                Dim oCustomer = Await FEB.Customer.Find(exs, oWinner.Distribuidor.Guid)
                If exs.Count = 0 Then
                    ...
                    With item
                        ...
                        .sku = Await FEB.ProductSku.Find(exs, _Raffle.Product.Guid, GlobalVariables.Emp.Mgz)
                        .qty = 1
                        .Pending = .Qty
    ...
    Private Async Sub AfterDeliver(sender As Object, e As MatEventArgs)
        ...
        If FEB.Raffle.Load(_Raffle, exs) Then
            ...
            If Not Await FEB.Raffle.Update(exs, _Raffle) Then
