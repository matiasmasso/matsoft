Public Class Menu_BookFra
    ...
    Private Async Sub Do_Excel(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Dim oExcel = Await FEB.Bookfras.Excel(exs, oExercici, DTO.GlobalVariables.Today())
        If exs.Count = 0 Then
    ...
    Private Async Sub Do_SendToSii(entorno As DTO.Defaults.Entornos)
        Dim exs As New List(Of Exception)
        Dim oTaskResult = Await FEB.Bookfras.SendToSii(Current.Session.Emp, entorno, _BookFras, _ShowProgress, exs)
    ...
    Private Async Sub Do_SiiLog()
        ...
        If FEB.BookFra.Load(exs, oBookFra) Then
            Dim oX509Cert = Await FEB.Cert.X509Certificate2(Current.Session.Emp.Org, exs)
            If exs.Count = 0 Then
                ...
                Else
                    ...
                    If Await FEB.BookFra.LogSii(exs, oBookFra) Then
