Public Class Menu_Noticia
    ...
    Private Sub Do_Browse(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Dim url = FEB.Noticia.UrlFriendly(_Noticia, True)
        UIHelper.ShowHtml(url)
    End Sub

    Private Sub Do_CopyDirectLink()
        Dim sUrl = FEB.UrlHelper.Factory(True, "news", _Noticia.Guid.ToString())
        Clipboard.SetDataObject(sUrl, True)
    End Sub

    Private Sub Do_CopyMailLink(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim sUrl As String = FEB.Mailing.BodyUrl(DTODefault.MailingTemplates.Noticia, _Noticia.Guid.ToString())
        Clipboard.SetDataObject(sUrl, True)
    End Sub
    ...
    Private Async Sub Do_ProLeads()
        Dim exs As New List(Of Exception)
        If FEB.Noticia.Load(exs, _Noticia) Then
            Dim oBrands As New List(Of DTOProductBrand)
            If DirectCast(_Noticia, DTONoticia).Product IsNot Nothing Then
                Dim oBrand = FEB.Product.Brand(exs, DirectCast(_Noticia, DTONoticia).Product)
                oBrands.Add(oBrand)
            End If
            ...
            Else
                oChannels = Await FEB.DistributionChannels.Headers(GlobalVariables.Emp, Current.Session.Lang, exs)
            End If
    ...
    Private Async Sub Do_Logs()
        ...
        Dim oLogs = Await FEB.WebLogBrowses.All(exs, _Noticia)
        If exs.Count = 0 Then
    ...
    Private Async Sub refreshLogs(sender As Object, e As MatEventArgs)
        ...
        Dim oLogs = Await FEB.WebLogBrowses.All(exs, _Noticia) ' NoticiaLoader.WebLogs(_Noticia)
        If exs.Count = 0 Then
