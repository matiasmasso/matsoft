Public Class Menu_RecallCli
    ...
    Private Async Sub Do_LabelsOutlook()
        Dim exs As New List(Of Exception)
        If FEB.RecallCli.Load(exs, _RecallCli) Then
            Dim oPdfStream As Byte() = LegacyHelper.PdfRecallLabel.Factory(_RecallCli)
            Dim oMailMessage As DTOMailMessage = Await FEB.RecallCli.MailMessage(_RecallCli, oPdfStream, exs)
            If exs.Count = 0 Then
    ...
    Private Async Sub Do_MailVivace()
        ...
        If FEB.RecallCli.Load(exs, _RecallCli) Then
            Dim oPdfStream As Byte() = LegacyHelper.PdfRecallLabel.Factory(_RecallCli)
            Dim oMailMessage = Await FEB.RecallCli.MailMessageToVivace(_RecallCli, oPdfStream, exs)
            If exs.Count = 0 Then
                If Await OutlookHelper.Send(oMailMessage, exs) Then
                    If FEB.RecallCli.Load(exs, _RecallCli) Then
                        _RecallCli.FchVivace = DTO.GlobalVariables.Now()
                        If Await FEB.RecallCli.Update(exs, _RecallCli) Then
    ...
    Private Async Sub Do_Delivery(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If FEB.RecallCli.Load(exs, _RecallCli) Then
            ...
            If _RecallCli.Delivery IsNot Nothing Then
                oDelivery = FEB.Delivery.Factory(exs, _RecallCli.PurchaseOrder, GlobalVariables.Emp.Mgz, DTO.GlobalVariables.Today(), Current.Session.User)
                With oDelivery
                    .Address = New DTOAddress()
                    .Address.Text = _RecallCli.Address
                    .Address.Zip = Await FEB.Zip.FromZipCod(_RecallCli.Country, _RecallCli.Zip, exs)
                End With
            Else
    ...
    Private Async Sub Do_Pdc()
        ...
        If FEB.RecallCli.Load(exs, _RecallCli) Then
            If _RecallCli.PurchaseOrder Is Nothing Then
                ...
                If FEB.Customer.Load(_RecallCli.Customer, exs) Then
                    ...
                    Dim pOrder = Await FEB.PurchaseOrder.Update(exs, oPurchaseOrder)
                    If exs.Count = 0 Then
    ...
    Private Async Sub Do_Delete(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If rc = MsgBoxResult.Ok Then
            Dim exs As New List(Of Exception)
            If Await FEB.RecallCli.Delete(exs, _RecallClis.First) Then
