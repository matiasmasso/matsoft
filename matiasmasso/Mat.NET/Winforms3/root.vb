...
Public Module root
    ...
    Public Sub ShowIngresosYDespesesMensual(Optional ByVal iYea As Integer = 0, Optional ByVal iMes As Integer = 0)
        If iYea = 0 Or iMes = 0 Then
            Dim DtFch As Date = DTO.GlobalVariables.Today().AddMonths(-1)
            iYea = DtFch.Year
            iMes = DtFch.Month
        End If
    ...
    Public Async Function RepRetencionsSave(ByVal iYea As Integer, ByVal iQuarter As Integer, exs As List(Of Exception)) As Task(Of Boolean)
        ...
        Dim oReps = Await FEB.Reps.WithRetencions(exs, Current.Session.Emp, DtFirstDayFirstMonth, DtLastDayLastMonth)

        Dim oPdfRepRetencio As LegacyHelper.PdfRepRetencions = Nothing
        Dim oStream As Byte() = Nothing
        For Each oRep In oReps
            Dim oRepLiqs As List(Of DTORepLiq) = Await FEB.Repliqs.GetRepLiqsFromQuarter(exs, oRep, iYea, iQuarter)
            ...
            If exs.Count = 0 Then
                ...
                With oDoc
                    ...
                    .DocFile = oDocFile
                End With
                If Await FEB.ContactDoc.Update(oDoc, exs) Then
