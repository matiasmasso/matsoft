Public Class EdiOrderLoader


#Region "CRUD"

    Shared Function Find(oGuid As Guid) As DTOEdiOrder
        Dim retval As DTOEdiOrder = Nothing
        Dim oEDiversaOrder As New DTOEdiOrder(oGuid)
    ...
    Shared Function Load(ByRef oEDiOrder As DTOEdiOrder) As Boolean
        ...
        If Not oEDiOrder.IsLoaded And Not oEDiOrder.IsNew Then
            ...
            sb.AppendLine(", EDiversaOrderHeader.Nadms ")
            ...
            sb.AppendLine("WHERE EdiversaOrderHeader.Guid='" & oEDiOrder.Guid.ToString & "' ")
            sb.AppendLine("ORDER BY EdiversaOrderItem.Lin ")

            Dim SQL As String = sb.ToString
            Dim oItem As New DTOEdiOrder.Item
            Dim oDrd As SqlDataReader = SQLHelper.GetDataReader(SQL)
            Do While oDrd.Read
                If Not oEDiOrder.IsLoaded Then
                    With oEDiOrder
                        .DocNum = oDrd("DocNum")
                        .FchDoc = oDrd("FchDoc")
                        ...
                        If Not IsDBNull(oDrd("Comprador")) Then
                            Dim oMatchingComprador As DTOEdiOrder = Nothing
                        ...
                        If Not IsDBNull(oDrd("ReceptorMercancia")) Then
                            ...
                            Dim oMatchingReceptor As DTOEdiOrder = Nothing
                ...
                If Not IsDBNull(oDrd("ItemGuid")) Then
                    If Not oItem.Guid.Equals(oDrd("ItemGuid")) Then
                        oItem = New DTOEdiOrder.Item(oDrd("ItemGuid"))
                        With oItem
                            .Lin = oDrd("Lin")
            ...
            oDrd.Close()
        End If
        retval = oEDiOrder.IsLoaded
        Return retval
    End Function


    Shared Function searchByDocNum(docnum As String) As List(Of DTOEdiOrder)
        ...
        Dim oItem As New DTOEdiOrder.Item
        Dim oDrd As SqlDataReader = SQLHelper.GetDataReader(SQL)
        Dim oOrder As New DTOEdiOrder
        Do While oDrd.Read
            If Not oOrder.Guid.Equals(oDrd("Guid")) Then
                oOrder = New DTOEdiOrder(oDrd("Guid"))
                With oOrder
                    .DocNum = oDrd("DocNum")
                    .FchDoc = oDrd("FchDoc")
                    If Not IsDBNull(oDrd("FchDelivery")) Then
    ...
    Shared Function Update(oEDiversaOrder As DTOEdiOrder, exs As List(Of Exception)) As Boolean
        Dim retval As Boolean
    ...
    Shared Function Update(oEDiversaOrder As DTOEdiOrder) As DTOTaskResult
    ...
    Shared Function Update(oEDiversaOrder As DTOEdiOrder, oEdiversaFile As DTOEdiversaFile, oPurchaseOrder As DTOPurchaseOrder, ByRef exs As List(Of DTOEdiversaException)) As Boolean
        Dim retval As Boolean
    ...
    Shared Sub Update(oEDiversaOrder As DTOEdiOrder, ByRef oTrans As SqlTransaction)
        DeleteExceptions(oEDiversaOrder, oTrans)
        UpdateHeader(oEDiversaOrder, oTrans)
        UpdateItems(oEDiversaOrder, oTrans)
    End Sub

    Shared Sub DeleteExceptions(oEdiversaOrder As DTOEdiOrder, ByRef oTrans As SqlTransaction)
        Dim sb As New System.Text.StringBuilder
    ...
    Shared Sub UpdateHeader(oEDiversaOrder As DTOEdiOrder, ByRef oTrans As SqlTransaction)
        Dim sb As New System.Text.StringBuilder
    ...
    Shared Function SetResult(exs As List(Of Exception), oEdiOrder As DTOEdiOrder, oPurchaseOrder As DTOPurchaseOrder) As Boolean
    ...
    Protected Shared Sub UpdateItems(oEDiversaOrder As DTOEdiOrder, ByRef oTrans As SqlTransaction)
        ...
        For Each oItem As DTOEdiOrder.Item In oEDiversaOrder.Items
    ...
    Shared Function Delete(oEDiversaOrder As DTOEdiOrder, exs As List(Of Exception)) As Boolean
        Dim retval As Boolean
    ...
    Shared Sub Delete(oEDiversaOrder As DTOEdiOrder, ByRef oTrans As SqlTransaction)
        DeleteItems(oEDiversaOrder, oTrans)
        DeleteHeader(oEDiversaOrder, oTrans)
    End Sub

    Shared Sub DeleteHeader(oEDiversaOrder As DTOEdiOrder, ByRef oTrans As SqlTransaction)
        Dim SQL As String = "DELETE EdiversaOrderHeader WHERE Guid='" & oEDiversaOrder.Guid.ToString & "' "
        SQLHelper.ExecuteNonQuery(SQL, oTrans)
    End Sub

    Shared Sub DeleteItems(oEDiversaOrder As DTOEdiOrder, ByRef oTrans As SqlTransaction)
...
#End Region
    ...
    Shared Function Duplicates(oEDiversaOrder As DTOEdiOrder) As List(Of DTOEdiOrder)
        Dim retval As New List(Of DTOEdiOrder)
        ...
        Dim oDrd As SqlDataReader = SQLHelper.GetDataReader(SQL)
        Do While oDrd.Read
            Dim item As New DTOEdiOrder(oDrd("Guid"))
            With item
...
Public Class EDiOrdersLoader

    Shared Function Headers(oEmp As DTOEmp, year As Integer) As List(Of DTOEdiOrder)
        Dim retval As New List(Of DTOEdiOrder)
        Dim sb As New System.Text.StringBuilder
    ...
    Shared Function All(Optional OnlyOpenFiles As Boolean = True) As DTOEdiOrder.Collection
        Dim retval As New DTOEdiOrder.Collection
        Dim sb As New System.Text.StringBuilder
        sb.AppendLine("SELECT EDiversaOrderHeader.Guid, EDiversaOrderHeader.DocNum ")
        sb.AppendLine(", EDiversaOrderHeader.FchDoc, EDiversaOrderHeader.FchDelivery, EDiversaOrderHeader.FchLast ")
        sb.AppendLine(", EDiversaOrderHeader.Tipo, EDiversaOrderHeader.Funcion, EDiversaOrderHeader.Cur ")
        sb.AppendLine(", EDiversaOrderHeader.Nadms, EDiversaOrderHeader.Proveedor, EDiversaOrderHeader.Comprador, EDiversaOrderHeader.ReceptorMercancia, EDiversaOrderHeader.FacturarA ")
        sb.AppendLine(", EDiversaOrderHeader.CompradorEAN, EDiversaOrderHeader.ReceptorMercanciaEAN, EDiversaOrderHeader.FacturarAEAN ")
        sb.AppendLine(", EDiversaOrderHeader.Centro, EDiversaOrderHeader.Departamento ")
        sb.AppendLine(", EDiversaOrderHeader.Eur, EDiversaOrderHeader.Result, EDiversaOrderHeader.Obs ")
        sb.AppendLine(", EdiversaOrderItem.Guid AS ItemGuid, EdiversaOrderItem.Lin, EdiversaOrderItem.Ean ")
        sb.AppendLine(", EdiversaOrderItem.RefProveidor, EdiversaOrderItem.RefClient, EdiversaOrderItem.Dsc ")
        ...
        Dim SQL As String = sb.ToString
        Dim oDrd As SqlDataReader = SQLHelper.GetDataReader(SQL)
        Dim oOrder As New DTOEdiOrder(Guid.NewGuid)
        Dim oItem As New DTOEdiOrder.Item
    ...
    Shared Function DeleteDuplicates() As DTOTaskResult
        ...
        Return retval
    End Function
    Shared Function Descarta(oEDiversaOrders As List(Of DTOEdiOrder), ByRef exs As List(Of Exception)) As Boolean
        Dim retval As Boolean
    ...
    Shared Sub Descarta(oEDiversaOrders As List(Of DTOEdiOrder), ByRef oTrans As SqlTransaction)
        Dim sb As New System.Text.StringBuilder
        sb.AppendLine("UPDATE Edi SET Result=" & CInt(DTOEdiversaFile.Results.deleted) & " WHERE (")
        For Each item As DTOEdiOrder In oEDiversaOrders
    ...
    Shared Function ConfirmationPending() As List(Of DTOEdiOrder)
        Dim retval As New List(Of DTOEdiOrder)
        Dim sb As New Text.StringBuilder
