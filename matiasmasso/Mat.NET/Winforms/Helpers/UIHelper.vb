...
Module UIHelper
        ...
        Select Case visible
            Case True
                For Each oChild In panel.Controls
                    If TypeOf oChild Is Button Then
                ...
                'panel.Parent.Cursor = Cursors.WaitCursor
            Case Else
                For Each oChild In panel.Controls
                    If TypeOf oChild Is Button Then
    ...
    Public Sub CopyLink(oDocFile As DTODocFile, Optional msg As Boolean = True)
        Dim sUrl As String = FEB2.DocFile.DownloadUrl(oDocFile, True)
    ...
    Public Function LoadExcelSheetDialog(ByRef oSheet As MatHelperStd.ExcelHelper.Sheet, Optional sTitle As String = "") As Boolean
        Dim retval As Boolean
        ...
        If LoadExcelDialog(sFilename, sTitle) Then
            Dim exs As New List(Of Exception)
            If LegacyHelper.ExcelHelper.ExcelSheetFromFilename(sFilename, oSheet, exs) Then
                retval = True
            Else
    ...
    Public Function SaveExcelDialog(oSheet As MatHelperStd.ExcelHelper.Sheet, Optional ShowProgress As ProgressBarHandler = Nothing, Optional sTitle As String = "") As Boolean
        Dim retval As Boolean
        Dim oDlg As New SaveFileDialog
        With oDlg
            ...
            If .ShowDialog = DialogResult.OK Then
                ...
                Dim exs As New List(Of Exception)
                If LegacyHelper.ExcelHelper.Save(oSheet, exs) Then
                    retval = True
                Else
                    UIHelper.WarnError(exs, "error al desar el fitxer")
                End If
    ...
    Public Function SaveExcelDialog(oBook As MatHelperStd.ExcelHelper.Book, Optional ShowProgress As ProgressBarHandler = Nothing, Optional sTitle As String = "") As Boolean
        Dim retval As Boolean
        Dim oDlg As New SaveFileDialog
        With oDlg
            ...
            If .ShowDialog = DialogResult.OK Then
                ...
                Dim exs As New List(Of Exception)
                If LegacyHelper.ExcelHelper.Save(oBook, exs) Then
                    retval = True
                Else
                    UIHelper.WarnError(exs, "error al desar el fitxer")
                End If
    ...
    Public Function SavePdfDialog(oPdf As LegacyHelper._BaseC1PdfDocument, Optional sTitle As String = "") As Boolean
        Dim retval As Boolean
        Dim oDlg As New SaveFileDialog
        With oDlg
            If sTitle = "" Then sTitle = oPdf.Filename
            If sTitle > "" Then
                .Title = sTitle
                If oPdf.Filename > "" Then
                    Dim sFilename As String = oPdf.Filename
                    If Not sFilename.EndsWith(".pdf") Then
                        sFilename += ".pdf"
                    End If
                    .FileName = sFilename
                End If
            End If
            .Filter = "fitxers Pdf (*.pdf)|*.pdf|tots els documents (*.*)|*.*"
            If .ShowDialog = DialogResult.OK Then
                Try
                    oPdf.Save(.FileName)
                    retval = True
    ...
    Public Function LoadPdfDialog(ByRef oDocFile As DTODocFile, Optional sTitle As String = "", Optional sFilter As String = "documents Pdf (*.pdf)|*.pdf|tots els documents (*.*)|*.*") As Boolean
        ...
        With oDlg
            ...
            If .ShowDialog Then
                ...
                Dim tmp = LegacyHelper.DocfileHelper.Factory(.FileName, exs)
                If exs.Count = 0 Then
                    oDocFile = tmp
                    retval = True
                Else
    ...
    Public Function ShowExcel(oSheet As MatHelperStd.ExcelHelper.Sheet, exs As List(Of Exception)) As Boolean
        Dim retval As Boolean
        If oSheet Is Nothing Then
            exs.Add(New Exception("Excel buit"))
        Else
            Dim oStream As Byte() = Nothing
            If LegacyHelper.ExcelHelper.Stream(oSheet, oStream, exs) Then
                If oSheet.Filename = "" Then
    ...
    Public Function ShowExcel(oBook As MatHelperStd.ExcelHelper.Book, exs As List(Of Exception)) As Boolean
        Dim retval As Boolean
        If oBook Is Nothing Then
            exs.Add(New Exception("Excel buit"))
        Else
            Dim oStream As Byte() = Nothing
            If LegacyHelper.ExcelHelper.Stream(oBook, oStream, exs) Then
    ...
    Public Async Function ShowStreamAsync(exs As List(Of Exception), oDocFile As DTODocFile, Optional sSuggestedFileName As String = "", Optional BlDownloadFromWeb As Boolean = True) As Task(Of Boolean)
        ...
        ElseIf oDocFile.Stream IsNot Nothing Then
            If sSuggestedFileName = "" Then sSuggestedFileName = FEB2.DocFile.FileNameOrDefault(oDocFile)
            Select Case oDocFile.mime
                Case MimeCods.Pdf, MimeCods.Gif, MimeCods.Jpg, MimeCods.Png
        ...
        Else
            Dim pDocfile = Await FEB2.DocFile.FindWithStream(oDocFile.hash, exs)
            If exs.Count = 0 Then
                ...
                Else
                    Try
                        Process.Start(FEB2.DocFile.DownloadUrl(oDocFile, True))
                        retval = True
                    Catch ex As Exception
                        exs.Add(ex)
                    End Try
    ...
    Public Sub ShowPdf(oDocFile As DTODocFile, Optional sSuggestedFileName As String = "", Optional BlDownloadFromWeb As Boolean = True)
        ...
        Else
            If oDocFile.Stream Is Nothing Then
                Dim url = FEB2.DocFile.DownloadUrl(oDocFile, True)
                Process.Start(url)
            Else
    ...
    Public Sub ShowPdfStream(oBasePdf As LegacyHelper._BaseC1PdfDocument)
        Dim exs As New List(Of Exception)
        Dim oStream As Byte() = oBasePdf.Stream(exs)
        If exs.Count = 0 Then
            Dim sSuggestedFileName As String = ""
            If FileSystemHelper.SaveStream(oStream, exs, sSuggestedFileName) Then
                Process.Start(sSuggestedFileName)
            Else
                UIHelper.WarnError(exs, "error al desar el fitxer")
            End If
