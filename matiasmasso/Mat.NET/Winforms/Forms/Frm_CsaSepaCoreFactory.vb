Public Class Frm_CsaSepaCoreFactory
    ...
    Private Sub Frm_SepaCoreCsaFactory_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If FEB2.Banc.Load(_Banc, exs) Then
            Select Case _Format
    ...
    Private Async Sub refresca()
        ...
        Select Case _Format
            Case DTOCsa.FileFormats.RemesesExportacioLaCaixa
                ...
                Dim oAllCsbs = Await FEB2.Csbs.PendentsDeGirar(exs, GlobalVariables.Emp, oCountry:=Nothing, sepa:=False)
            ...
            Case Else
                UIHelper.ToggleProggressBar(Panel1, True)
                _Csbs = Await FEB2.Csbs.PendentsDeGirar(exs, GlobalVariables.Emp, oCountry:=Nothing, sepa:=True)
                UIHelper.ToggleProggressBar(Panel1, False)
                If exs.Count > 0 Then
                    UIHelper.WarnError(exs)
                    Me.Close()
    ...
    Private Sub WarnNoIban(exs As List(Of Exception), oCsbs As List(Of DTOCsb))
        ...
        For Each oCsb As DTOCsb In oCsbs
            If FEB2.Pnd.Load(oCsb.Pnd, exs) Then
    ...
    Private Async Sub ButtonOk_Click(sender As Object, e As EventArgs) Handles ButtonOk.Click
        ...
        Dim exs As New List(Of Exception)
        If Await FEB2.Csa.SaveRemesaCobrament(oCsa, Current.Session.User, exs) Then
            UIHelper.ToggleProggressBar(Panel1, False)
            Select Case oCsa.FileFormat
                ...
                Case Else
                    If FEB2.Csa.Load(oCsa, exs) Then 'per recuperar Id despres de desar
