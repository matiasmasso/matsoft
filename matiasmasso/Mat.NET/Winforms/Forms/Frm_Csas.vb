Public Class Frm_Csas
    ...
    Private Async Sub Frm_Csas_LoadAsync(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If _Banc Is Nothing Then
            iYears = Await FEB2.Csas.YearsAsync(Current.Session.Emp, exs)
        Else
            Me.Text = Me.Text & " de " & _Banc.AbrOrNom()
            iYears = Await FEB2.Csas.YearsAsync(_Banc, exs)
        End If
    ...
    Private Async Function refrescaAsync(exs As List(Of Exception)) As Task(Of Boolean)
        ...
        If _Banc Is Nothing Then
            ...
            _Csas = Await FEB2.Csas.AllAsync(Xl_Years1.Value, Current.Session.Emp, exs)
        Else
            _Csas = Await FEB2.Csas.AllAsync(Xl_Years1.Value, _Banc, exs)
        End If
        ...
        Select Case TabControl1.SelectedIndex
            Case Tabs.mailingLogs
                If _lastLogYear <> Xl_Years1.Value Then
                    Dim exs As New List(Of Exception)
                    ProgressBar1.Visible = True
                    Dim oCsbs = Await FEB2.Csbs.mailingLogs(exs, GlobalVariables.Emp, Xl_Years1.Value)
                    ProgressBar1.Visible = False
                    If exs.Count = 0 Then
