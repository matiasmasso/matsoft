Public Class Frm_BancTransferFactory
    ...
    Private Async Function LoadReps(exs As List(Of Exception)) As Task
        _Pool = Await FEB2.BancTransferPool.FromReps(Current.Session.User, exs)
    ...
    Private Async Function LoadStaff(exs As List(Of Exception)) As Task
        _Pool = Await FEB2.BancTransferPool.FromStaff(GlobalVariables.Emp, Current.Session.User, exs)
        Xl_BancTransferBeneficiarisFactory1.Load(_Pool.Beneficiaris)
        SetTotal()
    End Function
    ...
    Private Async Sub ButtonOk_Click(sender As Object, e As EventArgs) Handles ButtonOk.Click
        ...
        Dim exs As New List(Of Exception)
        Dim CcaId = Await FEB2.BancTransferPool.Save(exs, oPool)
        If exs.Count = 0 Then
            oPool.Cca.id = CcaId
            RaiseEvent AfterUpdate(Me, New MatEventArgs(oPool))
            Dim XMLSource As String = Await FEB2.SepaCreditTransfer.XML(Current.Session.Emp, oPool, exs)
            If exs.Count = 0 Then
