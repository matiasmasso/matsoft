Public Class Frm_Pagament
    ...
    Private Async Sub Frm_Pagament_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        FEB2.Contact.Load(_Proveidor, exs)
        If FEB2.Proveidor.Load(_Proveidor, exs) Then
            ...
            Dim oPnds = Await FEB2.Pnds.All(exs, GlobalVariables.Emp, _Proveidor)
    ...
    Private Sub SetCcaConcept()
        ...
        Select Case CurrentMode()
            Case PaymentModes.CreditTransfer
                s = String.Format("{0} - transferència {1}", CurrentBanc.Abr, _Proveidor.Nom)
            Case PaymentModes.DirectDebit
                s = String.Format("{0} - efecte {1}", CurrentBanc.Abr, _Proveidor.Nom)
            Case PaymentModes.Visa
                s = String.Format("{1} - Pagament amb Visa {0}", CurrentBanc.Abr, _Proveidor.Nom)
        End Select
    ...
    Private Function CurrentBanc() As DTOBanc
        Dim retval As DTOBanc = Xl_BancsComboBox1.SelectedItem
        Return retval
    End Function
    ...
    Private Function CtaDebit() As DTOPgcCta
        ...
        If oCur.isUSD Then
            retval = FEB2.PgcCta.FromCodSync(DTOPgcPlan.Ctas.ProveidorsUsd, Current.Session.Emp, exs)
        Else
            retval = FEB2.PgcCta.FromCodSync(DTOPgcPlan.Ctas.ProveidorsEur, Current.Session.Emp, exs)
        End If
        Return retval
    End Function
    ...
    Private Async Function LoadIban(exs As List(Of Exception)) As Task(Of Boolean)
        ...
        Dim oIban As DTOIban = Await FEB2.Iban.FromContact(exs, _Proveidor, DTOIban.Cods.Proveidor)
        If exs.Count = 0 Then
...
#Region "Navegacio"
    ...
    Private Async Function SaveVisa(exs As List(Of Exception)) As Task(Of Boolean)
        Dim retval As Boolean
        Dim oCtaCredit = Await FEB2.PgcCta.FromCod(DTOPgcPlan.Ctas.VisasPagadas, Current.Session.Emp, exs)
        ...
        Dim pCca = Await FEB2.Proveidor.SaveFactura(exs, oCca, oPnds)
        If exs.Count = 0 Then
            oCca = pCca
            _resultmsg = "Assentament " & oCca.Id & " registrat satisfactoriament"
            retval = True
        Else
            _resultmsg = exs.First.Message
        End If
        Return retval
    End Function
    ...
    Private Async Function SaveDirectDebit(exs As List(Of Exception)) As Task(Of Boolean)
        Dim retval As Boolean
        Dim oCtaCredit = Await FEB2.PgcCta.FromCod(DTOPgcPlan.Ctas.bancs, Current.Session.Emp, exs)
        Dim oAmt As DTOAmt = Xl_EurLiquid.Amt
        Dim oCca As DTOCca = DTOCca.Factory(DateTimePicker1.Value, Current.Session.User, DTOCca.CcdEnum.Pagament)
        oCca.Concept = TextBoxCcaConcept.Text
        oCca.DocFile = Xl_DocFile1.Value
    ...
    Private Async Function SaveCreditTransfer(exs As List(Of Exception)) As Task(Of Boolean)
        ...
        If oBankBranch Is Nothing Then
            ...
            oBankBranch = Await FEB2.Iban.GetBankBranchFromDigits(oIban, exs)
        End If
        If oBankBranch Is Nothing Then
        ...
        Else
            ...
            Dim CcaId = Await FEB2.BancTransferPool.Save(exs, _BancTransferPool)
            If exs.Count = 0 Then
    ...
    Private Async Sub ButtonSepaCreditTransfer_Click(sender As Object, e As EventArgs) Handles ButtonSepaCreditTransfer.Click
        ...
        Dim XmlSource As String = Await FEB2.SepaCreditTransfer.XML(Current.Session.Emp, _BancTransferPool, exs)
        If exs.Count = 0 Then
