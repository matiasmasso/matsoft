...
Public Class Frm_Holding
    ...
    Private Sub Frm_Holding_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If FEB2.Holding.Load(exs, _Holding) Then
            refresca()
    ...
    Private Async Sub ButtonOk_Click(sender As Object, e As EventArgs) Handles ButtonOk.Click
        ...
        End With
        If Await FEB2.Holding.Update(exs, _Holding) Then
    ...
    Private Async Sub ButtonDel_Click(sender As Object, e As EventArgs) Handles ButtonDel.Click
        ...
        If rc = MsgBoxResult.Ok Then
            If Await FEB2.Holding.Delete(exs, _Holding) Then
                RaiseEvent AfterUpdate(Me, New MatEventArgs(_Holding))
                Me.Close()
            Else
                UIHelper.WarnError(exs)
    ...
    Private Async Sub Xl_ClustersTree1_RequestToRefresh(sender As Object, e As MatEventArgs) Handles Xl_ClustersTree1.RequestToRefresh
        Dim exs As New List(Of Exception)
        UIHelper.ToggleProggressBar(Panel1, True)
        _Holding = Await FEB2.Holding.Find(exs, _Holding.Guid) '=========================== passar a FEB2.Holding.Clusters
        UIHelper.ToggleProggressBar(Panel1, False)
        If exs.Count = 0 Then
            With _Holding
                TextBoxNom.Text = String.Format("Holding: {0}", .nom)
        ...
        Select Case TabControl1.SelectedIndex
            Case Tabs.Pncs
                If Not _TabDone(Tabs.Pncs) Then
                    UIHelper.ToggleProggressBar(Panel1, True)
                    Dim oPncs = Await FEB2.PurchaseOrderItems.Pending(exs, _Holding, DTOPurchaseOrder.Codis.client, GlobalVariables.Emp.Mgz, DTOCustomer.GroupLevels.Holding)
                    UIHelper.ToggleProggressBar(Panel1, False)
                    If exs.Count = 0 Then
