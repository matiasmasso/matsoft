Public Class Frm_BritaxTarget
    ...
    Private Async Sub Frm_BritaxTarget_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        _orders = Await FEB2.PurchaseOrders.Headers(exs, Current.Session.Emp,
                                            Cod:=DTOPurchaseOrder.Codis.Proveidor,
                                                Contact:=oProveidor,
                                                FchCreatedFrom:=fchfrom,
                                                FchCreatedTo:=fchTo)
        If exs.Count = 0 Then
            _pendingOrders = Await FEB2.PurchaseOrders.Pending(exs, Current.Session.Emp, cod:=DTOPurchaseOrder.Codis.proveidor, contact:=oProveidor)
            _bookfras = Await FEB2.Bookfras.All(exs, DTOBookFra.Modes.all, DTOExercici.Current(Current.Session.Emp), , oProveidor)
            If exs.Count = 0 Then
    ...
    Private Sub ExcelToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ExcelToolStripMenuItem.Click
        Dim exs As New List(Of Exception)
        Dim oSheet = FEB2.Britax.ExcelTarget(_year, _bookfras, _orders, _pendingOrders)
