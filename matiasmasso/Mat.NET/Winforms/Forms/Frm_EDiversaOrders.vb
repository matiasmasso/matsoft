Public Class Frm_EDiversaOrders
    ...
    Private Async Sub Frm_EDiversaOrders_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        _AllOrders = Await FEB2.EdiversaOrders.OpenFiles(exs)
    ...
    Private Sub refrescaOrders()
        Dim oOrders As New List(Of DTOEdiversaOrder)
    ...
    Private Async Sub CheckBoxOnlyOpenOrders_CheckedChanged(sender As Object, e As EventArgs) Handles CheckBoxOnlyOpenOrders.CheckedChanged
        If _AllowEvents Then
            Xl_Years1.Visible = CheckBoxOnlyOpenOrders.Checked
            Application.DoEvents()
            Dim exs As New List(Of Exception)
            _AllOrders = Await FEB2.EdiversaOrders.OpenFiles(exs) '(CheckBoxOnlyOpenOrders.Checked)
            If exs.Count = 0 Then
                Xl_Brands_CheckList1.Load(_AllBrands, _AllBrands)
                refrescaOrders()
            Else
    ...
    Private Sub LoadYears()
        ...
        For i As Integer = Today.Year To 2009 Step -1
            oYears.Add(i)
        Next
    ...
    Private Async Sub TabControl1_SelectedIndexChanged(sender As Object, e As EventArgs) Handles TabControl1.SelectedIndexChanged
        ...
        Select Case TabControl1.SelectedIndex
            ...
            Case Tabs.Productes
                Dim oStocks = Await FEB2.ProductStocks.Skus(exs, GlobalVariables.Emp.Mgz)
                If exs.Count = 0 Then
            ...
            Case Tabs.ConfirmationPending
                Dim oEdiversaOrders As List(Of DTOEdiversaOrder) = Await FEB2.EdiversaOrders.ConfirmationPending(exs)
                Xl_EdiversaOrdersConfirmationPending.Load(oEdiversaOrders)
            Case Tabs.Confirmations
                Dim oOrdrSps = Await FEB2.EdiversaOrdrSps.Headers(exs)
                If exs.Count = 0 Then
                Else
                    UIHelper.WarnError(exs)
                End If
