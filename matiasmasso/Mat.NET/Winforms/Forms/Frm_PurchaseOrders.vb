Public Class Frm_PurchaseOrders
    ...
    Private Async Sub Frm_PurchaseOrders_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If _ContactMode = ContactModes.None Then
            Dim iYeas = Await FEB2.PurchaseOrders.Years(exs, Current.Session.Emp, DTOPurchaseOrder.Codis.Client, Nothing)
            If exs.Count = 0 Then
    ...
    Private Async Function Refresca() As Task
        ...
        Select Case _ContactMode
            Case ContactModes.None
                _Orders = Await FEB2.PurchaseOrders.Headers(exs, Current.Session.Emp, Cod:=DTOPurchaseOrder.Codis.Client, Contact:=_Contact, Year:=Xl_Years1.Value)
            Case ContactModes.Customer
                _Orders = Await FEB2.PurchaseOrders.Headers(exs, Current.Session.Emp, Cod:=DTOPurchaseOrder.Codis.Client, Contact:=_Contact)
            Case ContactModes.Ccx
                _Orders = Await FEB2.PurchaseOrders.Headers(exs, Current.Session.Emp, Cod:=DTOPurchaseOrder.Codis.Client, Ccx:=_Contact)
            Case ContactModes.Proveidor
                _Orders = Await FEB2.PurchaseOrders.Headers(exs, Current.Session.Emp, Cod:=DTOPurchaseOrder.Codis.Proveidor, Contact:=_Contact)
            Case ContactModes.Rep
                _Orders = Await FEB2.PurchaseOrders.Headers(exs, Current.Session.Emp, Cod:=DTOPurchaseOrder.Codis.Client, Rep:=_Contact)
        End Select
