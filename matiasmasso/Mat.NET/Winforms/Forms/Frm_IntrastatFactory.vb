...
Public Class Frm_IntrastatFactory
    ...
    Private Async Function refresca() As Task
        ...
        _Intrastat = Await FEB2.Intrastat.Factory(Current.Session.Emp, CurrentFlujo, Xl_YearMonth1.YearMonth, exs)
        _Intrastat.partidas = _Intrastat.partidas.Where(Function(x) x.importeFacturado > 0).ToList 'No s'admeten partides negatives
        TextBoxUrlIntrastat.Text = Await FEB2.Default.GlobalValue(DTODefault.Codis.UrlIntrastat, exs)
        If exs.Count = 0 Then
    ...
    Private Async Sub Button1_Click(sender As Object, e As EventArgs) Handles Button_Exit.Click
        ...
        If _dirtyIntrastatUrl Then
            If Await FEB2.Default.SetEmpValue(Current.Session.Emp, DTODefault.Codis.UrlIntrastat, TextBoxUrlIntrastat.Text, exs) Then
            Else
                UIHelper.WarnError(exs)
                Exit Sub
            End If
