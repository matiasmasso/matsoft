Public Class Frm_ProductSkuForecast
    ...
    Private Async Sub Frm_ProductSkuForecast_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        _Forecasts = Await FEB2.Forecasts.All(Current.Session.Emp.Mgz, exs, _Proveidor, _Product)
    ...
    Private Sub ExcelComandaPerCategoriesToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ExcelComandaPerCategoriesToolStripMenuItem.Click
        ...
        Dim query = items.GroupBy(Function(g) New With {Key g.Sku.Category.Brand, Key g.Sku.Category}).
            Select(Function(group) New With {.Brand = group.Key.Category.Brand, .Category = group.Key.Category, .Qty = group.Sum(Function(y) y.Qty), .Eur = group.Sum(Function(z) z.Qty * z.amount.Eur * (100 - z.Dto) / 100)})

        Dim oSheet As New MatHelperStd.ExcelHelper.Sheet
        For Each item In query
            Dim oRow = oSheet.AddRow()
            oRow.AddCell(item.Brand.Nom.Tradueix(DTOLang.ENG))
            oRow.AddCell(item.Category.Nom.Tradueix(DTOLang.ENG))
