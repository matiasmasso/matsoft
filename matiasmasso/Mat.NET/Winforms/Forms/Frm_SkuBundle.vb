Public Class Frm_SkuBundle
    ...
    Private Async Sub Frm_Properties_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If FEB2.ProductSku.Load(_ProductSku, exs) Then
            With _ProductSku
                ...
                Dim oImage = Await FEB2.ProductSku.Image(exs, _ProductSku)
                If exs.Count = 0 Then
                    Xl_Image1.Load(oImage, 700, 800, "imatge de producte")
                Else
    ...
    Private Async Function LoadAccessories() As Task
        Dim exs As New List(Of Exception)
        Dim oAccessories = Await FEB2.Product.Relateds(exs, DTOProduct.Relateds.Accessories, _ProductSku, GlobalVariables.Emp.Mgz)
        If exs.Count = 0 Then
    ...
    Private Async Function LoadSpares() As Task
        Dim exs As New List(Of Exception)
        Dim oSpares = Await FEB2.Product.Relateds(exs, DTOProduct.Relateds.Spares, _ProductSku, GlobalVariables.Emp.Mgz)
        If exs.Count = 0 Then
    ...
    Private Async Sub onAccessorySelected(sender As Object, e As MatEventArgs)
        Dim exs As New List(Of Exception)
        Dim oAccessories = Await FEB2.Product.Relateds(exs, DTOProduct.Relateds.Accessories, _ProductSku, GlobalVariables.Emp.Mgz, True, False)
        If exs.Count = 0 Then
            ...
            Else
                ...
                If Await FEB2.Product.UpdateRelateds(exs, DTOProduct.Relateds.Accessories, _ProductSku, oAccessories) Then
                    Await LoadAccessories()
                Else
                    UIHelper.WarnError(exs)
                End If
    ...
    Private Async Sub Xl_ProductSkusExtendedSpares_RequestToAddNewSku(sender As Object, e As MatEventArgs) Handles Xl_ProductSkusExtendedSpares.RequestToAddNewSku
        Dim exs As New List(Of Exception)
        Dim oSpares = Await FEB2.Product.Relateds(exs, DTOProduct.Relateds.Spares, _ProductSku, GlobalVariables.Emp.Mgz, True, False)
        If exs.Count = 0 Then
            ...
            Else
                ...
                If Await FEB2.Product.UpdateRelateds(exs, DTOProduct.Relateds.Spares, _ProductSku, oSpares) Then
                    Await LoadSpares()
                Else
                    UIHelper.WarnError(exs)
                End If
