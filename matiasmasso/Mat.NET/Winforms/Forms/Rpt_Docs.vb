...
#End Region
    ...
    Private CurrentDoc As DTODoc
    Private CurrentItm As DTODocItm
    ...
    Public WriteOnly Property DocRpt() As DTODocRpt
        Set(ByVal Value As DTODocRpt)
            mDocRpt = Value
        End Set
    End Property
    ...
    Private Sub DoClientne(ByVal oItm As DTODocItm)
        Dim oImport As DTOAmt = oItm.Import()
    ...
    Private Function DocEnd() As Boolean
        ...
        If mTot.Eur <> 0 Then
            ...
            If CurrentDoc.Descomptes() Then
                ...
                If CurrentDoc.Dto <> 0 Then
                    oTmp = mTot.Percent(CurrentDoc.Dto)
                    DrawGralDto(CurrentDoc.Dto, oTmp)
                    mTot.Substract(oTmp)
                End If
                If CurrentDoc.Dpp <> 0 Then
                    If CurrentDoc.Dto <> 0 Then DrawSum(SumConcepts.SumaParcial)
                    oTmp = mTot.Percent(CurrentDoc.Dpp)
                    DrawDpp(CurrentDoc.Dpp, oTmp)
                    mTot.Substract(oTmp)
                End If
            End If
            ...
            If CurrentDoc.Impostos Then
                ...
                oBase = mTot.Clone
                For Each oQuota As DTOTaxBaseQuota In CurrentDoc.IvaBaseQuotas

                    Select Case oQuota.Tax.Codi
                        Case DTOTax.Codis.Iva_Standard, DTOTax.Codis.Iva_Reduit, DTOTax.Codis.Iva_SuperReduit
                            oTmp = DrawIVA(oQuota.Tax.Tipus, oQuota.Base, oQuota.Base.unEquals(oBase))
                            mTot.Add(oTmp)
                        Case DTOTax.Codis.Recarrec_Equivalencia_Standard, DTOTax.Codis.Recarrec_Equivalencia_Reduit, DTOTax.Codis.Recarrec_Equivalencia_SuperReduit
                            oTmp = DrawREQ(oQuota.Tax.Tipus, oQuota.Base, oQuota.Base.unEquals(oBase))
                            mTot.Add(oTmp)
                    End Select
        ...
        If CurrentDoc.Coletillas IsNot Nothing Then
    ...
    Private Sub DrawDestinatari(ByVal oArrayList As List(Of String))
    ...
    Private Sub DrawObs(ByVal oArrayList As List(Of String))
    ...
    Private Sub DrawPreu(ByVal oPreu As DTOAmt)
        Dim tmpStr As String = Format(oPreu.Val, CurrentDoc.Cur.FormatString)
    ...
    Private Sub DrawImport(ByVal oImport As DTOAmt)
        Dim tmpStr As String = oImport.CurFormatted
    ...
    Private Sub DrawGralDto(ByVal SngDto As Decimal, ByVal oImporte As DTOAmt)
        Dim tmpStr As String
        tmpStr = CurrentDoc.Lang.Tradueix("Descuento", "Descompte", "Discount")
        tmpStr = tmpStr & " " & SngDto & " %"
        DrawEndLine(tmpStr, oImporte, False)
    End Sub

    Private Sub DrawDpp(ByVal SngDto As Decimal, ByVal oImporte As DTOAmt)
        Dim tmpStr As String
    ...
    Private Function DrawIVA(ByVal DcIva As Decimal, ByVal oBase As DTOAmt, ByVal BlPrintBase As Boolean) As DTOAmt
        Dim tmpStr As String
        ...
        DrawEndLine(tmpStr, oAmt, False)
        Return oAmt
    End Function

    Private Function DrawREQ(ByVal DcREQ As Decimal, ByVal oBase As DTOAmt, ByVal BlPrintBase As Boolean) As DTOAmt
        ...
        If BlPrintBase Then
            ...
            tmpStr = tmpStr & oBase.CurFormatted
        End If
        Dim retval As DTOAmt = oBase.Percent(DcREQ)
        DrawEndLine(tmpStr, retval, False)
        Return retval
    End Function

    Private Sub DrawEndLine(ByVal tmpStr As String, ByVal oImporte As DTOAmt, ByVal Raya As Boolean)
        Dim X As Integer
        If Raya Then
