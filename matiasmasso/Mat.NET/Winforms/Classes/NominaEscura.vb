...
Public Class NominaEscura
    ...
    Private Function DocFile(oShapeRange As Word.ShapeRange, oStaff As DTOStaff, ByRef exs As List(Of Exception)) As DTODocFile
        Dim sFilename As String = SavePage(oShapeRange, oStaff)
        Dim retval As DTODocFile = BLLDocFile.FromFile(sFilename, exs)
        Dim oThumb As Image = retval.Thumbnail
        Return retval
    End Function

    Private Function SavePage(oShapeRange As Word.ShapeRange, oStaff As DTOStaff) As String
    ...
    Shared Function LogoFilename() As String
        ...
        If Not Loaded Then
            ...
            Dim oThumb As Image = BLL.ImageHelper.GetThumbnailToFit(oLogo, 100)
            oThumb.Save(retval)
            Loaded = True
        End If
        Return retval
    ...
    Private Function ReadNomina(ByRef exs As List(Of Exception)) As Nomina
        ...
        Dim retval As New Nomina(_NumSeguretatSocial, _Fch)
        If retval.Staff Is Nothing Then
            retval = Nothing
            exs.Add(New Exception("no s'ha trobat cap empleat amb el següent numero de Seg.Social:" _
                                  & vbCrLf _
                                  & _NumSeguretatSocial _
                                  & vbCrLf _
                                  & "revisar les dades de " & _Name))
        Else
            For Each oEscuraItem As NominaEscuraItem In _Items
                ...
                With oItem
                    .Qty = oEscuraItem.Qty
                    .Preu = BLLApp.GetAmt(oEscuraItem.Price)
            ...
            With retval
                ...
                Else
                    If .Staff.Iban Is Nothing Then
                        exs.Add(New Exception(BLLStaff.AliasOrNom(.Staff) & " no té registrat cap compte corrent"))
                    Else
                        .Iban = .Staff.Iban
                    End If
                End If
                ...
                If IsNumeric(_TotalDevengat) Then
                    .Devengat = BLLApp.GetAmt(CDec(_TotalDevengat))
                End If

                .Fch = _Fch

                .Dietes = BLLApp.GetAmt(DcDietas)
                .Embargos = BLLApp.GetAmt(DcEmbargos)
                .Deutes = BLLApp.GetAmt(DcDeutes)
                .SegSocial = BLLApp.GetAmt(DcSegSoc)
                .Irpf = BLLApp.GetAmt(DcIrpf)

                If IsNumeric(Me.BaseIRPF) Then
                    .IrpfBase = BLLApp.GetAmt(CDec(Me.BaseIRPF))
                End If

                If IsNumeric(Me.Liquid) Then
                    .Liquid = BLLApp.GetAmt(CDec(Me.Liquid))
                Else
                    .Liquid = BLLApp.EmptyAmt
                End If
                ...
                Dim sNom As String = BLLStaff.AliasOrNom(.Staff)
                ...
                .Cca = BLLCca.Factory(.Fch, BLLSession.Current.User, DTOCca.CcdEnum.Nomina)
                .Cca.Concept = String.Format("{0}-{1}", sNom, sText)

                If .Devengat IsNot Nothing Then
                    If .Devengat.IsNotZero Then
                        Dim oDevengatMenysDietes As DTOAmt = .Devengat.Clone
                        oDevengatMenysDietes.Substract(.Dietes)
                        BLLCca.AddDebit(.Cca, oDevengatMenysDietes, DTOPgcPlan.Ctas.Nomina, .Staff)
                    End If
                End If

                If .Dietes.IsNotZero Then
                    BLLCca.AddDebit(.Cca, .Dietes, DTOPgcPlan.Ctas.Dietas, .Staff)
                End If

                If .SegSocial.IsNotZero Then
                    BLLCca.AddCredit(.Cca, .SegSocial, DTOPgcPlan.Ctas.SegSocialDevengo)
                End If

                If .Irpf.IsNotZero Then
                    BLLCca.AddCredit(.Cca, .Irpf, DTOPgcPlan.Ctas.IrpfTreballadors, .Staff)
                End If

                If .Embargos.IsNotZero Then
                    BLLCca.AddCredit(.Cca, .Embargos, DTOPgcPlan.Ctas.NominaEmbargos, .Staff)
                End If

                If .Deutes.IsNotZero Then
                    BLLCca.AddCredit(.Cca, .Deutes, DTOPgcPlan.Ctas.NominaDeutes, .Staff)
                End If

                If .Liquid.IsNotZero Then
                    BLLCca.AddCredit(.Cca, .Liquid, DTOPgcPlan.Ctas.PagasTreballadors, .Staff)
                End If
...
Public Class NominaEscuraItem
    ...
    Public Property Cod As DTONomina.Concepte
    Public Property Devengo As Decimal
    Public Property Deduccio As Decimal

    Public Sub New(oCod As DTONomina.Concepte)
        MyBase.New()
        _Cod = oCod
    End Sub
End Class
