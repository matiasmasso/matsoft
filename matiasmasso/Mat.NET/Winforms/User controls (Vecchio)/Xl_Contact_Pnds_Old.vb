...
Public Class Xl_Contact_Pnds_Old
    ...
    Private Sub Calcula()
        ...
        If oDeutor.Val = 0 Then
            LabelDeutor.Text = ""
        Else
            LabelDeutor.Text = "total deutor: " & oDeutor.CurFormatted
        End If

        If oCreditor.Val = 0 Then
            LabelCreditor.Text = ""
        Else
            LabelCreditor.Text = "total creditor: " & oCreditor.CurFormatted
        End If
    ...
    Private Sub ShowPnd()
        Dim oFrm As New Frm_Contact_Pnd(CurrentPnd.ToDTO)
        AddHandler oFrm.AfterUpdate, AddressOf RefreshRequest
        oFrm.Show()
    End Sub
    ...
    Private Sub ExtracteToolStripButton_Click(ByVal sender As Object, ByVal e As System.EventArgs) 'Handles ExtracteToolStripButton.Click
        ...
        Dim oPlan As PgcPlan = PgcPlan.FromToday
        Dim oRow As DataGridViewRow = DataGridView1.CurrentRow
        Dim oCta As DTOPgcCta = MaxiSrvr.PgcCta.FromNum(oPlan, oRow.Cells(Cols.Cta).Value).ToNewCta
        Dim oFrm As New Frm_Extracte(mContact.ToNewContact, oCta)
        oFrm.Show()
    End Sub
