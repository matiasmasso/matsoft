...
Public Class Xl_Pdc_LineItems_Proveidor
    ...
    Private _Pdc As DTOPurchaseOrder
    ...
    Public WriteOnly Property Pdc() As Pdc
        Set(ByVal value As Pdc)
            mPdc = value
            _Pdc = New DTOPurchaseOrder(mPdc.Guid)
            Refresca()
        End Set
    End Property
    ...
    Private Function GetItmFromRow(ByVal oRow As DataGridViewRow, Optional ByVal iCurrentLineNumber As Integer = 0) As LineItmPnc
        ...
        Dim oItm As LineItmPnc = Nothing
        If IsDBNull(oRow.Cells(Cols.PncGuid).Value) Then
            oItm = New LineItmPnc(mPdc)
        Else
            oItm = New LineItmPnc(CType(oRow.Cells(Cols.PncGuid).Value, Guid))
        End If
        With oItm
    ...
    Private Function GetArtFromDataGridViewRow(ByVal oRow As DataGridViewRow) As Art
        ...
        If Not IsDBNull(oRow.Cells(Cols.ArtGuid).Value) Then
    ...
    Private Function GetSkuFromDataGridViewRow(ByVal oRow As DataGridViewRow) As DTOProductSku
    ...
    Private Sub FeedRowFromArt(ByRef oRow As DataGridViewRow, ByVal oArt As Art)
        If oArt Is Nothing Then
            oRow.Cells(Cols.ArtGuid).Value = Guid.Empty
        ...
        Else
            ...
            oRow.Cells(Cols.PncGuid).Value = Guid.NewGuid
    ...
    Private Function CreateTable() As DataTable
        ...
        With oTb.Columns
            .Add("PncGuid", System.Type.GetType("System.Guid"))
    ...
    Private Function CurrentLine() As LineItmPnc
        ...
        If oRow IsNot Nothing Then
            ...
            If Not oArtGuid.Equals(Guid.Empty) Then
                Dim oArt As New Art(oArtGuid)
                oLine = New LineItmPnc(CType(oRow.Cells(Cols.pncGuid).Value, Guid))
                oLine.SetItm()
                oLine.Art = oArt
    ...
    Private Sub SetContextMenu()
        ...
        Try
            ...
            If oItm.Art IsNot Nothing Then
                Dim oGuid As Guid = oItm.Art.Guid
    ...
    Private Sub DataGridView1_CellFormatting(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellFormattingEventArgs) Handles DataGridView1.CellFormatting
        Select Case e.ColumnIndex
            ...
            Case Cols.Pendent
                ...
                'e.CellStyle.BackColor = DTO.Defaults.COLOR_NOTOK
                'End If
        End Select
    End Sub
        ...
        If iNewQty <> iOldQty Then
            ...
            If iPack > 1 Then
                ...
                If iMod <> 0 Then
                    ...
                    Dim oSku As DTOProductSku = GetSkuFromDataGridViewRow(oRow)
                    If Not BLL.BLLProduct.AllowUserToFraccionarInnerPack(oSku, BLL.BLLSession.Current.User) Then
        ...
        If Not IsDBNull(oRow.Cells(Cols.Lin).Value) Then
            ...
            If iLin > 0 Then
                ...
                If sNewValue <> sOldValue Then
                    'check sortides
                    Dim oLineItmPnc As New LineItmPnc(CType(oRow.Cells(Cols.PncGuid).Value, Guid))
    ...
    Private Function GetPendents(ByVal oRow As DataGridViewRow) As Integer
        ...
        If IsNumeric(oRow.Cells(Cols.Qty).Value) Then
    ...
    Private Sub CalculaImports(ByVal oRow As DataGridViewRow)
        ...
        If BlCarrec Then
            DcAmt = Math.Round(iQty * DcPvp * (100 - DcDto) / 100, 2, MidpointRounding.AwayFromZero)
        End If
    ...
    Private Sub CheckFreeUnits()
        ...
        If oArt IsNot Nothing Then
            If HasCarrec(oRow) And Not IsDBNull(oRow.Cells(Cols.Qty).Value) Then
                ...
                Dim oIncentius As Incentius = oProduct.Incentius(MaxiSrvr.Incentiu.Cods.FreeUnits, mPdc.Fch, New DTOCustomer(mPdc.Client.Guid))
