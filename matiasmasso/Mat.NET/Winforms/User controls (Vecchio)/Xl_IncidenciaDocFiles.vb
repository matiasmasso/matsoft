...
Public Class Xl_IncidenciaDocFiles
    ...
    Private _Mode As DTOIncidencia.AttachmentCods
    ...
    Public Shadows Sub Load(oIncidencia As DTOIncidencia, oMode As DTOIncidencia.AttachmentCods)
    ...
    Private Sub Refresca()
        ...
        Dim oFiles As List(Of DTODocFile) = IIf(_Mode = DTOIncidencia.AttachmentCods.imatge, _Incidencia.docFileImages, _Incidencia.purchaseTickets)
    ...
    Private Function CurrentDocFileName(oMime As MimeCods) As String
        Dim retval = String.Format("incidencia.{0}.{1}.{2:00}{3}", _Incidencia.num.ToString, _Mode.ToString, DataGridView1.CurrentRow.Index + 1, MediaHelper.GetExtensionFromMime(oMime))
        Return retval
    End Function
    ...
    Private Async Sub Do_ExportDocFile()
        ...
        With oDlg
            Dim oStream = Await FEB2.DocFile.StreamOrDownload(oDocFile, exs)
            If exs.Count = 0 Then
    ...
    Private Sub Do_CopyLinkDocFile()
        ...
        Clipboard.SetDataObject(FEB2.DocFile.DownloadUrl(oDocFile, True))
    End Sub
        ...
        Public Sub New(oDocFile As DTODocFile)
            ...
            With oDocFile
                If .Thumbnail Is Nothing Then
                    Dim oImage = MatHelperStd.ImageHelper.GetImgFromByteArray(.Stream)
                    _Thumbnail = LegacyHelper.ImageHelper.Converter(MatHelperStd.ImageHelper.GetThumbnailToFill(oImage, 100, 100))
                Else
                    _Thumbnail = LegacyHelper.ImageHelper.Converter(MatHelperStd.ImageHelper.GetThumbnailToFill(.Thumbnail, 100, 100))
                End If
            End With
        End Sub
