Public Class Xl_PurchaseOrderItems2
    ...
    Private Sub refresca()
        ...
        For Each item As DTOPurchaseOrderItem In _Order.Items
            ...
            If item.PackParent Is Nothing Or item.Equals(item.PackParent) Then
...
#End Region
    ...
    Public Function GetTotals() As DTOAmt
        ...
        Dim retval As DTOAmt = BLLApp.GetAmt(DcBase)
        Return retval
    End Function
    ...
    Private Function IsAllowed(oCustomer As DTOCustomer, oSku As DTOProductSku) As Boolean
        ...
        ElseIf oCustomer.Rol.Id = DTORol.Ids.Cli Then
            If BLLElCorteIngles.Belongs(oCustomer) Then
                retval = True
            Else
        ...
        If _AllowEvents Then
            Select Case e.ColumnIndex
                Case Cols.SkuNom
                    ...
                    Else
                        If oSku.Virtual Then BLLProductPack.Load(oSku)
                        Dim oControlItem As ControlItem = oRow.DataBoundItem
                        Dim item As DTOPurchaseOrderItem = oControlItem.Source
                        With item
                            ...
                            .Sku = oSku
                            Dim oCost As DTOAmt
                            oCost = BLL.BLLPricelistItemCustomer.GetCustomerCost(_Order.Customer, .Sku, _TarifaDtos, _Fch)
                            If oCost Is Nothing Then
                                .Price = BLLApp.GetAmt(0)
                                .Dto = 0
                            Else
                                If _Order.Customer.Ccx.Tarifa = DTOCustomer.Tarifas.FiftyFifty Then
                                    .Price = BLLApp.GetAmt(Math.Round(oCost.Eur / 2, 2))
                                Else
                                    .Price = oCost
                                End If
                            ...
                            If .Sku.Virtual Then LoadPackItems(oControlItem)
                        End With
                ...
                Case Cols.Qty
                    ...
                    If item.Sku.Virtual Then
                        For Each oPackedOrderItem As DTOPurchaseOrderItem In item.PackChildren()
                            Dim oPackItem As DTOProductPackItem = item.Sku.PackItems.Find(Function(x) x.Equals(oPackedOrderItem.Sku))
                            If oPackItem IsNot Nothing Then
                                oPackedOrderItem.Qty = item.Qty * oPackItem.Qty
                            End If
                        Next
                    End If
                ...
                Case Cols.Price
                    ...
                    Dim item As DTOPurchaseOrderItem = oControlItem.Source
                    With oControlItem
                        item.Price = BLLApp.GetAmt(.Price)
                        If .Price <> 0 Then
                            .Amt = Math.Round(.Price * .Qty * (100 - .Discount) / 100, 2)
                        End If
                    End With
    ...
    Private Sub LoadPackItems(oControlItem As ControlItem)
        Dim idx As Integer = _ControlItems.IndexOf(oControlItem)
        Dim oParent As DTOPurchaseOrderItem = oControlItem.Source
        oParent.PackParent = oParent

        Dim oSku As DTOProductSku = oParent.Sku
        oSku.PackItems = BLLProductPackItems.All(oSku)
        For Each oPackItem As DTOProductPackItem In oSku.PackItems
            Dim item As New DTOPurchaseOrderItem()
            With item
                .PurchaseOrder = oParent.PurchaseOrder
                .Qty = oPackItem.Qty * oControlItem.Qty
                .Pending = .Qty
                .Price = oPackItem.Price
                .Sku = oPackItem
                .PackParent = oParent
            End With

            oParent.PackChildren.Add(item)
        Next

        oSku.Stock = BLLProductPack.Stock(oSku)
        oSku.Clients = BLLProductPack.Clients(oSku)
        oSku.RRPP = BLLProductPack.Retail(oSku)
    End Sub
        ...
        Public Sub Load(item As DTOPurchaseOrderItem)
            ...
            If item.ChargeCod = DTOPurchaseOrderItem.ChargeCods.Chargeable Then
                ...
                If item.Price IsNot Nothing Then
                    ...
                    _Amt = BLL.BLLPurchaseOrderItem.Amt(item).Eur
                End If
            End If
