Public Class Xl_Deliveries
    ...
    Private Function FilteredValues() As List(Of DTODelivery)
        ...
        Else
            If IsNumeric(_Filter) Then
                retval = _Values.FindAll(Function(x) Not String.IsNullOrEmpty(x.Customer.Nom) AndAlso x.Customer.Nom.ToLower.Contains(_Filter.ToLower) Or x.Id.ToString.Contains(_Filter) Or Not String.IsNullOrEmpty(x.Customer.FullNom) AndAlso x.Customer.FullNom.ToLower.Contains(_Filter.ToLower))
            Else
                retval = _Values.FindAll(Function(x) Not String.IsNullOrEmpty(x.Customer.Nom) AndAlso x.Customer.Nom.ToLower.Contains(_Filter.ToLower) Or Not String.IsNullOrEmpty(x.Customer.FullNom) AndAlso x.Customer.FullNom.ToLower.Contains(_Filter.ToLower))
            End If
        End If
        Return retval
    End Function
    ...
    Private Sub SetProperties()
        Dim sameCustomer As Boolean = _Values.All(Function(x) x.Customer.Equals(_Values.First.Customer))
    ...
    Protected Class ControlItem
        ...
        Public Sub New(value As DTODelivery)
            ...
            With value
                ...
                _Fch = .Fch
                _Clx = .Customer.Nom
                If _Clx = "" Then _Clx = .Customer.FullNom
