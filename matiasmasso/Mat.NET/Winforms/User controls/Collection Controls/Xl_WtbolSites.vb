Public Class Xl_WtbolSites

    Inherits _Xl_ReadOnlyDatagridview

    Private _Values As List(Of DTOWtbolSite)
    ...
    Public Shadows Sub Load(values As List(Of DTOWtbolSite), Optional oDefaultValue As DTOWtbolSite = Nothing, Optional oSelectionMode As DTO.Defaults.SelectionModes = DTO.Defaults.SelectionModes.Browse)
        _Values = values
    ...
    Private Sub Refresca()
        _AllowEvents = False
        Dim oFilteredValues As List(Of DTOWtbolSite) = FilteredValues()
        _ControlItems = New ControlItems
        For Each oItem As DTOWtbolSite In oFilteredValues
        ...
        If _DefaultValue IsNot Nothing Then
            ...
            If rowIdx >= 0 Then
                MyBase.CurrentCell = MyBase.Rows(rowIdx).Cells(Cols.Web)
            End If
        End If
    ...
    Private Function FilteredValues() As List(Of DTOWtbolSite)
        Dim retval As List(Of DTOWtbolSite)
        If _Filter = "" Then
            retval = _Values
        Else
            retval = _Values.FindAll(Function(x) x.Web.ToLower.Contains(_Filter.ToLower) Or x.Customer.FullNom.ToLower.Contains(_Filter.ToLower))
        End If
        Return retval
    End Function
    ...
    Private Sub SetProperties()
        ...
        'MyBase.RowWtbolSite.DefaultCellStyle.BackColor = Color.Transparent
    ...
    Private Sub Xl_BudgetOrders_CellFormatting(sender As Object, e As DataGridViewCellFormattingEventArgs) Handles MyBase.CellFormatting
        Select Case e.ColumnIndex
            Case Cols.ico
                ...
                Dim oControlItem As ControlItem = oRow.DataBoundItem
                Dim oWtbolSite As DTOWtbolSite = oControlItem.Source
                If oWtbolSite.Active Then
    ...
    Protected Class ControlItem
        ...
        Public Sub New(value As DTOWtbolSite)
            MyBase.New()
            _Source = value
            With value
                _Web = .Web
                _Cli = .Customer.FullNom
            End With
        End Sub
