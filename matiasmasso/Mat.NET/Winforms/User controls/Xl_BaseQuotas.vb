...
Public Class Xl_BaseQuotas
    ...
    Private Async Sub Do_Extracte(oCtaCod As DTOPgcPlan.Ctas)
        ...
        Dim oCcbs = Await FEB2.Ccbs.All(exs, _IvaLiquidacio.Exercici, DTOPgcCta.FromCodi(_AllCtas, oCtaCod),, _IvaLiquidacio.YearMonth.LastFch)
        oCcbs = oCcbs.OrderBy(Function(x) x.cca.cdn).OrderBy(Function(y) y.cca.ccd).OrderBy(Function(z) z.cca.fch).ToList

        Dim oSheet = FEB2.Extracte.Excel(oCcbs, Current.Session.Lang)
    ...
    Private Async Sub Do_LlibreFacturesEmesesNacional()
        Dim exs As New List(Of Exception)
        Dim oAllInvoices = Await FEB2.Invoices.All(exs, _IvaLiquidacio.Exercici, _IvaLiquidacio.Month)
        If exs.Count = 0 Then
    ...
    Private Sub Do_LlibreFacturesEmeses(oInvoices As List(Of DTOInvoice), sFilename As String)
        Dim oSheet As New ExcelHelper.Sheet(sFilename)
        FEB2.Invoices.LoadExcel(oSheet, oInvoices)
        Dim exs As New List(Of Exception)
        If Not UIHelper.ShowExcel(oSheet, exs) Then
            UIHelper.WarnError(exs)
        End If
    ...
    Private Async Sub Do_BookFrasSoportatInterior()
        Dim exs As New List(Of Exception)
        Dim oAllBookFras = Await FEB2.Bookfras.All(exs, DTOBookFra.Modes.All, _IvaLiquidacio.Exercici, _IvaLiquidacio.Month)
        If exs.Count = 0 Then
    ...
    Private Sub Do_BookFras(oBookFras As List(Of DTOBookFra), sFilename As String)
        Dim oSheet As New ExcelHelper.Sheet(sFilename)
        FEB2.Bookfras.LoadExcel(oSheet, oBookFras)
        Dim exs As New List(Of Exception)
        If Not UIHelper.ShowExcel(oSheet, exs) Then
            UIHelper.WarnError(exs)
        End If
    ...
    Public Function ExcelSheet() As ExcelHelper.Sheet
        Dim retval As New ExcelHelper.Sheet
        With retval
            .AddColumn("Concepte", ExcelHelper.Sheet.NumberFormats.PlainText)
            .AddColumn("Base", ExcelHelper.Sheet.NumberFormats.Euro)
            .AddColumn("Tipus", ExcelHelper.Sheet.NumberFormats.Percent)
            .AddColumn("Quota", ExcelHelper.Sheet.NumberFormats.Euro)
        End With
