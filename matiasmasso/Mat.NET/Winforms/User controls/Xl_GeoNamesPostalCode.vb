Public Class Xl_GeoNamesPostalCode
    ...
    Private Async Sub TextBoxZip_Validated(sender As Object, e As EventArgs) Handles TextBoxZip.Validated
        ...
        Dim sCountryIso = ComboBoxCountry.SelectedValue
        Dim sZipCod = TextBoxZip.Text
        If sZipCod = "" Then
            Xl_Lookup_Zip1.Clear()
        Else
            Dim oPostalCodes As List(Of DT2.Google.Geonames.postalCode) = Await FEB2.GeoNames.Locations(exs, sCountryIso, sZipCod)
    ...
    Private Async Function loadZip(oPostalCode As DT2.Google.Geonames.postalCode) As Task
        Dim exs As New List(Of Exception)
        Dim oCountry = Await CurrentCountry(exs)
        If exs.Count = 0 Then
            Dim sZipCod As String = oPostalCode.postalCode
            Dim oZips = Await FEB2.Zips.All(exs, oCountry, sZipCod)
            If exs.Count = 0 Then
    ...
    Private Sub loadPlaceNames(oPostalCodes As List(Of DT2.Google.Geonames.postalCode))
        _AllowEvents = False
    ...
    Private Async Sub ComboBoxPlaceNames_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ComboBoxPlaceNames.SelectedIndexChanged
        Dim exs As New List(Of Exception)
        If _AllowEvents Then
            Dim oPostalCodes As List(Of DT2.Google.Geonames.postalCode) = ComboBoxPlaceNames.Tag
            ...
            Else
                ...
                Dim oZonas = Await FEB2.Zonas.All(exs, oPostalCode)
