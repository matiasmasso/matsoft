Public Class MatDataGridView
    ...
    Private Function CanTabToNextCell() As Boolean
        ...
        Do While (Not found)
            ...
            Dim tabStopRow As Boolean = Not MyBase.Rows(nextCellAddress.Y).ReadOnly

            If TypeOf MyBase.Rows(nextCellAddress.Y).Cells(nextCellAddress.X) Is ISupportTabStop Then
                Dim tabstopcell As ISupportTabStop =
                DirectCast(Me.Rows(nextCellAddress.Y).Cells(nextCellAddress.X), ISupportTabStop)
                If tabstopcell.TabStop And tabStopRow Then
                    found = True
                End If
            ElseIf tabStopRow Then
                found = True
            End If

            If Not found AndAlso
               nextCellAddress.X <= currentCellAddress.X AndAlso
    ...
    Private Function TabToCell(ByVal keyData As Keys) As Boolean
        ...
        Do While (Not found)
            ...
            If TypeOf Me.CurrentCell Is ISupportTabStop Then
                ...
                Dim tabstoprow As Boolean = MyBase.Rows(Me.CurrentCell.RowIndex).ReadOnly = False
                If tabstopcell.TabStop And tabstoprow Then
                    Return True
                End If
            Else
