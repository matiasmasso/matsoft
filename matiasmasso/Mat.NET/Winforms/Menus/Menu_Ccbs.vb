Public Class Menu_Ccbs
    ...
    Public Sub New(ByVal values As List(Of DTOCcb))
        ...
        If _Values.Count > 0 Then
            ...
            _IsAllowedToBrowse = FEB2.Cca.IsAllowedToBrowse(oFirstCca, Current.Session.User)
        End If
    End Sub
        ...
        Else
            ...
            Select Case oCcd
                Case DTOCca.CcdEnum.FacturaNostre
                    Dim oInvoice = FEB2.Invoice.FromNumSync(exs, Current.Session.Emp, _Values.First.Cca.Fch.Year, _Values.First.Cca.Cdn)
                    If exs.Count = 0 Then
                ...
                Case DTOCca.CcdEnum.Amortitzacions
                    Dim oAmortitzacioItem = FEB2.AmortizationItem.FromCcaSync(_Values.First.Cca, exs)
                ...
                Case DTOCca.CcdEnum.InmovilitzatAlta
                    Dim oAmortitzacio = FEB2.Amortization.FromAltaSync(exs, _Values.First.Cca)
                ...
                Case DTOCca.CcdEnum.InmovilitzatBaixa
                    Dim oAmortitzacio = FEB2.Amortization.FromBaixaSync(exs, _Values.First.Cca)
                    If oAmortitzacio Is Nothing Then
                        oMenuItem.Visible = False
                    Else
                        oMenuItem.Text = "Actiu..."
    ...
    Private Async Sub Do_BookFra(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim exs As New List(Of Exception)
        Dim oCca As DTOCca = _Values.First.Cca
        Dim oBookFra = Await FEB2.BookFra.FindOrNew(exs, oCca)
        If exs.Count = 0 Then
    ...
    Private Async Sub Do_Del(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If rc = MsgBoxResult.Ok Then
            ...
            For Each oCcb As DTOCcb In _Values
                Await FEB2.Cca.Delete(exs, oCcb.Cca)
            Next
            If exs.Count = 0 Then
    ...
    Private Async Sub Do_Specific_Csb(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim exs As New List(Of Exception)
        If FEB2.Cca.Load(_Values.First.Cca, exs) Then
            Dim oCsb = Await FEB2.Csb.Find(_Values.First.Cca.Ref.Guid, exs)
            If exs.Count = 0 Then
    ...
    Private Async Sub Do_Specific_CsbRevert(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim exs As New List(Of Exception)
        Dim oCca As DTOCca = _Values.First.Cca
        If Await FEB2.Csb.RevertVto(oCca, exs) Then
