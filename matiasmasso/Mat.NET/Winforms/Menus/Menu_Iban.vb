Public Class Menu_Iban
    ...
    Private Function MenuItem_Titular() As ToolStripMenuItem
        ...
        If _Iban.Titular IsNot Nothing Then
            Dim oMenuContact As New Menu_Contact(_Iban.Titular)
    ...
    Private Function MenuItem_Mandats() As ToolStripMenuItem
        Dim oMenuItem As New ToolStripMenuItem
        oMenuItem.Text = "Mandats contra compte nostre"
        AddHandler oMenuItem.Click, AddressOf Do_ShowMandatos
        Return oMenuItem
    End Function
    ...
    Private Sub Do_BrowseImg(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim sUrl = FEB2.Iban.ImgUrl(_Iban.Digits, True)
    ...
    Private Async Sub Do_BlankToSign()
        ...
        If FEB2.Iban.Load(_Iban, exs) Then
            ...
            Else
                If FEB2.Iban.Swift(_Iban.BankBranch, exs) Is Nothing Then
                    MsgBox("Codi Swift no registrat", MsgBoxStyle.Exclamation, "MAT.NET")
                Else
                    If FEB2.Contact.Load(_Iban.titular, exs) Then
                        Dim sSwift = FEB2.Iban.Swift(_Iban)
        ...
        With oDlg
            ...
            If .ShowDialog = DialogResult.OK Then
                ...
                If oDocfile Is Nothing Then
                    UIHelper.WarnError(exs)
                Else
                    If Await FEB2.Iban.UploadAndApprove(_Iban, oDocFile, Current.Session.User, exs) Then
    ...
    Private Async Sub Do_Del(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If rc = MsgBoxResult.Ok Then
            Dim exs As New List(Of Exception)
            If Await FEB2.Iban.Delete(_Iban, exs) Then
    ...
    Private Async Sub Do_Structure(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim exs As New List(Of Exception)
        Dim oCountry = Await FEB2.Iban.Country(_Iban, exs)
        Dim oIbanStructure = Await FEB2.IbanStructure.Find(oCountry, exs)
        If exs.Count = 0 Then
    ...
    Private Async Sub Do_SelectBankBranch()
        ...
        If _Iban.IbanStructure IsNot Nothing Then
            Dim oBankBranch As DTOBankBranch = Await FEB2.IbanStructure.GetBankBranch(_Iban, exs)
            If exs.Count = 0 Then
                If oBankBranch Is Nothing Then
                    Dim oBank = Await FEB2.IbanStructure.GetBank(_Iban, exs)
                    If exs.Count = 0 Then
    ...
    Private Sub Do_ShowMandatos()
        Dim oFrm As New Frm_SepaMandatos(_Iban)
        oFrm.Show()
    End Sub
