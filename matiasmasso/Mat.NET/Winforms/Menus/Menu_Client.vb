...
Public Class Menu_Client
    ...
    Public Function Range() As ToolStripMenuItem()
        ...
        MenuItem_Credit(),
        MenuItem_Sellout(),
        MenuItem_ExcelLast3yearsDelivered(),
        MenuItem_CitClis(),
        MenuItem_Tarifas(),
        MenuItem_Margins(),
    ...
    Private Function MenuItem_Group() As ToolStripMenuItem
        ...
        If FEB2.Customer.IsGroupSync(exs, _Customer) Then
    ...
    Private Function MenuItem_ExcelLast3yearsDelivered()
        Dim oMenuItem As New ToolStripMenuItem
        oMenuItem.Image = My.Resources.Excel
        AddHandler oMenuItem.Click, AddressOf Do_ExcelLast3yearsDelivered
        oMenuItem.Text = "envios 3 ultims anys"
        Return oMenuItem
    End Function
    ...
    Private Async Sub Do_NewPdc(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim exs As New List(Of Exception)
        If FEB2.Customer.Load(_Customer, exs) Then
            ...
            If Await FEB2.AlbBloqueig.BloqueigStart(Current.Session.User, oOrder.Contact, DTOAlbBloqueig.Codis.PDC, exs) Then
    ...
    Private Async Sub Do_LastAlbs(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim exs As New List(Of Exception)
        Dim oDeliveries = Await FEB2.Deliveries.Headers(exs, _Customer)
        If exs.Count = 0 Then
    ...
    Private Async Sub Do_NewSpv(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If FEB2.Customer.Load(_Customer, exs) Then
            With oSpv
                ...
                .Tel = Await FEB2.Contact.Tel(exs, _Customer)
            End With
    ...
    Private Async Sub Do_Sellout(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim exs As New List(Of Exception)
        Dim oSellout = Await FEB2.SellOut.Factory(exs, Current.Session.User,, DTOSellOut.ConceptTypes.Product)
        If exs.Count = 0 Then
            With oSellout
    ...
    Private Async Sub Do_ExcelLast3yearsDelivered()
        Dim exs As New List(Of Exception)
        Dim oSheet = Await FEB2.Deliveries.ExcelHistoricDetallat(_Customer, exs)
        If exs.Count = 0 Then
    ...
    Private Sub Do_CitClis(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim exs As New List(Of Exception)
        If FEB2.Contact.Load(_Customer, exs) Then
    ...
    Private Sub Do_Margins(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim exs As New List(Of Exception)
        Dim oCustomer = FEB2.Customer.CcxOrMe(exs, _Customer)
        If exs.Count = 0 Then
    ...
    Private Sub Do_ArtCustRefs(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim exs As New List(Of Exception)
        Dim oCustomer As DTOCustomer = FEB2.Customer.CcxOrMe(exs, _Customer)
        If exs.Count = 0 Then
    ...
    Private Async Sub Do_ExcelStocks()
        Dim exs As New List(Of Exception)
        Dim oSheet = Await FEB2.ProductStocks.Excel(exs, Current.Session.Emp, _Customer)
        If Not UIHelper.ShowExcel(oSheet, exs) Then
            UIHelper.WarnError(exs)
        End If
    End Sub
    ...
    Private Async Sub Do_BankTransferReminder()
        ...
        Dim oSubscriptors = Await FEB2.Subscriptors.All(exs, oSubscription, _Customer)
        ...
        End If
        If _Customer.lang Is Nothing Then FEB2.Customer.Load(_Customer, exs)
