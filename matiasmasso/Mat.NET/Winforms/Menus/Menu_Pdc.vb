...
Public Class Menu_Pdc
        ...
        If FEB2.PurchaseOrder.Load(exs, _PurchaseOrder, GlobalVariables.Emp.Mgz) Then
            Select Case _PurchaseOrder.cod
                Case DTOPurchaseOrder.Codis.client
                    If Await FEB2.AlbBloqueig.BloqueigStart(Current.Session.User, _PurchaseOrder.contact, DTOAlbBloqueig.Codis.PDC, exs) Then
    ...
    Private Sub Do_PdcHtml(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim sUrl As String = FEB2.PurchaseOrder.Url(_PurchaseOrder, True)
        Process.Start(sUrl)
    End Sub
    ...
    Private Sub Do_CopyLinkToDoc(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If _PurchaseOrder.docFile IsNot Nothing Then
            Dim sUrl As String = FEB2.DocFile.DownloadUrl(_PurchaseOrder.docFile, True)
    ...
    Private Sub Do_CopyTpv(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim url As String = FEB2.PurchaseOrder.UrlTpv(_PurchaseOrder, True)
    ...
    Private Async Sub Do_Del(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If rc = MsgBoxResult.Ok Then
            Dim exs As New List(Of Exception)
            If Await FEB2.PurchaseOrders.Delete(exs, _PurchaseOrders) Then
    ...
    Private Async Sub Do_DocumentImportar(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If UIHelper.LoadPdfDialog(oDocFile, sTitle) Then
            If FEB2.PurchaseOrder.Load(exs, _PurchaseOrder) Then
    ...
    Private Async Sub Do_Alb(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim exs As New List(Of Exception)
        If Await FEB2.AlbBloqueig.BloqueigStart(Current.Session.User, _PurchaseOrder.contact, DTOAlbBloqueig.Codis.ALB, exs) Then
    ...
    Private Async Sub Do_Clon(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim exs As New List(Of Exception)
        If FEB2.PurchaseOrder.Load(_PurchaseOrder, exs) Then
            Dim oClon = DTOPurchaseOrder.clon(_PurchaseOrder)
            oClon.UsrLog = DTOUsrLog.Factory(Current.Session.User)

            Select Case oClon.cod
                Case DTOPurchaseOrder.Codis.proveidor
                    If Await FEB2.AlbBloqueig.BloqueigStart(Current.Session.User, oClon.contact, DTOAlbBloqueig.Codis.PDC, exs) Then
    ...
    Private Sub Do_Edi_Export(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim exs As New List(Of Exception)
        Dim src As String = FEB2.EdiversaOrder.EdiMessage(Current.Session.Emp, _PurchaseOrder, exs)
        If exs.Count = 0 Then
            Dim oDlg As New SaveFileDialog
            With oDlg
                .FileName = String.Format("Edi Order {0} {1:yyMMddHHmm}.txt", _PurchaseOrder.num, Now)
    ...
    Private Async Sub Do_Edi_Queue(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Dim oEdiFile = FEB2.EdiversaOrder.EdiFile(Current.Session.Emp, _PurchaseOrder, exs)
        If exs.Count > 0 Then
            UIHelper.WarnError(exs)
        Else
            If oEdiFile Is Nothing Then
                UIHelper.WarnError(exs)
            Else
                If Await FEB2.EdiversaFile.Update(exs, oEdiFile) Then
                    RefreshRequest(Me, MatEventArgs.Empty)
                Else
    ...
    Private Async Sub Do_GenerateOrderToSuplier()
        ...
        For Each oCustomerOrder As DTOPurchaseOrder In _PurchaseOrders
            FEB2.PurchaseOrder.Load(exs, oCustomerOrder)
        Next
                    ...
                    For Each customerItem In oCustomerOrder.items
                        If oProveidor.Equals(DTOProduct.proveidor(customerItem.sku)) Then
                            ...
                            If suplierItem Is Nothing Then
                                Dim oCostItem = Await FEB2.ProductSku.Cost(exs, oSku)
    ...
    Private Async Sub Do_RecuperaLiniesDeSortides(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim exs As New List(Of Exception)
        Dim iCount As Integer = Await FEB2.PurchaseOrderItems.RecuperaLiniesDeSortides(exs, _PurchaseOrder)
        If exs.Count = 0 Then
            If iCount = 0 Then
    ...
    Private Async Sub Do_NoRep(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If FEB2.PurchaseOrder.Load(exs, _PurchaseOrder) Then
            ...
            Await FEB2.PurchaseOrder.Update(exs, _PurchaseOrder)
            If exs.Count = 0 Then
    ...
    Private Async Sub Do_RemovePromo()
        Dim exs As New List(Of Exception)
        If Await FEB2.PurchaseOrder.RemovePromo(exs, _PurchaseOrder) Then
    ...
    Private Async Sub Do_BlockPendingItems()
        Dim exs As New List(Of Exception)
        _PurchaseOrder = Await FEB2.PurchaseOrder.Find(_PurchaseOrder.Guid, exs)
        If exs.Count = 0 Then
            If Await FEB2.PurchaseOrder.BlockPendingItems(exs, _PurchaseOrder) Then
                RaiseEvent AfterUpdate(Me, New MatEventArgs(_PurchaseOrder))
            Else
    ...
    Private Sub Do_ConfirmacioEdiSave()
        Dim exs As New List(Of Exception)
        If FEB2.PurchaseOrder.Load(_PurchaseOrder, exs) Then
            Dim oEdiOrdrsp = DTOEdiOrdrsp.Factory(_PurchaseOrder, GlobalVariables.Emp.Org, Today, Today)
            Dim oDlg As New SaveFileDialog
            With oDlg
                .FileName = String.Format("ORDRSP {0} {1:yyMMddHHmm}.txt", _PurchaseOrder.num, Now)
                .InitialDirectory = My.Computer.FileSystem.SpecialDirectories.MyDocuments
                .Filter = "txt files (*.txt)|*.txt|All files (*.*)|*.*"
                .FilterIndex = 1
                If .ShowDialog() = System.Windows.Forms.DialogResult.OK Then
                    If FileSystemHelper.SaveTextToFile(oEdiOrdrsp.Stream, .FileName, exs) Then
                        RefreshRequest(Me, MatEventArgs.Empty)
                    Else
                        UIHelper.WarnError(exs, "Error al desar el fitxer")
                    End If
    ...
    Private Async Sub Do_ConfirmacioEdiFtp()
        ...
        If FEB2.PurchaseOrder.Load(_PurchaseOrder, exs) Then
            Dim oEdiOrdrsp = DTOEdiOrdrsp.Factory(_PurchaseOrder, GlobalVariables.Emp.Org, Today, Today)
            Dim msg = Await FEB2.Ftpserver.Send(exs, _PurchaseOrder.customer, DTOFtpserver.Path.Cods.Ordrsp, oEdiOrdrsp.ByteArray(), oEdiOrdrsp.DefaultFileName)
            If exs.Count = 0 Then
    ...
    Private Async Function BrowsePdf(ByVal BlProforma As Boolean) As Task(Of Task)
        Dim exs As New List(Of Exception)
        If _PurchaseOrders.Count = 1 Then
            Dim oPurchaseOrder As DTOPurchaseOrder = _PurchaseOrders.First
            If FEB2.PurchaseOrder.Load(exs, oPurchaseOrder) Then
                ...
                Dim oDoc = FEB2.PurchaseOrder.Doc(oPurchaseOrder, BlProforma)
                oPdf.PrintDoc(oDoc)
        ...
        For Each oPurchaseOrder In _PurchaseOrders
            If FEB2.PurchaseOrder.Load(exs, oPurchaseOrder) Then
                ...
                For Each oOrder In tmp
                    Dim oDoc = FEB2.PurchaseOrder.Doc(oOrder, False)
                    oPdf.PrintDoc(oDoc)
                Next
                ...
                If exs.Count = 0 Then
                    ...
                    If exs.Count = 0 Then
                        ...
                        Await FEB2.PurchaseOrder.Update(exs, oPurchaseOrder)
                        If exs.Count = 0 Then
    ...
    Private Async Sub Do_RecalculaPendents(sender As Object, e As System.EventArgs)
        Dim exs As New List(Of Exception)
        Dim iCount = Await FEB2.PurchaseOrder.RecalculaPendents(exs, _PurchaseOrder)
        If exs.Count = 0 Then
            RaiseEvent AfterUpdate(Me, MatEventArgs.Empty)
    ...
    Private Async Sub Do_DeletePendingItems()
        ...
        If rc = MsgBoxResult.Ok Then
            ...
            Dim exs As New List(Of Exception)
            If Await FEB2.PurchaseOrders.DeletePendingItems(_PurchaseOrders, AddressOf oFrm.ShowProgress, exs) Then
    ...
    Private Sub Do_Excel()
        Dim exs As New List(Of Exception)
        If FEB2.PurchaseOrder.Load(_PurchaseOrder, exs) Then
            If FEB2.Contact.Load(_PurchaseOrder.Contact, exs) Then
                Dim oSheet As MatHelperStd.ExcelHelper.Sheet = FEB2.PurchaseOrder.Excel(_PurchaseOrder)
