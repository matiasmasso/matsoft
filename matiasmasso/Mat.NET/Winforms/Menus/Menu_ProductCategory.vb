...
#End Region
    ...
    Private Sub AddMenuItems()
        ...
        MyBase.AddMenuItem(MenuItem_Online())
        MyBase.AddMenuItem(MenuItem_Circulars())

        Select Case Current.Session.Rol.id
            Case DTORol.Ids.SuperUser, DTORol.Ids.Marketing
...
#End Region
    ...
    Private Function SubMenuItem_Plugins() As ToolStripMenuItem
        ...
        With oMenuItem.DropDownItems
            ...
            .Add(SubMenuItem_PluginRelatedSkus)
    ...
    Private Function SubMenuItem_PluginRelatedSkus() As ToolStripMenuItem
        Dim oMenuItem As New ToolStripMenuItem
        oMenuItem.Text = "articles relacionats"
        AddHandler oMenuItem.Click, AddressOf Do_CopyPluginRelatedSkus
        Return oMenuItem
    End Function
    ...
    Private Async Sub Do_RankingCustomers()
        Dim exs As New List(Of Exception)
        Dim oRanking = Await FEB2.Ranking.CustomerRanking(exs, Current.Session.User, _ProductCategories.First)
        If exs.Count = 0 Then
    ...
    Private Sub Do_Web(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim url As String = _ProductCategory.GetUrl(Current.Session.Lang,, True)
        Process.Start(url)
    End Sub
    ...
    Private Sub Do_UrlBuilderBlogComments(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim sUrl As String = FEB2.ProductCategory.UrlBuilderForBlogComments(_ProductCategory, True)
        Clipboard.SetDataObject(sUrl)
    End Sub

    Private Sub Do_CopyUrlCompatibility()
        Dim sUrl = FEB2.ApiUrl("ProductDownloads/LastCompatibilityReport", _ProductCategory.Guid.ToString())
        Clipboard.SetDataObject(sUrl)
    End Sub

    Private Sub Do_UrlBuilderEmails(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim sUrl As String = FEB2.ProductCategory.UrlBuilderForEmails(_ProductCategory, True)
        Clipboard.SetDataObject(sUrl)
    End Sub
    ...
    Private Sub Do_CopyPluginCollection()
        Dim src As String = _ProductCategory.PluginCollection()
        Clipboard.SetDataObject(src, True)
        MsgBox("Plugin copiat al portapapers", MsgBoxStyle.Information, "Mat.Net")
    End Sub

    Private Sub Do_CopyPluginRelatedSkus()
        Dim src As String = _ProductCategory.PluginRelatedSkus()
        Clipboard.SetDataObject(src, True)
        MsgBox("Plugin copiat al portapapers", MsgBoxStyle.Information, "Mat.Net")
    End Sub
