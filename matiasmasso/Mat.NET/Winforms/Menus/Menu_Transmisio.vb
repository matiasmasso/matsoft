Public Class Menu_Transmisio
    ...
    Private Async Sub Do_GetXML(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Dim oByteArray = Await FEB2.Transmisio.XmlFile(_Transmisio, exs)
        ...
        If exs.Count = 0 Then
            ...
            With oDlg
                ...
                .FileName = FEB2.Transmisio.FileNameDades(_Transmisio)
    ...
    Private Async Sub Do_GetAlbs(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ' Dim url As String = FEB2.Transmisio.UrlAlbarans(_Transmisio)
        'UIHelper.ShowHtml(url)

        Dim exs As New List(Of Exception)

        MyBase.ToggleProgressBarRequest(True)
        Dim oByteArray = Await FEB2.Transmisio.PdfDeliveries(_Transmisio, exs)
        MyBase.ToggleProgressBarRequest(False)
        ...
        If exs.Count = 0 Then
            ...
            With oDlg
                ...
                .FileName = FEB2.Transmisio.FileNameDocs(_Transmisio)
    ...
    Private Sub Do_EmailBody()
        Dim url As String = FEB2.Transmisio.SendUrlBody(_Transmisio)
        UIHelper.ShowHtml(url)
    End Sub
    ...
    Private Async Sub Do_SendOutlook(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        MyBase.ToggleProgressBarRequest(True)
        Dim oAttachment = Await FEB2.Transmisio.XmlAttachment(_Transmisio, exs)
        MyBase.ToggleProgressBarRequest(False)

        If exs.Count = 0 Then
            Dim sendTo = Await FEB2.Transmisio.SendTo(exs)
            If exs.Count = 0 Then
                With _Transmisio
                    Dim oMailMessage = DTOMailMessage.Factory(sendTo)
                    With oMailMessage
                        .Subject = FEB2.Transmisio.SendSubject(_Transmisio)
                        .BodyUrl = FEB2.Transmisio.SendUrlBody(_Transmisio)
    ...
    Private Async Sub Do_SendExchange(ByVal sender As System.Object, ByVal e As System.EventArgs)
        Dim exs As New List(Of Exception)
        MyBase.ToggleProgressBarRequest(True)
        If Await FEB2.Transmisio.Send(exs, _Transmisio) Then
            MyBase.ToggleProgressBarRequest(False)
    ...
    Private Async Sub Do_Excel(ByVal sender As System.Object, ByVal e As System.EventArgs)
        Dim exs As New List(Of Exception)
        MyBase.ToggleProgressBarRequest(True)
        Dim oSheet = Await FEB2.Transmisio.Excel(_Transmisio, exs)
        MyBase.ToggleProgressBarRequest(False)
        If exs.Count = 0 Then
    ...
    Private Async Sub Do_ExcelECI(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        For i As Integer = 0 To _Transmisions.Count - 1
            _Transmisions(i) = Await FEB2.Transmisio.Find(_Transmisions(i).Guid, exs)
        Next
        Dim oDeliveries = _Transmisions.SelectMany(Function(x) x.deliveries).ToList()
        For i = 0 To oDeliveries.Count - 1
            oDeliveries(i) = Await FEB2.Delivery.Find(oDeliveries(i).Guid, exs)
