Public Class Frm_AlbNew2
    ...
    Public Sub New(oImportacio As DTOImportacio)
        ...
        FEB2.Importacio.Load(exs, oImportacio)
        _Proveidor = oImportacio.Proveidor
        _Delivery = FEB2.Delivery.Factory(exs, _Proveidor, Current.Session.User, GlobalVariables.Emp.Mgz)
    ...
    Private Async Sub Frm_AlbNew2_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If _Delivery.IsNew Then
            _Delivery.items = Await FEB2.DeliveryItems.Factory(exs, _Delivery.contact, _Delivery.cod, GlobalVariables.Emp.Mgz)
        ...
        Select Case _Delivery.Cod
            Case DTOPurchaseOrder.Codis.Proveidor
                ...
                If Not FEB2.Proveidor.Load(_Proveidor, exs) Then
                    UIHelper.WarnError(exs)
                End If
            Case Else
                _Customer = _Delivery.Customer
                _Ccx = FEB2.Customer.CcxOrMe(exs, _Customer)
                'FEB2.Customer.Load(_Ccx, exs)
        End Select
    ...
    Private Async Function refresca() As Task
        ...
        With _Delivery
            ...
            Select Case .Cod
                Case DTOPurchaseOrder.Codis.Client, DTOPurchaseOrder.Codis.Reparacio
                    If Await FEB2.Contact.IsImpagat(_Ccx, exs) Then
                    ...
                    Case DTOPurchaseOrder.Codis.Client, DTOPurchaseOrder.Codis.Reparacio
                        ...
                        Else
                            If _Delivery.CashCod = DTOCustomer.CashCodes.credit Then
                                If Not Await FEB2.Customer.EFrasEnabled(exs, _Delivery.Customer) Then
        ...
        Dim oMenuClient As New Menu_Contact(_Delivery.Contact)
    ...
    Private Sub ValidateCustomer()
        If _Delivery.Cod = DTOPurchaseOrder.Codis.client And _Delivery.IsNew Then
            ...
            Dim exs As New List(Of Exception)
            If Not FEB2.Customer.ValidateCustomer(oCustomer, exs) Then
    ...
    Private Async Function SetEmailLabel() As Task
        Dim exs As New List(Of Exception)
        Dim oRecipients = Await FEB2.Subscriptors.Recipients(exs, GlobalVariables.Emp, DTOSubscription.Wellknowns.ConfirmacioEnviament, _Delivery.contact)
        If oRecipients.Count = 0 Then
            Dim oUsers = Await FEB2.Users.All(exs, _Delivery.Contact)
            If exs.Count = 0 Then
                oUsers = oUsers.Where(Function(x) x.BadMail = False).ToList
    ...
    Private Sub SetReculliran()
        ...
        If oMgz.Address Is Nothing Then
            Dim exs As New List(Of Exception)
            If FEB2.Contact.Load(oMgz, exs) Then
            ...
            Select Case oCod
                ...
                Case DTOPurchaseOrder.Codis.Proveidor
                    ...
                    If oImportacio Is Nothing Then
                        oImportacio = Await FEB2.Importacio.FromDelivery(exs, _Delivery)
                        If exs.Count > 0 Then
                            UIHelper.WarnError(exs)
                        End If
                    End If
    ...
    Private Async Sub ButtonOk_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles ButtonOk.Click
        ...
        If Await FEB2.Delivery.CheckValidationErrors(exs, GlobalVariables.Emp, _Delivery) Then
            Save(exs)
        Else
    ...
    Private Sub LoadTransportFromForm()
        ...
        With _Delivery
            ...
            ' Dim oTrpCost As DTOTrpCost = FEB2.Delivery.SuggestedTransport(Xl_LookupZip1.Zip, CurrentPorts, DcM3)
    ...
    Private Async Sub Save(exs As List(Of Exception))
        ...
        If BlWasNew And _Delivery.Importacio IsNot Nothing Then
            Await FEB2.Importacio.Entrada(exs, _Delivery)
        Else
            Dim id = Await FEB2.Delivery.Update(exs, _Delivery)
            If exs.Count = 0 Then
                _Delivery.Id = id
            Else
                UIHelper.WarnError(exs)
        ...
        If exs.Count = 0 Then
            ...
            If _Delivery.contact.Equals(DTOCustomer.Wellknown(DTOCustomer.Wellknowns.tradeInn)) Then
    ...
    Private Async Function RecycleNum(exs As List(Of Exception)) As Task(Of Integer)
        ...
        If CheckBoxRecycle.Checked And IsNumeric(ComboBoxNum.Text) Then
            Dim oDelivery As DTODelivery = Await FEB2.Delivery.FromNum(Current.Session.Emp, DateTimePickerFch.Value.Year, ComboBoxNum.Text, exs)
            If exs.Count = 0 Then
    ...
    Private Function SetTotals(exs As List(Of Exception)) As Boolean
        ...
        Dim totalsText = FEB2.Delivery.TotalsText(_Delivery, Xl_DeliveryItems1.Items, GetExportCod(), _Ccx, _Delivery.Contact.Lang)
        ...
        Else
            Select Case _Delivery.Cod
                Case DTOPurchaseOrder.Codis.Proveidor
                    ...
                    TextBoxTotals.Text = String.Format("{0}   {1}", totalsText, FEB2.PaymentTerms.Text(oPaymentTerms, _Delivery.Contact.Lang))
                Case Else
    ...
    Private Async Function LoadRecicleNums() As Task(Of Boolean)
        ...
        Dim iNums = Await FEB2.Deliveries.NumsToRecycle(exs, Current.Session.Emp, DtFch)
        If exs.Count = 0 Then
    ...
    Private Async Function SetLastAlb() As Task
        Dim exs As New List(Of Exception)
        Dim oDelivery = Await FEB2.Deliveries.Last(exs, _Delivery.Contact)
        If exs.count = 0 Then
            If oDelivery Is Nothing Then
                With LabelLastAlb
                    .Text = "primer albará"
    ...
    Private Async Sub ButtonDel_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles ButtonDel.Click
        ...
        If rc = MsgBoxResult.Ok Then
            Dim exs As New List(Of Exception)
            If Await FEB2.Delivery.Delete(exs, _Delivery) Then
    ...
    Private Sub Frm_PurchaseOrder_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
        Dim exs As New List(Of Exception)
        If Not FEB2.AlbBloqueig.BloqueigEnd(Current.Session.User, _Delivery.Contact, DTOAlbBloqueig.Codis.ALB, exs) Then
            e.Cancel = True
            UIHelper.WarnError(exs)
        End If
    End Sub
...
#Region "Toolbar"
    ...
    Private Async Sub SetCreditDetails()
        ...
        If BlShowCreditAvailable Then
            ...
            Dim oTotal As DTOAmt = FEB2.Delivery.TotalLiquid(exs, _Delivery)
            Dim oCreditDisponible = Await FEB2.Risc.CreditDisponible(_Ccx, exs)
            If exs.Count = 0 Then
            Else
                UIHelper.WarnError(exs)
            End If
            ...
            If oPdc.PaymentTerms IsNot Nothing Then
                If Not DTOPaymentTerms.Match(oPdc.PaymentTerms, _Ccx.PaymentTerms) Then
                    ...
                    If Not BlFpgExists Then
                        ...
                        End If
                        sWarnText = sWarnText & "comanda " & oPdc.Num & ": " & FEB2.PaymentTerms.Text(oPdc.PaymentTerms, Current.Session.Lang)
                    End If
