...
Public Class Frm_Last_albs_enAltres
    ...
    Private Sub LoadGrid()
        ...
        'DTO.DTOCustomer.PortsCodes.Altres

        Dim SQL As String = "SELECT YEA,ALB,FCH,CLX,EUR+PT2,CASHCOD, FRA," _
        & "(CASE WHEN USR.LOGIN IS NULL THEN (CASE WHEN ALB.UsrCreatedGuid=ALB.CliGuid THEN '(client)' ELSE CAST(ALB.USRCREATED AS VARCHAR) END) ELSE USR.LOGIN END) AS USR,COD,ALB.RETENCIOCOD " _
        ...
        & "WHERE ALB.EMP=" & BLLApp.Emp.Id & " AND " _
        & "ALB.YEA=" & CurrentYea() & " " _
        & "AND ALB.COD > 1 " _
        & "AND ALB.PORTSCOD=" & CInt(DTO.DTOCustomer.PortsCodes.Altres) & " " _
        & "AND ALB.COD<>" & DTOPurchaseOrder.Codis.Proveidor & " " _
        & "AND MgzGuid='" & BLLApp.Mgz.Guid.ToString & "' " _
        & "ORDER BY ALB DESC"
    ...
    Private Sub SetYeas()
        ...
        mDsYeas =  DAL.SQLHelper.GetDataset(SQL, New List(Of Exception))
    ...
    Private Function CurrentAlb() As Alb
        ...
        If oRow IsNot Nothing Then
            ...
            oAlb = MaxiSrvr.Alb.FromNum(BLL.BLLApp.Emp, CurrentYea, LngId)
        End If
        Return oAlb
    End Function
