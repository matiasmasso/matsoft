...
Public Class Frm_Last_Fras
    ...
    Private Sub LoadGrid()
        Dim SQL As String = "SELECT Fra.Guid, Fra.CliGuid, (CASE WHEN CCA.Hash IS NULL THEN 0 ELSE 1 END) AS PDF, FRA.YEA,FRA.FRA,FRA.FCH,CLX.CLX,FRA.EURBASE,FRA.EURLIQ,FRA.FPG,FRA.CFP " _
        & "FROM fra " _
        & "INNER JOIN CliGral ON FRA.CliGuid = CliGral.Guid " _
        & "INNER JOIN CLX ON FRA.CliGuid=CLX.Guid " _
        & "LEFT OUTER JOIN CCA ON CCA.GUID = FRA.CCAGUID " _
        & "WHERE CliGral.EMP=" & mEmp.Id & " AND " _
    ...
    Private Sub SetMonths()
        Dim SQL As String = "SELECT MONTH(FCH) AS MES, COUNT(FRA) as List(Of Fra), SUM(EURBASE) AS EUR FROM FRA WHERE " _
        & "EMP=" & BLLApp.Emp.Id & " AND " _
        & "YEA=" & CurrentYea() & " " _
