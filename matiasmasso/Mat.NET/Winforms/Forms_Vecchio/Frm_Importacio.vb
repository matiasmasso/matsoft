...
Public Class Frm_Importacio
    ...
    Private Sub SetImportacio()
        If _Importacio.IsNew Then
            ...
            DateTimePickerETD.Value = Today
            DateTimePickerEta.Value = Today
    ...
    Private Sub RefrescaGral()
        With _Importacio
            ...
            Xl_ContactPlataformaDeCarga.Contact = .PlataformaDeCarga
            Xl_ContactTrp.Contact = .Transportista
    ...
    Private Async Sub ButtonOk_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles ButtonOk.Click
        ...
        Dim id = Await FEB2.Importacio.Update(_Importacio, exs)
        If exs.Count = 0 Then
            _Importacio.id = id
    ...
    Private Sub ToolStripButtonExcel_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        Dim oSheet = MatHelperStd.ExcelHelper.Sheet.Factory(mDsArts)
        Dim exs As New List(Of Exception)
    ...
    Private Async Function LoadIncoterms(exs As List(Of Exception)) As Task(Of Boolean)
        ...
        Dim oIncoterms As List(Of DTOIncoterm) = Await FEB2.Incoterms.All(exs)
        If exs.Count = 0 Then
    ...
    Private Async Function RefrescaPrevisions() As Task
        Dim exs As New List(Of Exception)
        _Importacio = Await FEB2.ImportPrevisions.Load(exs, _Importacio)
        If exs.Count = 0 Then
            ...
            If .ShowDialog Then
                If .FileName > "" Then
                    ...
                    If FileSystemHelper.IsFileLocked(.FileName, IO.FileMode.Open, IO.FileAccess.Read, exs) Then
                        UIHelper.WarnError(exs)
                    Else
                        'Dim oExcel As ExcelHelper.Book = MatExcel.Read2(.FileName)
    ...
    Private Async Sub Do_ImportaPrevisio(sender As Object, e As MatEventArgs)
        ...
        Dim exs As New List(Of Exception)
        If Await FEB2.ImportPrevisions.UploadExcel(exs, _Importacio, oSheet) Then
            Await RefrescaPrevisions()
        Else
            UIHelper.WarnError(exs)
        End If
    ...
    Private Async Sub Xl_ImportPrevisio1_onPurchaseOrderItemUpdateRequest(sender As Object, e As MatEventArgs) Handles Xl_ImportPrevisio1.onPurchaseOrderItemUpdateRequest
        ...
        If Not String.IsNullOrEmpty(oPrevisio.NumComandaProveidor) Then
            ...
            Dim oPncs = Await FEB2.PurchaseOrderItems.Pending(exs, _Importacio.Proveidor, DTOPurchaseOrder.Codis.Proveidor, GlobalVariables.Emp.Mgz)
        ...
        If Await FEB2.ImportPrevisions.Update(exs, _Importacio) Then
            Await RefrescaPrevisions()
        Else
            UIHelper.WarnError(exs)
        End If
