...
Public Class Frm_FlatFileFixLenType
    ...
    Private Sub LoadRegs()
        ...
        Dim oDs As DataSet = DAL.SQLHelper.GetDataset(SQL, New List(Of Exception), "@FileId", iFileId.ToString)
    ...
    Private Sub LoadFields()
        ...
        Dim oDs As DataSet = DAL.SQLHelper.GetDataset(SQL, New List(Of Exception), "@FileId", iFlatFileId, "@RegId", sRegId)
        Dim oTb As DataTable = oDs.Tables(0)

        Dim iPos As Integer = 1
        Dim iLen As Integer
        Dim oOpc As FlatFileFixLen.CampsOpcionals
        Dim oFormat As DTO.DTOFlatField.Formats
        For Each oRow As DataRow In oTb.Rows
            iLen = CInt(oRow(ColsFields.Len))
            oOpc = CType(oRow(ColsFields.Opcional), FlatFileFixLen.CampsOpcionals)
