...
Public Class Frm_Csas_Old
    ...
    Private Sub LoadGrid()
        Dim SQL As String = "SELECT 0 as ICO,CSA.Guid,CSA.CSB,CSA.FCH,CSA.BNC, " _
        ...
        & "CSB ON CSA.Guid=CSB.CsaGuid LEFT OUTER JOIN " _
        & "CLIGRAL ON CSA.BancGuid=CLIGRAL.Guid " _
        & "WHERE CSA.EMP=@EMP "

        If mBanc IsNot Nothing Then
            SQL = SQL & " AND CSA.BancGuid='" & mBanc.Guid.ToString & "' "
        End If
        ...
        Select Case CurrentMode()
            ...
            Case Modes.Tots
        End Select
        SQL = SQL & "GROUP BY CSA.Guid,CSA.CSB,CSA.FCH,CSA.BNC,CLIGRAL.NOMCOM,CLIGRAL.RAOSOCIAL,CSA.EFTS,CSA.EUR,CSA.DIAS,CSA.GTS,CSA.DESCOMPTAT,CSA.FILEFORMAT "
        SQL = SQL & "ORDER BY Year(CSA.Fch) DESC, CSA.CSB DESC"
    ...
    Private Function CurrentCsa() As DTOCsa
        Dim oCsa As DTOCsa = Nothing
        Dim oRow As DataGridViewRow = DataGridView1.CurrentRow
        If oRow IsNot Nothing Then
            oCsa = BLLCsa.Find(CType(oRow.Cells(Cols.Guid).Value, Guid))
        End If
        Return oCsa
    End Function

    Private Function SelectedCsas() As List(Of DTOCsa)
        Dim retval As New List(Of DTOCsa)

        If DataGridView1.SelectedRows.Count > 0 Then
            For Each oRow As DataGridViewRow In DataGridView1.SelectedRows
                Dim oCsa As New DTOCsa(CType(oRow.Cells(Cols.Guid).Value, Guid))
                retval.Add(oCsa)
            Next
        Else
            retval.Add(CurrentCsa)
        End If
        Return retval
    End Function
