...
Public Class Frm_Pagament_Old
    ...
    Public WriteOnly Property Proveidor() As Contact
        Set(ByVal Value As Contact)
            Dim oDefaultAmt As DTOAmt = BLLApp.EmptyAmt
            mProveidor = Value
            mEmp = mProveidor.Emp
    ...
    Public Shadows Function Update(ByRef exs As List(Of Exception)) As Boolean
        ...
        Try
            Dim oPnds As List(Of Pnd) = xl_pnd_select1.Pnds
            mCca = GetCca(oPnds)
            mCca.Update(oTrans,)
    ...
    Private Function GetCca(oPnds As List(Of Pnd)) As Cca
        ...
        Select Case TabControl1.SelectedTab.Text
            Case TabPageEnd.Text
                If CheckBoxEmail.Checked Then
                    ...
                    Dim oIban As DTOIban = BLL.BLLIban.FromContact(oProveidor, DTOIban.Cods.Proveidor) '  New Proveidor(mProveidor.Guid).FormaDePago.Iban
    ...
    Private Function AltresDivises() As Boolean
        Dim BlEur As Boolean = xl_pnd_select1.Amt.Cur.Equals(DTOCur.Eur)
        Return Not BlEur
    End Function

    Private Sub xl_pnd_select1_AfterUpdate() Handles xl_pnd_select1.AfterUpdate
        EnableButtons()
        Dim oAmt As DTOAmt = xl_pnd_select1.Amt
        Xl_AmtCurSel.Amt = oAmt
        Xl_AmtCurContraValor.Amt = BLLApp.EmptyAmt


        If AltresDivises() Then
            Xl_AmtCurSel.Enabled = False
            Xl_AmtCurSelEur.Amt = BLLApp.GetAmt(oAmt.Eur)
        ...
        Else
            ...
            Xl_AmtCurSelEur.Amt = BLLApp.EmptyAmt
    ...
    Private Sub Xl_AmtCurContraValor_AfterUpdate(ByVal sender As Object, ByVal e As System.EventArgs) Handles Xl_AmtCurContraValor.AfterUpdate
        Dim oSum As DTOAmt = Xl_AmtCurContraValor.Amt.Clone
        oSum.Add(Xl_AmtCurGts.Amt)
        Xl_AmtCurLiq.Amt = oSum

        Dim oDif As DTOAmt = Xl_AmtCurSelEur.Amt.Clone
