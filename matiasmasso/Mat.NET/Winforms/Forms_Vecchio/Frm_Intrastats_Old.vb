...
Public Class Frm_Intrastats_Old
    ...
    Private Sub LoadGrid()
        ...
        sb.AppendLine("SELECT INTRASTAT.GUID, INTRASTAT.MES, COUNT(DISTINCT IMPORTDTL.ID), SUM(CCB.EUR) As DTOAmt ")
        ...
        sb.AppendLine("WHERE INTRASTAT.EMP=" & CInt(BLLApp.Emp.Id) & " AND INTRASTAT.YEA=" & iYea & " ")
        sb.AppendLine("GROUP BY INTRASTAT.MES,INTRASTAT.GUID ")
        sb.AppendLine("ORDER BY INTRASTAT.MES DESC ")
    ...
    Private Sub LoadYeas()
        ...
        Dim oDrd As SqlDataReader = DAL.SQLHelper.GetDataReader(SQL, "@EMP", CInt(_emp.Id).ToString)
        Do While oDrd.Read
    ...
    Private Sub NouIntrastatToolStripMenuItem_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles NouIntrastatToolStripMenuItem.Click
        Dim oFrm As New Frm_IntrastatFactory
        AddHandler oFrm.AfterUpdate, AddressOf RefreshRequest
        oFrm.Show()
    End Sub
