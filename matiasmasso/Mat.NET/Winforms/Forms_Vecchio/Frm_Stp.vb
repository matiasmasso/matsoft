...
Public Class Frm_Stp
    ...
    Private Async Sub Frm_Stp_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If _Category.IsNew Then
            Dim oProductChannels = Await FEB2.ProductChannels.All(exs, _Category.brand)
            If exs.Count = 0 Then
            ...
            End If
        Else
            If FEB2.ProductCategory.Load(_Category, exs) Then
        ...
        With _Category
            ...
            'Xl_Image_Thumbnail.Load(.thumbnail, 100, 130)
            Xl_Image_Thumbnail.LoadAsync(_Category.ThumbnailUrl())
    ...
    Private Async Sub ButtonOk_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles ButtonOk.Click
        ...
        With _Category
            ...
            If mDirtyThumbnail Then
                .thumbnail = LegacyHelper.ImageHelper.Converter(Xl_Image_Thumbnail.Bitmap)
            ...
            If Await FEB2.ProductCategory.Update(_Category, exs) Then
                If mDirtyFeatures Then
                    Dim oFeatures As List(Of DTOProductFeatureImage) = Xl_ProductFeaturedImages1.Values
                    Await FEB2.ProductFeatureImages.Update(exs, _Category, oFeatures)
                End If
                If Xl_CheckedFilters1.isDirty Then
                    If Await FEB2.FilterTargets.Update(exs, _Category, Xl_CheckedFilters1.SelectedValues) Then
    ...
    Private Async Sub ButtonDel_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles ButtonDel.Click
        ...
        If rc = MsgBoxResult.Ok Then
            Dim exs As New List(Of Exception)
            If Await FEB2.ProductCategory.Delete(_Category, exs) Then
    ...
    Private Sub Xl_Image_Thumbnail_AfterUpdate1(ByVal sender As Object, ByVal e As System.EventArgs) Handles _
            Xl_Image_Thumbnail.AfterUpdate,
    ...
    Private Async Sub TabControl1_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles TabControl1.SelectedIndexChanged
        ...
        If Not CleanTab(iTab) Then
            Select Case iTab
                ...
                Case Tabs.Features
                    Dim oFeatures = Await FEB2.ProductFeatureImages.All(exs, _Category)
                    Await Xl_ProductFeaturedImages1.Load(oFeatures)
    ...
    Private Async Function LoadMediaResources() As Task
        ...
        Dim items = Await FEB2.MediaResources.AllWithThumbnails(exs, _Category)
    ...
    Private Async Function LoadAccessories() As Task
        Dim exs As New List(Of Exception)
        Dim oAccessories = Await FEB2.Product.Relateds(exs, DTOProduct.Relateds.Accessories, _Category, GlobalVariables.Emp.Mgz)
        If exs.Count = 0 Then
    ...
    Private Async Function LoadPlugins() As Task
        Dim exs As New List(Of Exception)
        Dim oPlugins = Await FEB2.ProductPlugins.All(exs, _Category)
        If exs.Count = 0 Then
    ...
    Private Async Function LoadSpares() As Task
        Dim exs As New List(Of Exception)
        Dim oSpares = Await FEB2.Product.Relateds(exs, DTOProduct.Relateds.Spares, _Category, GlobalVariables.Emp.Mgz)
        If exs.Count = 0 Then
    ...
    Private Async Function LoadProductDownloads() As Task
        Dim exs As New List(Of Exception)
        Dim oDownloads As List(Of DTOProductDownload) = Await FEB2.Downloads.FromProductOrParent(exs, _Category)
        If exs.Count = 0 Then
    ...
    Private Async Sub onAccessorySelected(sender As Object, e As MatEventArgs)
        Dim exs As New List(Of Exception)
        Dim oAccessories = Await FEB2.Product.Relateds(exs, DTOProduct.Relateds.Accessories, _Category, GlobalVariables.Emp.Mgz, True, False)
        If exs.Count = 0 Then
            ...
            Else
                ...
                If Await FEB2.Product.UpdateRelateds(exs, DTOProduct.Relateds.Accessories, _Category, oAccessories) Then
                    Await LoadAccessories()
                Else
                    UIHelper.WarnError(exs)
                End If
    ...
    Private Async Sub Xl_ProductSkusExtendedSpares_RequestToAddNewSku(sender As Object, e As MatEventArgs) Handles Xl_ProductSkusExtendedSpares.RequestToAddNewSku
        Dim exs As New List(Of Exception)
        Dim oSpares = Await FEB2.Product.Relateds(exs, DTOProduct.Relateds.Spares, _Category, GlobalVariables.Emp.Mgz, True, False)
        If exs.Count = 0 Then
            ...
            Else
                ...
                If Await FEB2.Product.UpdateRelateds(exs, DTOProduct.Relateds.Spares, _Category, oSpares) Then
                    Await LoadSpares()
                Else
                    UIHelper.WarnError(exs)
                End If
    ...
    Private Async Function LoadMovies() As Task
        ...
        Dim oMovies = Await FEB2.YouTubeMovies.All(exs, Current.Session.User, _Category)
        If exs.Count = 0 Then
    ...
    Private Async Function RefrescaChannels() As Task
        Dim exs As New List(Of Exception)
        Dim oProductChannels As List(Of DTOProductChannel) = Await FEB2.ProductChannels.All(exs, _Category)
        If exs.Count = 0 Then
            Xl_ProductChannels1.Load(oProductChannels, Xl_ProductChannels.Modes.ChannelsPerProduct)
        Else
            UIHelper.WarnError(exs)
    ...
    Private Async Function LoadBloggerPosts() As Task
        Dim exs As New List(Of Exception)
        Dim oBlogs = Await FEB2.BloggerPosts.FromProductOrParent(_Category, exs)
        If exs.Count = 0 Then
    ...
    Private Async Function LoadFilters() As Task
        ...
        If Not _FiltersLoaded Then
            ...
            If exs.Count = 0 Then
                Dim oCheckedItems = Await FEB2.FilterTargets.All(exs, _Category)
                If exs.Count = 0 Then
