...
Public Class Frm_Client_Risc
    ...
    Public Sub New(ByVal oCustomer As DTOCustomer)
        ...
        _Ccx = FEB2.Customer.CcxOrMe(EXS, oCustomer)
    End Sub
    ...
    Private Async Function refresca() As Task
        Dim exs As New List(Of Exception)
        Dim oAmtSdoFras = Await FEB2.Risc.FrasPendentsDeVencer(_Ccx, exs)
        Dim oAmtSdoAlbsACredit = Await FEB2.Risc.SdoAlbsACredit(_Ccx, exs)
        Dim oAmtSdoAlbsNoCredit = Await FEB2.Risc.SdoAlbsNoCredit(_Ccx, exs)
        Dim oAmtSdoEntregatACompte = Await FEB2.Risc.EntregatACompte(_Ccx, exs)
        Dim oAmtDiposit = Await FEB2.Risc.DipositIrrevocable(_Ccx, exs)
        Dim oClassificacio = Await FEB2.Risc.CreditLimit(_Ccx, exs)
        Dim oDisposat = Await FEB2.Risc.CreditDisposat(_Ccx, exs)
        Dim oDisponible = Await FEB2.Risc.CreditDisponible(_Ccx, exs)
    ...
    Private Async Sub ButtonAlbsCredit_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles ButtonAlbsCredit.Click
        Dim exs As New List(Of Exception)
        Dim oDeliveries = Await FEB2.Deliveries.Headers(exs, Current.Session.Emp, contact:=_Ccx, group:=True)
        If exs.Count = 0 Then
    ...
    Public Async Sub SetIndexImpagats(Optional ByVal iDesdeElsDarrersMesos As Integer = 6)
        ...
        Dim iIndex = Await FEB2.Csbs.IndexImpagats(exs, GlobalVariables.Emp, _Ccx, iDesdeElsDarrersMesos)
        If exs.Count = 0 Then
            Dim iCreditProtocol_MaxImpagatsIndex = Await FEB2.Default.EmpInteger(Current.Session.Emp, DTODefault.Codis.CreditProtocol_MaxImpagatsIndex, exs)
            If exs.Count = 0 Then
    ...
    Private Sub ButtonLimit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonLimit.Click
        ...
        With oCliCreditLog
            .Customer = _Ccx
            FEB2.Contact.Load(.Customer, exs)
        End With
        ...
        Select Case TabControl1.SelectedIndex
            Case Tabs.Classificacions
                If mDirtyClassificacions Then
                    Dim exs As New List(Of Exception)
                    Dim oLogs = Await FEB2.CliCreditLogs.All(_Ccx, exs)
                    If exs.Count = 0 Then
    ...
    Private Sub Xl_CliCreditLogs1_RequestToAddNew(sender As Object, e As MatEventArgs) Handles Xl_CliCreditLogs1.RequestToAddNew
        ...
        With oItem
            .Customer = _Ccx
        End With
        If FEB2.Contact.Load(oItem.Customer, exs) Then
