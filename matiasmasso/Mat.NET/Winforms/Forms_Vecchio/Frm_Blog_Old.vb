...
Public Class Frm_Blog_Old
    ...
    Private Sub LoadLeads()
        Dim oLeads = BLLUsers.All().OrderByDescending(Function(x) x.FchCreated).ToList
        Dim oControl As New Xl_Leads
        oControl.Load(oLeads)
        oControl.Dock = DockStyle.Fill
        SplitContainer1.Panel2.Controls.Add(oControl)
        oControl.BringToFront()
    End Sub

    Private Sub LoadLeadsOld(Optional oSortedColumn As DataGridViewColumn = Nothing)
            ...
            Case Epigrafes.Leads
                Select Case e.ColumnIndex
                    Case 1
                        ...
                        If Not BLLUser.CheckEmailSintaxis(sEmail) Then
    ...
    Private Sub Do_ImportarFitxerDeSubscriptorsDeWordPress()
        ...
        With oDlg
            ...
            If .ShowDialog = System.Windows.Forms.DialogResult.OK Then
                ...
                Dim oArray As ArrayList = BLL.FileSystemHelper.getTextLinesArrayFromFilename(.FileName)

                For Each sLine As String In oArray
                    iCount += 1
                    Dim sEmail As String = Microsoft.VisualBasic.Left(sLine, 100)
                    Dim oLead As DTOUser = BLLUser.FromEmail(sEmail)
                    If oLead Is Nothing Then
                        iNew += 1
                        oLead = BLLUser.Factory()
                        With oLead
                            .EmailAddress = sEmail
                            .Source = DTOUser.Sources.WpFollower
                        End With
                        Dim exs As New List(Of Exception)
                        If Not BLLUser.Update(oLead, exs) Then
                            UIHelper.WarnError(exs)
                        End If
                    ElseIf oLead.Source = DTOUser.Sources.WpComment Then
                        iSwitchedSrc += 1
                        oLead.Source = DTOUser.Sources.WpFollower
                        Dim exs As New List(Of Exception)
                        If Not BLLUser.Update(oLead, exs) Then
                            UIHelper.WarnError(exs)
                        End If
    ...
    Private Sub Do_ImportarEmailsDelsComentaris()
        ...
        Dim exs As New List(Of Exception)
        BLLWpBlog.ImportWordPressCommentEmails(sResultMsg, resultCod, exs)
        MsgBox(sResultMsg, MsgBoxStyle.Information)
    End Sub
    ...
    Private Sub Do_LeadNew()
        ...
        oUser.Source = DTOUser.Sources.Manual
        Dim oFrm As New Frm_User(oUser)
        AddHandler oFrm.AfterUpdate, AddressOf RefreshRequest
        oFrm.Show()
    End Sub
    ...
    Private Sub Do_OcultarValidats()
        ...
        LoadLeadsOld(DataGridView2.SortedColumn)
    End Sub
    ...
    Private Sub Do_LeadUnSubscribe()
        ...
        If BLL.BLLUser.Update(oUser, exs) Then
            ...
            Dim oTxt As DTOTxt = BLLTxt.Find(DTOTxt.Ids.LeadAccountBaixa)
            Dim sBody As String = BLLTxt.ToHtml(oTxt, oLang, BLL.BLLUser.GetSaludo(oUser), BLL.BLLUser.GetFirma(oUser))
    ...
    Private Sub Do_ExportActiveNonWpLeads()
        ...
        With oDlg
            ...
            If .ShowDialog = System.Windows.Forms.DialogResult.OK Then
                ...
                Dim oUsers As List(Of DTOUser) = BLLWpBlog.LeadsNoSuscritsAWordPress

                For Each oUser As DTOUser In oUsers
                    writeFile.WriteLine(oUser.EmailAddress)
                Next
                writeFile.Flush()
                writeFile.Close()
                writeFile = Nothing
                MsgBox(oUsers.Count & " emails no suscrits")
            End If
    ...
    Private Sub Do_ExportActiveLeads()
        ...
        With oDlg
            ...
            If .ShowDialog = System.Windows.Forms.DialogResult.OK Then
                Dim oLeads As List(Of DTOUser) = BLLUsers.All().
                    Where(Function(x) x.Rol.Id = DTORol.Ids.Lead).
                    Where(Function(y) y.FchDeleted = Nothing).
                    ToList

                Dim writeFile As System.IO.TextWriter = New System.IO.StreamWriter(.FileName)
                Dim iCount As Integer
                For Each oLead In oLeads
                    writeFile.WriteLine(oLead.EmailAddress)
                    iCount += 1
                Next
                writeFile.Flush()
                writeFile.Close()
        ...
        With oDlg
            ...
            If .ShowDialog = System.Windows.Forms.DialogResult.OK Then
                ...
                For Each sLine As String In sLines
                    ...
                    If oLead Is Nothing Then
                        oLead = BLLUser.Factory()
                        oLead.EmailAddress = sEmail
                        If sFields.Length > 1 Then
                            oLead.Cognoms = sFields(1)
                        End If
                        oLead.Source = DTOUser.Sources.Manual
                        Dim exs As New List(Of Exception)
                        If Not BLLUser.Update(oLead, exs) Then
                            UIHelper.WarnError(exs)
                        End If
...
#Region "DragDrop"
    ...
    Private Sub Do_Unsubscribe(objMI As Microsoft.Office.Interop.Outlook.MailItem)
        ...
        Dim oEmail As DTOUser = BLLUser.FromEmail(sFrom)
