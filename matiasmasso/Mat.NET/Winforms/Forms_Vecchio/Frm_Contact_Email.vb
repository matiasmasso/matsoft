...
Public Class Frm_Contact_Email
    ...
    Private Sub Refresca()
        With _User
            ...
            End If
            mBlocked = Not FEB2.User.IsAllowedToRead(Current.Session.User, _User)
            ...
            If .BadMail <> DTOEmail.BadMailErrs.None Then
            ...
            If Not FEB2.User.IsAllowedToRead(Current.Session.User, _User) Then
    ...
    Private Async Function LoadSscs() As Task
        Dim exs As New List(Of Exception)
        Dim oRolSscs = Await FEB2.Subscriptions.All(exs, Current.Session.Emp, _User.Rol)
        Dim oUserSscs = Await FEB2.Subscriptions.All(exs, Current.Session.Emp, _User)
        If exs.Count = 0 Then
    ...
    Private Async Function LoadBadMails() As Task
        Dim exs As New List(Of Exception)
        Dim items = Await FEB2.User.BadMailReasons(exs)
        If exs.Count = 0 Then
    ...
    Private Async Sub ButtonOk_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles ButtonOk.Click
        ...
        Else
            ...
            With _User
                ...
                Else
                    .BadMail = DTOEmail.BadMailErrs.None
                End If
            ...
            If Await FEB2.User.Update(exs, _User) Then
                If mDirtySsc Then
                    ...
                    If Not Await FEB2.Subscriptions.Update(exs, _User, oSscs) Then
    ...
    Private Async Sub TextBoxEmail_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles TextBoxEmail.Validated
        ...
        If _User.EmailAddress <> sText Or sText.IndexOf("<") >= 0 Then
            ...
            Dim oSourceUser = Await FEB2.User.FromEmail(exs, Current.Session.Emp, TextBoxEmail.Text.Trim)
            If exs.Count = 0 Then
    ...
    Private Sub TextBoxPwd_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles TextBoxPwd.DoubleClick
        Dim BlAllow As Boolean = FEB2.User.IsAllowedToRead(Current.Session.User, _User)
        ...
        Select Case TabControl1.SelectedIndex
            ...
            Case Tabs.Logs
                If Not _LogsLoaded Then
                    Dim items = Await FEB2.User.WebLogs(exs, _User)
                    If exs.Count = 0 Then
