...
Public Class Frm_CreditsClients
    ...
    Private Sub LoadGrid()
        Dim SQL As String = "SELECT CLX.Guid, CLX.clx, CCC.Eur, X.RISC,(CASE WHEN CCC.EUR>0 THEN X.RISC/CCC.Eur ELSE 0 END) AS DISPOSAT, IBAN.CCC, IBAN.Mandato_DiesDevolucio, IBAN.Mandato_Fch " _
        & "FROM            CLX " _
        & "INNER JOIN CliGral ON Clx.Guid = CliGral.Guid " _
        & "LEFT OUTER JOIN Credit_ClassificacioClients AS CCC ON CLX.Guid = CCC.CliGuid " _
        & "LEFT OUTER JOIN IBAN ON CCC.CliGuid = IBAN.ContactGuid AND IBAN.COD = 2 AND (IBAN.Caduca_Fch IS NULL OR IBAN.Caduca_Fch > GETDATE()) " _
        & "LEFT OUTER JOIN (SELECT        ContactGuid, SUM(CASE WHEN CCB.DH = 1 THEN CCB.EUR ELSE - CCB.EUR END) AS RISC FROM Ccb WHERE Cta = '43000' AND yea = YEAR(GETDATE()) GROUP BY ContactGuid) AS X ON X.ContactGuid = CLX.Guid " _
        & "WHERE  CliGral.EMP=" & BLLApp.Emp.Id & " AND  (CCC.Eur <> 0 OR X.RISC <> 0) " _
        & "ORDER BY RISC DESC"
    ...
    Private Function CurrentContact() As DTOContact
        ...
        If oRow IsNot Nothing Then
            Dim oGuid As Guid = DataGridView1.CurrentRow.Cells(Cols.CliGuid).Value
