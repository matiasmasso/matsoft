...
Public Class Wz_Girs
    ...
    Private Async Sub Wz_Girs_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        _Bancs = Await FEB2.Bancs.AllActive(GlobalVariables.Emp, exs)
        If exs.Count = 0 Then
            _Csbs = Await FEB2.Csbs.PendentsDeGirar(exs, GlobalVariables.Emp, oCountry:=DTOCountry.Wellknown(DTOCountry.Wellknowns.Spain), sepa:=True)
            If exs.Count = 0 Then
    ...
    Private Async Function Grabacio() As Task
        ...
        For Each oCsa In oCsas
            With oCsa
                ...
                End If
                oCsa.Cca = Await FEB2.Csa.BuildCca(exs, oCsa, Current.Session.User)
            End With
        Next

        Dim caption As String = ""
        Dim ids = Await FEB2.Csas.Update(oCsas, exs)
        If exs.Count = 0 Then
            If ids.Count = 1 Then
