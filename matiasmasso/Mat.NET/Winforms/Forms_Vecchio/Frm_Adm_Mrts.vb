...
Public Class Frm_Adm_Mrts
    ...
    Private Sub SetCtas(ByVal IntYea As Integer)
        ...
        & "LEFT OUTER JOIN " _
        & "(SELECT  MRT.EMP, CTA, SUM(CASE WHEN MRT.BAIXACCAGUID IS NULL AND Year(Mr2.Fch)=" & IntYea & " then MR2.eur else 0 end) AS CURRENTYEA, " _
        & "SUM(CASE WHEN MRT.BAIXACCAGUID IS NULL THEN MR2.eur ELSE 0 END) AS AMORT FROM MRT INNER JOIN " _
    ...
    Private Sub LoadItms(ByVal IntYea As Integer, ByVal oCta As PgcCta)
        ...
        & "CCA ALTA ON MRT.AltaCcaGuid=ALTA.Guid LEFT OUTER JOIN " _
        & "CCA ON MRT.BaixaCcaGuid=CCA.Guid " _
        & "WHERE MRT.CTA LIKE '" & oCta.Id & "' AND " _
        & "Alta.Yea<=" & IntYea & " " _
        & "ORDER BY (CASE WHEN MRT.BAIXACCAGUID IS NULL THEN 0 ELSE 1 END), MRT.FCH"
    ...
    Private Sub ToolStripButtonRetroceder_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripButtonRetroceder.Click
        ...
        If rc = MsgBoxResult.Ok Then
            ...
            SQL = "SELECT Guid FROM CCA WHERE " _
            & "EMP=" & mEmp.Id & " AND " _
            ...
            Do While oDrd.Read
                oCca = New Cca(CType(oDrd("Guid"), Guid))
