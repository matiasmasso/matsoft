...
Public Class Frm_Forecast_Old
    ...
    Public Sub New(oProveidor As DTOProveidor)
        MyBase.New
        InitializeComponent()
        BLLContact.Load(oProveidor)
        _Proveidor = oProveidor
        mMode = Modes.Proveidor
        Me.Text = String.Format("Forecast {0}", _Proveidor.FullNom)
        LoadYeas()
    ...
    Public Sub New(oBrand As DTOProductBrand)
        MyBase.New
        InitializeComponent()
        BLLProductBrand.Load(oBrand)
        _Brand = oBrand
        _Proveidor = _Brand.Proveidor
        mMode = Modes.Tpa
        Me.Text = String.Format("Forecast {0}", _Brand.Nom)
        LoadYeas()
        If ComboBoxYeas.Items.Count > 0 Then
            LoadGridFcast()
        End If
    ...
    Public Sub New(oCategory As DTOProductCategory)
        MyBase.New
        InitializeComponent()
        BLLProductCategory.Load(oCategory)
        _Category = oCategory
        _Brand = _Category.Brand
        _Proveidor = _Brand.Proveidor
        mMode = Modes.Stp
        Me.Text = String.Format("Forecast {0}/{1}", _Brand.Nom, _Category.Nom)
        LoadYeas()
        If ComboBoxYeas.Items.Count > 0 Then
            LoadGridFcast()
        End If
...
#Region "ForeCast"
    ...
    Private Sub LoadGridFcast()
        ...
        sb.AppendLine("SELECT Tpa.Dsc, Tpa.Ord, Stp.Codi, Stp.Dsc, Stp.Ord, Art.Ord, Art.Guid ")
        sb.AppendLine(", " & RowsFcast.Fcast & " AS CodLin, Art.LastProduction ")
        sb.AppendLine(", SUM(VwForecast.qty) AS Tot ")

        For i As Integer = 1 To 12
            sb.AppendLine(", SUM(CASE WHEN VwForecast.MES =" & i.ToString & " THEN VwForecast.QTY ELSE 0 END) AS M" & i.ToString & " ")
        Next
        ...
        sb.AppendLine("LEFT OUTER JOIN VwForecast ON Art.Guid = VwForecast.Sku AND VwForecast.yea =" & CurrentYea() & " ")

        sb.AppendLine("WHERE Art.Emp =" & BLLApp.Emp.Id & " ")
        Select Case mMode
            Case Modes.Proveidor
                sb.AppendLine("AND Tpa.Proveidor='" & _Proveidor.Guid.ToString & "' ")
                sb.AppendLine("AND Stp.Codi <2 ") 'Standard i accessoris
            Case Modes.Tpa
                sb.AppendLine("AND Stp.Brand='" & _Brand.Guid.ToString & "' ")
                sb.AppendLine("AND Stp.Codi <2 ") 'Standard i accessoris
            Case Modes.Stp
                sb.AppendLine("AND Art.Category='" & _Category.Guid.ToString & "' ")
        End Select

        If CheckBoxHideLastInProduction.Checked Then
            sb.AppendLine("AND Art.LastProduction = 0 ")
        End If

        sb.AppendLine("GROUP BY Tpa.Ord, Tpa.Dsc, Stp.Codi, STP.ord, ART.Guid, STP.dsc, ART.ord, Art.Obsoleto, Art.LastProduction ")
        If CheckBoxHideObsoletos.Checked Then
            sb.AppendLine("HAVING SUM(VwForecast.qty)>0 or (Art.Obsoleto=0 AND Art.LastProduction=0) ")
        End If
        ...
        If Not CheckBoxHideSales.Checked Then
            sb.AppendLine("UNION ")
            sb.AppendLine("SELECT  Tpa.Dsc, Tpa.Ord, Stp.Codi, STP.DSC, STP.ORD, ART.ORD, ART.Guid ")
            sb.AppendLine(", " & RowsFcast.Sales & " AS CODLIN, 0 as LASTPRODUCTION ")
            sb.AppendLine(", SUM(PNC.qty) AS TOT ")

            For i As Integer = 1 To 12
                sb.AppendLine(", SUM(CASE WHEN MONTH(PDC.FCH) =" & i.ToString & " THEN PNC.QTY ELSE 0 END) AS M" & i.ToString & " ")
            Next
            ...
            sb.AppendLine("WHERE ART.emp =" & BLLApp.Emp.Id & " ")
            sb.AppendLine("AND Pdc.Yea =" & CurrentYea() & " ")
            sb.AppendLine("AND PNC.Cod =" & DTOPurchaseOrder.Codis.Client & " ")
            ...
            Select Case mMode
                ...
                Case Modes.Tpa
                    ...
                    sb.AppendLine("AND Stp.Codi <2 ") 'Standard i accessoris
                Case Modes.Stp
                    sb.AppendLine("AND Stp.Guid='" & _Category.Guid.ToString & "' ")
            End Select
            ...
            sb.AppendLine("GROUP BY Tpa.Ord, Tpa.Dsc, Stp.Codi, STP.ord, ART.Guid, STP.dsc, ART.ord ")
        End If

        sb.AppendLine("ORDER BY Tpa.Ord, Tpa.Dsc, Stp.Codi, Stp.Ord, Stp.Dsc, Art.Ord, Art.Guid, CodLin")
        ...
        With DataGridViewFcast
            ...
            If .Rows.Count > 0 Then
                ...
                End With
            End If
            With .Columns(ColsFcast.TpaNom)
                .HeaderText = "marca"
                .Width = 90
                .AutoSizeMode = DataGridViewAutoSizeColumnMode.None
            End With
            With .Columns(ColsFcast.TpaOrd)
                .Visible = False
            End With
            With .Columns(ColsFcast.StpCodi)
                .Visible = False
            End With
            ...
            End With
            With .Columns(ColsFcast.ArtGuid)
                .Visible = False
            End With
        ...
        If Not CheckBoxHideSales.Checked Then
            ...
            For iCol = ColsFcast.Tot To oGrid.Columns.Count - 1
                ...
                For iRow = 2 To oGrid.Rows.Count - 2 Step 2
                    ...
                    If oDgRowFcast.Cells(ColsFcast.ArtGuid).Value = oDgRowSales.Cells(ColsFcast.ArtGuid).Value Then
    ...
    Private Function CurrentArt() As Art
        ...
        If oRow IsNot Nothing Then
            If Not IsDBNull(oRow.Cells(ColsFcast.ArtGuid).Value) Then
                Dim oGuid As Guid = oRow.Cells(ColsFcast.ArtGuid).Value
                oArt = New MaxiSrvr.Art(oGuid)
            End If
        End If
        Return oArt
    End Function

    Private Function CurrentArts() As List(Of Art)
        Dim oArts As New List(Of Art)
        ...
        Else
            For Each oRow As DataGridViewRow In DataGridViewFcast.SelectedRows
                If Not IsDBNull(oRow.Cells(ColsFcast.ArtGuid).Value) Then
                    Dim oGuid As Guid = oRow.Cells(ColsFcast.ArtGuid).Value
                    Dim oArt As New MaxiSrvr.Art(oGuid)
                    oArts.Add(oArt)
                End If
            Next
        End If
    ...
    Private Sub DataGridViewFcast_CellFormatting(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellFormattingEventArgs) Handles DataGridViewFcast.CellFormatting
        If DataGridViewFcast.Columns(e.ColumnIndex).Visible Then
            ...
            If oCurrentCell.RowIndex > 0 Then
                Select Case oCurrentCell.ColumnIndex
                    Case ColsFcast.ArtNom
                        If Not IsDBNull(DataGridViewFcast.CurrentRow.Cells(ColsFcast.ArtGuid).Value) Then
                            Dim oArts As List(Of Art) = CurrentArts()
                            Dim oMenuArt As New Menu_Art(oArts.First)
    ...
    Private Sub CellPaste(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Dim oUser As DTOUser = BLLSession.Current.User
        ...
        For iTbRow As Integer = 0 To tblExcelData.Rows.Count - 1
            For iTbCol As Integer = 0 To tblExcelData.Columns.Count - 1
                If iTbRow + iRowMin < DataGridViewFcast.Rows.Count Then
                    ...
                    Dim oGuid As Guid = DataGridViewFcast.Rows(iTbRow + iRowMin).Cells(ColsFcast.ArtGuid).Value
                    Dim iMes As Integer = (iColMin - ColsFcast.M1) + iTbCol + 1

                    If iMes > 0 And iMes < 13 Then
                        If IsNumeric(tblExcelData.Rows(iTbRow)(iTbCol)) Then
                            Dim iQty As Integer = tblExcelData.Rows(iTbRow)(iTbCol)
                            Dim sb As New Text.StringBuilder
                            sb.AppendLine("INSERT INTO FORECAST(Sku,Yea,Mes,Qty,UsrCreated) VALUES ")
                            sb.AppendLine("('" & oGuid.ToString & "' ")
                            sb.AppendLine(", " & iYea)
                            sb.AppendLine(", " & iMes)
                            sb.AppendLine(", " & iQty)
                            sb.AppendLine(", '" & oUser.Guid.ToString & "')")
                            Dim SQL As String = sb.ToString
...
#Region "Pro"
    ...
    Private Sub LoadGridPro()
        ...
        Dim sb As New Text.StringBuilder
        sb.AppendLine("SELECT ART.Guid, STP.dsc, ART.ord ")
        sb.AppendLine(", 0 AS OPTIMITZAT,0 AS PRO, ArtStock.Stock AS Stk, ArtPn1.Qty as Pn1, ArtPn2NoPn3.Qty AS Pn2, 0 AS PRE ")
        sb.AppendLine(", (CASE WHEN ART.HEREDADIMENSIONS=1 THEN STP.M3 ELSE ART.M3 END) AS M3, ART.M3 AS M3X ")
        sb.AppendLine(", (CASE WHEN ART.HEREDADIMENSIONS=1 THEN STP.INNERPACK ELSE ART.INNERPACK END) AS INNERPACK ")
        sb.AppendLine(", (CASE WHEN ART.HEREDADIMENSIONS=1 THEN STP.OUTERPACK ELSE ART.OUTERPACK END) AS OUTERPACK ")
        sb.AppendLine("FROM Art ")
        sb.AppendLine("INNER JOIN Stp ON Art.Category = Stp.Guid")
        sb.AppendLine("INNER JOIN Tpa ON Stp.Brand = Tpa.Guid ")
        sb.AppendLine("LEFT OUTER JOIN ArtStock ON ART.Guid = ArtStock.ArtGuid AND ArtStock.MgzGuid='" & oMgz.Guid.ToString & "' ")
        sb.AppendLine("LEFT OUTER JOIN ArtPn2NoPn3 ON Art.Guid=ArtPn2NoPn3.ArtGuid ")
        sb.AppendLine("LEFT OUTER JOIN ArtPn1 ON Art.Guid=ArtPn1.ArtGuid ")
        sb.AppendLine("WHERE ART.emp =" & BLLApp.Emp.Id & " ")
        ...
        sb.AppendLine(" ORDER BY STP.ord, STP.Guid, ART.ord")
        ...
        For Each oRow As DataRow In oTb.Rows
            Dim oArt As New Art(CType(oRow(ColsPro.ArtGuid), Guid))
    ...
    Private Sub SetContextMenuPro()
        ...
        If oCurrentCell IsNot Nothing Then
            Dim oArt As New Art(CType(DataGridViewPro.CurrentRow.Cells(ColsPro.ArtGuid).Value, Guid))
    ...
    Private Sub DataGridViewPro_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles DataGridViewPro.DoubleClick
        Dim oGuid As Guid = DataGridViewPro.CurrentRow.Cells(ColsPro.ArtGuid).Value
...
#Region "Control"
    Private Sub LoadGridControl()
        ...
        Dim sb As New Text.StringBuilder
        sb.AppendLine("SELECT ART.Guid, STP.dsc, ART.ord ")
        sb.AppendLine(",0 AS M0PREV,0 AS M0TMP,0 AS M0SALES,0 AS M0PCT ")
        sb.AppendLine(",0 AS M1PREV,0 AS M1SALES,0 AS M1PCT ")
        sb.AppendLine(",0 AS M2PREV,0 AS M2SALES,0 AS M2PCT ")
        sb.AppendLine("FROM Art ")
        sb.AppendLine("INNER JOIN Stp ON Art.Category = Stp.Guid ")
        sb.AppendLine("INNER JOIN Tpa ON Stp.Brand = Tpa.Guid ")
        sb.AppendLine("WHERE ART.emp =" & BLLApp.Emp.Id & " ")
        ...
        sb.AppendLine("ORDER BY STP.Ord, Stp.Guid, ART.Ord, Art.Guid")
        Dim SQL As String = sb.ToString
        Dim oDs As DataSet = MaxiSrvr.GetDataset(SQL, MaxiSrvr.Databases.Maxi)
        Dim oTb As DataTable = oDs.Tables(0)
        Dim DtFch As Date = Today
        For Each oRow As DataRow In oTb.Rows
            Dim oGuid As Guid = oRow(ColsControl.ArtGuid)
            Dim oArt As New Art(oGuid)
            Dim DtFromFch As Date = DtFch.AddDays(-DtFch.Day + 1)
            Dim DtToFch As Date = DtFch.AddDays(-DtFch.Day).AddMonths(1)
            Dim iPrev As Integer = oArt.Forecast(DtFromFch, DtToFch)
            Dim iTmp As Integer = oArt.Forecast(DtFromFch, DtFch)
            Dim iVenut As Integer = oArt.Sales(DtFromFch, DtFch)
        ...
        With DataGridViewControl
            ...
            With .Columns(ColsControl.ArtGuid)
                .Visible = False
            End With
...
#End Region
    ...
    Private Sub ToolStripButtonProNewPdc_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripButtonProNewPdc.Click
        BLLContact.Load(_Proveidor)
        BLLProveidor.Load(_Proveidor)
        Dim oPdc As New Pdc(BLLApp.Emp, DTOPurchaseOrder.Codis.Proveidor)
        With oPdc
            .Client = New Client(_Proveidor.Guid)
            .Fch = Today
            .Obs = _Proveidor.Lang.Tradueix("plazo de entrega: ", "plaç d'entrega: ", "delivery time: ") & DateTimePickerDeliver.Value.ToShortDateString
            .Text = "regular"
            .Source = DTOPurchaseOrder.Sources.eMail
            .Cur = _Proveidor.Cur
            .Mgz = BLL.BLLApp.Mgz
            .DeliveryCod = Alb.DeliveryCods.Mgz
            .EntregarEn = New Contact(.Mgz.Guid)

            Dim oItm As LineItmPnc
            Dim oRow As DataGridViewRow
            For Each oRow In DataGridViewPro.Rows
                If oRow.Cells(ColsPro.Optimitzat).Value > 0 Then
                    oItm = New LineItmPnc(oPdc)
                    With oItm
                        .Qty = oRow.Cells(ColsPro.Optimitzat).Value
                        .Pendent = .Qty
                        .Art = New Art(CType(oRow.Cells(ColsPro.ArtGuid).Value, Guid))
                        .Preu = .Art.Cost
        ...
        If Not BLL.BLLAlbBloqueig.BloqueigStart(BLL.BLLSession.Current.User, _Proveidor, DTOAlbBloqueig.Codis.PDC, exs) Then
    ...
    Private Sub ToolStripButtonExcelPrevisioCompres_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripButtonExcelPrevisioCompres.Click
        ...
        Dim sFormulaVol As String = "=RC[-1]+RC[-2]-RC[-3]-RC[-4]"
        ...
        For Each oRow In oTb.Rows
            ...
            With oSheet
                ...
                Else
                    .Cells(i, 7) = 0
                End If
                .Cells(i, 8) = oRow(ColsPro.M3)
                If .Cells(i, 7).value > 0 And .Cells(i, 8).value > 0 Then
                    .Cells(i, 9).FormulaR1C1 = "=RC[-2]*RC[-1]"
                Else
                    .Cells(i, 9) = 0
                End If
            End With
    ...
    Private Sub CheckBox_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles _
     ...
     CheckBoxHideLastInProduction.CheckedChanged,
     CheckBoxHideSales.CheckedChanged
        If mAllowEvents Then
