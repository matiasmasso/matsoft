Public Class Frm_CliProductBlocked
    ...
    Private Async Sub Frm_CliProductBlocked_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If _CliProductBlocked.Contact IsNot Nothing AndAlso _CliProductBlocked.Product IsNot Nothing Then
            If FEB2.CliProductBlocked.Load(exs, _CliProductBlocked) Then
                Await Refresca()
                _AllowEvents = True
            Else
    ...
    Private Async Function LoadClientsAmbExclusiva() As Task
        Dim exs As New List(Of Exception)
        Dim oContacts = Await FEB2.CliProductBlocked.AltresEnExclusiva(exs, _CliProductBlocked.Contact, _CliProductBlocked.Product)
        If exs.Count = 0 Then
            With ListBoxClis
                .DisplayMember = "Nom"
                .ValueMember = "Guid"
    ...
    Private Sub EnableControls()
        ...
        If _CliProductBlocked.Product IsNot Nothing Then
            Dim exs As New List(Of Exception)
            FEB2.Product.Load(_CliProductBlocked.Product, exs)
            If FEB2.Product.BrandCodDist(_CliProductBlocked.Product) = DTOProductBrand.CodDists.Free Then
    ...
    Private Async Sub ButtonOk_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonOk.Click
        With _CliProductBlocked
            ...
            Else
                ...
                If Await FEB2.CliProductBlocked.Update(exs, _CliProductBlocked) Then
    ...
    Private Async Sub Xl_LookupProduct1_AfterUpdate(sender As Object, e As MatEventArgs) Handles Xl_LookupProduct1.AfterUpdate
        ...
        PanelDetall.Visible = True
        'PictureBoxLogo.Image = FEB2.Product.Brand(exs,oProduct).Logo
        _CliProductBlocked = Await FEB2.CliProductBlocked.Find(exs, oContact, oProduct)
        If exs.Count = 0 Then
