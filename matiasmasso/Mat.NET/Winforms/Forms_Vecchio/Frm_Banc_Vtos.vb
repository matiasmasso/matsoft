...
Public Class Frm_Banc_Vtos
    ...
    Private Sub LoadGrid()
        ...
        SQL.Append("SELECT Csb.Guid, (CASE WHEN IMPAGAT = 1 THEN 2 WHEN RECLAMAT = 1 THEN 3 WHEN CCAVTOCCA > 0 THEN 1 ELSE 0 END) AS WARN, ")
        ...
        If oCsb IsNot Nothing Then
            If oCsb.Impagat Then
                ...
                With oFrm
                    ...
                    '.Impagat = BLLImpagat.FromCsb(oCsb)
                    '.Show()
                End With
            Else
    ...
    Private Sub Do_CopyTpv()
        Dim oCsb As DTO.DTOCsb = CurrentCsb()
        BLL.BLLIban.Load(oCsb.Iban)
        Dim oImpagat As DTOImpagat = BLLImpagat.FromCsb(oCsb)
        Dim url As String = BLLTpvRequest.Url(oImpagat)
    ...
    Private Function CurrentOldCsb() As Csb
        Dim retval As Csb = Nothing
        Dim oRow As DataGridViewRow = DataGridView1.CurrentRow
        If oRow IsNot Nothing Then
            Dim iYea As Integer = oRow.Cells(Cols.Yea).Value
            Dim iCsa As Integer = oRow.Cells(Cols.Csa).Value
            retval = New Csb(MaxiSrvr.Csa.FromNum(mBanc.Emp, iYea, iCsa), oRow.Cells(Cols.Id).Value)
        End If
        Return retval
    End Function

    Private Function CurrentCsb() As DTO.DTOCsb
        Dim retval As DTO.DTOCsb = Nothing
        Dim oRow As DataGridViewRow = DataGridView1.CurrentRow
        If oRow IsNot Nothing Then
            Dim oGuid As Guid = oRow.Cells(Cols.Guid).Value
            retval = New DTO.DTOCsb(oGuid)
        End If
        Return retval
    End Function
