...
Public Class EPPlusHelper
    ...
    Shared Function Read(exs As List(Of Exception), oByteArray As Byte(), Optional Filename As String = "", Optional Title As String = "", Optional hasHeaderRow As Boolean = False) As ExcelHelper.Sheet
        Dim retval As New ExcelHelper.Sheet(Title, Filename)
        Try
            ...
            If hasHeaderRow Then
                Dim oRow = retval.Rows.First
                For Each oCell In oRow.Cells
                    retval.Columns.Add(New ExcelHelper.Column(oCell.Content.ToString, ExcelHelper.Sheet.NumberFormats.NotSet))
                Next
                retval.Rows.Remove(oRow)
            End If
        Catch ex As Exception
            exs.Add(ex)
        End Try
    ...
    Shared Function Read(exs As List(Of Exception), sFilename As String, ByRef oBook As ExcelHelper.Book, Optional hasHeaderRow As Boolean = False) As Boolean
        ...
        Try
            ...
            For Each worksheet As ExcelWorksheet In workbook.Worksheets
                If worksheet.Dimension IsNot Nothing Then
                    ...
                    Next
                    If hasHeaderRow Then
                        Dim oRow = oSheet.Rows.First
                        For Each oCell In oRow.Cells
                            oSheet.Columns.Add(New ExcelHelper.Column(oCell.Content.ToString, ExcelHelper.Sheet.NumberFormats.NotSet))
                        Next
                        oSheet.Rows.Remove(oRow)
                    End If
                End If
            Next
        ...
        Try
            ...
            Using xlPackage As New ExcelPackage(oMemoryStream)
                For Each oSheet As ExcelHelper.Sheet In oBook.Sheets
                    Dim idx As Integer = oBook.Sheets.IndexOf(oSheet)
                    If oSheet.Name = "" Then oSheet.Name = "Hoja " & idx + 1
    ...
    Private Shared Function Load(WorkSheet As ExcelWorksheet, oSheet As ExcelHelper.Sheet, Optional exs As List(Of Exception) = Nothing) As Boolean
        ...
        Try
            ...
            For col = 0 To oSheet.Columns.Count - 1
                ...
                'If oSheet.Columns(col).ForeColor <> Nothing Then
                'WorkSheet.Column(col + 1).Style.Font.Color.SetColor(oSheet.Columns(col).ForeColor)
                'End If
                Select Case oSheet.Columns(col).NumberFormat
                    ...
                    Case ExcelHelper.Sheet.NumberFormats.Kg
                        ...
                        WorkSheet.Column(col + 1).Width = 50
                    Case ExcelHelper.Sheet.NumberFormats.KgD1
                        WorkSheet.Column(col + 1).Style.HorizontalAlignment = Style.ExcelHorizontalAlignment.Right
                        WorkSheet.Column(col + 1).Style.Numberformat.Format = "#,##0.0 \K\g;-#,##0.0 \K\g;#"
                        WorkSheet.Column(col + 1).Width = 50
            ...
            If oSheet.Columns.Count > 0 Then
                ...
                With oRange.Style
                    ...
                    Select Case oSheet.HeaderRowStyle
                        Case ExcelHelper.HeaderRowStyles.Default
                            .Fill.BackgroundColor.SetColor(System.Drawing.Color.LightBlue)
                            .Font.Bold = True
                        Case ExcelHelper.HeaderRowStyles.ElCorteIngles
                            Dim oBackColor = System.Drawing.Color.FromArgb(0, 176, 80)
                            .Fill.BackgroundColor.SetColor(oBackColor)
                            .Font.Color.SetColor(System.Drawing.Color.White)
                            .Font.Bold = True
                            .VerticalAlignment = Style.ExcelVerticalAlignment.Center
                            .HorizontalAlignment = Style.ExcelHorizontalAlignment.Center
                            ...
                            WorkSheet.Rows(startRow).Height = 30
                    End Select
                End With
                ...
                For col = 0 To oSheet.Columns.Count - 1
                    ...
                    'If oSheet.Columns(col).BackColor <> Nothing Then
                    'oRange = WorkSheet.Cells(startRow + 1, col + 1, oSheet.Rows.Count + 1, col + 1)
                    'With oRange.Style
                    '.Fill.PatternType = OfficeOpenXml.Style.ExcelFillStyle.Solid
                    '.Fill.BackgroundColor.SetColor(oSheet.Columns(col).BackColor)
                    'End With
                    'End If
                Next
            End If
            ...
            For SheetRow = 0 To oSheet.Rows.Count - 1
                ...
                For col = 0 To oSheet.Rows(SheetRow).Cells.Count - 1
                    ...
                    'If src.ForeColor <> Nothing Then
                    'cell.Style.Font.Color.SetColor(src.ForeColor)
                    'ElseIf oSheet.Rows(SheetRow).Color <> Nothing Then
                    'cell.Style.Font.Color.SetColor(oSheet.Rows(SheetRow).Color)
                    'End If

                    'If src.BackColor <> Nothing Then
                    'cell.Style.Fill.BackgroundColor.SetColor(src.ForeColor)
                    'ElseIf oSheet.Rows(SheetRow).BackColor <> Nothing Then
                    'cell.Style.Fill.BackgroundColor.SetColor(oSheet.Rows(SheetRow).BackColor)
                    'End If
            ...
            'format full
            If oSheet.Columns.Count > 0 And oSheet.Rows.Count > 0 Then
