...
Public Class ExcelHelper
    ...
    Public Shared Function getExcelFileFromStream(exs As List(Of Exception), ByVal bytes() As Byte, Optional ByVal filename As String = "") As String
        filename = If(String.IsNullOrEmpty(filename), "Excel", filename)
        Dim filenameWithExtension = If(filename.EndsWith("xlsx"), filename, String.Format("{0}.xlsx", filename))
        Dim path = String.Format("{0}\tmp\{1}", Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), filenameWithExtension)
        System.IO.File.WriteAllBytes(filename, bytes)
        Return filename
    End Function

    Shared Function ExcelSheetFromFilename(exs As List(Of Exception), sFilename As String, ByRef oBook As Book, Optional hasHeaderRow As Boolean = False) As Boolean
        Dim retval As Boolean = EPPlusHelper.Read(exs, sFilename, oBook, hasHeaderRow)
        Return retval
    End Function

    Shared Function ExcelSheetFromFilename(exs As List(Of Exception), sFilename As String, ByRef oSheet As Sheet, Optional hasHeaderRow As Boolean = False) As Boolean
        Dim oBook As New Book
    ...
    Shared Function Stream(oBook As Book, ByRef oStream As Byte(), Optional exs As List(Of Exception) = Nothing) As Boolean
        Dim retval As Boolean = EPPlusHelper.Stream(oBook, oStream, exs)
        Return retval
    End Function

    Shared Function Stream(oSheet As Sheet, ByRef oStream As Byte(), Optional exs As List(Of Exception) = Nothing) As Boolean
        Dim oBook As New Book
    ...
    Shared Function Save(oBook As Book, Optional exs As List(Of Exception) = Nothing) As Boolean
        Dim retval As Boolean = EPPlusHelper.Save(oBook, exs)
        Return retval
    End Function

    Shared Function Save(oSheet As Sheet, Optional exs As List(Of Exception) = Nothing) As Boolean
        Dim oBook As New Book
        oBook.Sheets.Add(oSheet)
        oBook.Filename = oSheet.Filename
    ...
    Public Class Row
        ...
        Public Function GetInt(CellIdx As Integer) As Integer
            ...
            If Integer.TryParse(src, result) Then
                retval = Integer.Parse(src, _CultureInfo)
            End If
            Return retval
        End Function
        ...
        Public Function GetDecimal(CellIdx As Integer) As Decimal
            ...
            Dim src = GetString(CellIdx)
            Dim result As Integer
            If Decimal.TryParse(src, result) Then
        ...
        Public Function GetFchSpain(CellIdx As Integer) As Date
            ...
            If Not String.IsNullOrEmpty(src) Then
                ...
                If segments.Length >= 3 Then
                    ...
                    Dim year = CInt(segments(2).Substring(6,4))
