...
namespace Services
{
    ...
    {
        public static string ApiUrl(bool useLocalApi, params string[] segments) => Globals.ApiUrl(useLocalApi, segments);
        ...
        public static async Task<T?> Get2Async<T>(HttpClient http, bool useLocalApi, params string[] segments)
        {
            T? retval;
            var apiresponse = await GetAsync<T>(http, useLocalApi, segments);
        ...
        public static async Task<T?> Post2Async<U, T>(HttpClient http, bool useLocalApi, U payload, params string[] segments)
        {
            T? retval;
            var apiresponse = await PostAsync<U, T>(http, useLocalApi, payload, segments);
            if (apiresponse.Success())
                retval = apiresponse.Value;
            else
                throw apiresponse?.ProblemDetails?.ToException(segments) ?? new Exception("error http");
        ...
        public static async Task<ApiResponse<T>> GetAsync<T>(HttpClient http, bool useLocalApi, params string[] segments)
        {
            string url = Globals.ApiUrl(useLocalApi, segments);
        ...
        public static async Task<ApiResponse<T>> PostAsync<U, T>(HttpClient http, bool useLocalApi, U payload, params string[] segments)
        {
            var url = segments.Count() == 1 && segments.First().StartsWith("http") ? segments.First() : Globals.ApiUrl(useLocalApi, segments);
        ...
        public static async Task<ApiResponse<T>> PostAsync<T>(HttpClient http, bool useLocalApi, HttpContent content, params string[] segments)
        {
            var url = segments.Count() == 1 && segments.First().StartsWith("http") ? segments.First() : Globals.ApiUrl(useLocalApi, segments);
            return await PostAsync<T>(http, url, content);
        }
        ...
        public static async Task<T?> PostMultipartAsync<U, T>(HttpClient http, bool useLocalApi, U payload, DocfileModel? docfile = null, params string[] segments)
        {
            List<DocfileModel> docfiles = docfile == null ? new() : new() { docfile };
            return await PostMultipartAsync<U, T>(http, useLocalApi, payload, docfiles, segments);
        }

        public static async Task<T?> PostMultipartAsync<U, T>(HttpClient http, bool useLocalApi, U payload, List<DocfileModel> docfiles, params string[] segments)
        {
            T? retval = default;
            ...
            var apiresponse = await PostAsync<T>(http, useLocalApi, content, segments);
            if (apiresponse.Success())
                retval = apiresponse.Value;
            else
                throw apiresponse?.ProblemDetails?.ToException(segments) ?? new Exception("error http");
