...
namespace Web.Services
{
    public class MailService
    {
        ...
        public async static Task SendEmailAsync(string toEmailAddress, string emailSubject, string emailMessage, LangDTO? lang = null, bool copyToInfo = true)
        {
            using (var message = new MailMessage())
            {
                ...
                {
                    message.From = new MailAddress("portugal@matiasmasso.es", "M+O MATIAS MASSO, S.A.", Encoding.UTF8);
                   if(copyToInfo) message.Bcc.Add(new MailAddress("info@matiasmasso.es", "M+O MATIAS MASSO, S.A.", Encoding.UTF8));
                }
                else
                {
                    message.From = new MailAddress("info@matiasmasso.es", "M+O MATIAS MASSO, S.A.", Encoding.UTF8);
                    if (copyToInfo) message.Bcc.Add(new MailAddress("info@matiasmasso.es", "M+O MATIAS MASSO, S.A.", Encoding.UTF8));
                }
        ...
        public static string TemplatedBody(string body)
        {
            ...
            sb.AppendLine("        <td>");
            sb.AppendFormat("            &copy; {0} - Copyright MATIAS MASSO, S.A.", @DateTime.Today.Year);
            sb.AppendLine("        </td>");
            sb.AppendLine("    </tr>");
