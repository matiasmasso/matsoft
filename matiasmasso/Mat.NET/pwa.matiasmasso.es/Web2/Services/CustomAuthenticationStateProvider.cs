...
namespace Web.Services
{
    public class CustomAuthenticationStateProvider : AuthenticationStateProvider
    {
        ...
        public CustomAuthenticationStateProvider(ProtectedSessionStorage sessionStorage, HttpClient http, CookieService cookie)
        {
        ...
        private async Task<UserModel?> PersistedUserAsync(bool useLocalApi = false)
        {
            ApiResponse<UserModel>? apiResponse = null;
            var hash = await _cookie.GetValue("hash");
            if (hash != null) apiResponse = await HttpService.PostAsync<string, UserModel>( http, useLocalApi,hash, "User/fromHash", ((int)EmpModel.EmpIds.MatiasMasso).ToString());
