...
{
    public class ContactInvoicesViewModel
    {
        private Shared.AppState AppState;
        public string? SearchTerm;
        public InvoiceListDTO Model = new();
        ...
        public ContactInvoicesViewModel(Shared.AppState appstate, Guid contact)
        {
            AppState = appstate;
            Contact = contact;
        }

        public List<InvoiceListDTO.Item> Items()
        {
        ...
        public async void Fetch(HttpClient http)
        {
            ...
            try
            {
                var url = AppState.ApiUrl("InvoiceList", Contact.ToString());
                var response = await http.GetAsync(url);
                if (response.IsSuccessStatusCode)
                {
                    var responseText = await response.Content.ReadAsStringAsync();
                    Model = Newtonsoft.Json.JsonConvert.DeserializeObject<InvoiceListDTO>(responseText);
