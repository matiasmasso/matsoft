...
namespace Spa.ViewModels
{
    public class MultasViewModel
    {
        ...
        public List<MultaModel> Model = new();
        private List<MultaModel> items = new();

        public Globals.LoadStatusEnum LoadStatus = Globals.LoadStatusEnum.Empty;
        public delegate void StateChange();
        public event StateChange? OnStateChange;

        public MultasViewModel(Shared.AppState appstate, BaseGuid target)
        {
            AppState = appstate;
            Target = target;
        }

        public List<MultaModel> Items()
        {
        ...
        public async void Fetch(HttpClient http)
        {
            ...
            try
            {
                var url = AppState.ApiUrl("Multas", Target.Guid.ToString());
                var response = await http.GetAsync(url);
                if (response.IsSuccessStatusCode)
                {
                    var responseText = await response.Content.ReadAsStringAsync();
                    items = Newtonsoft.Json.JsonConvert.DeserializeObject<List<MultaModel>>(responseText);
