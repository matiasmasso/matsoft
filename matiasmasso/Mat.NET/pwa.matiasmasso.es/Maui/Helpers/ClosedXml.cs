        ...
        public static Byte[] Bytes(Book book)
        {
            using (var workbook = new XLWorkbook())
            {
                foreach (var sheet in book.Sheets)
                {
                    ...
                    //set column captions
                    if (sheet.Columns.Any(x => !string.IsNullOrEmpty(x.Caption)))
                    {
                        rowIdx += 1;
                    ...
                    foreach (var row in sheet.Rows)
                    {
                        ...
                        foreach (var cell in row.Cells)
                        {
                            ...
                            if (!string.IsNullOrEmpty(cell.Hyperlink))
                                wsCell.SetHyperlink(new XLHyperlink(cell.Hyperlink));
