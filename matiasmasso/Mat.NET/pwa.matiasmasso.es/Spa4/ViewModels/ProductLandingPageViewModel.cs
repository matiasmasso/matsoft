...
namespace Spa4.ViewModels
{
    public class ProductLandingPageViewModel
    {
        ...
        public async void Fetch(HttpClient http, SessionDTO session)
        {
            LoadStatus = Globals.LoadStatusEnum.Loading;
            AppState.ProblemDetails = null;
            try
            {
                var url = Globals.ApiUrl("ProductLandingPage");
               
                var dto = Request(Catchall, session.Lang);
        ...
        private ContentRequestDTO Request(string catchall, LangDTO lang)
        {
