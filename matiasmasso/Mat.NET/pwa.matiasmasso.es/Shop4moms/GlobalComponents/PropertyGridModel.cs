...
namespace Shop4moms.GlobalComponents
{
    public class PropertyGridModel
    {
        ...
        public Item AddString(int field, string? value = null, string? label = null, bool multiline = false)
        {
            var item = new Item();
            item.Field = field;
            item.Value = value;
            ...
            item.Label = label ?? "";
            item.Cod = multiline! ? Cods.Multiline : Cods.Text;
            Items.Add(item);
            return item;
        }
        ...
        public Item AddDropdown<T>(int field, List<T> values, T? value = default, string? label = null) where T : IModel
        {
            ...
            item.Id = value == null ? null : value.ToString();
            item.Label = label ?? "";
        ...
        public Item AddFiles(int field, List<DocFileModel> docfiles, string? label = null, string? accept = null, int? mbMaxlength = null)
        {
            var item = new Item();
            item.Field = field;
            item.Value = docfiles;
        ...
        public List<DocFileModel>? GetDocfiles(int field)
        {
            var item = Items.FirstOrDefault(x => x.Field == (int)field);
            List<DocFileModel> retval = (List<DocFileModel>?)item?.Value ?? new();
            return retval;
        }
        ...
        public DateTime? GetFch(int field)
        {
            ...
            if (item?.Value != null)
            {
                ...
                retval = DateTime.ParseExact(data, "yyyy-MM-dd", System.Globalization.CultureInfo.InvariantCulture);
        ...
        public List<DocFileModel> GetFiles(int field)
        {
            List<DocFileModel> retval = new();
            Item ? item = Items?.FirstOrDefault(x => x.Field == field);
            if (item != null)
            {
                retval = item.Value as List<DocFileModel> ?? new();
            }
            return retval;
        }
        ...
        public class Item
        {
            ...
            public int? MbMaxLength { get; set; }

            public string? Accept { get; set; } // mime extensions for file uploads
