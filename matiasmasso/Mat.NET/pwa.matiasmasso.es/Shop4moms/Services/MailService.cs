        ...
        public async static Task SendEmailAsync(string toEmailAddress, string emailSubject, string emailMessage, LangDTO? lang = null, bool copyToInfo = true)
        {
            using (var message = new MailMessage())
            {
                ...
                if (lang != null && lang.IsPor())
                {
                   ...
                   if(copyToInfo) message.Bcc.Add(new MailAddress("4moms@matiasmasso.es", "4moms Portugal", Encoding.UTF8));
                }
                else
                {
                    message.From = new MailAddress("4moms@matiasmasso.es", "4moms España", Encoding.UTF8);
                    if (copyToInfo) message.Bcc.Add(new MailAddress("4moms@matiasmasso.es", "4moms España", Encoding.UTF8));
                }
