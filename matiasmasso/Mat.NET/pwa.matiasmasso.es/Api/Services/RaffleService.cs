    ...
    public class RaffleService
    {
        ...
        public static RaffleModel.Player Player(Entities.MaxiContext db, Guid raffle, UserModel user)
        {
            ...
            if (retval.Raffle?.Product != null)
            {
                ...
                var product = cache.ProductFromGuid(retval.Raffle.Product.Guid);
                if (product != null)
                {
        ...
        public static RaffleModel? Find(Entities.MaxiContext db, Guid guid)
        {
            ...
            .Select(x => new RaffleModel(x.Guid)
            {
                ...
                FchFrom = x.FchFrom,
                FchTo = x.FchTo,
                Product = x.Art == null ? null : new GuidNom((Guid)x.Art!, ""),
                Question = x.Question,
                Answers = x.Answers,
            ...
            if (retval?.Product != null)
            {
                var cache = CacheService.CatalogRequest();
                product = cache.ProductFromGuid(retval.Product.Guid);
                if (product != null)
                {
                    retval.Product.Nom = cache.FullNom(product.Guid, retval.Lang);
                }
            }
        ...
        public static Byte[]? Thumbnail(Guid guid)
        {
            ...
            using (var db = new Entities.MaxiContext())
            {
                retval = db.Sorteos.Where(x => x.Guid.Equals(guid)).FirstOrDefault()?.ImgFbFeatured116;
            }
            return retval;
        }
        ...
        public static Byte[]? ImgBanner600(Guid guid)
        {
            ...
            using (var db = new Entities.MaxiContext())
            {
                retval = db.Sorteos.Where(x => x.Guid.Equals(guid)).FirstOrDefault()?.ImgBanner600;
            }
            return retval;
        }
        ...
        public static Byte[]? RaffleWinnerImg(Guid guid)
        {
            ...
            using (var db = new Entities.MaxiContext())
            {
                retval = db.Sorteos.Where(x => x.Guid.Equals(guid)).FirstOrDefault()?.ImgWinner;
            }
            return retval;
        }
