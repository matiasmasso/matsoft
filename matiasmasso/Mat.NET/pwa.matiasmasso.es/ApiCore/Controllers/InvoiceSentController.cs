...
namespace Api.Controllers
{
    ...
    [ApiController]
    [Route("[controller]")]
    public class InvoicesSentController : ControllerBase
    {
        ...
        public IActionResult FromUser()
        {
            ...
            try
            {
                ...
                var values = InvoicesSentService.FromUser(user);
                retval = Ok(values);
            }
            catch (Exception ex)
            {
        ...
        public IActionResult FromCustomer(Guid contact)
        {
            ...
            try
            {
                var values = InvoicesSentService.FromCustomer(contact);
                retval = Ok(values);
            }
            catch (Exception ex)
            {
        ...
        public IActionResult FromEmpYear(int emp, int year)
        {
            ...
            try
            {
                var values = InvoicesSentService.GetValues(emp, year);
                retval = Ok(values);
            }
            catch (Exception ex)
            {
