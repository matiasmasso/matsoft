...
namespace Api.Services
{
    ...
    {
        public static List<CanonicalUrlModel> CanonicalUrls()
        {
            using(var db = new Entities.MaxiContext())
            {
        ...
        public static List<CanonicalUrlModel> CanonicalUrls(Entities.MaxiContext db)
        {
            List<CanonicalUrlModel> retval = new();
            foreach (var x in db.VwProductUrlCanonicals)
            {
                var item = CanonicalUrlModel.Factory(x.Guid
                    , x.UrlBrandEsp
                    , x.UrlBrandCat
                    , x.UrlBrandEng
                    , x.UrlBrandPor
