...
namespace Api.Services
{
    public class EdiversaInvRptsService
    {
        ...
        public static Byte[] Excel(Guid holding, DateTime fch, CacheDTO cache)
        {
            var model = EdiversaInvRptsService.GetLastValues(holding, fch);
            ...
            foreach (var rpt in model)
            {
                ...
                row.AddCell(cache.Contacts.FirstOrDefault(x => rpt.Ccx != null && x.Guid == rpt.Ccx.Guid)?.Nom);
