...
namespace Api.Services
{
    public class ZipService
    {
        ...
        public static ZipModel? UpdateFromGeocode(GoogleApi.GeoCodeModel geocode)
        {
            ...
            _ = MailService.SendEmail("matias@matiasmasso.es", "test from M+O server ZipService.UpdateFromGeopCode", "this is the message body");
            return retval;
        }
    ...
    {
        public static List<ZipModel> GetValues()
        {
            using(var db = new Entities.MaxiContext())
            {
                return GetValues(db);
        ...
        public static List<ZipModel> GetValues(Entities.MaxiContext db)
        {
            return db.Zips
                        .AsNoTracking()
                            .Select(x => new ZipModel(x.Guid)
        ...
        //for eCommerce address query, it gets a google formated object with ZipGuid and Location nom
        public static List<DTO.Integracions.Google.Geonames.Zip> Geocode(string countryIso, string zipcod)
        {
            using (var db = new Entities.MaxiContext())
            {
                return db.VwZips
