Public Class ImportPrevisions
    ...
    Shared Async Function UploadExcel(exs As List(Of Exception), oImportacio As DTOImportacio, oSheet As ExcelHelper.Sheet) As Task(Of Boolean)
        Return Await Api.Execute(Of ExcelHelper.Sheet, Boolean)(oSheet, exs, "ImportPrevisions/UploadExcel", oImportacio.Guid.ToString())
    End Function
        ...
        Else
            ...
            Else
                ...
                Else
                    ...
                    FEB2.Contact.Load(oImportacio.proveidor, exs)

                    Dim sNum As String = Guid.NewGuid().ToString.Substring(1, 4)

                    Dim oXmlDocument As New DTOXmlDocument(includeProlog:=False)

                    Dim oRootSegment As DTOXmlSegment = oXmlDocument.AddSegment("DOCUMENT")
                    oRootSegment.AddAttributes("TYPE", "AVISOCAMION", "DATE", TextHelper.VbFormat(DateTime.Today, "dd/MM/yyyy"), "NUMERO", sNum, "REMESA", oImportacio.Guid.ToString("N").ToUpper())
                    ...
                    For Each item As DTOImportPrevisio In Items
                        ...
                        oItem.AddAttribute("DSC", Net.WebUtility.HtmlEncode(item.Sku.NomLlarg.Tradueix(oLang)))
