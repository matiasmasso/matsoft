Public Class Mayborn
    ...
    Shared Async Function ValidateCustomerOrder(exs As List(Of Exception), oSheet As ExcelHelper.Sheet, oEmp As DTOEmp) As Task(Of ExcelHelper.Sheet)
        ...
        Dim oPrvCliNums = Await FEB2.PrvCliNums.All(oProveidor, exs)
        Dim oInventari = Await FEB2.Mgz.Inventari(exs, oEmp.Mgz, oProveidor)
    ...
    Shared Async Function CustomerOrdersFactory(exs As List(Of Exception), oSheet As ExcelHelper.Sheet, oUser As DTOUser, oMgz As DTOMgz) As Task(Of List(Of DTOPurchaseOrder))
        Dim oCandidate = DTOExcelMaybornPOFarma.Factory(oSheet)
        Dim oProveidor = DTOProveidor.Wellknown(DTOProveidor.Wellknowns.Mayborn)
        Dim oPrvCliNums = Await FEB2.PrvCliNums.All(oProveidor, exs)
        Dim oInventari = Await FEB2.Mgz.Inventari(exs, oMgz, oProveidor)
    ...
    Shared Async Function SalesCsv(exs As List(Of Exception), Optional year As Integer = 0) As Task(Of DTOCsv)
        ...
        Dim items = Await FEB2.Mayborn.Sales(exs, year)
        If exs.Count = 0 Then
