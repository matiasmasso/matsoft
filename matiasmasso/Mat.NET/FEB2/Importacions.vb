Public Class Importacio
        ...
        If oDoc IsNot Nothing Then
            ...
            If TextHelper.VbIsNumeric(sNum) Then
                ...
                If DateTime.TryParse(sFch, DtFch) Then
                    retval = Await FEB2.Importacio.Find(exs, New Guid(sGuid))
                    retval.restoreObjects()
                Else
    ...
    Shared Async Function Confirm(exs As List(Of Exception), oConfirmation As DTOImportacio.Confirmation, ShowProgress As ProgressBarHandler) As Task(Of Boolean)
        ...
        Dim oImportacio As DTOImportacio = Await FEB2.Importacio.Find(exs, oConfirmation.Importacio.Guid)
        If exs.Count = 0 Then
            ...
            Dim oCtas = Await FEB2.PgcCtas.All(exs, oPlan)
            ...
            If exs.Count = 0 Then
                ...
                Dim oInvoicesReceived = Await FEB2.InvoicesReceived.FromConfirmation(exs, oConfirmation)
                If exs.Count = 0 Then
                    ...
                    For Each invoice In oInvoicesReceived
                        ...
                        invoice = Await FEB2.InvoiceReceived.Find(exs, invoice.Guid)
                        ...
                        If Await CheckFacturaDoesNotExists(exs, invoice, oConfirmation.User.Emp) Then
                            ...
                            Dim oDelivery = Await FEB2.InvoiceReceived.Delivery(exs, invoice, oConfirmation.User)
                            ...
                            If Await FEB2.Delivery.Update(exs, oDelivery) Then
                                ...
                                'desa la factura
                                If Await FEB2.InvoiceReceived.Update(exs, invoice) Then
                    ...
                    Await FEB2.Importacio.Update(oImportacio, exs)
                End If
            End If
        End If
    ...
    Public Shared Async Function GenerateCca(exs As List(Of Exception), invoice As DTOInvoiceReceived, oImportacio As DTOImportacio, oCtas As List(Of DTOPgcCta), oUser As DTOUser) As Task(Of DTOCca)
        ...
        Dim oProveidor = Await FEB2.Proveidor.Find(oImportacio.proveidor.Guid, exs)
        ...
        'Dim oCtaTotal = Await FEB2.PgcCta.FromCod(oCtaTotalCod, oUser.Emp, exs)
        ...
        Else
            ...
            retval = Await FEB2.Proveidor.SaveFactura(exs, oCca, oPnds, oImportacio)
    ...
    Private Shared Async Function CheckFacturaDoesNotExists(exs As List(Of Exception), oInvoice As DTOInvoiceReceived, oEmp As DTOEmp) As Task(Of Boolean)
        ...
        Dim oDuplicada As DTOCca = Await FEB2.Proveidor.CheckFacturaAlreadyExists(exs, oProveidor, DTOExercici.Current(oEmp), oInvoice.DocNum)
    ...
    Shared Async Function MailMessageOrdenDeCarga(exs As List(Of Exception), oEmp As DTOEmp, oImportacio As DTOImportacio) As Task(Of DTOMailMessage)
        ...
        If FEB2.Importacio.Load(exs, oImportacio) Then
            ...
            Dim sRecipients = Await FEB2.Subscriptors.Recipients(exs, oEmp, DTOSubscription.Wellknowns.TransportOrdenDeCarga, oImportacio.transportista)
            If exs.Count = 0 Then
                If sRecipients.Count = 0 Then
                    Dim oEmails = Await FEB2.Emails.All(exs, oImportacio.Transportista)
                    sRecipients = oEmails.Select(Function(x) x.EmailAddress).ToList
                End If

                retval = DTOMailMessage.Factory(sRecipients)
                Dim oTxt = Await FEB2.Txt.Find(DTOTxt.Ids.MailTrpLoad, exs)

                Dim oPlataformaDeCarga = DTOImportacio.PlataformaDeCargaOProveidor(oImportacio)
                If FEB2.Contact.Load(oPlataformaDeCarga, exs) Then
                    retval.body = oTxt.ToHtml(DTOLang.ESP,
                                          ...
                                          TextHelper.VbFormat(DateTime.Now, "dd/MM/yyyy"),
    ...
    Shared Async Function DiscrepancyReport(exs As List(Of Exception), oInvoicesReceived As List(Of DTOInvoiceReceived), oImportacio As DTOImportacio) As Task(Of ExcelHelper.Sheet)
        Dim retval As New ExcelHelper.Sheet("DiscrepancyReport " & oImportacio.id)
        With retval
            .AddColumn("Item code")
            .AddColumn("Item name")
            .AddColumn("Shipment")
    ...
    Shared Async Function ExcelGoods(exs As List(Of Exception), oImportacio As DTOImportacio, oLang As DTOLang) As Task(Of ExcelHelper.Sheet)
        Dim retval As New ExcelHelper.Sheet("Remesa Importacio " & oImportacio.id)
        If FEB2.Importacio.Load(exs, oImportacio) Then
            ...
            For Each document In oImportacio.items
                If document.srcCod = DTOImportacioItem.SourceCodes.alb Then
                    Dim oDelivery = Await FEB2.Delivery.Find(document.Guid, exs)
                    If exs.Count = 0 Then
...
Public Class Importacions
    ...
    Shared Async Function Excel(oEmp As DTOEmp, exs As List(Of Exception)) As Task(Of ExcelHelper.Sheet)
        Return Await Api.Fetch(Of ExcelHelper.Sheet)(exs, "Importacions/Excel", oEmp.Id)
    End Function

    Shared Function ExcelUrl(oEmp As DTOEmp) As String
        Return FEB2.ApiUrl("importacions/Excel", oEmp.Id)
    End Function

    Shared Function Excel(oImportacions As List(Of DTOImportacio), oLang As DTOLang) As ExcelHelper.Sheet
        Dim retval As New ExcelHelper.Sheet("Remeses de Importacio")
        With retval
            .AddColumn(oLang.Tradueix("remesa", "remesa", "Id"))
            .AddColumn("ETD", ExcelHelper.Sheet.NumberFormats.DDMMYY)
            .AddColumn("ETA", ExcelHelper.Sheet.NumberFormats.DDMMYY)
            .AddColumn(oLang.Tradueix("valor", "valor", "value"), ExcelHelper.Sheet.NumberFormats.Euro)
            ...
            .AddColumn(oLang.Tradueix("volumen", "volum", "volume"), ExcelHelper.Sheet.NumberFormats.m3D2)
            .AddColumn(oLang.Tradueix("transporte", "transport", "transport"), ExcelHelper.Sheet.NumberFormats.Euro)
