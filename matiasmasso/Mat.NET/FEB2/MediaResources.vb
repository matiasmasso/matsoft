Public Class MediaResource
    ...
    Shared Async Function Find(exs As List(Of Exception), hash As String) As Task(Of DTOMediaResource)
        Dim retval = Await Api.Fetch(Of DTOMediaResource)(exs, "MediaResource", CryptoHelper.UrlFriendlyBase64(hash))
        Return retval
    End Function

    Shared Async Function Thumbnail(exs As List(Of Exception), oMediaResource As DTOMediaResource) As Task(Of Byte())
        Return Await Api.FetchImage(exs, "MediaResource/thumbnail", CryptoHelper.UrlFriendlyBase64(oMediaResource.Hash))
    End Function
    ...
    Shared Function Load(exs As List(Of Exception), ByRef oMediaResource As DTOMediaResource, Optional LoadThumbnail As Boolean = False) As Boolean
        ...
        If Not oMediaResource.IsNew Then
            Dim pMediaResource = Api.FetchSync(Of DTOMediaResource)(exs, "MediaResource", urlFriendlyHash)
            If exs.Count = 0 Then
                ...
                If LoadThumbnail Then
                    oMediaResource.Thumbnail = Api.FetchImageSync(exs, "MediaResource/thumbnail", urlFriendlyHash)
                End If
            End If
        End If
    ...
    Shared Function Url(oMediaResource As DTOMediaResource, Optional AbsoluteUrl As Boolean = False) As String
        Return UrlHelper.Factory(AbsoluteUrl, "MediaResource/media", CryptoHelper.UrlFriendlyBase64(oMediaResource.Hash))
    End Function

    Shared Function ThumbnailUrl(oMediaResource As DTOMediaResource, Optional AbsoluteUrl As Boolean = False)
        Return UrlHelper.Factory(AbsoluteUrl, "MediaResource/thumbnail", CryptoHelper.UrlFriendlyBase64(oMediaResource.Hash))
    End Function
End Class
...
Public Class MediaResources
    ...
    Private Shared Async Function ThumbnailsLoaded(exs As List(Of Exception), items As List(Of DTOMediaResource)) As Threading.Tasks.Task(Of List(Of DTOMediaResource))
        ...
        If items IsNot Nothing AndAlso items.Count > 0 Then
            ...
            Dim oSpriteImage = Await Api.downloadImage(Of List(Of String))(sHashes, exs, "MediaResources/Sprite")
    ...
    Shared Function DownloadAllText(items As List(Of DTOMediaResource), oLang As DTOLang) As String
        ...
        Dim sWeight As String = FEB2.MediaHelper.LengthFormatted(dWeight)
