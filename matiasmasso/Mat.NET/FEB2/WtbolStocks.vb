...
Public Class WtbolStocks
    ...
    Shared Async Function Upload(oSite As DTOWtbolSite, oSheet As ExcelHelper.Sheet, exs As List(Of Exception)) As Task(Of Boolean)
    ...
    Shared Function JStocks(oSite As DTOWtbolSite, oSheet As ExcelHelper.Sheet, exs As List(Of Exception)) As JObject
        Dim retval = New JObject
        If FEB2.WtbolSite.Load(oSite, exs) Then
            ...
            For iRow As Integer = 1 To oSheet.Rows.Count - 1
                Dim oRow As ExcelHelper.Row = oSheet.Rows(iRow)
    ...
    Shared Async Function Xml(exs As List(Of Exception), oEmp As DTOEmp, oSite As DTOWtbolSite) As Task(Of String)
        Dim oStocks = Await FEB2.WtbolStocks.Merged(exs, oSite)
