...
Public Class PriceListItemSupplier
    ...
    Shared Async Function GetPreusDeCost(exs As List(Of Exception), oSku As DTOProductSku) As Task(Of DTOPriceListItem_Supplier)
        Dim oBrand = FEB2.Product.Brand(exs, oSku)

        Dim oProveidor As DTOProveidor = oBrand.Proveidor
        Dim DtFch As Date = DateTime.Today
        Dim retval = Await FEB2.PriceListItemSupplier.FromSku(exs, oProveidor, oSku.RefProveidor, DtFch)
        Return retval
    End Function
End Class
...
Public Class PriceListItemsSupplier
    ...
    Shared Async Function Delete(exs As List(Of Exception), values As List(Of DTOPriceListItem_Supplier)) As Task(Of Boolean)
        Return Await Api.Delete(Of List(Of DTOPriceListItem_Supplier))(values, exs, "PriceListItemSupplier")
    End Function

    Shared Async Function FromExcelSheet(exs As List(Of Exception), oPriceList As DTOPriceListSupplier, oSheet As ExcelHelper.Sheet) As Task(Of List(Of DTOPriceListItem_Supplier))
        'REF / EAN / DESCRIPTION / PRICE / RETAIL / INNER PACK
        Dim retval As New List(Of DTOPriceListItem_Supplier)
        'Dim oProveidorSkus As List(Of DTOProductSku) = FEB2.ProductSkus.All(Proveidor:=oPriceList.Proveidor)
        Dim oProveidorSkus = Await FEB2.ProductSkus.All(exs, oPriceList.Proveidor)
