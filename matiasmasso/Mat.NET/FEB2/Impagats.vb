Public Class Impagat
    ...
    Shared Async Function CalculaGastos(oEmp As DTOEmp, oImpagat As DTOImpagat, exs As List(Of Exception)) As Task(Of DTOAmt)
        ...
        Dim oMinimGastos As DTOAmt = Await FEB2.Default.EmpAmt(oEmp, DTODefault.Codis.DespesesImpagoMinim, exs)
        If exs.Count = 0 Then
            Dim DcGastosPercentage As Decimal = Await FEB2.Default.EmpDecimal(oEmp, DTODefault.Codis.DespesesImpago, exs)
    ...
    Shared Async Function WarnReps(exs As List(Of Exception), oUser As DTOUser, oImpagat As DTOImpagat) As Task(Of Boolean)
        ...
        If FEB2.Contact.Load(oLliurat, exs) Then
            Dim oAdr As DTOAddress = oLliurat.Address
            Dim oChannel = DTOContact.DistributionChannel(oLliurat)
            Dim oEmails = Await FEB2.Reps.Emails(exs, oAdr.Zip, oChannel)
            ...
            With oMailMessage
                .Cc = recipients
            End With
            retval = Await FEB2.MailMessage.Send(exs, oUser, oMailMessage)
        End If
        Return retval
    End Function
    ...
    Shared Function CobraPerVisa(oEmp As DTOEmp, oLog As DTOTpvLog, exs As List(Of Exception)) As DTOTpvLog
        ...
        If FEB2.Impagat.Load(oImpagat, exs) Then
            ...
            If oDespeses.IsNotZero Then
                ...
                oCcbCredit.Cta = FEB2.PgcCta.FromCodSync(DTOPgcPlan.Ctas.impagats, oEmp, exs)

                Dim oCtaDespeses = FEB2.PgcCta.FromCodSync(DTOPgcPlan.Ctas.ImpagosRecuperacioDespeses, oEmp, exs)
                oLog.Result.AddSaldo(oCtaDespeses)
            End If

            oLog = FEB2.Impagat.CobraPerVisa(oEmp, oLog, exs)
        End If
    ...
    Shared Async Function Update(exs As List(Of Exception), oImpagats As List(Of DTOImpagat), oBanc As DTOBanc, DtFch As Date, oNominal As DTOAmt, oCca As DTOCca, oUser As DTOUser) As Task(Of DTOCca)
        ...
        If exs.Count = 0 Then
            ...
            For Each oImpagat As DTOImpagat In oImpagats
                ...
                Dim DcGtsPercent As Decimal = Await FEB2.Default.EmpDecimal(oUser.Emp, DTODefault.Codis.despesesImpago, exs)
                Dim oGtsMin As DTOAmt = Await FEB2.Default.EmpAmt(oUser.Emp, DTODefault.Codis.despesesImpagoMinim, exs)
