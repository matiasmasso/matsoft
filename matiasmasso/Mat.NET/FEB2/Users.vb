Public Class User
    ...
    Shared Async Function UpdateUser(oSrc As DTOUser.Sources,
        ...
        Dim oUser As DTOUser = Await FEB2.User.FromEmail(exs, New DTOEmp(DTOEmp.Ids.MatiasMasso), sEmail) ' TO DEPRECATE
        With oUser
            .Source = oSrc
            .Nom = sFirstname
            .Cognoms = sSurname
            .NickName = sNickName
            .Country = Await FEB2.Country.Find(oCountry, exs)
            .ZipCod = sZipcod
            .Tel = sTel
    ...
    Shared Async Function ValidatePassword(exs As List(Of Exception), oEmp As DTOEmp, sEmail As String, sPassword As String) As Task(Of DTOUser.ValidatePasswordModel)
        ...
        Else
            retval.user = Await FEB2.User.FromEmail(exs, oEmp, sEmail)
    ...
    Shared Function IsAllowedToRead(oLector As DTOUser, oPropietari As DTOContact) As Boolean
        ...
        Select Case oLector.Rol.Id
            ...
            Case DTORol.Ids.SalesManager
                If oPropietari.Rol Is Nothing Then FEB2.Contact.Load(oPropietari, exs)
    ...
    Public Shared Async Function ValidateEmail(exs As List(Of Exception), oEmp As DTOEmp, sEmail As String) As Task(Of DTOUser.ValidationResults)
        ...
        Else
            Dim oUser As DTOUser = Await FEB2.User.FromEmail(exs, oEmp, sEmail)
    ...
    Shared Async Function BadMailReasons(exs As List(Of Exception), oLang As DTOLang) As Task(Of List(Of DTOGuidNom))
        ...
        Dim oCods = Await FEB2.Cods.All(exs, oParent)
...
Public Class Users
    ...
    Shared Async Function ProfessionalsExcel(exs As List(Of Exception), oEmp As DTOEmp, oContacts As List(Of DTOAtlas.Contact)) As Task(Of ExcelHelper.Sheet)
        Return Await Api.Execute(Of List(Of DTOAtlas.Contact), ExcelHelper.Sheet)(oContacts, exs, "Users/professionals/Excel", oEmp.Id)
    End Function
