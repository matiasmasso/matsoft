Public Class PurchaseOrder
    ...
    Shared Function SetIncentius(exs As List(Of Exception), oUser As DTOUser, ByRef oPurchaseOrder As DTOPurchaseOrder) As Boolean
        ...
        Dim oCcx = FEB2.Customer.CcxOrMe(exs, oPurchaseOrder.Contact)
        If Not oCcx.NoIncentius Then
            Dim oIncentius = FEB2.Incentius.AllSync(exs, oUser, False).FindAll(Function(x) x.OnlyInStk = False).ToList
    ...
    Shared Async Function BlockPendingItems(exs As List(Of Exception), oPurchaseOrder As DTOPurchaseOrder) As Task(Of Boolean)
        ...
        If dirty Then Await FEB2.PurchaseOrder.Update(exs, oPurchaseOrder)
        Return exs.Count = 0
    End Function
    ...
    Shared Function Doc(oPurchaseOrder As DTOPurchaseOrder, BlProforma As Boolean) As DTODoc
        ...
        If FEB2.PurchaseOrder.Load(oPurchaseOrder, exs) Then
            If FEB2.Contact.Load(oPurchaseOrder.Contact, exs) Then
                ...
                With retval
                    ...
                    If oPurchaseOrder.cod = DTOPurchaseOrder.Codis.proveidor Then
                        ...
                        If oPurchaseOrder.platform IsNot Nothing Then
                            ...
                            Dim oAdresses As List(Of DTOAddress) = FEB2.Addresses.AllSync(oMgzEntregarEn, exs)
                    ...
                    For Each item In oPurchaseOrder.items
                        ...
                        If item.Bundle.Count > 0 Then
                            ...
                            For Each oChildItem In item.bundle
                                Dim sSkuNom = oChildItem.sku.nomLlarg.Esp
    ...
    Shared Async Function MailConfirmation(exs As List(Of Exception), oUser As DTOUser, oPurchaseOrder As DTOPurchaseOrder, Optional sRecipients As List(Of String) = Nothing) As Task(Of Boolean)
        ...
        Dim retval = Await FEB2.MailMessage.Send(exs, oUser, oMailMessage)
        Return retval
    End Function

    Shared Function Excel(oOrder As DTOPurchaseOrder) As ExcelHelper.Sheet
        Dim retval As New ExcelHelper.Sheet("M+O")
        ...
        With retval
            ...
            .addColumn(, ExcelHelper.Sheet.NumberFormats.PlainText)
            .addColumn(, ExcelHelper.Sheet.NumberFormats.PlainText)
            .addColumn(, ExcelHelper.Sheet.NumberFormats.PlainText)
            .addColumn(, ExcelHelper.Sheet.NumberFormats.Integer)
            If DTOPurchaseOrder.pendentsExist(oOrder) Then
                .addColumn(, ExcelHelper.Sheet.NumberFormats.Integer)
            End If
            .addColumn(, ExcelHelper.Sheet.NumberFormats.Euro)
            If DTOPurchaseOrder.discountsExist(oOrder) Then
                .addColumn(, ExcelHelper.Sheet.NumberFormats.Percent)
            End If
            .addColumn(, ExcelHelper.Sheet.NumberFormats.Euro)
            .addColumn(, ExcelHelper.Sheet.NumberFormats.m3)
            .addColumn(, ExcelHelper.Sheet.NumberFormats.m3)
            .addColumn(, ExcelHelper.Sheet.NumberFormats.Kg)
            .AddColumn(, ExcelHelper.Sheet.NumberFormats.Kg)
            .AddColumn(, ExcelHelper.Sheet.NumberFormats.PlainText)
        End With
        ...
        Dim oCell As ExcelHelper.Cell = oRow.AddCell(oOrder.caption(oLang))
    ...
    Shared Function Excel(items As List(Of DTOPurchaseOrderItem), Optional oLang As DTOLang = Nothing) As ExcelHelper.Sheet
        'pendents d'entrega
        ...
        For Each item As DTOPurchaseOrderItem In items
            ...
            If oCod = DTOPurchaseOrder.Codis.proveidor Then
                oRow.addCell(item.purchaseOrder.num, FEB2.PurchaseOrder.Url(item.purchaseOrder, True))
                oRow.addCell(item.sku.refProveidor, item.sku.GetUrl(oLang))
                oRow.addCell(item.sku.nomProveidor)
            Else
                oRow.addCell(item.purchaseOrder.concept, FEB2.PurchaseOrder.Url(item.purchaseOrder, True))
                oRow.addCell(item.sku.id, item.sku.GetUrl(oLang))
                oRow.addCell(item.sku.nom)
            End If
            oRow.addCell(item.pending)
        Next
        Return retval
...
Public Class PurchaseOrders
    ...
    Shared Async Function DeletePendingItems(oPurchaseOrders As List(Of DTOPurchaseOrder), ShowProgress As ProgressBarHandler, exs As List(Of Exception)) As Task(Of Boolean)
        ...
        For Each order As DTOPurchaseOrder In oPurchaseOrders
            If FEB2.PurchaseOrder.Load(order, exs) Then
                ...
                If dirty Then
                    ...
                    Dim pOrder = Await FEB2.PurchaseOrder.Update(ex2s, order)
                    If exs.Count > 0 Then
    ...
    Shared Function ExcelPdcs(oPurchaseOrders As List(Of DTOPurchaseOrder)) As ExcelHelper.Sheet
        Dim retval As New ExcelHelper.Sheet("promos")
        With retval
            .AddColumn("Comanda", ExcelHelper.Sheet.NumberFormats.Integer)
        ...
        For Each order In oPurchaseOrders
            Dim oRow As ExcelHelper.Row = retval.AddRow()
            With oRow
                .AddCell(order.Num, FEB2.PurchaseOrder.Url(order, True))
                .AddCell(order.Fch)
    ...
    Shared Function ExcelLinies(exs As List(Of Exception), oPurchaseOrders As List(Of DTOPurchaseOrder)) As ExcelHelper.Sheet
        Dim retval As New ExcelHelper.Sheet("promos")
        With retval
            .AddColumn("Comanda", ExcelHelper.Sheet.NumberFormats.Integer)
            .AddColumn("Data", ExcelHelper.Sheet.NumberFormats.DDMMYY)
            ...
            .AddColumn("Quantitat", ExcelHelper.Sheet.NumberFormats.Integer)
            .AddColumn("Preu", ExcelHelper.Sheet.NumberFormats.Euro)
            .AddColumn("Descompte", ExcelHelper.Sheet.NumberFormats.Percent)
            .AddColumn("Import Descompte", ExcelHelper.Sheet.NumberFormats.Euro)
            .AddColumn("Import Net", ExcelHelper.Sheet.NumberFormats.Euro)
        End With
        For Each order In oPurchaseOrders
            FEB2.PurchaseOrder.Load(order, exs)
