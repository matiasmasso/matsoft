...
Public Class DocFile
    ...
    Shared Async Function Find(hash As String, exs As List(Of Exception)) As Task(Of DTODocFile)
        Return Await Api.Fetch(Of DTODocFile)(exs, "docfile", CryptoHelper.UrlFriendlyBase64(hash))
    End Function
    ...
    Shared Function Load(exs As List(Of Exception), ByRef oDocFile As DTODocFile, Optional LoadStream As Boolean = False, Optional LoadThumbnail As Boolean = False) As Boolean
        Dim sBase64Hash = CryptoHelper.UrlFriendlyBase64(oDocFile.Hash)
        Dim pDocfile = Api.FetchSync(Of DTODocFile)(exs, "Docfile", sBase64Hash)
        If exs.Count = 0 Then
            DTOBaseGuid.CopyPropertyValues(Of DTODocFile)(pDocfile, oDocFile, exs)
            If LoadStream Then
                oDocFile.Stream = Api.FetchBinarySync(exs, "docfile/Stream", sBase64Hash)
            End If
            If LoadThumbnail Then
                oDocFile.Thumbnail = Api.FetchImageSync(exs, "Docfile/thumbnail", sBase64Hash)
            End If
        End If
    ...
    Shared Async Function FindWithStream(hash As String, exs As List(Of Exception)) As Task(Of DTODocFile)
        ...
        If exs.Count = 0 Then
            If retval IsNot Nothing Then
                retval.Stream = Await Api.FetchBinary(exs, "docfile/Stream", CryptoHelper.UrlFriendlyBase64(hash))
            End If
        End If
        Return retval
    End Function
    ...
    Shared Async Function StreamOrDownload(oDocfile As DTODocFile, exs As List(Of Exception)) As Task(Of Byte())
        ...
        If oDocfile.Stream Is Nothing Then
            retval = Await Api.FetchBinary(exs, "docfile/Stream", CryptoHelper.UrlFriendlyBase64(oDocfile.Hash))
        End If
        Return retval
    End Function
    ...
    Shared Function FindSync(hash As String) As DTODocFile
        ...
        Return Api.FetchSync(Of DTODocFile)(exs, "docfile", CryptoHelper.UrlFriendlyBase64(hash))
    End Function
    ...
    Shared Async Function Thumbnail(exs As List(Of Exception), hash As String, Optional width As Integer = 0) As Task(Of Byte())
        ...
        If width = 0 Then
            retval = Await Api.FetchImage(exs, "Docfile/thumbnail", CryptoHelper.UrlFriendlyBase64(hash))
        Else
            retval = Await Api.FetchImage(exs, "Docfile/thumbnail", CryptoHelper.UrlFriendlyBase64(hash), width)
        End If
        Return retval
    End Function
    ...
    Shared Async Function Log(oDocFile As DTODocFile, oUser As DTOUser, exs As List(Of Exception)) As Task(Of Boolean)
        ...
        Return Await Api.Fetch(Of Boolean)(exs, "docfile/log", oGuid.ToString, CryptoHelper.UrlFriendlyBase64(oDocFile.Hash))
    End Function
    ...
    Shared Function FileNameOrDefault(oDocFile As DTODocFile) As String
        ...
        If retval = "" Then
            retval = FEB2.DocFile.HexHash(oDocFile) & "." & oDocFile.Mime.ToString
        End If
        Return retval
    End Function
