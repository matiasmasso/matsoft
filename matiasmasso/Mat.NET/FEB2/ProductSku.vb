...
Public Class ProductSku
    ...
    Shared Async Function Cost(exs As List(Of Exception), oSku As DTOProductSku) As Task(Of DTOPriceListItem_Supplier)
        Dim retval = Await FEB2.PriceListItemSupplier.GetPreusDeCost(exs, oSku)
        Return retval
    End Function
    ...
    Shared Async Function CostNet(exs As List(Of Exception), oSku As DTOProductSku) As Task(Of DTOAmt)
        ...
        Dim item = Await FEB2.PriceListItemSupplier.GetPreusDeCost(exs, oSku)
    ...
    Shared Function Url(oSku As DTOProductSku, Optional oTab As DTOProduct.Tabs = DTOProduct.Tabs.general, Optional AbsoluteUrl As Boolean = False) As String
        ...
        If oCategory Is Nothing Then
            retval = FEB2.UrlHelper.Factory(AbsoluteUrl, "sku", oSku.Guid.ToString())
        Else
        ...
        If retval = "" Then
            retval = FEB2.UrlHelper.Factory(AbsoluteUrl, "product", oSku.Guid.ToString())
        End If
    ...
    Shared Async Function Price(exs As List(Of Exception), oSku As DTOProductSku, oCustomer As DTOCustomer, Optional DtFch As Date = Nothing) As Task(Of DTOAmt)
        Return Await FEB2.Customer.SkuPrice(exs, oCustomer, oSku, DtFch)
    End Function
    ...
    Shared Async Function IsAllowedOrderQty(exs As List(Of Exception), oSku As DTOProductSku, ByVal iQty As Integer, Optional oUser As DTOUser = Nothing) As Task(Of Boolean)
        ...
        If iQty Mod iMoq <> 0 Then
            If oUser Is Nothing Then
                RetVal = False
            Else
                RetVal = Await FEB2.Product.AllowUserToFraccionarInnerPack(exs, oSku, oUser)
            End If
        End If
        Return RetVal
    End Function
...
Public Class ProductSkus
    ...
    Shared Async Function Obsolets(exs As List(Of Exception), oUser As DTOUser, oLang As DTOLang, fchFrom As DateTime) As Task(Of ExcelHelper.Sheet)
        Dim retval = Await Api.Execute(Of String, ExcelHelper.Sheet)(fchFrom.ToString("o"), exs, "ProductSkus/Obsolets", oUser.Guid.ToString, oLang.Tag)
        Return retval
    End Function
