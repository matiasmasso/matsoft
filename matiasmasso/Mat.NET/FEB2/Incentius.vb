Public Class Incentiu
    ...
    Shared Async Function ExcelFiresLocals(exs As List(Of Exception), oProveidor As DTOProveidor, oUser As DTOUser) As Task(Of ExcelHelper.Sheet)
        Dim oIncentiu = DTOIncentiu.Wellknown(DTOIncentiu.Wellknowns.FeriasLocales)
        Dim oOrders = Await FEB2.Incentiu.PurchaseOrders(exs, oIncentiu, oUser)
        ...
        Dim retval As New ExcelHelper.Sheet(sTitle)
        With retval
        ...
        For Each oOrder In oOrders
            ...
            oRow.AddCell(oOrder.Num, FEB2.PurchaseOrder.Url(oOrder, True))
    ...
    Shared Async Function ExcelResults(exs As List(Of Exception), oIncentiu As DTOIncentiu) As Task(Of ExcelHelper.Sheet)
        Return Await Api.Fetch(Of ExcelHelper.Sheet)(exs, "Incentiu/ExcelResults", oIncentiu.Guid.ToString())
    End Function

    Shared Async Function ExcelDetall(exs As List(Of Exception), oIncentiu As DTOIncentiu, oUser As DTOUser) As Task(Of ExcelHelper.Sheet)
        Dim sTitle As String = oIncentiu.Title.Esp
        Dim retval As New ExcelHelper.Sheet("lineas de pedido", sTitle)
        With retval
            .AddColumn("Fecha", ExcelHelper.Sheet.NumberFormats.DDMMYY)
            .AddColumn("Pedido", ExcelHelper.Sheet.NumberFormats.Integer)
            .AddColumn("Cliente", ExcelHelper.Sheet.NumberFormats.PlainText)
            .AddColumn("Producto", ExcelHelper.Sheet.NumberFormats.PlainText)
            .AddColumn("Cantidad", ExcelHelper.Sheet.NumberFormats.Integer)
            .AddColumn("Precio", ExcelHelper.Sheet.NumberFormats.Euro)
            .AddColumn("Descuento", ExcelHelper.Sheet.NumberFormats.Percent)
            .AddColumn("Importe", ExcelHelper.Sheet.NumberFormats.Euro)
        End With
    ...
    Shared Async Function ExcelDeliveryAddresses(exs As List(Of Exception), oIncentiu As DTOIncentiu, oUser As DTOUser) As Task(Of ExcelHelper.Sheet)
        Return Await Api.Fetch(Of ExcelHelper.Sheet)(exs, "Incentiu/DeliveryAddresses", oIncentiu.Guid.ToString())
    End Function
End Class
