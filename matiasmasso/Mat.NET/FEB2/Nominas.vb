    ...
    Shared Function Cca(ByRef oNomina As DTONomina, DtFch As Date, oCtas As List(Of DTOPgcCta), oUser As DTOUser, exs As List(Of Exception)) As DTOCca
        ...
        With retval
            ...
            If oNomina.SegSocial.IsNotZero Then
                .AddCredit(oNomina.SegSocial, Cta(oCtas, DTOPgcPlan.Ctas.SegSocialDevengo, exs))
            End If
...
Public Class Nominas
    ...
    Shared Async Function Llibre(oExercici As DTOExercici, exs As List(Of Exception)) As Task(Of ExcelHelper.Sheet)
        Dim retval As New ExcelHelper.Sheet(oExercici.Year, "M+O Nomines")
        With retval
            .DisplayTotals = True
            .AddColumn("assentament", ExcelHelper.Sheet.NumberFormats.W50)
            .AddColumn("data", ExcelHelper.Sheet.NumberFormats.DDMMYY)
            .AddColumn("treballador", ExcelHelper.Sheet.NumberFormats.PlainText)
            .AddColumn("devengat", ExcelHelper.Sheet.NumberFormats.Euro)
            .AddColumn("dietes", ExcelHelper.Sheet.NumberFormats.Euro)
            .AddColumn("seg.social", ExcelHelper.Sheet.NumberFormats.Euro)
            .AddColumn("Irpf", ExcelHelper.Sheet.NumberFormats.Euro)
            .AddColumn("altres", ExcelHelper.Sheet.NumberFormats.Euro)
            .AddColumn("liquid", ExcelHelper.Sheet.NumberFormats.Euro)
        End With

        Dim oNominas = Await FEB2.Nominas.All(exs, oExercici)
        If exs.Count = 0 Then
            oNominas = oNominas.
