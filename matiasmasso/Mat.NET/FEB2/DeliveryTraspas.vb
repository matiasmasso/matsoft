Public Class DeliveryTraspas
    ...
    Shared Function Excel(oDeliveryTraspas As DTODeliveryTraspas, exs As List(Of Exception)) As ExcelHelper.Sheet
        Dim retval As New ExcelHelper.Sheet("Traspas de magatzem " & oDeliveryTraspas.Id)
        If Load(oDeliveryTraspas, exs) Then
            For Each item In oDeliveryTraspas.Items
                Dim oRow = retval.AddRow()
                oRow.AddCell(item.Sku.Id)
                oRow.AddCell(item.Sku.Category.Brand.Nom)
                oRow.AddCell(item.Sku.Category.Nom)
