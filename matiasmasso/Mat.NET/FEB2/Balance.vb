Public Class Balance
    ...
    Shared Async Function Tree(oEmp As DTOEmp, Optional DtFchTo As Date = Nothing) As Task(Of List(Of DTOPgcClass))
        ...
        If DtFchTo = Nothing Then DtFchTo = DateTime.Today
        Dim oPlan = FEB2.PgcPlan.FromYearSync(DtFchTo.Year, exs)
        Dim AllClasses = Await FEB2.PgcClasses.All(exs, oPlan)
        ...
        Dim oSumasYSaldos As List(Of DTOBalanceSaldo) = FEB2.Balance.SumasySaldosSync(exs, oExercici, DtFchTo)

        For Each oClass As DTOPgcClass In retval
            FEB2.Balance.Merge(oClass, oSumasYSaldos)
        Next
        ...
        If oPassiu IsNot Nothing AndAlso oResultados IsNot Nothing Then
            FEB2.Balance.InsertResultadosIntoPassiu(oPassiu, oResultados)
        End If
    ...
    Shared Async Function Ratios(oEmp As DTOEmp, Optional DtFchTo As Date = Nothing) As Task(Of List(Of DTORatio))
        ...
        retval.Add(FEB2.Balance.FondoDeManiobra(oTree))
        retval.Add(FEB2.Balance.RatioDeLiquidez(oTree))
        retval.Add(FEB2.Balance.RatioDeTesoreria(oTree))
        retval.Add(FEB2.Balance.RatioDeDisponibilidad(oTree))
        retval.Add(FEB2.Balance.RatioDeEndeudamiento(oTree))
        retval.Add(FEB2.Balance.RatioCalidadDeuda(oTree))
        Return retval
    End Function


    Shared Async Function Excel(oEmp As DTOEmp, Optional DtFchTo As Date = Nothing) As Task(Of ExcelHelper.Book)
        Dim oLang As DTOLang = DTOApp.Current.Lang
        Dim oTree As List(Of DTOPgcClass) = Await Tree(oEmp, DtFchTo)
        Dim retval As New ExcelHelper.Book(oEmp.Org.PrimaryNifValue() & "." & DtFchTo.Year & ".Balanç i Compte resultats.xlsx")

        Dim oResultados As DTOPgcClass = oTree.Find(Function(x) x.Cod = DTOPgcClass.Cods.b_Cuenta_Explotacion)
        Dim oSheetResultados As New ExcelHelper.Sheet
        retval.Sheets.Add(oSheetResultados)
        oSheetResultados.Name = oResultados.Nom.Tradueix(oLang)
        FEB2.Balance.FormatBalanceSheet(oEmp, oSheetResultados, DtFchTo)

        Dim oPassiu As DTOPgcClass = oTree.Find(Function(x) x.Cod = DTOPgcClass.Cods.aB_Pasivo)
        Dim oSheetPassiu As New ExcelHelper.Sheet
        retval.Sheets.Add(oSheetPassiu)
        FEB2.Balance.FormatBalanceSheet(oEmp, oSheetPassiu, DtFchTo)
        ...
        Dim oSheetActiu As New ExcelHelper.Sheet
        retval.Sheets.Add(oSheetActiu)
        FEB2.Balance.FormatBalanceSheet(oEmp, oSheetActiu, DtFchTo)
        oSheetActiu.Name = oActiu.Nom.Tradueix(oLang)

        FEB2.Balance.AddClass(oSheetActiu, oActiu.Cod, oActiu, oLang)
        FEB2.Balance.AddClass(oSheetPassiu, oPassiu.Cod, oPassiu, oLang)
        FEB2.Balance.AddClass(oSheetResultados, oResultados.Cod, oResultados, oLang)
    ...
    Shared Sub InsertResultadosIntoPassiu(oPassiu As DTOPgcClass, oResultados As DTOPgcClass)
        ...
        If oResultado IsNot Nothing Then
            ...
            End With
            FEB2.Balance.Propagate(oResultado, oResultado.CurrentDeb, oResultado.CurrentHab, oResultado.PreviousDeb, oResultado.PreviousHab)
        End If
    End Sub
...
#Region "Excel"
    Shared Sub FormatBalanceSheet(oEmp As DTOEmp, ByRef oSheet As ExcelHelper.Sheet, DtFchTo As Date)
        If DtFchTo = Nothing Then DtFchTo = DateTime.Today
        Dim oExercici As DTOExercici = DTOExercici.FromYear(oEmp, DtFchTo.Year)
        ...
        With oSheet
            ...
            .AddColumn(sHeaderCurrent, ExcelHelper.Sheet.NumberFormats.Euro)
            .AddColumn(sHeaderPrevious, ExcelHelper.Sheet.NumberFormats.Euro)
        End With
    End Sub
    Shared Sub AddClass(ByRef oSheet As ExcelHelper.Sheet, oSheetCod As DTOPgcClass.Cods, oClass As DTOPgcClass, oLang As DTOLang)
