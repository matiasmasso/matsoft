...
#Region "CRUD"
    ...
    Shared Async Function Excel(oTransmisio As DTOTransmisio, exs As List(Of Exception)) As Task(Of ExcelHelper.Book)
        Return Await Api.Fetch(Of ExcelHelper.Book)(exs, "transmisio/Excel", oTransmisio.Guid.ToString())
    End Function
...
#End Region
    ...
    Shared Async Function Send(exs As List(Of Exception), oEmp As DTOEmp) As Task(Of Boolean)
        ...
        Dim oDeliveries As List(Of DTODelivery) = Await FEB2.Deliveries.PendentsDeTransmetre(oEmp.Mgz, exs)
        If exs.Count = 0 AndAlso oDeliveries.Count > 0 Then
            Dim oTransmisio = DTOTransmisio.Factory(oEmp, oEmp.Mgz, oDeliveries)
            retval = Await FEB2.Transmisio.Update(oTransmisio, exs)
            If exs.Count = 0 Then
                retval = Await FEB2.Transmisio.Send(exs, oTransmisio)
            End If
        End If
        Return retval
    End Function
    ...
    Shared Async Function NextTime(exs As List(Of Exception)) As Task(Of DateTimeOffset)
        ...
        Dim oTask = Await FEB2.Task.Find(pTask.Guid, exs)
        If exs.Count = 0 Then
    ...
    Shared Async Function XmlAttachment(oTransmisio As DTOTransmisio, exs As List(Of Exception)) As Task(Of DTOMailMessage.Attachment)
        ...
        Dim oByteArray = Await FEB2.Transmisio.XmlFile(oTransmisio, exs)
        If exs.Count = 0 Then
            ...
            Dim sFilename = String.Format("{0}\{1}", sTempPath, FEB2.Transmisio.FileNameDades(oTransmisio))
    ...
    Shared Async Function SendTo(exs As List(Of Exception)) As Task(Of String)
        ...
        Dim retval As String = Await FEB2.Emp.GetEmpValue(oEmp, DTODefault.Codis.EmailTransmisioVivace, exs)
        Return retval
    End Function
    ...
    Shared Async Function XmlFile(oTransmisio As DTOTransmisio, exs As List(Of Exception)) As Task(Of Byte())
        Return Await FEB2.Api.FetchBinary(exs, "transmisio/XmlFile", oTransmisio.Guid.ToString())
    End Function

    Shared Async Function PdfDeliveries(oTransmisio As DTOTransmisio, exs As List(Of Exception)) As Task(Of Byte())
        Return Await FEB2.Api.FetchBinary(exs, "transmisio/PdfDeliveries", oTransmisio.Guid.ToString())
    End Function

    Shared Function UrlPdfDeliveries(oTransmisio As DTOTransmisio) As String
        Return FEB2.Api.ApiUrl("transmisio/PdfDeliveries", oTransmisio.Guid.ToString())
    End Function
    ...
    Shared Function FileNameDocs(oTransmisio As DTOTransmisio) As String
        Return String.Format("{0}.documentacio.pdf", FilePrefix(oTransmisio))
    End Function
