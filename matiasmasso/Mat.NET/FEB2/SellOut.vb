Public Class SellOut
    ...
    Shared Async Function Factory(exs As List(Of Exception),
        ...
        If oYearMonthTo Is Nothing Then oYearMonthTo = New DTOYearMonth(DateTime.Today.Year, 12)
    ...
    Shared Async Function Factory(oUser As DTOUser, year As Integer, conceptType As DTOSellOut.ConceptTypes, format As DTOSellOut.ConceptTypes, brand As String, category As String, channel As String, country As String, zona As String, location As String, contact As String, groupbyholding As Boolean) As Task(Of DTOSellOut)
        ...
        Dim retval = Await FEB2.SellOut.Factory(exs, oUser, oYearMonthTo, conceptType, format)

        With retval

            If GuidHelper.IsGuid(category) Then
                Dim oProduct = Await FEB2.ProductCategory.Find(exs, New Guid(category))
            ...
            ElseIf GuidHelper.IsGuid(brand) Then
                Dim oProduct = Await FEB2.ProductBrand.Find(exs, New Guid(brand))
                Dim value = New DTOGuidNom(oProduct.Guid, oProduct.Nom.Esp)
                Dim oFilter = retval.GetFilter(DTOSellOut.Filter.Cods.Product)
                oFilter.Values.Add(value)
            End If
            ...
            If GuidHelper.IsGuid(channel) Then
                Dim oChannel = Await FEB2.DistributionChannel.Find(New Guid(channel), exs)
            ...
            If GuidHelper.IsGuid(contact) Then
                Dim value = Await FEB2.Customer.Find(exs, New Guid(contact))
            ...
            ElseIf GuidHelper.IsGuid(location) Then
                Dim value = Await FEB2.Location.Find(New Guid(location), exs)
                Dim oFilter = retval.GetFilter(DTOSellOut.Filter.Cods.Atlas)
                oFilter.Values.Add(value)
            ElseIf GuidHelper.IsGuid(zona) Then
                Dim value = Await FEB2.Zona.Find(New Guid(zona), exs)
                Dim oFilter = retval.GetFilter(DTOSellOut.Filter.Cods.Atlas)
                oFilter.Values.Add(value)
            ElseIf GuidHelper.IsGuid(country) Then
                Dim oCountry = Await FEB2.Country.Find(New Guid(country), exs)
                Dim oFilter = retval.GetFilter(DTOSellOut.Filter.Cods.Atlas)
    ...
    Shared Function RawDataLast12MonthsCsvUrl(Optional AbsoluteUrl As Boolean = False) As String
        Dim retval As String = FEB2.UrlHelper.Dox(AbsoluteUrl, DTODocFile.Cods.rawdatalast12monthscsv)
        Return retval
    End Function

    Shared Sub SetCcx(ByRef oSellOut As DTOSellOut, oCcx As DTOCustomer)
        FEB2.SellOut.ClearFilter(oSellOut, DTOSellOut.Filter.Cods.Customer)
        FEB2.SellOut.AddFilter(oSellOut, DTOSellOut.Filter.Cods.Customer, {oCcx})
    ...
    Shared Function ExcelUrl(oSellout As DTOSellOut, Optional AbsoluteUrl As Boolean = False) As String
        ...
        Dim retval As String = FEB2.UrlHelper.Factory(DTODocFile.Cods.selloutexcel, oParameters, AbsoluteUrl)
        Return retval
    End Function

    Shared Async Function ExcelRawData(exs As List(Of Exception), year As Integer, oUser As DTOUser) As Task(Of ExcelHelper.Sheet)
        Dim retval = Await Api.Fetch(Of ExcelHelper.Sheet)(exs, "Sellout/Excel", year, oUser.Guid.ToString())
        Return retval
    End Function

    Shared Async Function RawData(exs As List(Of Exception), oUser As DTOUser, year As Integer) As Task(Of ExcelHelper.Sheet)
        Dim retval = Await Api.Fetch(Of ExcelHelper.Sheet)(exs, "sellout/data", oUser.Guid.ToString(), year)
        Return retval
    End Function
    ...
    Shared Async Function FromParameters(exs As List(Of Exception), oParameters As Dictionary(Of String, String)) As Task(Of DTOSellOut)
        Dim oYearMonthTo As New DTOYearMonth(DateTime.Today.Year, 12)
        ...
        If oParameters.ContainsKey("User") Then
            oUser = Await FEB2.User.Find(New Guid(oParameters("User")), exs)
        End If

        Dim retval As DTOSellOut = Await FEB2.SellOut.Factory(exs, oUser, oYearMonthTo, oConceptType, oFormat)
        retval.GroupByHolding = BlGroupByHolding

        If oParameters.ContainsKey("Area") Then
            Dim value As DTOGuidNom = FEB2.Area.FindSync(New Guid(oParameters("Area")), exs)
            FEB2.SellOut.AddFilterValues(retval, DTOSellOut.Filter.Cods.Atlas, {value})
        End If

        Dim oProducts As New List(Of DTOProduct)
        If oParameters.ContainsKey("Product") Then
            Dim oProduct = Await FEB2.Product.Find(exs, New Guid(oParameters("Product")))
            Dim value As New DTOGuidNom(oProduct.Guid, oProduct.Nom.Esp)
            FEB2.SellOut.AddFilterValues(retval, DTOSellOut.Filter.Cods.Product, {value})
        End If
        ...
        If oParameters.ContainsKey("Proveidor") Then
            ...
            If Not FEB2.SellOut.FilterValues(retval, DTOSellOut.Filter.Cods.Provider).Any(Function(x) x.Guid.Equals(oProviderGuid)) Then
                Dim value As DTOGuidNom = Await FEB2.Proveidor.Find(oProviderGuid, exs)
                FEB2.SellOut.AddFilterValues(retval, DTOSellOut.Filter.Cods.Provider, {value})
            End If
        End If

        Dim oCustomers As New List(Of DTOCustomer)
        If oParameters.ContainsKey("Customer") Then
            Dim value As DTOGuidNom = Await FEB2.Customer.Find(exs, New Guid(oParameters("Customer")))
            FEB2.SellOut.AddFilterValues(retval, DTOSellOut.Filter.Cods.Customer, {value})
        End If

        Dim oReps As New List(Of DTORep)
        If oParameters.ContainsKey("Rep") Then
            Dim value As DTOGuidNom = Await FEB2.Rep.Find(New Guid(oParameters("Rep")), exs)
            FEB2.SellOut.AddFilterValues(retval, DTOSellOut.Filter.Cods.Rep, {value})
        End If

        Dim oChannels As New List(Of DTODistributionChannel)
        If oParameters.ContainsKey("Channel") Then
            Dim oChannel = Await FEB2.DistributionChannel.Find(New Guid(oParameters("Channel")), exs)
            Dim value As New DTOGuidNom(oChannel.Guid, oChannel.LangText.Tradueix(DTOLang.ESP))
            FEB2.SellOut.AddFilterValues(retval, DTOSellOut.Filter.Cods.Channel, {value})
        End If
