Public Class RaffleParticipant
    ...
    Shared Async Function Enroll(oRaffleParticipant As DTORaffleParticipant, oUser As DTOUser, oEmp As DTOEmp, exs As List(Of Exception)) As Task(Of Boolean)
        Dim retval As Boolean = Await FEB2.RaffleParticipant.Update(exs, oRaffleParticipant)
        If retval Then
            Dim oMailMessage = Await FEB2.RaffleParticipant.RaffleParticipationMailMessage(exs, oEmp, oRaffleParticipant)
    ...
    Shared Async Function RaffleParticipationMailMessage(exs As List(Of Exception), oEmp As DTOEmp, oParticipant As DTORaffleParticipant) As Task(Of DTOMailMessage)
        ...
        If FEB2.Raffle.Load(oParticipant.Raffle, exs) Then
            Dim oLang As DTOLang = oParticipant.User.Lang
            Dim oBrand = FEB2.Product.Brand(exs, oParticipant.Raffle.Product)
            ...
            With retval
                .Bcc = DTOSubscriptor.Recipients(Await FEB2.Subscription.Subscriptors(oSsc, exs))
                ...
                .BodyUrl = FEB2.Mailing.BodyUrl(DTODefault.MailingTemplates.RaffleParticipation, oParticipant.Guid.ToString())
            End With
        End If
        Return retval
    End Function
    ...
    Shared Async Function RaffleWinnerCongrats(exs As List(Of Exception), oEmp As DTOEmp, oWinner As DTORaffleParticipant) As Task(Of DTOMailMessage)
        ...
        If FEB2.Raffle.Load(oWinner.Raffle, exs) Then
            ...
            Dim oBrand = FEB2.Product.Brand(exs, oWinner.Raffle.Product)
            Dim sBrandNom = DTOProduct.GetNom(oBrand)
            Dim oSsc = DTOSubscription.Wellknown(DTOSubscription.Wellknowns.CopiaSorteigConfirmacioParticipacio)
            ...
            With retval
                ...
                .BodyUrl = FEB2.Mailing.BodyUrl(DTODefault.MailingTemplates.RaffleWinnerCongrats, oWinner.Guid.ToString())
            End With
        End If
        Return retval
    End Function
    ...
    Shared Async Function Validate(oRaffle As DTORaffle, oUser As DTOUser, exs As List(Of DTORaffleParticipant.Exception)) As Task(Of Boolean)
        ...
        Dim oParticipant = Await FEB2.RaffleParticipant.search(exs2, oRaffle, oUser)
    ...
    Shared Function DistributorNameAndAddressHtml(oParticipant As DTORaffleParticipant) As String
        ...
        If FEB2.Contact.Load(oDistributor, exs) Then
    ...
    Shared Async Function SetAsWinner(oWinner As DTORaffleParticipant, exs As List(Of Exception)) As Task(Of Boolean)
        ...
        If oWinner IsNot Nothing Then
            ...
            If FEB2.Raffle.Load(oRaffle, exs) Then
                ...
                If Await FEB2.Raffle.Update(exs, oRaffle) Then
                    retval = True
                End If
...
Public Class RaffleParticipants
    ...
    Shared Async Function ExcelParticipantsList(exs As List(Of Exception), oRaffle As DTORaffle) As Task(Of ExcelHelper.Sheet)
        Dim items = Await FEB2.RaffleParticipants.Compact(oRaffle, exs)
