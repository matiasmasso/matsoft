Public Class Repliq
    ...
    Public Shared Async Function RegeneraPdf(exs As List(Of Exception), ByVal oRepLiq As DTORepLiq, oEmp As DTOEmp) As Task(Of Boolean)
        If Not oRepLiq.IsLoaded Then oRepLiq = Await FEB2.Repliq.Find(oRepLiq.Guid, exs)
        If exs.Count = 0 Then
            Dim oPdfRepLiq As New LegacyHelper.PdfRepLiq(oRepLiq)
            Dim oCert = Await FEB2.Cert.Find(oEmp.Org, exs)
            ...
            If exs.Count = 0 Then
                ...
                If exs.Count = 0 Then
                    ...
                    If Not oCca.IsLoaded Then oCca = Await FEB2.Cca.Find(oCca.Guid, exs)
                    If exs.Count = 0 Then
    ...
    Shared Function DownloadUrl(oRepLiq As DTORepLiq, Optional AbsoluteUrl As Boolean = False) As String
        ...
        If oRepLiq IsNot Nothing Then
            retval = FEB2.Cca.DownloadUrl(oRepLiq.Cca, AbsoluteUrl)
        End If
        Return retval
    End Function
    ...
    Shared Async Function GetCca(exs As List(Of Exception), oRepLiq As DTORepLiq, oUser As DTOUser) As Task(Of DTOCca)
        ...
        With retval
            ...
            Dim oRaoSocial As DTOProveidor = FEB2.Rep.RaoSocialFacturacio(oRepLiq.Rep)
...
Public Class Repliqs
        ...
        For Each oRep In oReps
            If FEB2.Rep.Load(exs, oRep) Then
                ...
                With oRepLiq
                    ...
                    .Cca = Await FEB2.Repliq.GetCca(exs, oRepLiq, oUser)
                End With
                retval.Add(oRepLiq)
            End If
        Next
    ...
    Shared Async Function Update(exs As List(Of Exception), oRepliqs As List(Of DTORepLiq), oUser As DTOUser, ShowProgress As ProgressBarHandler) As Task(Of Boolean)
        ...
        If Not cancelRequest Then
            ...
            For Each oRepLiq In oRepliqs
                ...
                Dim oRepLiqResult = Await FEB2.Repliq.Update(exs2, oRepLiq)
                ...
                If exs2.Count = 0 Then
                    ...
                    If Not Await FEB2.Repliq.RegeneraPdf(exs2, oRepLiqResult, oUser.Emp) Then
    ...
    Shared Async Function GetRepLiqsFromQuarter(exs As List(Of Exception), oRep As DTORep, ByVal iYea As Integer, ByVal iQuarter As Integer) As Task(Of List(Of DTORepLiq))
        ...
        Dim retval = Await FEB2.Repliqs.Headers(exs, oRep)
    ...
    Shared Function Excel(oRep As DTORep, oRepLiqs As List(Of DTORepLiq)) As ExcelHelper.Sheet
        Dim sCaption = String.Format("M+O {0} {1}", oRep.Lang.Tradueix("Liquidaciones", "Liquidacions", "Commission Statements"), oRep.NickName)
        Dim retval As New ExcelHelper.Sheet(sCaption)
        With retval
            .AddColumn(oRep.Lang.Tradueix("Liquidación", "Liquidació", "Statement Id"))
            .AddColumn(oRep.Lang.Tradueix("Fecha", "Data", "Date"), ExcelHelper.Sheet.NumberFormats.DDMMYY)
            .AddColumn(oRep.Lang.Tradueix("Base", "Base", "Base"), ExcelHelper.Sheet.NumberFormats.Euro)
            .AddColumn(oRep.Lang.Tradueix("Devengado", "Devengat", "Commission"), ExcelHelper.Sheet.NumberFormats.Euro)
            .AddColumn(oRep.Lang.Tradueix("IVA", "IVA", "VAT"), ExcelHelper.Sheet.NumberFormats.Euro)
            .AddColumn(oRep.Lang.Tradueix("IRPF", "IRPF", "IRPF"), ExcelHelper.Sheet.NumberFormats.Euro)
            .AddColumn(oRep.Lang.Tradueix("Liquido", "Liquid", "Cash"), ExcelHelper.Sheet.NumberFormats.Euro)
        End With
        For Each item In oRepLiqs
            Dim oRow = retval.AddRow
            oRow.AddCell(item.FormattedId, FEB2.Repliq.DownloadUrl(item, True))
