...
Public Class EdiversaFileSystem 'FEBL, temporal
    ...
    Shared Function Execute(oTask As DTOTask, exs As List(Of Exception)) As Boolean
        Try
            ...
            If FEB2.Task.LogSync(oTask, exs) Then
                Select Case oTask.Cod
                    Case DTOTask.Cods.EdiReadFromInbox
                        FEB2.EdiversaFileSystem.ReadInboxSync(exs)
                    Case DTOTask.Cods.EdiWriteToOutbox
                        FEB2.EdiversaFileSystem.WriteFilesToOutboxSync(exs)
                End Select

                oTask.SetResult(exs)
                FEB2.TaskLog.UpdateSync(oTask, exs)
            Else
                exs.Add(New Exception("FEB2.EdiversaFileSystem.ExecuteIfDue: Error al registrar el començament de tasca"))
            End If

        Catch ex As Exception
            exs.Add(New Exception("FEB2.EdiversaFileSystem.ExecuteIfDue: Error al executar la tasca " & oTask.Cod.ToString()))
            exs.Add(ex)
        End Try
    ...
    Shared Function WriteFilesToOutboxSync(exs As List(Of Exception)) As Boolean
        Try
            Dim oEdiversaFiles As List(Of DTOEdiversaFile) = FEB2.EdiversaFileSystem.PendingToWriteToOutboxSync(exs)
                ...
                For Each oEdiversaFile As DTOEdiversaFile In oEdiversaFiles
                    ...
                    Select Case oEdiversaFile.tag
                        Case "ORDERS_D_96A_UN_EAN008"
                            ...
                            sb.Append("." & TextHelper.VbFormat(DateTime.Now, "yyyy.MM.dd_HH.mm.ss"))
                            sb.Append(".txt")
                            ...
                            If SaveFile(oEdiversaFile, sFullPath, exs) Then
                                ...
                                If Not FEB2.EdiversaFileSystem.UpdateSync(oEdiversaFile, exs) Then
                        ...
                        Case "INVOIC_D_93A_UN_EAN007", "INVOIC_D_01B_UN_EAN010"
                            ...
                            oEdiversaFile.FileName = sb.ToString
                            Dim sFullPath As String = FolderPath(FolderPaths.outbox) & oEdiversaFile.FileName
                            If FEB2.EdiversaFileSystem.SaveFile(oEdiversaFile, sFullPath, exs) Then
                                oEdiversaFile.Result = DTOEdiversaFile.Results.Processed
                                If Not FEB2.EdiversaFileSystem.UpdateSync(oEdiversaFile, exs) Then
                                    exs.Add(New Exception("fitxer " & oEdiversaFile.FileName & " desat pero base de dades no actualitzada"))
                                End If
            ...
            End If
        Catch ex As Exception
            exs.Add(New Exception("FEB2.EdiversaFileSystem.WriteFilesToOutbox: Error " & ex.Message))
            exs.Add(ex)
        End Try
        Return exs.Count = 0
    End Function
