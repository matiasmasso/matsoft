...
#Region "CONSTRUCTORS"
    ...
    Sub New(ByVal Hostname As String, ByVal Username As String, ByVal Password As String, Optional ByVal BlEnableSsl As Boolean = False)
        _hostname = Hostname
        _username = Username
        _password = Password
        _enableSSL = BlEnableSsl
    End Sub

#End Region

    Public Function Directoris(exs As List(Of Exception), sUrl As String) As List(Of String)
        Dim retval As New List(Of String)
        Try
            Dim oParent As FTPdirectory = ListDirectoryDetail(sUrl)
            Dim oItems As List(Of FTPfileInfo) = oParent.FindAll(Function(x) x.FileType = FTPfileInfo.DirectoryEntryTypes.Directory)
            For Each oItem As FTPfileInfo In oItems
                retval.Add(oItem.Filename)
            Next
    ...
    Public Function Fitxers(sUrl As String) As List(Of String)
        Dim retval As New List(Of String)
        Dim oParent As FTPdirectory = ListDirectoryDetail(sUrl)
        Dim oFiles As List(Of FTPfileInfo) = oParent.FindAll(Function(x) x.FileType = FTPfileInfo.DirectoryEntryTypes.File)
        For Each oFile As FTPfileInfo In oFiles
            retval.Add(oFile.Filename)
        Next
        Return retval
    End Function

    Public Function Imatges(sUrl As String) As List(Of String)
        Dim retval As New List(Of String)
        Dim oParent As FTPdirectory = ListDirectoryDetail(sUrl)
        Dim oFiles As List(Of FTPfileInfo) = oParent.FindAll(Function(x) x.FileType = FTPfileInfo.DirectoryEntryTypes.File)
        For Each oFile As FTPfileInfo In oFiles
            If ImageHelper.IsImage(oFile.Filename) Then
                retval.Add(oFile.Filename)
            End If
        Next
        Return retval
...
#Region "Upload: File transfer TO ftp server"
    ...
    Public Function Upload(exs As List(Of Exception), ByVal localFilename As String, Optional ByVal targetFilename As String = "") As Boolean

        '1. check source
        If Not File.Exists(localFilename) Then
            exs.Add(New Exception("File " & localFilename & " not found"))
        End If

        'copy to FI
        Dim fi As New FileInfo(localFilename)
        Return Upload(exs, fi, targetFilename)
    End Function
    ...
    Public Function Upload(exs As List(Of Exception), ByVal fi As FileInfo, Optional ByVal targetFilename As String = "") As Boolean
    ...
    Public Function Upload(ByVal oArray As Byte(), Optional ByVal targetFilename As String = "", Optional label As String = "", Optional ShowProgress As ProgressBarHandler = Nothing) As Boolean
        ...
        'Notify FTP of the expected size
        ftp.ContentLength = oArray.Length

        'create byte array to store: ensure at least 1 byte!
        Const BufferSize As Integer = 2048
        Dim content(BufferSize - 1) As Byte, dataRead As Integer

        Dim BytesDone As Integer
        Dim fs As Stream = New MemoryStream(oArray)
        Using rs As Stream = ftp.GetRequestStream
            ...
            Do
                ...
                If ShowProgress IsNot Nothing Then
                    ...
                    Dim msg = IIf(label = "", "pujant fitxer...", label)
                    ShowProgress(0, oArray.Length - 1, BytesDone, msg, cancel)
...
#Region "Download: File transfer FROM ftp server"
    ...
    'Version taking string/FileInfo
    Public Function Download(ByVal url As String) As Byte()
        Dim retval As Byte()
        ...
        Dim oresponse As FtpWebResponse = ftp.GetResponse()
        Dim oMemoryStream As New MemoryStream
        myStreamWriter = New System.IO.StreamWriter(oMemoryStream)

        myStreamWriter.Write(New System.IO.StreamReader(oresponse.GetResponseStream()).ReadToEnd)
...
#Region "private supporting fns"
    ...
    Private Function GetDirectory(Optional ByVal directory As String = "") As String
        ...
        ElseIf directory.StartsWith("ftp://") Then
            URI = directory
...
Public Class FtpHelper
    ...
    Public Shared Function Download(url As String, username As String, password As String) As Byte()
        ...
        Using stream As New System.IO.MemoryStream()
            Dim oWebResponse As WebResponse = request.GetResponse()
            Dim oFtpWebResponse = CType(oWebResponse, FtpWebResponse)
            Dim oStream = oFtpWebResponse.GetResponseStream()
