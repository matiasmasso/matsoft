...
Public Class PdfSepaMandatoManual
    ...
    Private mCasillaSingleLineHeight As Integer = 25 ' 13

    Private mFontValue As New Font("Helvetica", 14, FontStyle.Regular)
    ...
    Public Sub New(oEmp As DTOEmp, oGuid As Guid, Optional oLang As DTOLang = Nothing)
        MyBase.New()
    ...
    Private Function C1Pdf(oSepaTexts As List(Of DTOSepaText)) As C1PdfDocument
        ...
        mCasillaX = mRectangleHeader.X + 150 ' 180
        ...
        oPdf.DrawString(_Guid.ToString, mFontTitle, Brushes.Black, mRectangleTitle, sF)
        ...
        iY = iY + DrawCasilla(oPdf, True, iY, ss(0), ss(1), "", 1)

        ss = oSepaTexts(texts.DeutorAdr).LangText.Tradueix(_Lang).Split(vbLf)
        iY = iY + DrawCasilla(oPdf, True, iY, ss(0), ss(1), "", 2)

        ss = oSepaTexts(texts.DeutorZipyCit).LangText.Tradueix(_Lang).Split(vbLf)
        DrawCasilla(oPdf, True, iY, , ss(0), "", , , 65)
        iY = iY + DrawCasilla(oPdf, True, iY, , ss(1), "", 3, mCasillaX + 75, mCasillaWidth - 75)

        iY = iY + DrawCasilla(oPdf, True, iY, , oSepaTexts(texts.DeutorPais).LangText.Tradueix(_Lang), "", 4)

        ss = oSepaTexts(texts.Iban).LangText.Tradueix(_Lang).Split(vbLf)
        iY = iY + DrawCasilla(oPdf, True, iY, ss(0), ss(1), "", 5)

        iY = iY + DrawCasilla(oPdf, True, iY, , oSepaTexts(texts.Swift).LangText.Tradueix(_Lang), "", 6)

        iY = iY + DrawCasilla(oPdf, False, iY, oSepaTexts(texts.CreditorNom).LangText.Tradueix(_Lang), oSepaTexts(texts.CreditorNom).LangText.Tradueix(_Lang), _Org.Nom, 7)

        Dim sPresentador_Id As String = IdentificadorDelInterviniente(DTOAddress.Country(_Org.Address).ISO, "000", _Org.PrimaryNifValue())
        iY = iY + DrawCasilla(oPdf, False, iY, , oSepaTexts(texts.CreditorId).LangText.Tradueix(_Lang), sPresentador_Id, 8)

        iY = iY + DrawCasilla(oPdf, False, iY, , oSepaTexts(texts.CreditorAdr).LangText.Tradueix(_Lang), _Org.Address.Text, 9)

        ss = oSepaTexts(texts.CreditorZipyCit).LangText.Tradueix(_Lang).Split(vbLf)
        DrawCasilla(oPdf, False, iY, , ss(0), _Org.Address.Zip.ZipCod, , , 65)
        iY = iY + DrawCasilla(oPdf, False, iY, , ss(1), DTOAddress.Location(_Org.Address).Nom, 10, mCasillaX + 75, mCasillaWidth - 75)

        iY = iY + DrawCasilla(oPdf, False, iY, , oSepaTexts(texts.CreditorPais).LangText.Tradueix(_Lang), DTOAddress.Country(_Org.address).LangNom.Tradueix(_Lang), 11)

        ss = oSepaTexts(texts.TipoDePago).LangText.Tradueix(_Lang).Split(vbLf)
        Dim iShortLen As Integer = 28 ' 16
        DrawCasilla(oPdf, True, iY, ss(0), ss(1), "X", , , iShortLen)
        iY = iY + DrawCasilla(oPdf, False, iY, , ss(2), , 12, mCasillaX + 75, iShortLen)

        ss = oSepaTexts(texts.SigLocation).LangText.Tradueix(_Lang).Split(vbLf)
        DrawCasilla(oPdf, True, iY, ss(0), ss(1), "", , , mCasillaWidth - 75)
        iY = iY + DrawCasilla(oPdf, True, iY, , ss(2), , 13, mCasillaX + mCasillaWidth - 65, 65)
        ...
        iY = iY + DrawCasilla(oPdf, True, iY, ss(0), ss(1), , , , , 90) ' 45)

        Dim sbs As New System.Text.StringBuilder
        sbs.Append(_Lang.Tradueix("En caso de persona jurídica se requiere sello, nombre y apellidos y DNI del firmante", "En cas de persona jurídica es requereix segell, nom cognoms i DNI del signant", "Stamp, full name and signee Id number in case of society"))
        'sbs.Append(_Iban.PersonNom)
        'sbs.Append(_Lang.Tradueix(" con DNI ", " amb DNI ", " with Id number "))
        'sbs.Append(_Iban.PersonDni)
        oPdf.DrawString(sbs.ToString, mFontSubtitle, Brushes.Navy, New PointF(mCasillaX + 10, iY - 25))
    ...
    Private Function DrawCasilla(oPdf As C1PdfDocument, BlObligatori As Boolean, iY As Integer, Optional sHeader As String = "", Optional sSubHeader As String = "", Optional sValue As String = "", Optional iOrdre As Integer = 0, Optional iX As Integer = 0, Optional iWidth As Integer = 0, Optional iCasillaHeight As Integer = 0) As Integer
        Dim iLineHeight As Integer = 33 ' 17
        ...
        If BlObligatori Then
            ...
            Dim oRectangleAsterisk As New Rectangle(iX - 12, iY, 12, oCasilla.Height) ' 12)
        ...
        If sValue > "" Then
            ...
            Dim oRectangle As New Rectangle(oCasilla.X + 10, oCasilla.Y, oCasilla.Width - 11, oCasilla.Height)
            oPdf.DrawString(sValue, mFontValue, Brushes.Navy, oRectangle, sF)
        End If
        ...
        If iOrdre <> 0 Then
            ...
            Dim sF As New StringFormat()
            sF.Alignment = StringAlignment.Far
            Dim oRectangle As New Rectangle(mRectangleMain.Right - iOrdreWidth - iRightMargin, iY + (oCasilla.Height / 3), iOrdreWidth, oCasilla.Height) ' mFontStandard.Height)
    ...
    Public Function Filename() As String
        Dim s As String = "mandato SEPA CORE.pdf"
        Return s
    End Function
