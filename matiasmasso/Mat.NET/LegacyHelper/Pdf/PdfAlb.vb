Public Class PdfAlb
    ...
    Public Sub New(ByVal oInvoices As List(Of DTOInvoice), Optional ByVal BlSigned As Boolean = False)
        Dim exs As New List(Of Exception)
        'mPdf = New C1.C1Pdf.C1PdfDocument(Printing.PaperKind.A4)
        Dim EmpId As Integer = 1
        If oInvoices.Count > 0 Then EmpId = oInvoices.First.Emp.Id
        If oInvoices.First.Fch < CDate("1/11/2007") Then
            mPdf = New PdfCorpTemplateStGervasi(PdfCorpTemplateStGervasi.Templates.invoice)
        Else
            mPdf = New PdfCorpTemplateDiagonal(PdfCorpTemplateStGervasi.Templates.invoice, EmpId)
        End If
        ...
        For Each oInvoice As DTOInvoice In oInvoices
            SetDetailTabWidths(oInvoice.Customer.MostrarEANenFactura)
            Dim oDoc As DTODoc = DTOInvoice.doc(oInvoice)
            PrintDoc(oDoc)
        Next
    ...
    Public Sub New(ByVal oInvoice As DTOInvoice)
        ...
        mSigned = True
        'mPdf = New C1.C1Pdf.C1PdfDocument(Printing.PaperKind.A4)
        If oInvoice.Fch < CDate("1/11/2007") Then
            mPdf = New PdfCorpTemplateStGervasi(PdfCorpTemplateStGervasi.Templates.invoice)
        Else
            mPdf = New PdfCorpTemplateDiagonal(PdfCorpTemplateStGervasi.Templates.invoice, oInvoice.Emp.Id)
        End If
        mRight = mPdf.PageRectangle.Width - 20
        mBottom = mPdf.PageRectangle.Bottom - 20
        Dim oDoc As DTODoc = DTOInvoice.doc(oInvoice)
    ...
    Public Function Stream(exs As List(Of Exception), oCert As DTOCert) As Byte()
        Dim retval As Byte() = Nothing
        If mSigned Then
            ' Dim oCert = Await FEBL.Cert.Find(oOrg, exs)
            If exs.Count = 0 Then
                retval = CType(mPdf, C1PdfHelper.Document).SignedStream(exs, oCert.Stream, oCert.Pwd)
            End If
        Else
            retval = CType(mPdf, C1PdfHelper.Document).Stream(exs)
        End If
        Return retval
    End Function

    Public Function StreamSync(exs As List(Of Exception), oCert As DTOCert, Optional ByVal BlSign As Boolean = False, Optional oOrg As DTOContact = Nothing) As Byte()
        Dim retval As Byte() = Nothing
        If mSigned Then
            'Dim oCert = bEBL.Cert.Find(oOrg, exs).Result
            If exs.Count = 0 Then
    ...
    Private Sub PrintDoc(ByVal oCurrentDoc As DTODoc, Optional iCopias As Integer = 0)
        mCurrentDoc = oCurrentDoc
        Dim oLegacyColor = oCurrentDoc.BackColor
        mBackBrush = New System.Drawing.Pen(oLegacyColor).Brush
        ...
        For iCopia = 1 To iCopias
            ...
            NewPage()
            mArrastre = DTOAmt.Factory(oCurrentDoc.cur)
            PrintDetall()
    ...
    Private Sub PrintTotals()
        ...
        'mCurrentDoc.ivaBaseQuotas = New List(Of DTOTaxBaseQuota)
    ...
    Public Sub DrawRowEnd(ByVal oEndRow As EndRows, ByVal ParamArray sStrings() As String)
        ...
        Select Case oEndRow
            ...
            Case EndRows.REQstd, EndRows.REQred, EndRows.REQsuperRed
                ...
                mArrastre.add(oAmt)
            Case EndRows.Total
                'mCurrentDoc.incoterm = DTOIncoterm.Factory(DTOIncoterm.Ids.DDP)
