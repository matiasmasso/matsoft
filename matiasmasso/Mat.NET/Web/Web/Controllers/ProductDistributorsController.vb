Public Class ProductDistributorsController
        ...
        If oUser IsNot Nothing AndAlso oUser.Rol.IsAuthenticated Then
            Select Case oUser.Rol.id
                Case DTORol.Ids.manufacturer
                    Dim oProveidors = Await FEB2.User.BrandManufacturers(exs, oUser)
                    If oProveidors.Count > 0 Then
                        Dim oProveidor As DTOProveidor = oProveidors(0)
                        Dim src As String = Await FEB2.ProductDistributors.ManufacturerCsv(oProveidor)
    ...
    Public Async Function RepList(Optional rep As Nullable(Of Guid) = Nothing) As Threading.Tasks.Task(Of ActionResult)
        ...
        Else
            oRep = Await FEB2.Rep.Find(rep, exs)
        End If

        Dim Model = Await FEB2.ProductDistributors.FromRep(exs, oRep)
        retval = View(Model)
        Return retval
    End Function
    ...
    Public Async Function RepDistribuidoresOficialesList(brandnom As String) As Threading.Tasks.Task(Of ActionResult)
        Dim exs As New List(Of Exception)
        Dim retval As ActionResult = Nothing
        Dim oBrand = Await FEB2.ProductBrand.FromNom(exs, GlobalVariables.Emp, brandnom)
        Dim oUser = ContextHelper.GetUser()

        Dim Model As List(Of DTOProductDistributor) = Await FEB2.ProductDistributors.DistribuidorsOficials(exs, oUser, oBrand)
        retval = Await LoginOrView("RepList", Model, {DTORol.Ids.SuperUser, DTORol.Ids.Admin, DTORol.Ids.SalesManager, DTORol.Ids.Comercial, DTORol.Ids.Rep, DTORol.Ids.Operadora, DTORol.Ids.Manufacturer})
        Return retval
    End Function


    Shadows Async Function LoginOrView(sView As String, Model As Object, Optional oRols As DTORol.Ids() = Nothing) As Threading.Tasks.Task(Of ActionResult)
        Dim exs As New List(Of Exception)
        Dim retval As ActionResult = Nothing
        Dim oUser = ContextHelper.GetUser()
        If oUser IsNot Nothing AndAlso oUser.Rol.IsAuthenticated Then
