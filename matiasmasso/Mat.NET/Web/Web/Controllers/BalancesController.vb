    ...
    Async Function Index(guid As Nullable(Of Guid)) As Task(Of ActionResult)
        ...
        Select Case MyBase.Authorize({DTORol.Ids.SuperUser,
            ...
            Case AuthResults.success
                ...
                Dim DtFch As Date
                Dim sFch As String = Await FEB2.Default.EmpValue(GlobalVariables.Emp, DTODefault.Codis.LastBalanceQuadrat, exs)
                If IsNumeric(sFch) Then
                    DtFch = Date.FromOADate(sFch)
                Else
                    DtFch = Today
                End If
                Dim Model As List(Of DTOPgcClass) = Await FEB2.Balance.Tree(GlobalVariables.Emp, DtFch)
                ViewBag.Fch = DtFch
