...
Public Class BlogPostController
    ...
    Async Function CatchAll() As Threading.Tasks.Task(Of ActionResult)
        ...
        If MyBase.UrlSplit(HttpContext.Request.Url, oMainSegment, ".html", oDomain, oLang, sFriendlySegment) Then
            If String.IsNullOrEmpty(sFriendlySegment) Then
                oModel = Await FEB2.BlogPost.LastModel(exs, oLang)
                If exs.Count = 0 Then
                    ViewBag.Lang = oLang
                ...
                End If
            Else
                oModel = Await FEB2.BlogPost.FromFriendlySegment(exs, sFriendlySegment, oLang)
                If exs.Count = 0 Then
    ...
    Async Function Index(guid As Nullable(Of Guid)) As Threading.Tasks.Task(Of ActionResult)
        ...
        If guid Is Nothing Then
            model = Await FEB2.BlogPost.LastModel(exs, oLang)
        Else
            Dim oPost As New DTOBlogPost(guid)
            model = Await FEB2.BlogPost.Model(exs, oPost, oLang)
        End If
    ...
    Async Function Posts() As Threading.Tasks.Task(Of ActionResult)
        ...
        Dim exs As New List(Of Exception)
        Dim oLang = ContextHelper.Lang
        Dim model = Await FEB2.BlogPosts.Models(exs, oLang)
        If exs.Count = 0 Then
            ViewBag.Lang = oLang
    ...
    Async Function SignUp(returnPostGuid As Guid) As Threading.Tasks.Task(Of ActionResult)
        ...
        Dim oBlogPost = Await FEB2.BlogPost.Find(exs, returnPostGuid)
        If exs.Count = 0 Then
    ...
    Async Function About() As Threading.Tasks.Task(Of ActionResult)
        ...
        model.Posts = Await FEB2.BlogPosts.Models(exs, oLang)
    ...
    Async Function Consultas(returnPostGuid As Nullable(Of Guid)) As Threading.Tasks.Task(Of ActionResult)
        ...
        If returnPostGuid Is Nothing Then
            oTree = Await FEB2.PostComments.Tree(exs, DTOContent.Wellknown(DTOContent.Wellknowns.consultasBlog), DTOPostComment.ParentSources.Blog, ContextHelper.Lang, 15, 0)
        Else
            ...
            End With
            oTree = Await FEB2.PostComments.Tree(exs, oPostComment)
        End If
        ...
        End With
        'model.Posts = Await FEB2.BlogPosts.Models(exs, oLang)
        ViewBag.Title = Mvc.ContextHelper.Tradueix("Consultas", "Consultes", "Any questions?")
    ...
    Async Function WithComment(id As Guid) As Threading.Tasks.Task(Of ActionResult)
        ...
        Dim oComment = Await FEB2.PostComment.Find(exs, id)
        Dim oPost As New DTOBlogPost(oComment.Parent)
        Dim oModel As DTOBlogPostModel = Await FEB2.BlogPost.Model(exs, oPost, oLang)
        oModel.Comments = Await FEB2.PostComments.Tree(exs, oPost, DTOPostComment.ParentSources.Blog, ContextHelper.Lang, 15, 0)
        If exs.Count = 0 Then
            If oModel Is Nothing Then
                Return Await ErrorNotFoundResult()
            Else
    ...
    Async Function Consulta(model As BlogConsultaModel) As Threading.Tasks.Task(Of PartialViewResult)
        ...
        If ContextHelper.GetUser Is Nothing Then
            Dim oUser As DTOUser = Await FEB2.User.FromEmail(exs, New DTOEmp(DTOEmp.Ids.MatiasMasso), model.EmailAddress) ' TO DEPRECATE
            If oUser Is Nothing Then
                oUser = New DTOUser
                oUser.Emp = GlobalVariables.Emp
        ...
        If Await FEB2.PostComment.Update(exs, oComment) Then
            Await FEB2.PostComment.EmailPendingModeration(exs, GlobalVariables.Emp, oComment)
