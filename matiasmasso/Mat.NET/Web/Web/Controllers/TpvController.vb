Public Class TpvController
    ...
    Public Async Function Index(Optional Base64Json As String = "") As Threading.Tasks.Task(Of ActionResult) '(Cod As Nullable(Of Tpv.Modes), guid As Nullable(Of Guid)) As ActionResult
        ...
        If Base64Json = "" Then
            ...
            oModel = Await FEB2.TPVRequest.FromMode(oMode, oGuid, oLang, exs)
        ...
        Else
            ...
            If oMode = DTOTpvRequest.Modes.Free Then
                Dim oTpvLog = Await FEB2.TpvLog.Find(oGuid, exs)
    ...
    Public Async Function Index(oModel As DTOTpvRequest) As Threading.Tasks.Task(Of ActionResult)
        ...
        If ModelState.IsValid Then
            ...
            Dim oConfig As DTOPaymentGateway = Await FEB2.TpvRedSys.Config(exs, GlobalVariables.Emp, DTOPaymentGateway.Environments.Production)
            Dim oTpvRedsys As DTOTpvRedsysPasarela = Await FEB2.TpvRedSys.SignedRequest(exs, oConfig, oUser, ContextHelper.GetLang, oModel.Mode, oModel.Ref, DTOAmt.Factory(oModel.Eur), oModel.Concept)
    ...
    Public Shadows Async Function Log() As Threading.Tasks.Task
        ...
        Try
            ...
            Else
                If Await FEB2.TpvRedSys.LogResponse(GlobalVariables.Emp, oTpvLog, exs) Then
                    'avisa a les noies del resultat de la operació
                    Dim oMailMessage = DTOTpvLog.MailMessage(oTpvLog)
                    Dim oUser = Await FEB2.User.Find(DTOUser.Wellknown(DTOUser.Wellknowns.info).Guid, exs)
                    Await FEB2.MailMessage.Send(exs, oUser, oMailMessage)
                Else
                    exs.Add(New Exception("error en FEB2.TpvRedSys.LogResponse"))
                    If oTpvLog Is Nothing Then
        ...
        Finally
            ...
            FEB2.MailMessage.SendSync(DTOUser.Wellknown(DTOUser.Wellknowns.matias), oMailMessage, exs)
        End Try
    End Function
    ...
    Private Async Function GetLogFromContext() As Threading.Tasks.Task(Of DTOTpvLog)
        ...
        Dim retval As DTOTpvLog = Await FEB2.TpvRedSys.GetLogFromEncriptedData(GlobalVariables.Emp, MerchantParameters, signatureReceived)
        Return retval
    End Function
