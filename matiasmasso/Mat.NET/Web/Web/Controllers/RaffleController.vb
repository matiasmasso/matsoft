...
Public Class RaffleController
    ...
    Async Function Zoom(guid As Guid) As Threading.Tasks.Task(Of ActionResult)
        ...
        Dim oRaffle = Await FEB2.Raffle.Find(guid, exs)
    ...
    Async Function Play2(guid As Guid, user As Guid) As Threading.Tasks.Task(Of ActionResult)
        ...
        If exs.Count = 0 And oUser IsNot Nothing Then
            ...
            ViewBag.Title = Mvc.ContextHelper.Tradueix("Inscripción en sorteo", "Inscripció en sorteig", "Raffle subscription", "Inscrição no sorteio")
            Dim oRaffle As New DTORaffle(guid)
            Dim oLang = ContextHelper.Lang
            Dim model As DTORafflePlayModel = Await FEB2.RaffleParticipant.PlayModelFactory(exs, oRaffle, oUser, ContextHelper.Lang)
    ...
    Async Function Share() As Threading.Tasks.Task(Of JsonResult)
        ...
        If GuidHelper.IsGuid(sGuid) Then
            ...
            Dim oGuid = Await FEB2.Feedback.Update(exs, oFeedback)
            If exs.Count = 0 Then
    ...
    Async Function Locations_DEPRECATED(raffle As Guid, zonaGuid As Guid) As Threading.Tasks.Task(Of JsonResult)
        Dim exs As New List(Of Exception)
        Dim oRaffle = Await FEB2.Raffle.Find(raffle, exs)
        Dim oZona = DTOArea.Factory(zonaGuid, DTOArea.Cods.Zona)
        Dim oLocations = Await FEB2.Raffle.Locations(exs, GlobalVariables.Emp, oRaffle, oZona)
    ...
    Async Function Distributors_DEPRECATED(raffle As Guid, locationGuid As Guid) As Threading.Tasks.Task(Of JsonResult)
        ...
        Dim oDistributors = Await FEB2.Raffle.Distributors(exs, GlobalVariables.Emp, oRaffle, oLocation)
    ...
    Async Function Update(token As Guid, raffle As Guid, answer As Integer, distributor As Guid) As Threading.Tasks.Task(Of PartialViewResult)
        ...
        Try
            oRaffle = Await FEB2.Raffle.Find(raffle, exs)
            oUser = Await FEB2.User.Find(token, exs)
            ...
            If oRaffle IsNot Nothing Then
                ...
                Dim oUserInfo = Await FEB2.User.Find(DTOUser.Wellknown(DTOUser.Wellknowns.info).Guid, exs)
                If Await FEB2.RaffleParticipant.Enroll(oParticipant, oUserInfo, GlobalVariables.Emp, exs) Then
    ...
    Async Function MailConfirmation(raffleparticipant As String) As System.Threading.Tasks.Task
        Dim exs As New List(Of Exception)
        Dim oRaffleParticipant = Await FEB2.RaffleParticipant.Find(exs, New Guid(raffleparticipant))
        Dim oMailMessage = Await FEB2.RaffleParticipant.RaffleParticipationMailMessage(exs, GlobalVariables.Emp, oRaffleParticipant)
        Dim oUserInfo = Await FEB2.User.Find(DTOUser.Wellknown(DTOUser.Wellknowns.info).Guid, exs)
        If Not Await FEB2.MailMessage.Send(exs, oUserInfo, oMailMessage) Then
    ...
    Public Shadows Async Function onAuthentication() As Threading.Tasks.Task(Of JsonResult)
        ...
        Dim oRaffle = Await FEB2.Raffle.Find(raffle, ex2)
        Dim oUser = Await FEB2.User.FromEmail(ex2, GlobalVariables.Emp, email)
        Dim myData As Object = Nothing
        Dim exs As New List(Of DTORaffleParticipant.Exception)
        Dim isValidUser As Boolean = Await FEB2.RaffleParticipant.Validate(oRaffle, oUser, exs)
        If isValidUser Then
...
Public Class RafflesController
    ...
    Async Function Index() As Threading.Tasks.Task(Of ActionResult)
        ...
        Dim Model = Await FEB2.Raffles.Model(exs, oLang, 5, 0, MyBase.User)
        ContextHelper.NavViewModel.ResetCustomMenu()
    ...
    Async Function partialRaffles() As Threading.Tasks.Task(Of PartialViewResult)
        ...
        Dim oLang As DTOLang = ContextHelper.Lang()
        Dim Model = Await FEB2.Raffles.Model(exs, oLang, take, takeFrom, MyBase.User)
