Public Class NewsController
    ...
    Async Function Index(Catchall As String) As Threading.Tasks.Task(Of ActionResult)
        ...
        If GuidHelper.IsGuid(sUserGuid) Then
            Dim pUser = Await FEB2.User.Find(New Guid(sUserGuid), exs)
        ...
        If MyBase.UrlSplit(HttpContext.Request.Url, oMainSegment, ".html", oDomain, oLang, sFriendlySegment) Then
            ...
            Else
                If GuidHelper.IsGuid(sFriendlySegment) Then
                    ...
                    Else
                        oModel = Await FEB2.Noticia.Find(exs, oGuid)
                    End If
                Else
                    oModel = Await FEB2.Noticia.SearchByUrl(exs, sFriendlySegment)
                End If

                If oModel Is Nothing Then
                    Dim oBlogPost = If(GuidHelper.IsGuid(sFriendlySegment), Await FEB2.BlogPost.Find(exs, New Guid(sFriendlySegment)), Await FEB2.BlogPost.FromFriendlySegment(exs, sFriendlySegment, oLang))
                ...
                If FEB2.Noticia.IsAllowed(GlobalVariables.Emp, oUser, oModel) Then
                    Await FEB2.Session.Log(exs, oUser, oModel.Guid)
    ...
    Async Function NoticiaView(oLang As DTOLang, Catchall As String) As Threading.Tasks.Task(Of ActionResult)
        ...
        Else
            If GuidHelper.IsGuid(Catchall) Then
                ...
                oNoticia = Await FEB2.Noticia.Find(exs, oGuid)
            Else
                oNoticia = Await FEB2.Noticia.SearchByUrl(exs, Catchall)
            End If

            If oNoticia Is Nothing Then
                retval = Await ErrorNotFoundResult()
            Else
                oNoticia.Comments = Await FEB2.PostComments.Tree(exs, oNoticia, DTOPostComment.ParentSources.News, oLang, 15, 0)
                ...
                Else
                    If FEB2.Noticia.IsAllowed(GlobalVariables.Emp, oUser, oNoticia) Then
                        Await FEB2.Session.Log(exs, oUser, oNoticia.Guid)
    ...
    Async Function Noticias() As Threading.Tasks.Task(Of ActionResult)
        ...
        If exs.Count = 0 Then
            ...
            Dim model = Await FEB2.Noticias.LastNoticias(exs, oUser, Lang)
            If exs.Count = 0 Then
                ViewBag.Lang = oLang
    ...
    Async Function PartialNoticias(lang As String) As Threading.Tasks.Task(Of PartialViewResult)
        ...
        ViewBag.Lang = oLang
        If exs.Count = 0 Then
            Dim model = Await FEB2.Noticias.LastNoticias(exs, oUser, oLang)
            If exs.Count = 0 Then
    ...
    Async Function iMat(id As Guid) As Threading.Tasks.Task(Of ActionResult)
        Dim exs As New List(Of Exception)
        Dim oNoticia = Await FEB2.Noticia.Find(exs, id)
        Dim oUser = ContextHelper.GetUser()
        Await FEB2.Session.Log(exs, oUser, oNoticia.Guid)
    ...
    Async Function LastNoticiaForMobile() As Threading.Tasks.Task(Of ActionResult)
        Dim exs As New List(Of Exception)
        Dim oUser = ContextHelper.GetUser
        Dim oNoticia = Await FEB2.Noticias.LastNoticia(exs, oUser, ContextHelper.Lang())
        Await FEB2.Session.Log(exs, oUser, oNoticia.Guid)
    ...
    Async Function FromGuid(guid As Guid) As Threading.Tasks.Task(Of ActionResult)
        Dim exs As New List(Of Exception)
        Dim Model = Await FEB2.Noticia.Find(exs, guid)
    ...
    Async Function Categoria(Nom As String) As Threading.Tasks.Task(Of ActionResult)
        ...
        If Nom = "" Then
            ...
            ' Dim oNoticias = Await FEB2.Noticias.Headers(exs, DTOContent.Srcs.News, Not showPro, OnlyVisible:=True)
        ...
        Else
            ...
            Dim oCategoria = Await FEB2.CategoriaDeNoticia.FromNom(srcNom, exs)
