...
@Code
    ...
    Dim items As List(Of DTOBalanceSaldo) = FEB2.Balance.CceSync(exs, Mvc.GlobalVariables.Emp, Model, ViewBag.fch)
...
<div>
    ...
    Cta: @DTOPgcCta.FullNom(Model, Mvc.ContextHelper.Lang())
</div>
...
<div class="Grid">
    ...
    @For Each item As DTOBalanceSaldo In items
        @<div Class="Row SelectableRow" data-url="@FEB2.Ccd.Url(Model, item.Contact, ViewBag.fch)">
            <div Class="CellTxt">
            ...
            <div Class="CellBigNum CurrentYear">
                @Format(FEB2.Balance.CurrentYearEur(item, oCod), "#,##0.00;-#,##0.00;#")
            </div>
            <div Class="CellBigNum PreviousYear">
                @Format(FEB2.Balance.PreviousYearEur(item, oCod), "#,##0.00;-#,##0.00;#")
            </div>
        </div>
    Next
</div>
