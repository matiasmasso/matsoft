...
<section class="Grid">
    <div class="RowHdr">
        <div class="CellShortTxt CellDesktop">@Mvc.ContextHelper.Tradueix("Cuenta", "Compte", "Account")</div>
        <div class="CellShortTxt CellMobile">@Mvc.ContextHelper.Tradueix("Cuenta", "Compte", "Account")</div>
        <div class="CellTxt">@Mvc.ContextHelper.Tradueix("Contacto", "Contacte", "Contact")</div>
        ...
        <div class="CellAmt CellMobile">@Mvc.ContextHelper.Tradueix("Haber", "Credit", "Credit")</div>
    </div>

    @For Each item As DTOCcb In Model.Items

        @<div class="Row SelectableRow" data-url="@FEB2.Ccd.Url(item.Cta, item.Contact, MatHelperStd.TimeHelper.LastDayOfYear(item.Cca.Fch))">
            <div class="CellShortTxt CellDesktop">
                @DTOPgcCta.FullNom(item.Cta, Mvc.ContextHelper.Lang())
            </div>
            <div class="CellShortTxt CellMobile">
                <a href="@FEB2.PgcExtracte.Url(Model.Fch.Year, item.Cta, item.Contact)">
                    @item.Cta.Id
                </a>
            </div>
...
@If Model.DocFile IsNot Nothing Then
    @<section class="DocFile">
        <a href="@FEB2.DocFile.DownloadUrl(Model.DocFile, False)">
            <img src="@FEB2.DocFile.ThumbnailUrl(Model.DocFile, True)" />
        </a>
    </section>
End If
