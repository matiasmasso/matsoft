...
@Code
    ...
    Dim oCustomers = FEB2.User.CustomersForBasketSync(exs, oUser)
...
<div class="pagewrapper">
    ...
    @If Model.Count = 0 Then
        @<div>
            @Mvc.ContextHelper.Tradueix("No disponemos de excedentes de stocks en estos momentos", "No disposem de excedents de stocks en aquests moments", "No overstocks available currently")
        </div>
    ElseIf Not OrderEnabled Then
        @<div>
            @Mvc.ContextHelper.Tradueix("Su correo electrónico no está asociado a ninguna ficha activa de cliente. Por favor póngase en contacto con nuestras oficinas para actualizar sus datos.", "El seu correu electronic no está associat a cap fitxa de client en actiu. Si us plau posis en contacte amb les nostres oficines per actualitzar les seves dades.", "Sorry your email is not linked to any active customer account. Please contact our offices to update your profile.")
        </div>
    Else
        @<div class="DataCollection">
    @If (oUser.Rol.id = DTORol.Ids.CliFull Or oUser.Rol.id = DTORol.Ids.CliLite) Then
    @<p>
        @Mvc.ContextHelper.Tradueix("Para cursar un pedido rellene las cantidades en la columna de la derecha y pulse el botón al final del listado", "Per cursar una comanda ompli les quantitats a la columna de la dreta i premi el botó que trobará al final del llistat", "Fill your quantities on right column and press the button at the bottom of the list to submit your order")
    </p>
    End If
    ...
    <div Class="Grid Outlet">
        ...
        <div Class="RowHdr">
            ...
            <div Class="CellAmt" @IIf(OrderEnabled, "", "hidden='hidden'")>
                @Mvc.ContextHelper.Tradueix("Coste", "Cost", "Cost")
            </div>
            <div Class="CellDto">
                @Mvc.ContextHelper.Tradueix("Dto", "Dte", "Dct")
            </div>
            <div Class="CellNum">
                @Mvc.ContextHelper.Tradueix("Min", "Min", "MOQ")
            </div>
            <div Class="CellAmt" @IIf(OrderEnabled, "", "hidden='hidden'")>
                @Mvc.ContextHelper.Tradueix("Pedido", "Comanda", "Order")
            </div>
        </div>
        ...
        @For Each oSku As DTOProductSku In Model
            ...
            @<div class="Row">
                ...
                <div Class="CellTxt CellSku">
                    <a href="@oSku.GetUrl(Mvc.ContextHelper.Lang)" target="_blank">@oSku.Nom.Tradueix(Mvc.ContextHelper.Lang())</a>
                </div>
                <div Class="CellAmt" @IIf(OrderEnabled, "", "hidden='hidden'")>
    ...
    <div Class="CellDto">
                ...
                <div Class="CellNum">
                    @FEB2.Outlet.MOQ(oSku)
                </div>
                <div Class="CellAmt" @IIf(OrderEnabled, "", "hidden='hidden'")>
                    <input Class="Qty" type="number" data-sku="@oSku.Guid.ToString" data-price="0" min="@FEB2.Outlet.MOQ(oSku)" max="@oSku.Stock" />
                </div>
    ...
    @If OrderEnabled Then
        @<div class="Submit" @IIf(OrderEnabled, "data-customer=" & oCustomers.First.Guid.ToString, "")>
            <input type="button" value='@Mvc.ContextHelper.Tradueix("enviar pedido", "enviar comanda", "submit order")' />
        </div>
    End If
</div>
