...
@If Model.Count = 0 Then
    @<div>
        <p>
            @Mvc.ContextHelper.Tradueix("No nos consta ninguna cuenta vinculada a este usuario.",
                                           "No ens consta cap compte vinculat a aquest usuari.",
                                           "No account seems to be currently linked to this user.")
            <br />
            @Mvc.ContextHelper.Tradueix("Por favor contacte con nuestras oficinas para tramitarlo.",
...
ElseIf Model.Count = 1 Then
    ...
    @<div class="SelectedContact" data-guid="@Model(0).Guid.ToString">
        @Html.Raw(FEB2.Contact.HtmlNameAndAddress(Model(0)))
    </div>
...
Else
    ...
    @<div class="SelectContact">
        <p>
            @Mvc.ContextHelper.Tradueix("Por favor seleccione una cuenta:",
        ...
        @For Each oContact As DTOContact In Model
            @<div class="Contact">
                <a href='#' data-guid="@oContact.Guid.ToString">
                    @Html.Raw(FEB2.Contact.HtmlNameAndAddress(oContact))
                </a>
            </div>
        Next
    </div>
