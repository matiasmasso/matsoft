...
<div id="ControlPanel">
    <div id="Filters">
        <select id="CurrentYear">
            ...
            @For year As Integer = Today.Year - 1 To 2017 Step -1
                @<option value="@year">@year</option>
            Next
        </select>
        <select id="CurrentUnits">
            <option value="0" selected>@Mvc.ContextHelper.Tradueix("Importes", "Imports", "Amounts")</option>
            <option value="1">@Mvc.ContextHelper.Tradueix("Unidades", "Unitats", "Units")</option>
        </select>
        <select id="CurrentBrand" hidden="hidden">
            <option value="" selected>@Mvc.ContextHelper.Tradueix("Todas las marcas", "Totes les marques", "All brands")</option>
        </select>
        <select id="CurrentCategory" hidden="hidden" ">
            <option value="" selected>@Mvc.ContextHelper.Tradueix("Todas las categorías", "Totes les categories", "All categories")</option>
        </select>
        <select id="CurrentSku" hidden="hidden">
            <option value="" selected>@Mvc.ContextHelper.Tradueix("Todos los productos", "Tots els productes", "All products")</option>
        </select>
    </div>
    <div>
        <a href='#' id="ExcelRequest" title='@(Mvc.ContextHelper.Tradueix("Descargar fichero Excel", "Descarregar fitxer Excel", "Download Excel file")) '>
...
<div id="Grid-Container" hidden="hidden">
    <div class="Row Header">
        <div>@Mvc.ContextHelper.Tradueix("Concepto", "Concepte", "Concept")</div>
        <div class="Total">@Mvc.ContextHelper.Tradueix("Totales", "Totals", "Totals")</div>
        @For i = 1 To 12
            @<div class="Month">@Mvc.ContextHelper.Lang.MesAbr(i)</div>
        Next
    </div>
</div>
