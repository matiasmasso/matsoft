$(document).ready(function () {
    ...
    function refresca() {
        ...
        $(".loading").show();
        $('#divItems').load('/EciSalesReport', data(), function () {
            $(".loading").hide();
        });
    }

    $(document).on('click', '#ExcelDownload', function (event) {
        event.preventDefault();
        $.ajax({
            cache: false,
            url: '/EciSalesReport/Excel',
            data: data(),
            type: 'POST',
            success: function (result) {
                $(".loading").hide();
                var downloadUrl = '/doc/Excel?fileGuid=' + result.FileGuid
