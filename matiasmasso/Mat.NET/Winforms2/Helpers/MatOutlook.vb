Module MatOutlook
    ...
    Public Async Function RemittanceAdvice(oCca As DTOCca, exs As List(Of Exception)) As Task(Of Boolean)
        Dim oRemittanceAdvice = Await FEB.RemittanceAdvice.FromCca(exs, oCca)
        Dim oProveidor As DTOProveidor = oRemittanceAdvice.Proveidor
        If FEB.Contact.Load(oProveidor, exs) Then
            Dim sRecipients = Await FEB.Subscriptors.Recipients(exs, GlobalVariables.Emp, DTOSubscription.Wellknowns.Comptabilitat, oProveidor)
            ...
            With oMailMessage
                ...
                .BodyUrl = FEB.UrlHelper.Factory(True, "mail/remittanceAdvice", oCca.Guid.ToString())
            End With
    ...
    Public Async Function RemittanceAdvice(oBeneficiari As DTOBancTransferBeneficiari, exs As List(Of Exception)) As Task(Of Boolean)
        Dim sRecipients = Await FEB.Subscriptors.Recipients(exs, GlobalVariables.Emp, DTOSubscription.Wellknowns.Comptabilitat, oBeneficiari.Contact)
        Dim oMailMessage = DTOMailMessage.Factory(sRecipients)
        With oMailMessage
            .Bcc = {"matias@matiasmasso.es"}.ToList
            .Subject = oBeneficiari.Contact.Lang.Tradueix("Aviso de transferencia", "Avis de transferència", "Bank transfer notification")
            .BodyUrl = FEB.UrlHelper.Factory(True, "mail/remittanceAdvice2", oBeneficiari.Guid.ToString())
        End With
