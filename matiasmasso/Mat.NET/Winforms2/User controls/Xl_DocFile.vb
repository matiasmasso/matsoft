...
Public Class Xl_DocFile
    ...
    Property _DefaultMimeCod As MimeCods = MimeCods.Pdf
    ...
    Public Shadows Async Function Load(oDocFile As DTODocFile, Optional defaultmimeCod As MimeCods = MimeCods.Pdf) As Task
        _DocFile = oDocFile
        _DefaultMimeCod = defaultmimeCod
        SetContextMenu()
        Try
            ...
            Else
                ...
                If oDocFile.Hash > "" Then
                    If oDocFile.Thumbnail Is Nothing Then
                        Dim oImage = FEB.DocFile.Thumbnail(exs, oDocFile.Hash)
                        If exs.Count = 0 Then
                            PictureBox1.Image = LegacyHelper.ImageHelper.Converter(Await FEB.DocFile.Thumbnail(exs, oDocFile.Hash))
                        Else
    ...
    Private Async Sub Do_ImportaFileAsync()
        ...
        With oDlg
            Select Case _DefaultMimeCod
                Case MimeCods.Xlsx
                    .Filter = "fitxers Excel|*.xlsx;*.xls|tots els documents|*.*"
                Case Else
    ...
    Private Async Function ImportaStreamAsync(oByteArray As Byte(), exs As List(Of Exception)) As Task(Of Boolean)
        ...
        Dim sha256 As String = CryptoHelper.Sha256(oByteArray)

        Dim oDocfile = Await FEB.DocFile.FindBySha256(sha256, exs)
        If exs.Count = 0 Then
    ...
    Private Async Function DisplayFileOrServerCopy() As Task
        Dim exs As New List(Of Exception)
        Dim oServerCopy As DTODocFile = Await FEB.DocFile.Find(_DocFile.Hash, exs)
        If exs.Count = 0 Then
            If oServerCopy Is Nothing Then
                _IsInedit = True
                TextboxFeatures.BackColor = Color.LightYellow
                PictureBox1.Image = LegacyHelper.ImageHelper.FromBytes(_DocFile.Thumbnail)
                TextboxFeatures.Text = DTODocFile.Features(_DocFile)
                SetContextMenu()
            Else
                _IsInedit = False
