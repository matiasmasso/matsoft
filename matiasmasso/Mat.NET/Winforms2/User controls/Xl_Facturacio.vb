Public Class Xl_Facturacio
    ...
    Public Shadows Async Function Load(values As List(Of DTOInvoice), oLastInvoicesEachSerie As List(Of DTOInvoice), DtMinFch As DateTimeOffset, Optional oDefaultValue As DTOInvoice = Nothing) As Task
        _Values = values
        _NextNum = 1
        _NextRct = 1
        _NextSmp = 1
        _LastInvoicesEachSerie = oLastInvoicesEachSerie
    ...
    Private Sub SetFchs()
        ...
        For Each oControlItem As ControlItem In _ControlItems
            ...
            If Not oControlItem.FixedFch Then
                Dim oLastInvoice = _LastInvoicesEachSerie.FirstOrDefault(Function(x) x.Serie = oInvoice.Serie)
                Dim DtLastInvoice = If(oLastInvoice Is Nothing, _MinFch, oLastInvoice.Fch)
                Dim DtFirstDeliveryFch As Date = oInvoice.Deliveries.Min(Function(x) x.Fch)
                _MinFch = {_MinFch, DtFirstDeliveryFch, DtLastInvoice}.Max
