Public Class Xl_RepComLiquidableNewRepLiq
    ...
    Public Event ToggleProggressBar(sender As Object, e As MatEventArgs)
    ...
    Public Function CheckedReps() As List(Of DTORep)
        Return Xl_RepLiqNew_RepsSplit1.Reps
    End Function
    ...
    Public Async Function RepLiqs(exs As List(Of Exception)) As Task(Of List(Of DTORepLiq))
        ...
        Dim retval As List(Of DTORepLiq) = Await FEB.Repliqs.Factory(oRepComsLiquidables, DtFch, Current.Session.User, exs)
        Return retval
    End Function
    ...
    Private Async Sub refresca()
        ...
        RaiseEvent ToggleProggressBar(Me, New MatEventArgs(True))
        _RepComsLiquidables = Await FEB.RepComLiquidables.PendentsDeLiquidar(exs)
        RaiseEvent ToggleProggressBar(Me, New MatEventArgs(False))
        If exs.Count = 0 Then
    ...
    Private Async Function LoadPendents() As Task
        ...
        For Each oRepComLiquidable As DTORepComLiquidable In _RepComsLiquidables
            ...
            If oRepComLiquidable.Comisio.Eur > 0 Then
                ...
                Dim oTuple As Tuple(Of NewRepLiqControlItem.Codis, Boolean) = Await Liquidable(exs, oRepComLiquidable, DtFchRepLiq)
    ...
    Private Async Function Liquidable(exs As List(Of Exception), ByVal oRepComLiquidable As DTORepComLiquidable, ByVal DtLiq As Date) As Task(Of Tuple(Of NewRepLiqControlItem.Codis, Boolean))
        Dim retval As Boolean
        Dim oFra = oRepComLiquidable.Fra
        Dim oCodi As NewRepLiqControlItem.Codis
        If oRepComLiquidable.Status = DTORepComLiquidable.PaymentStatus.Unpayments Then
            oCodi = NewRepLiqControlItem.Codis.Impagat
        ElseIf oRepComLiquidable.Status = DTORepComLiquidable.PaymentStatus.Insolvent Then
        ...
        Else
            ...
            Select Case oCfp
                Case DTOPaymentTerms.CodsFormaDePago.comptat
                    retval = True
                    Dim oAlbs = Await FEB.Deliveries.All(exs, oFra)
                    If exs.Count = 0 Then
    ...
    Private Async Sub onDescartaCheck(sender As Object, e As EventArgs)
        ...
        If Not Await FEB.RepComLiquidable.Descarta(oRepComLiquidable, exs) Then
    ...
    Private Function LastFinDeMes() As Date
        Dim Fch As Date = DTO.GlobalVariables.Today()
