...
Public Class Xl_Contact_Logo
    ...
    Public Shadows Async Function Load(exs As List(Of Exception), value As DTOContact) As Task(Of Boolean)
        ...
        Else
            _Contact.Logo = Await FEB.Contact.Logo(exs, value)
            If exs.Count = 0 Then
                If _Contact.Logo Is Nothing Then
                    PictureBox1.Image = My.Resources.DefaultContact
                Else
                    Dim ms As New IO.MemoryStream(_Contact.Logo)
    ...
    Private Async Function SetContextMenu() As Task
        ...
        If _Contact IsNot Nothing Then
            ...
            Dim oContactMenu = Await FEB.ContactMenu.Find(exs, _Contact)
                ...
                If Validate Then
                    If FEB.Contact.Load(_Contact, exs) Then
                        Using ms As New IO.MemoryStream
                            oImg.Save(ms, oImg.RawFormat)
                            _Contact.Logo = ms.ToArray()
                        End Using
