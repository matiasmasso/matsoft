...
Public Class Xl_FormaDePago
    ...
    Private Async Function Refresca() As Task
        ...
        LabelText.Text = FEB.PaymentTerms.Text(_PaymentTerms, Current.Session.Lang)
    ...
    Private Async Sub BankToolStripMenuItem_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BankToolStripMenuItem.Click
        If _PaymentTerms.Iban IsNot Nothing Then
            ...
            Dim oBank = Await FEB.IbanStructure.GetBank(_PaymentTerms.Iban.Digits, exs)
            If exs.Count = 0 Then
    ...
    Private Async Sub BranchToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles BranchToolStripMenuItem.Click
        If _PaymentTerms.Iban IsNot Nothing Then
            Dim exs As New List(Of Exception)
            Dim oBranch = Await FEB.IbanStructure.GetBankBranch(_PaymentTerms.Iban.Digits, exs)
            If exs.Count = 0 Then
    ...
    Private Async Sub RefreshIban(sender As Object, e As MatEventArgs)
        Dim exs As New List(Of Exception)
        Dim oIban = Await FEB.Iban.FromContact(exs, _Contact, mTipus)
        If exs.Count = 0 Then
    ...
    Private Async Sub Xl_Iban1_AfterUpdate(sender As Object, e As MatEventArgs) Handles Xl_Iban1.AfterUpdate
        Dim exs As New List(Of Exception)
        Dim oIban As DTOIban = e.Argument
        oIban.Titular = _Contact
        oIban.Cod = mTipus
        oIban.Emp = GlobalVariables.Emp.Trimmed
        If oIban.FchFrom = Nothing Then oIban.FchFrom = DTO.GlobalVariables.Today()
