Public Class Xl_MediaResources
    ...
    Public Shadows Sub Load(oProduct As DTOProduct, items As List(Of DTOMediaResource))
        ...
        For Each item As DTOMediaResource In items
            ...
            ListView1.LargeImageList.Images.Add(item.Hash, LegacyHelper.ImageHelper.Converter(item.Thumbnail))
        Next
    ...
    Private Sub Do_AddNew()
        Dim oFrm As New Frm_MediaResource(_Product)
    ...
    Private Async Sub Xl_MediaResources_DragDrop(sender As Object, e As DragEventArgs) Handles ListView1.DragDrop
        ...
        If DragDropHelper.GetDroppedMediaResources(e, oMediaResources, Current.Session.User, exs) Then
            If oMediaResources.Count > 0 Then
                Dim tmp = Await FEB.MediaResource.FromHash(exs, oMediaResources.First().Hash)
                If exs.Count = 0 Then
                    If tmp Is Nothing Then
                        ...
                        oFrm.Show()
                    Else
                        Dim oFrm As New Frm_MediaResource(oMediaResources.First)
                        AddHandler oFrm.AfterUpdate, AddressOf RefreshRequest
                        oFrm.Show()
                    End If
                Else
    ...
    Private Async Function Upload(exs As List(Of Exception), oMediaResources As List(Of DTOMediaResource)) As Task(Of Boolean)
        ...
        For Each item As DTOMediaResource In oMediaResources
            With item
                .Products.Add(New Models.Base.GuidNom(_Product.Guid, _Product.FullNom()))
                .UsrLog.usrLastEdited = Current.Session.User
            End With

            Await FEB.MediaResource.Update(item, exs)
        Next
