Public Class Xl_DocfileSrcs
    ...
    Private _specificCods As List(Of DTODocFile.Cods)
    ...
    Public Shadows Sub Load(values As List(Of DTODocFileSrc), Optional oDefaultValue As DTODocFileSrc = Nothing, Optional oSelectionMode As DTO.Defaults.SelectionModes = DTO.Defaults.SelectionModes.browse, Optional specificCods As List(Of DTODocFile.Cods) = Nothing)
    ...
    Private Sub SetContextMenu()
        ...
        End If
        If _specificCods Is Nothing Then
            oContextMenu.Items.Add("afegir", Nothing, AddressOf Do_AddNew)
        Else
            For Each cod In _specificCods
                oContextMenu.Items.Add(AddSpecificMenuItem(cod))
            Next
        End If
    ...
    Private Function AddSpecificMenuItem(cod As DTODocFile.Cods) As ToolStripMenuItem
        Dim nom = cod.ToString()
        Select Case cod
        ...
        With retval
            .Text = String.Format("afegir {0}", nom)
            .Tag = cod
        End With
        AddHandler retval.Click, AddressOf Do_AddSpecific
        Return retval
    End Function

    Private Sub Do_AddSpecific(sender As Object, e As EventArgs)
        Dim oMenuItem As ToolStripMenuItem = sender
        If oMenuItem.Tag Is Nothing Then
            RaiseEvent RequestToAddNew(Me, MatEventArgs.Empty)
        Else
            RaiseEvent RequestToAddNew(Me, New MatEventArgs(oMenuItem.Tag))
        End If
    End Sub
