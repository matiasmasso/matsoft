    ...
    Private Sub DataGridView1_CellFormatting(sender As Object, e As DataGridViewCellFormattingEventArgs) Handles MyBase.CellFormatting
        Select Case e.ColumnIndex
            Case Cols.ico
                ...
                'Dim oThumbnail = FEB.ProductSku.ThumbnailSync(exs, oSku, 70, 80)
                Dim oThumbnailBytes = oControlItem.Source.Thumbnail
                If oThumbnailBytes IsNot Nothing Then
                    Dim oThumbnail = LegacyHelper.ImageHelper.FromBytes(oThumbnailBytes)
                    e.Value = LegacyHelper.ImageHelper.GetThumbnailToFill(oThumbnail, 70, 80)
                End If
            Case Cols.Noms
                Dim oRow As DataGridViewRow = DirectCast(sender, DataGridView).Rows(e.RowIndex)
                Dim oControlItem As ControlItem = oRow.DataBoundItem
                If oControlItem.Source.Product.obsoleto Then
