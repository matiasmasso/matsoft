Public Class Xl_ProductSkusExtended
    ...
    Private Sub Refresca()
        ...
        For Each oItem As DTOProductSku In oFilteredValues
            If _ShowObsolets Or oItem.ObsoletoConfirmed = Nothing Or oItem.Stock <> 0 Or oItem.Clients <> 0 Or oItem.Proveidors <> 0 Then
    ...
    Private Sub Do_Excel()
        ...
        Dim oSheet As New MatHelper.Excel.Sheet("Stocks")
        With oSheet
            .addColumn("ref", MatHelper.Excel.Cell.NumberFormats.W50)
            .addColumn("nom", MatHelper.Excel.Cell.NumberFormats.W50)
            .addColumn("stock", MatHelper.Excel.Cell.NumberFormats.Integer)
            .addColumn("retail", MatHelper.Excel.Cell.NumberFormats.Euro)
        End With
        ...
        Select Case e.ColumnIndex
            ...
            Case Cols.LastProductionIco
                ...
                If oProductSku.obsoleto Then
                    If oProductSku.ObsoletoConfirmed <> Nothing Then
                        e.Value = My.Resources.del
                    Else
                        e.Value = My.Resources.aspa
                    End If
                ElseIf oProductSku.LastProduction Then
            ...
            With value
                ...
                Select Case oBundleCod
                    Case DTOProductSku.BundleCods.parent
                        _Stock = value.BundleSkus.Min(Function(x) x.Sku.Stock)
                        _Clients = value.BundleSkus.Max(Function(x) x.Sku.Clients - (x.Sku.ClientsAlPot + x.Sku.ClientsEnProgramacio))
                        _Proveidors = value.BundleSkus.Min(Function(x) x.Sku.Proveidors)
                    Case DTOProductSku.BundleCods.none, DTOProductSku.BundleCods.child
                        ...
                        If Not ShowRealStocks Then
                            _Stock = _Stock - .ClientsBlockStock 'IIf(_Stock > .ClientsBlockStock, _Stock - .ClientsBlockStock, 0)
                            _Clients = _Clients - .ClientsBlockStock ' IIf(_Clients > .ClientsBlockStock, _Clients - .ClientsBlockStock, 0)
                            '_Proveidors = _Proveidors - .ClientsBlockStock ' IIf(_Proveidors > .ClientsBlockStock, _Proveidors - .ClientsBlockStock, 0)
                        End If
                End Select
            End With
        End Sub
