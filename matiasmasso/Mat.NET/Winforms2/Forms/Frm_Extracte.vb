Public Class Frm_Extracte
    ...
    Private Async Sub Form1_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        Select Case _Modalitat
            Case Modalitats.SingleAccount
                ...
                Dim oExercicis = Await FEB.Exercicis.All(exs, Current.Session.Emp, oCta:=CurrentCta)
                If exs.Count = 0 Then
                    ...
                    Else
                        ...
                        If CurrentExercici() IsNot Nothing Then
                            oContacts = Await FEB.Contacts.All(exs, CurrentExercici, _Cta)
                            If exs.Count > 0 Then
                                UIHelper.WarnError(exs)
                            End If
                        End If

                        Await oXl_Contacts.Load(oContacts)
            ...
            Case Modalitats.SingleContact
                ...
                Dim oExercicis = Await FEB.Exercicis.All(exs, Current.Session.Emp, CurrentContact)
                If exs.Count = 0 Then
                    If oExercicis.Count = 0 Then
                        MsgBox("no hi han comptes registrades a aquest nom", MsgBoxStyle.Information, "Mat.NET")
                        Me.Close()
    ...
    Private Async Function LoadCtas() As Task
        Dim exs As New List(Of Exception)
        Dim oCtas = Await FEB.PgcCtas.All(exs, CurrentExercici, CurrentContact)
        If exs.Count = 0 Then
    ...
    Private Async Function LoadCcbs() As Task
        ...
        Dim oCcbs = Await FEB.Ccbs.All(exs, CurrentExercici, CurrentCta, CurrentContact)
    ...
    Private Async Sub Xl_Exercicis1_onItemSelected(sender As Object, e As MatEventArgs) Handles Xl_Exercicis1.ValueChanged
        ...
        Select Case _Modalitat
            Case Modalitats.SingleAccount
                ...
                Dim oContacts = Await FEB.Contacts.All(exs, oExercici, _Cta)
                If exs.Count = 0 Then
                    Await oXl_Contacts.Load(oContacts)
                    Await LoadCcbs()
                Else
    ...
    Private Async Sub onContactChanged(sender As Object, e As MatEventArgs)
        ...
        Dim exs As New List(Of Exception)
        Dim oCcbs = Await FEB.Ccbs.All(exs, CurrentExercici, _Cta, oContact)
        If exs.Count = 0 Then
            Xl_Extracte_Ccbs1.Load(oCcbs, _SelectionMode)
        Else
            UIHelper.WarnError(exs)
    ...
    Private Async Function RefrescaPnds() As Task
        ...
        If _Contact IsNot Nothing Then
            Dim oPnds = Await FEB.Pnds.All(exs, Current.Session.Emp, _Contact)
            Dim oImpagats = Await FEB.Impagats.All(exs, Current.Session.Emp, _Contact)
            If exs.Count = 0 Then
    ...
    Private Sub Xl_Extracte_Ccbs1_RequestToToggleProgressBar(sender As Object, e As MatEventArgs) Handles Xl_Extracte_Ccbs1.RequestToToggleProgressBar
