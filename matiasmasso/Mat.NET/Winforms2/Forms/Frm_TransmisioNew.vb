Public Class Frm_TransmisioNew
    ...
    Private Async Sub Frm_TransmisioNew_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        Dim oDeliveries = Await FEB.Deliveries.PendentsDeTransmetre(GlobalVariables.Emp.Mgz, exs)
        If exs.Count = 0 Then
            Xl_Transmisio_SelectAlbs1.Load(oDeliveries)
            TextBoxEmail.Text = Await FEB.Default.EmpValue(Current.Session.Emp, DTODefault.Codis.EmailTransmisioVivace, exs)
            If exs.Count = 0 Then
                SetStatus()
                SetMenu()
            Else
    ...
    Private Async Sub ButtonOk_Click(sender As Object, e As EventArgs) Handles ButtonOk.Click
        ...
        If Await FEB.Transmisio.Update(oTransmisio, exs) Then
            If CheckBoxEmail.Checked Then
                If Await FEB.Transmisio.Send(exs, oTransmisio, TextBoxEmail.Text) Then
    ...
    Private Sub SetStatus()
        ...
        Else
            ...
            TextBoxStatus.Text = String.Format("{0} albarans per {1}", items.Count, DTOAmt.CurFormatted(FEB.Deliveries.Sum(items)))
