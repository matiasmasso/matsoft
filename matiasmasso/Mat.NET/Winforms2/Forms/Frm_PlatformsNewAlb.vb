Public Class Frm_PlatformsNewAlb
    ...
    Private Async Sub Frm_PlatformsNewAlb_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If _Holding Is Nothing Then
            _AllPurchaseOrders = Await FEB.PurchaseOrders.PendingForPlatforms(exs, _Customer)
        Else
            _AllPurchaseOrders = Await FEB.PurchaseOrders.PendingForPlatforms(exs, _Holding)
        End If

        If exs.Count = 0 Then
            _StocksAvailable = Await StocksAvailable(exs)
            If exs.Count = 0 Then
                _Groups = Await FEB.ECITransmGroups.All(exs)
    ...
    Private Function CurrentDeliveries() As List(Of DTODelivery)
        ...
        For Each oDelivery In oDeliveries
            oDelivery.Import = oDelivery.totalCash()
        Next
    ...
    Private Async Sub ExcelGrupsTransmisióToolStripMenuItem_Click(sender As Object, e As EventArgs)
        Dim exs As New List(Of Exception)
        Dim retval As New MatHelper.Excel.Sheet("El Corte Ingles")
        With retval
            .AddColumn("Grup")
            .AddColumn("Centre")
            .AddColumn("Comanda")
        ...
        Dim oGroups = Await FEB.ECITransmGroups.All(exs)
    ...
    Private Async Sub ButtonOk_Click(sender As Object, e As EventArgs) Handles ButtonOk.Click
        ...
        If Await FEB.AlbBloqueig.BloqueigAllStart(Current.Session.User, DTOAlbBloqueig.Codis.ALB, exs) Then
            Dim oUpdatedDeliveries As New List(Of DTODelivery)
            For Each odelivery In oDeliveries
                Dim exs2 As New List(Of Exception)
                Dim id As Integer = Await FEB.Delivery.Update(exs2, odelivery)
        ...
        If Not Await FEB.AlbBloqueig.BloqueigAllEnd(Current.Session.User, DTOAlbBloqueig.Codis.ALB, exs) Then
    ...
    Private Async Function StocksAvailable(exs As List(Of Exception)) As Task(Of List(Of DTOStockAvailable))
        ...
        If _Holding Is Nothing Then
            retval = Await FEB.PurchaseOrders.StocksAvailableForPlatforms(exs, GlobalVariables.Emp, _Customer)
        Else
            retval = Await FEB.PurchaseOrders.StocksAvailableForPlatforms(exs, GlobalVariables.Emp, _Holding)
        End If
        Return retval
    End Function
    ...
    Private Async Sub PreviewAlbaranesToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles PreviewAlbaranesToolStripMenuItem.Click
        Dim exs As New List(Of Exception)
        Dim oDeliveries = CurrentDeliveries()
        Dim oStream As Byte() = Await FEB.Deliveries.Pdf(exs, oDeliveries, False)
        If exs.Count = 0 Then
