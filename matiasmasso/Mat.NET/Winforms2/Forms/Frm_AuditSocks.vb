Public Class Frm_AuditSocks
    ...
    Private Async Sub Frm_AuditSocks_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Xl_Yea1.Yea = DTO.GlobalVariables.Today().Year - 1
        Await refresca()
    End Sub
    ...
    Private Async Function refresca() As Task
        ...
        Dim items = Await FEB.AuditStocks.All(exs, oExercici)
        If exs.Count = 0 Then
    ...
    Private Async Sub onColumnsMapped(sender As Object, e As MatEventArgs)
        ...
        If exs.Count = 0 Then
            If Await FEB.AuditStocks.Update(items, exs) Then
                Await refresca()
            Else
                UIHelper.WarnError(exs)
            End If
    ...
    Private Function GetItemsFromSheet(oSheet As MatHelper.Excel.Sheet, exs As List(Of Exception)) As List(Of DTOAuditStock)
        ...
        For i As Integer = iFirstRow To oSheet.Rows.Count - 1
            Dim oRow As MatHelper.Excel.Row = oSheet.Rows(i)
    ...
    Private Async Sub AsignarUltimCostToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles AsignarUltimCostToolStripMenuItem.Click
        ...
        For Each item In items
            If item.Sku IsNot Nothing Then
                Dim oAmt = Await FEB.ProductSku.LastCost(exs, GlobalVariables.Emp, item.Sku, item.FchEntrada)
                If oAmt IsNot Nothing Then
                    item.Cost = oAmt.eur
                    Await FEB.AuditStock.Update(item, exs)
