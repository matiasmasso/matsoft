Public Class Frm_Usuari
    ...
    Private Sub Frm_Properties_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If FEB.User.Load(exs, _User) Then
            With _User
                ...
                UIHelper.LoadComboFromEnum(ComboBoxSource, GetType(DTOUser.Sources), "(seleccionar font)", DTOUser.Sources.Manual)
                ComboBoxSource.SelectedValue = .Source

                If .FchCreated = Nothing Then
                    DateTimePickerFchCreated.Value = DTO.GlobalVariables.Now()
                Else
    ...
    Private Async Sub ButtonDel_Click(sender As Object, e As EventArgs) Handles ButtonDel.Click
        ...
        If rc = MsgBoxResult.Ok Then
            If Await FEB.User.Delete(exs, _User) Then
                RaiseEvent AfterUpdate(Me, New MatEventArgs(_User))
                Me.Close()
            Else
    ...
    Private Async Sub TabControl1_SelectedIndexChanged(sender As Object, e As EventArgs) Handles TabControl1.SelectedIndexChanged
        ...
        Select Case TabControl1.SelectedIndex
            Case Tabs.Raffles
                'Dim oParticipants As List(Of DTORaffleParticipant) = FEB.RaffleParticipant.FromUser(_User)
                'Xl_RaffleParticipants1.Load(oParticipants, Xl_RaffleParticipants.Modes.FromUsuari)
            Case Tabs.Contacts
                Dim oContacts = Await FEB.User.Contacts(exs, _User)
                If exs.Count = 0 Then
    ...
    Private Sub Xl_Contact_Add1_RequestToAdd(sender As Object, e As MatEventArgs) Handles Xl_Contact_Add1.RequestToAdd
        ...
        'Dim exs as New List(Of exception)
        'BLL.BLLUser.AddContact(_User, oContact, exs)
        'Dim oContacts As List(Of DTOContact) = febl.User.Contacts(_User)
