...
Public Class Frm_PremiumLine
    ...
    Private Sub Frm_Properties_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If FEB.PremiumLine.Load(_PremiumLine, exs) Then
            With _PremiumLine
                TextBoxNom.Text = .nom
                If .fch > DateTimePicker1.MinDate Then
                    DateTimePicker1.Value = .fch
                End If
                RadioButtonDefaultExclude.Checked = .Codi = DTOPremiumLine.Codis.defaultExclude
                RadioButtonDefaultInclude.Checked = .Codi = DTOPremiumLine.Codis.defaultInclude
    ...
    Private Sub Control_Changed(sender As Object, e As EventArgs) Handles _
        TextBoxNom.TextChanged,
         DateTimePicker1.ValueChanged,
          RadioButtonDefaultExclude.CheckedChanged,
           RadioButtonDefaultInclude.CheckedChanged
    ...
    Private Async Sub ButtonOk_Click(sender As Object, e As EventArgs) Handles ButtonOk.Click
        With _PremiumLine
            ...
            .Codi = If(RadioButtonDefaultExclude.Checked, DTOPremiumLine.Codis.defaultExclude, DTOPremiumLine.Codis.defaultInclude)
            .products = Xl_ProductCategories1.Values
        End With

        Dim exs As New List(Of Exception)
        UIHelper.ToggleProggressBar(Panel1, True)
        If Await FEB.PremiumLine.Update(_PremiumLine, exs) Then
    ...
    Private Async Sub ButtonDel_Click(sender As Object, e As EventArgs) Handles ButtonDel.Click
        ...
        If rc = MsgBoxResult.Ok Then
            If Await FEB.PremiumLine.Delete(_PremiumLine, exs) Then
                RaiseEvent AfterUpdate(Me, New MatEventArgs(_PremiumLine))
                Me.Close()
            Else
    ...
    Private Async Function refrescaCustomers() As Task
        Dim exs As New List(Of Exception)
        _PremiumCustomers = Await FEB.premiumCustomers.All(exs, _PremiumLine)
        If exs.Count = 0 Then
            Xl_PremiumCustomers1.Load(_PremiumCustomers)
        Else
            UIHelper.WarnError(exs)
        End If
    End Function
    ...
    Private Async Function LoadMailings() As Task
        ...
        If _AllEmailRecipients Is Nothing Then
            _AllEmailRecipients = Await FEB.PremiumLine.EmailRecipients(exs, _PremiumLine)
    ...
    Private Sub ButtonExcelMailing_Click(sender As Object, e As EventArgs) Handles ButtonExcelMailing.Click
        ...
        Dim oSheet = FEB.PremiumLine.ExcelEmailRecipients(oRecipients)
        If exs.Count = 0 Then
    ...
    Private Async Sub ExcelToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ExcelToolStripMenuItem.Click
        ...
        Dim oSheet As New MatHelper.Excel.Sheet(_PremiumLine.nom, "M+O Premiun customers")
        For Each item In _PremiumCustomers.Where(Function(x) x.docFile IsNot Nothing).OrderByDescending(Function(x) x.UsrLog.FchLastEdited.Date)
            Dim oRow = oSheet.AddRow()
            With item
                oRow.AddCell(.UsrLog.FchLastEdited.Date)
                ...
                oRow.AddCell(.customer.nomAndNomComercial(), item.docFile.DownloadUrl(True))
            End With
