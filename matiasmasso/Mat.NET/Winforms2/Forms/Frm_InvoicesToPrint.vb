Public Class Frm_InvoicesToPrint
    ...
    Private Async Function refresca() As Task
        ...
        Dim AllItems = Await FEB.Invoices.PrintPending(exs, Current.Session.Emp)
        If exs.Count = 0 Then
    ...
    Private Async Sub ButtonOk_Click(sender As Object, e As EventArgs) Handles ButtonOk.Click
        ...
        Dim exs As New List(Of Exception)
        Await PrintInvoices(oInvoices, Current.Session.User, DTO.GlobalVariables.Now(), AddressOf Xl_ProgressBar1.ShowProgress, exs)
    ...
    Private Async Function PrintInvoices(oInvoices As List(Of DTOInvoice), oUser As DTOUser, DtFch As Date, ShowProgress As ProgressBarHandler, exs As List(Of Exception)) As Task(Of Boolean)
        ...
        Await FEB.Invoices.SendToEmail(oInvoicesForEmail, oUser, DtFch, AddressOf Xl_ProgressBar1.ShowProgress, exs)
        Await FEB.Invoices.SendToEdi(Current.Session.Emp, oInvoicesForEdi, oUser, DtFch, AddressOf Xl_ProgressBar1.ShowProgress, exs)
        Return True
    End Function
    ...
    Private Async Sub Xl_Lookup_PrintedInvoices1_AfterUpdate(sender As Object, e As MatEventArgs)
        Dim exs As New List(Of Exception)
        Dim items = Await FEB.Invoices.PrintPending(exs, Current.Session.Emp, e.Argument)
        If exs.Count = 0 Then
            Dim oSortedItems As List(Of DTOInvoice) = items.OrderBy(Function(x) x.Num).OrderBy(Function(x) x.Fch.Year).ToList
            Xl_InvoicesToPrint1.Load(oSortedItems)
        Else
    ...
    Private Async Sub Xl_InvoicesToPrint1_RequestToCheckOut(sender As Object, e As MatEventArgs) Handles Xl_InvoicesToPrint1.RequestToCheckOut
        ...
        Dim exs As New List(Of Exception)
        Xl_ProgressBar1.Load("Descartant " & DTOInvoice.Caption(oInvoices))
        If Await FEB.Invoices.CheckOutFromPrinting(exs, oInvoices, Current.Session.User, DTO.GlobalVariables.Now(), AddressOf Xl_ProgressBar1.ShowProgress) Then
            Await refresca()
