Public Class Frm_ProductDescription
    ...
    Private _CategoryInheritsFrom As DTOProductCategory
    ...
    Private Async Sub Frm_ProductDescription_Load(sender As Object, e As EventArgs) Handles Me.Load
        Dim exs As New List(Of Exception)
        If _Product.IsNew Then
            Me.Text = "Noms i Descripcions de nou producte"
        Else
            FEB.Product.Load(_Product, exs)
        ...
        If exs.Count = 0 Then
            With _Product
                ...
                TextBoxSeoTitleEsp.Text = .SeoTitleOrDefault().Esp
                TextBoxSeoTitleCat.Text = .SeoTitleOrDefault().Cat
                TextBoxSeoTitleEng.Text = .SeoTitleOrDefault().Eng
                TextBoxSeoTitlePor.Text = .SeoTitleOrDefault().Por
                ...
                If TypeOf _Product Is DTOProductSku Then
                    ...
                    CheckBoxInheritContent.Checked = oSku.Hereda
                    Await refrescaSkuInheritance(exs, oSku)
    ...
    Private Async Function refrescaSkuInheritance(exs As List(Of Exception), oSku As DTOProductSku) As Task
        If CheckBoxInheritExtracte.Checked Then
            If _CategoryInheritsFrom Is Nothing Then
                _CategoryInheritsFrom = Await FEB.ProductCategory.Find(exs, oSku.Category.Guid)
            End If

            TextBoxExcerptEsp.Text = _CategoryInheritsFrom.Excerpt.Esp
            TextBoxExcerptCat.Text = _CategoryInheritsFrom.Excerpt.Cat
            TextBoxExcerptEng.Text = _CategoryInheritsFrom.Excerpt.Eng
            TextBoxExcerptPor.Text = _CategoryInheritsFrom.Excerpt.Por

            Xl_LangsTextDsc.Load(_CategoryInheritsFrom.Content)

            TextBoxCanonicalUrlEsp.Text = _CategoryInheritsFrom.UrlCanonicas.CanonicalUrl(DTOLang.ESP) '.Urls.Canonical(DTOLang.ESP, True)
            TextBoxCanonicalUrlCat.Text = _CategoryInheritsFrom.UrlCanonicas.CanonicalUrl(DTOLang.CAT) '.Urls.Canonical(DTOLang.CAT, True)
            TextBoxCanonicalUrlEng.Text = _CategoryInheritsFrom.UrlCanonicas.CanonicalUrl(DTOLang.ENG) '.Urls.Canonical(DTOLang.ENG, True)
            TextBoxCanonicalUrlPor.Text = _CategoryInheritsFrom.UrlCanonicas.CanonicalUrl(DTOLang.POR) '.Urls.Canonical(DTOLang.POR, True)
        Else
            TextBoxExcerptEsp.Text = oSku.Excerpt.Esp
            TextBoxExcerptCat.Text = oSku.Excerpt.Cat
            TextBoxExcerptEng.Text = oSku.Excerpt.Eng
            TextBoxExcerptPor.Text = oSku.Excerpt.Por

            Xl_LangsTextDsc.Load(oSku.Content)

            TextBoxCanonicalUrlEsp.Text = oSku.UrlCanonicas.CanonicalUrl(DTOLang.ESP) '.Urls.Canonical(DTOLang.ESP, True)
            TextBoxCanonicalUrlCat.Text = oSku.UrlCanonicas.CanonicalUrl(DTOLang.CAT) '.Urls.Canonical(DTOLang.CAT, True)
            TextBoxCanonicalUrlEng.Text = oSku.UrlCanonicas.CanonicalUrl(DTOLang.ENG) '.Urls.Canonical(DTOLang.ENG, True)
            TextBoxCanonicalUrlPor.Text = oSku.UrlCanonicas.CanonicalUrl(DTOLang.POR) '.Urls.Canonical(DTOLang.POR, True)
        End If

        TextBoxExcerptEsp.Enabled = Not CheckBoxInheritExtracte.Checked
        TextBoxExcerptCat.Enabled = Not CheckBoxInheritExtracte.Checked
        TextBoxExcerptEng.Enabled = Not CheckBoxInheritExtracte.Checked
        TextBoxExcerptPor.Enabled = Not CheckBoxInheritExtracte.Checked

        Xl_LangsTextDsc.Enabled = Not CheckBoxInheritExtracte.Checked

        TextBoxCanonicalUrlEsp.Enabled = Not CheckBoxInheritExtracte.Checked
        TextBoxCanonicalUrlCat.Enabled = Not CheckBoxInheritExtracte.Checked
        TextBoxCanonicalUrlEng.Enabled = Not CheckBoxInheritExtracte.Checked
        TextBoxCanonicalUrlPor.Enabled = Not CheckBoxInheritExtracte.Checked
    End Function
    ...
    Private Async Sub ButtonOk_Click(sender As Object, e As EventArgs) Handles ButtonOk.Click
        ...
        Else
            ...
            If TypeOf _Product Is DTOProductBrand Then
                ...
                If FEB.ProductBrand.Load(oBrand, exs) Then
                    ReadFromForm()
                    If Await FEB.ProductBrand.Update(oBrand, exs) Then
            ...
            ElseIf TypeOf _Product Is DTOProductCategory Then
                ...
                If FEB.ProductCategory.Load(oCategory, exs) Then
                    ReadFromForm()
                    If Await FEB.ProductCategory.Update(oCategory, exs) Then
                        RaiseEvent AfterUpdate(Me, New MatEventArgs(_Product))
                        Me.Close()
                    Else
                        UIHelper.ToggleProggressBar(Panel1, False)
            ...
            ElseIf TypeOf _Product Is DTODept Then
                ...
                If FEB.Dept.Load(oDept, False, exs) Then
                    ReadFromForm()
                    If Await FEB.Dept.Upload(oDept, exs) Then
                        RaiseEvent AfterUpdate(Me, New MatEventArgs(_Product))
                        Me.Close()
                    Else
                        UIHelper.ToggleProggressBar(Panel1, False)
    ...
    Private Sub ReadFromForm()
        With _Product
            ...
            End With
            With .Content
                .Esp = Xl_LangsTextDsc.Value.Esp
                .Cat = Xl_LangsTextDsc.Value.Cat
                .Eng = Xl_LangsTextDsc.Value.Eng
                .Por = Xl_LangsTextDsc.Value.Por
            End With

            Dim formSeoTitle = New DTOLangText(.Guid, DTOLangText.Srcs.SeoTitle, TextBoxSeoTitleEsp.Text, TextBoxSeoTitleCat.Text, TextBoxSeoTitleEng.Text, TextBoxSeoTitlePor.Text)
            If formSeoTitle.Equals(.DefaultSeoTitle) Then
                .SeoTitle.clear()
            Else
    ...
    Private Async Sub CheckBoxInherit_CheckedChanged(sender As Object, e As EventArgs) Handles CheckBoxInheritExtracte.CheckedChanged, CheckBoxInheritContent.CheckedChanged
        Dim exs As New List(Of Exception)
        If _AllowEvents Then
            _AllowEvents = False
            ...
            If _Product.SourceCod = DTOProduct.SourceCods.Sku Then
                Await refrescaSkuInheritance(exs, _Product)
                If exs.Count = 0 Then
                    ButtonOk.Enabled = True
                    _AllowEvents = True
                Else
    ...
    Private Sub TextBoxSeoTitle_TextChanged(sender As Object, e As EventArgs) Handles TextBoxSeoTitleEsp.TextChanged,
            TextBoxSeoTitleCat.TextChanged,
            TextBoxSeoTitleEng.TextChanged,
            TextBoxSeoTitlePor.TextChanged

        UpdateCounter(TextBoxSeoTitleEsp, TextBoxSeoTitleCountEsp)
        UpdateCounter(TextBoxSeoTitleCat, TextBoxSeoTitleCountCat)
        UpdateCounter(TextBoxSeoTitleEng, TextBoxSeoTitleCountEng)
        UpdateCounter(TextBoxSeoTitlePor, TextBoxSeoTitleCountPor)
    End Sub

    Private Sub UpdateCounter(TextControl As TextBox, CounterControl As TextBox)
        Dim count As Integer = TextControl.Text.Length
        CounterControl.Text = count.ToString
        Select Case count
            Case < 55
                CounterControl.BackColor = Color.FromKnownColor(KnownColor.Control)
            Case > 65
                CounterControl.BackColor = Color.LightSalmon
            Case Else
                CounterControl.BackColor = Color.Yellow
        End Select
    End Sub
End Class