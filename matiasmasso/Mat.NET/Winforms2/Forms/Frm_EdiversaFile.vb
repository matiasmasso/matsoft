Public Class Frm_EdiversaFile
    ...
    Private Async Function refresca() As Task
        ...
        If FEB.EdiversaFile.Load(_File, exs) Then
            LoadProperties()
            LoadCodiGrid()
    ...
    Private Async Function LoadExceptions() As Task
        Dim exs As New List(Of Exception)
        Dim oEdiversaExceptions = Await FEB.EdiversaExceptions.All(_File, exs)
        If exs.Count = 0 Then
    ...
    Private Async Sub ButtonOk_Click(sender As Object, e As System.EventArgs) Handles ButtonOk.Click
        ...
        _File.Stream = sText
        Dim exs As New List(Of Exception)
        If Await FEB.EdiversaFile.Update(exs, _File) Then
    ...
    Private Async Sub RestoreToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles RestoreToolStripMenuItem.Click
        Dim exs As New List(Of Exception)
        Dim oFile = Await FEB.EdiversaFile.Restore(exs, GlobalVariables.Emp, _File)
        If exs.Count = 0 Then
            _File = oFile
            If Await FEB.EdiversaFile.Update(exs, _File) Then
                Await refresca()
