Public Class Frm_PurchaseOrder_Proveidor
    ...
    Private Async Sub ButtonOk_Click(sender As Object, e As EventArgs) Handles ButtonOk.Click
        If Xl_PurchaseOrderItems1.MultiTruck Then
            Await SaveMultiTruckOrders()
        Else
            Await SaveSingleOrder()
        End If
    End Sub

    Private Async Function SaveSingleOrder() As Task
        Dim exs As New List(Of Exception)
        ...
        If SetOrderFromForm(exs) Then
            ...
            If exs.Count = 0 Then
                ...
                If wasNew Then
                    ...
                    pOrder = Await FEB.PurchaseOrder.Update(exs, _PurchaseOrder)
    ...
    Private Async Function SaveMultiTruckOrders() As Task
        Dim exs As New List(Of Exception)
        UIHelper.ToggleProggressBar(Panel1, True)
        'Dim oOrders As New List(Of DTOPurchaseOrder)
        For Each oTruck In Xl_PurchaseOrderItems1.Trucks
            Dim oOrder = DTOPurchaseOrder.Factory(GlobalVariables.Emp, _PurchaseOrder.Proveidor, Current.Session.User, DateTimePickerFch.Value,, TextBoxObs.Text)
            With oOrder
                ...
                For Each item In oTruck.Items
                    Dim oItem As New DTOPurchaseOrderItem(item.Guid)
                    With oItem
                        .Bundle = item.Bundle
                        .ChargeCod = item.ChargeCod
                        .CustomLin = item.CustomLin
                        ...
                        .Price = item.Price
                        .PurchaseOrder = New DTOPurchaseOrder(oOrder.Guid)
                        .Qty = item.Qty
                        .RepCom = item.RepCom
                        .Sku = New DTOProductSku(item.Sku.Guid)
                    End With
            ...
            Dim pOrder = Await FEB.PurchaseOrder.Update(exs, oOrder)
            Dim oPdf As New LegacyHelper.PdfAlb2({oOrder}.ToList, True)
            Dim oCert = Await FEB.Cert.Find(GlobalVariables.Emp.Org, exs)
            Dim oStream = oPdf.Stream(exs, oCert)
            Dim oDocfile = LegacyHelper.DocfileHelper.Factory(exs, oStream)
            oOrder.DocFile = oDocfile
            'desa un segon cop dons no podem redactar el pdf abans de saber el número de la comanda
            oOrder.IsNew = False 'perque gestioni l'update correctament el segon cop
            pOrder = Await FEB.PurchaseOrder.Update(exs, oOrder)
        Next
...
#End Region
    ...
    Private Sub Frm_PurchaseOrder_Proveidor_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        Dim exs As New List(Of Exception)
        If Not FEB.AlbBloqueig.BloqueigEnd(Current.Session.User, _PurchaseOrder.Proveidor, DTOAlbBloqueig.Codis.PDC, exs) Then
            e.Cancel = True
            UIHelper.WarnError(exs)
        End If
    End Sub
