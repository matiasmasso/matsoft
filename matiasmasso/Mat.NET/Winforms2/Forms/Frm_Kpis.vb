Public Class Frm_Kpis
    ...
    Private Sub Frm_Kpis_Load(sender As Object, e As EventArgs) Handles Me.Load
        _yearFrom = DTO.GlobalVariables.Today().Year - 2
    ...
    Private Async Function Balance(exs As List(Of Exception)) As Task(Of DTOBalance)
        If _Balance Is Nothing Then
            _Balance = Await FEB.Balance.Tree(exs, GlobalVariables.Emp, DTO.GlobalVariables.Today().Year - 2)
        End If
        Return _Balance
    End Function
    ...
    Private Async Function Comandes(exs As List(Of Exception)) As Task(Of List(Of DTOKpi))
        If _Comandes Is Nothing Then
            _Comandes = Await FEB.PurchaseOrderItems.Kpis(exs, GlobalVariables.Emp, DTO.GlobalVariables.Today().Year - 2)
        End If
        Return _Comandes
    End Function
    ...
    Private Async Function Impagats(exs As List(Of Exception)) As Task(Of List(Of DTOKpi))
        If _Impagats Is Nothing Then
            _Impagats = Await FEB.Impagats.Kpis(exs, GlobalVariables.Emp, DTO.GlobalVariables.Today().Year - 2)
            Dim indexImpagats = DTOKpi.Factory(DTOKpi.Ids.Index_Impagats)
            Dim oRange = DTOYearMonth.Range(New DTOYearMonth(DTO.GlobalVariables.Today().Year - 2, 1), DTOYearMonth.Current)
