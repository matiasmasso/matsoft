Public Class Frm_Vivace_Stocks
    ...
    Private Function ReadFile(oSheet As MatHelper.Excel.Sheet, exs As List(Of Exception)) As List(Of DTOVivaceStock)
        Dim retval As New List(Of DTOVivaceStock)

        Dim idx As Integer
        Dim startRow = If(oSheet.ColumnHeadersOnFirstRow, 1, 0)
        For Each oRow As MatHelper.Excel.Row In oSheet.Rows
            idx += 1
            If Not (idx = 1 And oSheet.ColumnHeadersOnFirstRow) Then
    ...
    Private Async Function LoadFchs() As Task
        ...
        Dim fchs = Await FEB.VivaceStocks.Fchs(exs)
        If exs.Count = 0 Then
            With ComboBoxFch
                .DataSource = Await FEB.VivaceStocks.Fchs(exs)
    ...
    Private Async Function refresca() As Task
        ...
        Dim items = Await FEB.VivaceStocks.All(exs, Current.Session.Emp, DtFch)
        If exs.Count = 0 Then
    ...
    Private Async Function Save(DtFch As Date, items As List(Of DTOVivaceStock)) As Task
        Dim exs As New List(Of Exception)
        If Await FEB.VivaceStocks.Update(exs, GlobalVariables.Emp, DtFch, items) Then
            Await LoadFchs()
