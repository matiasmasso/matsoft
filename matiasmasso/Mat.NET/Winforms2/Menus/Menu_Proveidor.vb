...
Public Class Menu_Proveidor
    ...
    Private Function MenuItem_AvisCamion_NoImport() As ToolStripMenuItem
        ...
        Select Case Current.Session.User.Rol.id
            Case DTORol.Ids.superUser, DTORol.Ids.admin, DTORol.Ids.logisticManager, DTORol.Ids.accounts
                ...
                oMenuItem.Text = "Avis Camion (manual)"
            Case Else
                oMenuItem.Enabled = False
        End Select
        Return oMenuItem
    ...
    Private Sub Do_LastPdcs(ByVal sender As Object, ByVal e As System.EventArgs)
        'Dim oPurchaseOrders =await FEB.PurchaseOrders.Headers(exs, Cod:=DTOPurchaseOrder.Codis.Proveidor, Contact:=_Proveidor)
    ...
    Private Async Sub Do_NewAlb(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim exs As New List(Of Exception)
        Dim oDelivery = FEB.Delivery.Factory(exs, _Proveidor, Current.Session.User, GlobalVariables.Emp.Mgz)
        If exs.Count = 0 Then
    ...
    Private Async Sub Do_SellOut(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If exs.Count = 0 Then
            FEB.SellOut.AddFilterValues(oSellout, DTOSellOut.Filter.Cods.Provider, _Proveidors.ToArray)
    ...
    Private Sub Do_NewFra(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim oFrm As New Wz_Proveidor_NewFra(_Proveidor, DTO.GlobalVariables.Today())
        oFrm.Show()
    End Sub
    ...
    Private Sub Do_Margins()
        Dim oFrm As New Frm_Margins(Models.MarginsModel.Modes.Proveidor, _Proveidor)
        oFrm.Show()
    End Sub
    ...
    Private Async Sub Do_ExcelFiresLocals()
        Dim exs As New List(Of Exception)
        Dim oSheet = Await FEB.Incentiu.ExcelFiresLocals(exs, _Proveidor, Current.Session.User)
        If exs.Count = 0 Then
    ...
    Private Async Sub onColumnsMapped(sender As Object, e As MatEventArgs)
        ...
        Dim oCountries As List(Of DTOCountry) = Await FEB.Countries.All(DTOLang.ESP, exs)
        Dim iNotFound As Integer
        Dim iUpdateErrors As Integer
        For Each oRow In oSheet.Rows
            Dim oSku = Await FEB.ProductSku.FromProveidor(exs, _Proveidor, oRow.Cells.First.Content)
            If exs.Count = 0 Then
                ...
                Else
                    If FEB.ProductSku.Load(oSku, exs) Then
                        ...
                        End If
                        If BlProcede Then
                            If Not Await FEB.ProductSku.Update(oSku, exs) Then
                                iUpdateErrors += 1
                            End If
                        End If
                    Else
