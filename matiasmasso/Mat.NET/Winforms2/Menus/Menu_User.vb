...
Public Class Menu_User
    ...
    Private Function MenuItem_Advanced() As ToolStripMenuItem
        ...
        oMenuItem.DropDownItems.Add(MenuItem_HashPwd)
        Return oMenuItem
    End Function
    ...
    Private Function MenuItem_HashPwd() As ToolStripMenuItem
        Dim oMenuItem As New ToolStripMenuItem
        oMenuItem.Text = "copiar Hash pwd"
        Select Case Current.Session.User.Rol.id
            Case DTORol.Ids.superUser
            Case Else
                oMenuItem.Visible = False
        End Select
        AddHandler oMenuItem.Click, AddressOf Do_HashPwd
        Return oMenuItem
    End Function
    ...
    Private Sub Do_CopyPwd(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If FEB.User.IsAllowedToRead(Current.Session.User, oContact) Then
    ...
    Private Async Sub Do_MailPwd(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim exs As New List(Of Exception)
        If Await FEB.User.EmailPassword(_User, exs) Then
    ...
    Private Sub Do_WebPro(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If FEB.User.Load(exs, _User) Then
            ...
            If FEB.User.IsAllowedToRead(Current.Session.User, oContact) Then
                Dim sUrl As String = FEB.UrlHelper.Factory(True, "guid", _User.Guid.ToString(), Current.Session.User.Guid.ToString())
                UIHelper.ShowHtml(sUrl)
            Else
                MsgBox("operacio no autoritzada per politica de privacitat", MsgBoxStyle.Exclamation, "MAT.NET")
                'FEB.MailMessage.MailErr(Current.Session.User, My.User.Name & " browse " & _User.EmailAddress)
            End If
        Else
            UIHelper.WarnError(exs)
        End If
    ...
    Private Async Sub Do_BaixaConsumidor()
        ...
        Dim oUser = Await FEB.User.Find(_User.Guid, exs)
        ...
        Else
            oUser.FchDeleted = DTO.GlobalVariables.Now()
    ...
    Private Async Sub Do_ExcelStocks()
        ...
        Dim oUser = Await FEB.User.Find(_User.Guid, exs)
        If exs.Count = 0 Then
            Dim oSheet = Await FEB.ProductStocks.Excel(exs, Current.Session.Emp, oUser)
    ...
    Private Sub Do_HashPwd()
        UIHelper.CopyToClipboard(_User.HashIdentityPassword())
    End Sub
