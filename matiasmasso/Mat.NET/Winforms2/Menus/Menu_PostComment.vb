Public Class Menu_PostComment
    ...
    Private Function MenuItem_MailAnswer() As ToolStripMenuItem
        ...
        If oAnswering IsNot Nothing Then
            ...
            If FEB.PostComment.Load(exs, _PostComment) Then
    ...
    Private Async Sub Do_Validate(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If FEB.PostComment.Load(exs, _PostComment) Then
            With _PostComment
                ...
                .FchDeleted = Nothing
            End With
            If Await FEB.PostComment.Update(exs, _PostComment) Then
    ...
    Private Async Sub Do_Delete(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim exs As New List(Of Exception)
        If FEB.PostComment.Load(exs, _PostComment) Then
            _PostComment.FchDeleted = DTO.GlobalVariables.Now()
            If Await FEB.PostComment.Delete(exs, _PostComment) Then
                RaiseEvent AfterUpdate(sender, New MatEventArgs(_PostComment))
            Else
                UIHelper.WarnError(exs)
            End If
    ...
    Private Async Sub Do_MailAnswer()
        Dim exs As New List(Of Exception)
        If Await FEB.PostComment.EmailAnswer(exs, GlobalVariables.Emp, _PostComment) Then
    ...
    Private Async Sub Do_Browse()
        ...
        Select Case _PostComment.ParentSource
            Case DTOPostComment.ParentSources.Blog
                Dim oBlogPost = Await FEB.BlogPost.Find(exs, _PostComment.Parent)
                If exs.Count = 0 Then
            ...
            Case DTOPostComment.ParentSources.News, DTOPostComment.ParentSources.Noticia
                ...
                Dim oNoticia = Await FEB.Noticia.Find(exs, _PostComment.Parent)
                If exs.Count = 0 And oNoticia Is Nothing Then exs.Add(New Exception("No s'ha trobat la noticia"))
                If exs.Count = 0 Then
                    UIHelper.ShowHtml(oNoticia.Url().AbsoluteUrl(Current.Session.Lang))
