...
#End Region
    ...
    Private Async Sub Do_ExcelTarifaCost(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If oBrand.proveidor Is Nothing Then
            ...
            Dim otarifaVigent = Await FEB.PriceListItemsSupplier.Vigent(exs, oBrand.proveidor)
    ...
    Private Async Sub Do_ExcelTarifaPvp(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim exs As New List(Of Exception)
        Dim oTarifa = Await FEB.CustomerTarifa.Load(exs, Current.Session.User)
        'TODO: extrau nomes la de la marca
        If exs.Count = 0 Then
            Dim oExcelSheet As MatHelper.Excel.Sheet = oTarifa.Excel(Current.Session.Lang)
        Else
            UIHelper.WarnError(exs)
        End If
    End Sub
    ...
    Private Sub Do_CopyTarifas(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If IsDate(sFch) Then
            Dim sUrl = FEB.CustomerTarifa.TarifaUrl(True, CDate(sFch))
    ...
    Private Async Sub Do_RankingCustomers()
        Dim exs As New List(Of Exception)
        Dim oRanking = Await FEB.Ranking.CustomerRanking(exs, Current.Session.User, _Brands.First)
        If exs.Count = 0 Then
    ...
    Private Async Sub Do_Delete(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If rc = MsgBoxResult.Ok Then
            Dim exs As New List(Of Exception)
            If Await FEB.ProductBrand.Delete(_Brands(0), exs) Then
    ...
    Private Sub Do_StoreLocator()
        Dim oFrm As New Frm_StoreLocator(_Brands.First())
        oFrm.Show()
    End Sub

    Private Async Sub Do_Sellout(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim exs As New List(Of Exception)
        Dim oSellout = Await FEB.SellOut.Factory(exs, Current.Session.User, , DTOSellOut.ConceptTypes.Geo)
        If exs.Count = 0 Then
    ...
    Private Sub Do_Incidencies()
        Dim oFrm As New Frm_Last_Incidencies(_Brands.First)
        oFrm.Show()
    End Sub
    ...
    Private Async Sub Do_CsvEmailsOficialDistributors()
        ...
        If FEB.ProductBrand.Load(oBrand, exs) Then
            Dim oEmails = Await FEB.CliProductsBlocked.DistribuidorsOficialsActiveEmails(exs, oBrand)
            If exs.Count = 0 Then
    ...
    Private Async Sub Do_MailReps()
        ...
        Dim oRepEmails = Await FEB.Reps.Emails(exs, oProduct)
        If exs.Count = 0 Then
    ...
    Private Async Sub Do_RepAdrs()
        ...
        If _Brands.Count > 0 Then
            Dim oReps = Await FEB.Reps.All(exs, _Brands.First)
            For Each oRep As DTORep In oReps
                Dim sMovil = Await FEB.Contact.Movil(oRep, exs)
                sb.Append(FEB.Rep.RaoSocialFacturacio(oRep))
                sb.Append(";")
    ...
    Private Sub Do_Web_ES(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim url As String = _Brands.First.GetUrl(DTOLang.ESP, AbsoluteUrl:=True)
        Process.Start(url)
    End Sub
    Private Sub Do_Web_CA(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim url As String = _Brands.First.GetUrl(DTOLang.CAT, AbsoluteUrl:=True)
        Process.Start(url)
    End Sub
    Private Sub Do_Web_EN(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim url As String = _Brands.First.GetUrl(DTOLang.ENG, AbsoluteUrl:=True)
        Process.Start(url)
    End Sub
    Private Sub Do_Web_PT(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim url As String = _Brands.First.GetUrl(DTOLang.POR, AbsoluteUrl:=True)
        Process.Start(url)
    End Sub

    Private Sub Do_CopyLinkUrlJsonSalePoints()
        Dim sUrl As String = FEB.ProductDistributors.UrlJSON(_Brands.First, True)
        UIHelper.CopyLink(sUrl)
    End Sub

    Private Sub Do_CopyLinkUrlXMLSalePoints()
        Dim sUrl As String = FEB.ProductDistributors.UrlXML(_Brands.First, True)
        UIHelper.CopyLink(sUrl)
    End Sub
End Class
