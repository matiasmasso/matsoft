...
#End Region
    ...
    Private Async Sub frm__Default_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        DTOApp.Current = Await FEB2.App.InitializeAsync(exs, DTOApp.AppTypes.Spv, "https://matiasmasso-api.azurewebsites.net", 55836)
    ...
    Private Async Sub ButtonSpvPrnt_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles ButtonSpvPrnt.Click
        ...
        If IsNumeric(TextBoxSpv.Text) Then
            iId = TextBoxSpv.Text
        Else
            MsgBox("falta el número", MsgBoxStyle.Exclamation, "SERVICIO TECNICO")
            Exit Sub
        End If

        Dim exs As New List(Of Exception)
        Dim oSpv As DTOSpv = Await FEB2.Spv.FromId(GlobalVariables.Emp, IntYea, iId, exs)
        If exs.Count = 0 Then
    ...
    Private Async Sub BtnSpvsNotRead_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSpvsNotRead.Click
        Dim exs As New List(Of Exception)
        Dim oSpvs = Await FEB2.Spvs.SpvsNotRead(Current.Session.User, exs)
        If exs.Count = 0 Then
            If oSpvs.Count = 0 Then
                MsgBox("No hay hojas para bajar", MsgBoxStyle.Information, "SERVICIO TECNICO")
            Else
                PrintSpvs(oSpvs)
            End If
        Else
    ...
    Private Async Sub ButtonSpvOutPrnt_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonSpvOutPrnt.Click
        ...
        Dim exs As New List(Of Exception)
        Dim oDelivery As DTODelivery = Await FEB2.Delivery.FromNum(GlobalVariables.Emp, IntYea, LngId, exs)
        If exs.Count = 0 Then
            If oDelivery.Cod <> 4 Then
                MsgBox("este número no corresponde al taller!", MsgBoxStyle.Exclamation, "SERVICIO TECNICO")
            End If
    ...
    Private Sub ApiToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ApiToolStripMenuItem.Click
        FEB2.Api.UseLocalApi(ApiToolStripMenuItem.Checked)
    End Sub
End Class
