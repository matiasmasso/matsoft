Public Class GoogleMapsHelper
    Protected Shared _APIKey As String = "AIzaSyC3O2n2r1p1w-9JkC-f-yI7HWQfkst053I" ' "AIzaSyBwCj6irp55gfyydVdY_3bz2gnWO9FVeEs" ' "AIzaSyC3O2n2r1p1w-9JkC-f-yI7HWQfkst053I" ' "AIzaSyCUJivhLPW2XSvKRRr_K1Fdlw1UO5QuiU0"
    Protected Shared _StaticImageWidth As Integer = 640
    Protected Shared _StaticImageHeight As Integer = 320


    Shared Function Normalized(oCoordenadas As GeoHelper.Coordenadas) As String
    ...
    Shared Function Url(sAddress As String, sLocationNom As String, sZipCod As String, Optional oCoordenadas As GeoHelper.Coordenadas = Nothing) As String
        ...
        If oCoordenadas IsNot Nothing Then
            If oCoordenadas.latitud <> 0 Then
                retval = Url(oCoordenadas.latitud, oCoordenadas.longitud)
            Else
                retval = sb.ToString
            End If
        Else
    ...
    Shared Function Parameters(Optional coordenadas As GeoHelper.Coordenadas = Nothing, Optional zoom As Integer = 8, Optional size As System.Drawing.Size = Nothing) As List(Of String)
    ...
    Shared Function Markers(oCoordenadas As List(Of GeoHelper.Coordenadas), Optional size As String = "mid", Optional color As String = "blue", Optional label As String = "S") As String
    ...
    Shared Function GeoCode(exs As List(Of Exception), googleText As String) As GeoHelper.Coordenadas
        ...
        Try
            ...
            If WebApiHelper.SendGetRequest(sUrl, parameters, jsonOutput, exs) Then
                Dim data As Newtonsoft.Json.Linq.JObject = Newtonsoft.Json.Linq.JObject.Parse(jsonOutput)
                If data("results").Count = 0 Then
                    Dim sb As New Text.StringBuilder
                    If data("error_message") IsNot Nothing Then sb.Append(data("error_message"))
                    If data("status") IsNot Nothing And data("error_message") IsNot Nothing Then sb.AppendLine()
                    If data("status") IsNot Nothing Then sb.Append(data("status"))
                    Dim err As String = sb.ToString()
                    Throw New Exception(err)
                Else
                    Dim lat As Double = data("results")(0)("geometry")("location")("lat")
                    Dim lng As Double = data("results")(0)("geometry")("location")("lng")
    ...
    Public Class GlGeoCode2
        Public Property results As ResultsClass
        Public Class ResultsClass
            Public Property geometry As GeometryClass
        End Class
    ...
    Shared Function IpLookupUrl(ip As String, width As Integer, height As Integer, Optional AppendApiKey As Boolean = False) As String
        ...
        'Dim oCoordenadas As New GeoHelper.Coordenadas(result("latitude"), result("longitude"))
        Dim oCoordenadas As New GeoHelper.Coordenadas(result("lat"), result("lon"))

        Dim retval As String = GoogleMapsHelper.StaticImageUrl(oCoordenadas, width, height, AppendApiKey)
        Return retval
    End Function

    Shared Function StaticImageUrl(oCoordenadas As GeoHelper.Coordenadas, width As Integer, height As Integer, Optional AppendApiKey As Boolean = False) As String
        Dim sb As New System.Text.StringBuilder
    ...
    Protected Shared Function QueryString(EncodedAddresses As List(Of String)) As String
        ...
        For Each sEncodedAddress In EncodedAddresses
            ...
            sb.Append("label:" & TextHelper.VbChr(idx + 65))
            sb.Append("%7C") 'separator
