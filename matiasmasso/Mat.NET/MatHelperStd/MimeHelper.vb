...
Public Class MimeHelper
    ...
    Shared Function GuessMime(oStream As Byte()) As MimeCods
        ...
        If oStream.Length > 2 Then
            Dim sHeader As String = TextHelper.VbChr(oStream(0)).ToString() & TextHelper.VbChr(oStream(1)).ToString & TextHelper.VbChr(oStream(2)).ToString & TextHelper.VbChr(oStream(3)).ToString & TextHelper.VbChr(oStream(4)).ToString
    ...
    Shared Function IsVideo(oMimeCod As MimeCods) As Boolean
        Dim videos = {MimeCods.Mp4, MimeCods.Mov, MimeCods.Mpg, MimeCods._3Gp}
        Dim retval = videos.Contains(oMimeCod)
        Return retval
    End Function
    Shared Function IsImage(oMimeCod As MimeCods) As Boolean
        Dim videos = {MimeCods.Jpg, MimeCods.Gif, MimeCods.Png, MimeCods.Tif, MimeCods.Tiff, MimeCods.Bmp}
        Dim retval = videos.Contains(oMimeCod)
        Return retval
    End Function
