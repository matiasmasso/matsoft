...
Public Class FileSystemHelper
    ...
    Shared Function StreamToByteArray(inputStream As Stream) As Byte()
    ...
    Shared Function SaveStream(ByVal oBuffer As Byte(), exs As List(Of Exception), ByRef sFilename As String) As Boolean
        ...
        ElseIf sFilename.Contains("\") Then
            ...
            Dim folder = sFilename.Substring(0, sFilename.LastIndexOf("\"))
    ...
    Shared Function SaveStream(exs As List(Of Exception), ByVal oBuffer As Byte(), sFilename As String) As Boolean
        Dim retval As Boolean
    ...
    Shared Function TmpFolder() As String
        Dim retval As String = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) & "\tmp"
    ...
    Shared Function GetPdfTmpFileName(Optional ByVal sFileName As String = "", Optional exs As List(Of Exception) = Nothing) As String
        ...
        Dim sTempPath As String = TmpFolder()
        ...
        If sFileName > "" Then
            Select Case sFileName.Substring(sFileName.Length - 4, 4).ToUpper
                Case ".PDF", ".JPG", ".JPEG", ".GIF", ".PNG", "XLSX", ".ZIP"
                Case Else
                    sFileName += ".pdf"
            End Select
        Else
    ...
    Shared Async Function ReadHtmlAsync(exs As List(Of Exception), url As String) As Task(Of String)
        Dim hc As New HttpClient()
        Dim uri As New Uri(url)
        Dim result = hc.GetStreamAsync(uri)
        Dim vs As Stream = Await result
        Dim am As New StreamReader(vs)
    ...
    Shared Function GetXMLfromByteStream(ByVal oStream As Byte()) As Xml.XmlDocument
        ...
        For i As Integer = iInitCount To oStream.Length - 1
            s = s & TextHelper.VbChr(oStream(i))
        Next
    ...
    Shared Function IsFileLocked(sFilename As String, oFileMode As FileMode, oFileAccess As System.IO.FileAccess, exs As List(Of Exception)) As Boolean
        ...
        Catch ex3 As PathTooLongException
            ...
            exs.Add(New Exception("el nom del fitxer es massa llarg '" & sFilename & "'"))
        Catch ex4 As DirectoryNotFoundException
            'The specified path Is invalid, (for example, it Is on an unmapped drive).
            Dim iPos As Integer = sFilename.LastIndexOf("\")
            Dim sFolder = sFilename.Substring(0, iPos)
            exs.Add(New Exception("no s'ha trobat la carpeta '" & sFolder & "'"))
        Catch ex6 As UnauthorizedAccessException
            ...
            exs.Add(New Exception("acces no autoritzat al fitxer '" & sFilename & "'"))
        ...
        Catch ex9 As NotSupportedException
            ...
            exs.Add(New Exception("format de ubicacio incorrecte per '" & sFilename & "'"))
        Catch ex2 As ArgumentException
            'Path Is a zero-length String, contains only white space, Or contains one Or more invalid characters as defined by InvalidPathChars.
            exs.Add(New Exception("caracters no valids a la ubicació del fitxer '" & sFilename & "'"))
        Finally
    ...
    Shared Function DownloadHtml(exs As List(Of Exception), sUrl As String, ByRef sResult As String, Optional username As String = "", Optional password As String = "") As Boolean
        Dim retval As Boolean
        Dim oStream As MemoryStream = Nothing
        Dim pExs As New List(Of Exception)
        If DownloadStream(pExs, sUrl, oStream, username, password) Then
            oStream.Position = 0
            Try
    ...
    Shared Function DownloadStream(exs As List(Of Exception), ByVal sUrl As String, ByRef oResultStream As MemoryStream, Optional username As String = "", Optional password As String = "") As Boolean
        Dim retval As Boolean
        ...
        Try
            ...
            oRequest.UserAgent = "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/51.0.2704.103 Safari/537.36"
            oRequest.CookieContainer = New System.Net.CookieContainer

            'Dim target As New Uri("https://account.amazon.com")
            'oRequest.CookieContainer.Add(New System.Net.Cookie("sso-cd-state", "eyJ6aXAiOiJERUYiLCJlbmMiOiJBMjU2R0NNIiwiYWxnIjoiQTI1NktXIn0.Ap-na5-ULHxwqC73WcgulaY-nZ6eREj9-9ZWg6N0RzBBBecVrjqG9w.JF531rHG5BDVpo9f.NEaQmuYvEX_jFGqlZflJH5nKjg8tqe-en21RiltS_PnxbgNJwfCCMLQm1Y4yBfFGzv-K-jFKPAEyHWRR_GiqFbnS6-WZCzxfYtMenJFh6-JngBgY7aH5WAYwIsCw64KIas_Dm7oUDE2y7AjfpN09oVqFmxZKCJEAqLj6i4FFos5QXaS8n37nI5Jvfa8pnctKSANDjV-ckfWi2fA2Dv1YUC-mS5bPL5TH1p8.u-Yn6dHv3dvDp9zcDH4Ezg") With {.Domain = target.Host})

            'target = New Uri("https://amazon.co.uk")
            'oRequest.CookieContainer.Add(New System.Net.Cookie("session-id", "262-9568922-1649548") With {.Domain = target.Host})
            'oRequest.CookieContainer.Add(New System.Net.Cookie("session-id-time", "2240066803l") With {.Domain = target.Host})
            'oRequest.CookieContainer.Add(New System.Net.Cookie("session-token", "bKA6WAjjRCKluJz6OIlB1RAc0FALLWiapz87+5PjuQlP2TTA8kA4x1HcgkXAGOOR/z0lw8YtYKDDb8+57hgDKTgeWOGEJpqiPVPQ3+AlsQjDwzF38QQ17Dg2ZMHtnVGhF8GKhx0QkwgolSAHafLgVXxPWB8OXlyUuSGzF8lDeHZAiX8MwGUa6ZIIu8sB687EZAR2YGbgcRaHKYRQLBWuj9ZAAQE7yD38") With {.Domain = target.Host})
            'oRequest.CookieContainer.Add(New System.Net.Cookie("signin-sso-state-gb", "4d9dce05-1d3d-4a45-aace-18012e455842") With {.Domain = target.Host})
            'oRequest.CookieContainer.Add(New System.Net.Cookie("ubid-acbuk", "259-0063198-0037051") With {.Domain = target.Host})

            'target = New Uri("https://sellercentral.amazon.co.uk")
            'oRequest.CookieContainer.Add(New System.Net.Cookie("csm-hit", "tb:s-H220VNC9G2DBNRQ143PZ|1609346804128&t:1609346804366&adb:adblk_no") With {.Domain = target.Host})
