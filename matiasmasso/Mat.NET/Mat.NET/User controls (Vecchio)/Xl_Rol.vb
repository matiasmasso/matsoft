...
#End Region
    ...
    Public Property Rol() As DTORol
        ...
        Set(ByVal Value As DTORol)
            ...
            If mRol Is Nothing Then
                Label1.Text = ""
            Else
                Label1.Text = BLL.BLLRol.Nom(mRol, BLL.BLLApp.Lang)
                ...
                Select Case BLL.BLLSession.Current.User.Rol.id
                    ...
                    Case DTORol.Ids.Accounts, DTORol.Ids.LogisticManager, DTORol.Ids.Operadora, DTORol.Ids.Comercial, DTORol.Ids.Marketing
                        Select Case mRol.Id
                            Case DTORol.Ids.Guest, DTORol.Ids.Comercial, DTORol.Ids.NotSet, DTORol.Ids.Cli
                                DisableChange = False
                        End Select
    ...
    Private Sub Change()
        Dim oNewRol As DTORol = mRol
        If oNewRol Is Nothing Then oNewRol = DefaultRol()
        If root.SelectRol(oNewRol) Then
            Me.Rol = oNewRol
            RaiseEvent AfterUpdate(mRol, EventArgs.Empty)
