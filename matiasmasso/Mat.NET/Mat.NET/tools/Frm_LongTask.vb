...
Public Class Frm_LongTask
    ...
    Private Sub BackgroundWorker_DoWork(sender As Object, e As DoWorkEventArgs)
        ...
        Dim oDrd As SqlClient.SqlDataReader = MatSQL.GetDataReader(SQL, Databases.Maxi)
        Do While oDrd.Read
            ...
            If Not IsDBNull(oDrd("Pdf")) Then
                oByteArray = oDrd("Pdf")
                Dim sHash As String = BLL.CryptoHelper.Hash(oByteArray)
                Dim exs As New List(Of Exception)

                Dim oDocFile As DTODocFile = Nothing
                If BLL_DocFile.LoadFromStream(oByteArray, oDocFile, exs, "w.pdf") Then
        ...
        For Each oPair As KeyValuePair(Of Guid, String) In _DataSource
            ...
            Dim i As Integer = MatSQL.ExecuteNonQuery(SQL, Databases.Maxi, "@Guid", oPair.Key.ToString, "@Hash", oPair.Value)
            _BackgroundWorker.ReportProgress(idx)
        Next
    End Sub
