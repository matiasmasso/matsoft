...
Public Class Frm_Nominas
    ...
    Private Sub LoadGrid()
        ...
        & "FROM CliStaff " _
        & "WHERE CliStaff.EMP=" & App.Current.Emp.Id & " AND " _
    ...
    Private Sub SaveCca(ByVal oRow As DataGridViewRow)
        ...
        Dim oAmtDevengo As New MaxiSrvr.Amt(CDec(oRow.Cells(Cols.Devengat).Value - oRow.Cells(Cols.Dietas).Value))
        Dim oAmtDietas As New MaxiSrvr.Amt(CDec(oRow.Cells(Cols.Dietas).Value))
        Dim oAmtSegSoc As New MaxiSrvr.Amt(CDec(oRow.Cells(Cols.SegSoc).Value))
        Dim oAmtIrpf As New MaxiSrvr.Amt(CDec(oRow.Cells(Cols.Irpf).Value))
        Dim oAmtLiq As New MaxiSrvr.Amt(CDec(oRow.Cells(Cols.LiqCalc).Value))
