...
Public Class Frm_Pnds
    ...
    Private Sub LoadGrid()
        ...
        Dim SQL As String = "SELECT ID, '0' AS WARN, PND.vto, PND.eur, PND.PGCPLAN, PND.Cta, PND.fra, PND.fch, CLX.clx, PND.fpg, PND.Cli " _
        ...
        For Each oRow In oTb.Rows
            If oRow(Cols.Warn) = 0 Then
                oCta = MaxiSrvr.PgcCta.FromNum(CInt(oRow(Cols.Plan)), oRow(Cols.Cta))
                oCcd = New Ccd(MaxiSrvr.Contact.FromNum(mEmp, oRow(Cols.Cli)), Today.Year, oCta)
                BlCuadra = oCcd.Cuadra
                For j = i To oTb.Rows.Count - 1
                    If oTb.Rows(j)(Cols.Cli) = oRow(Cols.Cli) Then
                        If oTb.Rows(j)(Cols.Cta) = oRow(Cols.Cta) Then
