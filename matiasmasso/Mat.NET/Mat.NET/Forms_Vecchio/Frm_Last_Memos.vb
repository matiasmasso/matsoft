...
Public Class Frm_Last_Memos
    ...
    Private Sub LoadGrid()
        ...
        & "(CASE WHEN USR.LOGIN IS NULL THEN CAST(MEM.USR AS VARCHAR) ELSE USR.LOGIN END) AS USR, " _
        & "MEM.cod " _
        & "FROM Mem INNER JOIN " _
        & "CLX ON MEM.EMP=CLX.EMP AND MEM.CLI=CLX.CLI LEFT OUTER JOIN " _
        & "EMPUSR ON MEM.EMP=EMPUSR.EMP AND MEM.USR=EMPUSR.CLI INNER JOIN " _
        & "USR ON EmpUsr.UsrGuid = Usr.Guid " _
        & "WHERE MEM.EMP=@Emp AND " _
        & "YEAR(MEM.FCH)=@Yea " _
        & "ORDER BY MEM.ID DESC"

        Dim oDs As DataSet = MaxiSrvr.GetDataset(SQL, MaxiSrvr.Databases.Maxi, "@Emp", mEmp.Id, "@Yea", iYea)
