...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        Dim resources As System.Resources.ResourceManager = New System.Resources.ResourceManager(GetType(Rpt_Docs))
...
#End Region
    ...
    Private mEnsobrado As MaxiSrvr.DocRpt.Ensobrados
        ...
        If mTot <> 0 Then
            ...
            If CurrentDoc.Descomptes Then
                ...
                If CurrentDoc.Dto <> 0 Then
                    DblTmp = Math.Round(mTot * CurrentDoc.Dto / 100, Decimals)
                ...
                If CurrentDoc.Dpp <> 0 Then
                    ...
                    DblTmp = Math.Round(mTot * CurrentDoc.Dpp / 100, Decimals)
    ...
    Private Sub DrawObs(ByVal oArrayList As ArrayList)
        ...
        For Each itm In oArrayList
            ...
            tmpStr = CutString(itm, hTabHdrAdr - X, FontStd, mEv)
            mEv.Graphics.DrawString(tmpStr, FontStd, Brushes.Black, X, Y)
            Y = Y + FontStd.Height
        Next
    End Sub
            ...
            Case SumConcepts.Total
                Select Case CurrentDoc.Incoterm
                    Case Doc.incoterms.CIF
                        tmpStr = "CIF " & CurrentDoc.Cur.Id
                    Case Else
                        tmpStr = CurrentDoc.Lang.Tradueix("Total", "Total", "Total") & " " & CurrentDoc.Cur.Id
                End Select
    ...
    Private Function DrawIVA(ByVal DcIva As Decimal, ByVal oBase As maxisrvr.Amt, ByVal BlPrintBase As Boolean) As Decimal
        Dim tmpStr As String
        ...
        If BlPrintBase Then
            ...
            tmpStr = tmpStr & oBase.CurFormat
        End If
        Dim oAmt As maxisrvr.Amt = oBase.Percent(DcIva)
        Dim DblAmt As Decimal = oAmt.Val
        DrawEndLine(tmpStr, DblAmt, False)
        Return DblAmt
    End Function

    Private Function DrawREQ(ByVal DcREQ As Decimal, ByVal oBase As maxisrvr.Amt, ByVal BlPrintBase As Boolean) As Decimal
        ...
        If BlPrintBase Then
            ...
            tmpStr = tmpStr & oBase.CurFormat
        End If
        Dim oAmt As maxisrvr.Amt = oBase.Percent(DcREQ)
        Dim DblAmt As Decimal = oAmt.Val
        DrawEndLine(tmpStr, DblAmt, False)
        Return DblAmt
    End Function
    ...
    Private Sub DrawTemplate()
        ...
        Select Case mDocRpt.Estilo
            Case maxisrvr.DocRpt.Estilos.Comanda
                sConcept = oLang.Tradueix("pedido", "comanda", "order")
            Case maxisrvr.DocRpt.Estilos.Albara
                sConcept = oLang.Tradueix("albarán", "albará", "packing list")
            Case maxisrvr.DocRpt.Estilos.Factura
                sConcept = oLang.Tradueix("factura", "factura", "invoice")
            Case maxisrvr.DocRpt.Estilos.Proforma
    ...
    Public Sub print(ByVal oKind As DocRpt.FuentePapel, ByVal eEnsobrado As DocRpt.Ensobrados, Optional ByVal CopiasExtra As Integer = 0)
        ...
        Select Case oKind
            Case maxisrvr.DocRpt.FuentePapel.Original
