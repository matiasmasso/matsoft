...
Public Class Frm_ArtXStp_Repeticions
    ...
    Private Function CurrentContact() As Contact
        ...
        Dim oRow As DataGridViewRow = DataGridViewHdr.CurrentRow
        If oRow IsNot Nothing Then
            Dim iContact As Integer = oRow.Cells(ColsDtl.cli).Value
            oContact = MaxiSrvr.Contact.FromNum(mEmp, iContact)
        End If
        Return oContact
    End Function

    Private Sub LoadGridDtl()
        Dim SQL As String = "SELECT pdc.cli, COUNT(DISTINCT Pdc.pdc) AS PDCS, SUM(PNC.qty) AS UNITS, SUM(PNC.QTY*PNC.EUR*(100-PNC.DTO)/100) AS EUR, CLX.clx " _
        & "FROM ART INNER JOIN " _
        & "STP ON ART.Category = Stp.Guid INNER JOIN " _
        & "PNC ON ART.Guid = PNC.ArtGuid INNER JOIN " _
        ...
        SQL = SQL & " AND PDC.cod = 2 " _
        & "GROUP BY PDC.CLI, CLX.CLX " _
        & "HAVING (COUNT(DISTINCT Pdc.pdc)=" & CurrentRepeticions() & ") " _
    ...
    Private Sub SetContextMenuDtl()
        ...
        If oRow IsNot Nothing Then
            Dim oContact As Contact = MaxiSrvr.Contact.FromNum(mEmp, CInt(oRow.Cells(ColsDtl.cli).Value))
