...
Public Class Frm_Trp_Tarifa
    ...
    Public Sub New(ByVal oZon As TrpZon)
        ...
        If sTrpNom = "" Then sTrpNom = mTrpZon.Transportista.Nom_o_NomComercial
        ...
        PictureBoxLogo.Image = mTrpZon.Transportista.Img48
        LoadZonas()
        LoadCostos()
    ...
    Private Sub LoadZonas()
        Dim SQL As String = "SELECT Zona.Guid, " _
        & "Country.Iso+' '+Zona.Nom AS NOM " _
        & "FROM TRPZON INNER JOIN " _
        & "Zona ON TRPZON.Zona = Zona.Guid " _
        & "INNER JOIN Country ON Zona.Country=Country.Guid " _
        & "WHERE TRPZON.TrpGuid ='" & mTrpZon.Transportista.Guid.ToString & "' " _
        & "AND TRPZON.TrpZon =" & mTrpZon.Id & " " _
        ...
        SQL = "SELECT Zona.Guid, " _
        & "Country.Iso+' '+Zona.Nom AS NOM " _
        & "FROM Zona " _
        & "INNER JOIN Country ON Zona.Country=Country.Guid " _
        & "WHERE Zona.Guid NOT IN (SELECT TrpZon.Zona FROM TrpZon WHERE TRPZON.TrpGuid ='" & mTrpZon.Transportista.Guid.ToString & "') " _
        & "ORDER BY Country.Iso+' '+Zona.Nom"
    ...
    Private Function GetZonasFromListBox() As List(Of Zona)
        Dim retval As New List(Of Zona)

        Dim oTb As DataTable = mDsZonSi.Tables(0)
        For Each oRow As DataRow In oTb.Rows
            Dim oGuid As Guid = oRow(0)
            Dim oZona As New Zona(oGuid)
            retval.Add(oZona)
        Next
        Return retval
    End Function
    ...
    Private Sub ButtonCalc_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonCalc.Click
        ...
        Dim oCost As maxisrvr.Amt = mTrpZon.Cost(SngM3, SngKg)
        If oCost Is Nothing Then
            MsgBox("volum/pes no tarifat", MsgBoxStyle.Exclamation, "MAT.NET")
        Else
            LabelCalcCost.Text = "cost: " & oCost.CurFormat
        End If
