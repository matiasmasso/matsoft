...
Public Class Frm_Client_Historial
    ...
    Private Sub LoadGrid()
        ...
        If mGrouped Then
            SQLSELECT = "SELECT PDC.Guid, PDC.pdc, 0 AS LIN, PDC.fch, SUM(PNC.qty) AS QTY, TPA.DSC + ' / ' + STP.DSC AS DSC, SUM(PNC.Pn2) AS PN2, 0 AS dto, 0 AS VAL " _
        ...
        Else
            SQLSELECT = "SELECT PDC.Guid, PDC.pdc, PNC.LIN, PDC.fch, PNC.qty, ART.myD, PNC.Pn2, PNC.dto, (CASE WHEN CARREC=1 THEN PNC.eur ELSE 0 END) AS VAL " _
            & "FROM PDC INNER JOIN " _
            & "PNC ON PDC.Guid = PNC.PdcGuid INNER JOIN " _
    ...
    Private Function CurrentLin() As LineItmPnc
        ...
        If oRow IsNot Nothing Then
            ...
            Dim IntLin As Long = oRow.Cells(Cols.Lin).Value
            Dim oPdc As New Pdc(oGuid)
            oPdc.Client = mClient
            oLin = New LineItmPnc(oPdc, IntLin)
            oLin.SetItm()
        End If
        Return oLin
    End Function
    ...
    Private Sub LoadStps()
        ...
        If oBrand Is Nothing Then
            ComboBoxStp.Visible = False
        Else
            Dim oCategories As List(Of DTOProductCategory) = BLL.BLLProductCategories.FromCustomerOrders(oCustomer, oBrand)
