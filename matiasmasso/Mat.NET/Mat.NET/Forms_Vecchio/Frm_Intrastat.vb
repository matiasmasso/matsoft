...
Public Class Frm_Intrastat
    ...
    Private Sub LoadGrid()
        ...
        If _Intrastat.IsNew Then
            SQL = "SELECT IMPORTDTL.HeaderGuid, 0 AS GRP, 0 AS CHK, IMPORTDTL.Yea, IMPORTDTL.Id, CCA.fch, SUM(CCB.eur) AS AMT, IMPORTHDR.PrvGuid, CCA.txt, IMPORTDTL.GUID, ImportDtl.Intrastat " _
            ...
            & "CCB ON Ccb.CcaGuid = Cca.Guid AND CCB.CTA LIKE '6%' LEFT OUTER JOIN " _
            & "Country P ON IMPORTHDR.PAISORIGEN LIKE P.ISO " _
            & "WHERE IMPORTDTL.INTRASTAT IS NULL AND P.CEE=1 AND IMPORTDTL.SRCCOD=" & ImportacioItem.SourceCodes.Fra & " " _
            ...
            oDs = MaxiSrvr.GetDataset(SQL, MaxiSrvr.Databases.Maxi, "@EMP", App.Current.Emp.Id)
            oTb = oDs.Tables(0)
        Else
            SQL = "SELECT IMPORTDTL.HeaderGuid, 0 AS GRP, 2 AS CHK, IMPORTDTL.Yea, IMPORTDTL.Id, CCA.fch, SUM(CCB.eur) AS AMT, '" & System.Guid.Empty.ToString & "' AS PrvGuid, CCA.txt, IMPORTDTL.GUID " _
            & "FROM IMPORTDTL INNER JOIN " _
            & "CCA ON IMPORTDTL.Guid = CCA.Guid INNER JOIN " _
            & "CCB ON Ccb.CcaGuid = Cca.Guid AND CCB.CTA LIKE '6%' " _
            & "WHERE IMPORTDTL.INTRASTAT = @GUID " _
            & "GROUP BY IMPORTDTL.HeaderGuid, IMPORTDTL.Yea, IMPORTDTL.Id, CCA.txt, CCA.fch, IMPORTDTL.GUID " _
            & "ORDER BY IMPORTDTL.Yea DESC, IMPORTDTL.Id DESC, CCA.txt"
            oDs = MaxiSrvr.GetDataset(SQL, MaxiSrvr.Databases.Maxi, "@EMP", App.Current.Emp.Id, "@GUID", _Intrastat.Guid.ToString)
            oTb = oDs.Tables(0)
            ...
            With .ImportacioItems
                For i As Integer = 0 To DataGridView1.Rows.Count - 1
                    ...
                    If BlChecked And BlLineItm Then
                        ...
                        Dim oItem As New ImportacioItem(oHeader, MaxiSrvr.ImportacioItem.SourceCodes.Fra, oGuid)
                        .Add(oItem)
                    End If
                Next
            End With
    ...
    Private Sub DataGridView1_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles DataGridView1.DoubleClick
        Dim oImportacio As Importacio = CurrentImportacio()
