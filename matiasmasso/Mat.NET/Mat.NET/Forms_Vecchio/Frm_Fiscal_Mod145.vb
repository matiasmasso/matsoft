...
Public Class Frm_Fiscal_Mod145
    ...
    Public Sub New(Optional oStaff As Staff = Nothing)
        MyBase.New()
    ...
    Private Sub LoadGrid()
        Dim SQL As String = "SELECT CLIDOCS.guid, (CASE WHEN BAJA IS NULL THEN 0 ELSE 1 END) AS BAJA, CliStaff.cli, CliStaff.Abr, CLIDOCS.fch " _
        & "FROM            CliStaff LEFT OUTER JOIN " _
        & "CLIDOCS ON CliStaff.emp = CLIDOCS.emp AND CliStaff.cli = CLIDOCS.cli AND CLIDOCS.src = 8 "

        If mStaff IsNot Nothing Then
            SQL = SQL & " WHERE CliDocs.Cli=" & mStaff.Id.ToString & " AND CliDocs.Emp=" & mStaff.Emp.Id & " "
        End If

        SQL = SQL & "GROUP BY CLIDOCS.guid, CliStaff.cli, CliStaff.Abr, CliStaff.Baja, CLIDOCS.fch " _
    ...
    Private Function CurrentItm() As BigFileSrc
        Dim oItm As BigFileSrc = Nothing
        Dim oRow As DataGridViewRow = DataGridView1.CurrentRow
        If oRow IsNot Nothing Then
            Dim oGuid As Guid = CType(oRow.Cells(Cols.Guid).Value, Guid)
            oItm = New BigFileSrc(DTODocFile.Cods.CliDoc, oGuid)
        End If
        Return oItm
    End Function
    ...
    Private Sub SetContextMenu()
        ...
        If oRow IsNot Nothing Then
            ...
            End If
            Dim iStaff As Integer = CInt(oRow.Cells(Cols.Cli).Value)
            Dim oContact As Contact = MaxiSrvr.Contact.FromNum(BLL.BLLApp.Emp, iStaff)
    ...
    Private Sub Do_AddNew()
        Dim oCliDoc As New CliDoc(mStaff)
        oCliDoc.Type = CliDoc.Types.Model_145
