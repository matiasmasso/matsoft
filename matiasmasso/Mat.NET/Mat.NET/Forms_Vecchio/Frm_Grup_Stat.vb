...
Public Class Frm_Grup_Stat
    ...
    Private Sub LoadGridClis()
        ...
        Select Case RadioButtonArc.Checked
            Case True
                SQL = "SELECT  CliClient.Guid, CliClient.ref, 0,'',SUM(ARC.qty * ARC.eur*(100-ARC.DTO)/100) AS TOT "
                ...
                & "WHERE CliClient.Emp =" & mClient.Emp.Id & " AND " _
                & "Alb.Yea =" & CurrentYea() & " AND " _
                & "(CliClient.Cli =" & mClient.Id & " OR CliClient.ccx =" & mClient.Id & ") " _
                & "GROUP BY CliClient.Guid, CliClient.ref " _
                & "ORDER BY CliClient.ref"
            Case False
                SQL = "SELECT  CliClient.Guid, CliClient.ref, 0,'',SUM(PNC.qty * PNC.eur*(100-PNC.DTO)/100) AS TOT "
        ...
        mDs(Tabs.GridClis) = maxisrvr.GetDataset(SQL, maxisrvr.Databases.Maxi)
    ...
    Private Sub LoadGridArts()
        ...
        Select Case RadioButtonArc.Checked
            Case True
                SQL = "SELECT  STP.Guid, TPA.DSC,  STP.stp, STP.dsc AS STPDSC, SUM(ARC.qty) AS TOT "
                ...
                & "GROUP BY TPA.ORD, Stp.Guid, TPA.DSC, STP.ord, STP.tpa, STP.dsc, TPA.DSC, STP.stp " _
                & "ORDER BY TPA.ORD, STP.ord, STPDSC"
            Case False
                SQL = "SELECT  STP.Guid, TPA.DSC,  STP.stp,STP.dsc AS STPDSC, SUM(PNC.qty) AS TOT "
        ...
        mDs(Tabs.GridArts) = maxisrvr.GetDataset(SQL, maxisrvr.Databases.Maxi)
    ...
    Private Sub LoadYeas()
        ...
        Dim oDs As DataSet = MaxiSrvr.GetDataset(SQL, MaxiSrvr.Databases.Maxi, "@Guid", mClient.Guid.ToString)
    ...
    Private Sub ToolStripButton1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripButton1.Click
        MatExcel.GetExcelFromDataset(mDs(TabControl1.SelectedIndex)).Visible = True
    End Sub
