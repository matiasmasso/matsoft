...
Public Class Frm_Cobrament_Reembolsos
    ...
    Private Sub SaveMail()
        mMail = New Mail(mEmp, DateTimePicker1.Value)
        With mMail
            .Cod = DTO.DTOCorrespondencia.Cods.Rebut
            .Contacts.Add(mTransportista)
            .Subject = "RELACIÓ DE REEMBOLSOS " & Xl_AlbsReembols1.Amt.CurFormat
    ...
    Private Sub SaveCca()
        Dim oCtaHab As PgcCta = mPlan.Cta(DTOPgcPlan.ctas.DeutorsVaris)
        Dim oCtaDeb As PgcCta = Xl_Cobrament1.Cta
        Dim oAmt As maxisrvr.Amt = Xl_AlbsReembols1.Amt
        ...
        With oCca
            ...
            .Txt = "n/ref." & mMail.Id & " " & Xl_Cobrament1.Concepte

            .ccbs.Add(New Ccb(oCca, oCtaDeb, Xl_Cobrament1.SubCta, oAmt, DTOCcb.DhEnum.Debe))
            .ccbs.Add(New Ccb(oCca, oCtaHab, mTransportista, oAmt, DTOCcb.DhEnum.Haber))

            Dim exs as New List(Of exception)
            If .Update( exs) Then
                With TextBoxEnd
                    .Text = .Text & " assentament " & oCca.Id
                End With
            Else
    ...
    Private Sub SaveCcaItm(ByVal oalb As Alb)
        ...
        Dim oAmt As maxisrvr.Amt = oalb.Reembolso
        ...
        Dim SQL As String = "UPDATE ALB SET COBRO='" & Format(oCca.fch, "yyyyMMdd") & "' WHERE " _
        & "EMP=" & mEmp.Id & " AND " _
        & "YEA=" & oalb.Yea & " AND " _
        & "ALB=" & oalb.Id
    ...
    Private Sub SaveXec()
        Dim oCta As PgcCta = mPlan.Cta(DTOPgcPlan.ctas.DeutorsVaris)
        Dim oAmt As maxisrvr.Amt = Xl_AlbsReembols1.Amt

        Dim oPnd As New Pnd()
        With oPnd
            .Contact = mTransportista
            .Amt = oAmt
            .Cod = Pnd.Codis.Deutor
            .Cta = oCta
            .Fch = Today
            .Vto = Xl_Cobrament1.XecVto
            .Cfp = DTOCustomer.FormasDePagament.ReposicioFons
        ...
        Dim oXec As New Xec(mTransportista, Xl_Cobrament1.XecIBAN, Xl_Cobrament1.XecNum, oPnds)
        If Xl_Cobrament1.CodiFpg = Xl_Cobrament.Fpgs.Pagare Then
            oXec.Vto = Xl_Cobrament1.XecVto
        End If

        Dim exs as New List(Of exception)
        If XecLoader.UpdateXec(oXec, exs) Then
