...
Public Class Frm_Rep_Transfers
    ...
    Public WriteOnly Property NewTransferCod() As NewTransferCods
        Set(ByVal value As NewTransferCods)
            ...
            Dim sConcepte As String = currentbanc.nom & "-Transferencies comisions a representants"
            mBancTransfer = New BancTransfer(App.Current.Emp.DefaultBanc, DtFch, sConcepte)
            ...
            ElseIf mNewTransferCod = NewTransferCods.Staff Then
                ...
                Me.Text = "TRANSFERENCIES AL PERSONAL"
            End If
        ...
        Else
            Select Case mNewTransferCod
                Case NewTransferCods.Reps
                    Dim oCta As PgcCta = mPlan.Cta(DTOPgcPlan.ctas.proveidorsEur)
                    SQL = "SELECT CCB.ContactGuid, " _
                     ...
                     & "CCB.PGCPLAN, " _
                     & "'" & oCta.Id & "' AS CTAID, " _
                     & "'" & oCta.FullNom & "' AS CTANOM, " _
                     & "'' as WARNCCC, " _
                     & "'' AS CCC, " _
                     & "'' AS CONCEPTE " _
                     & "FROM  CCB INNER JOIN " _
                     & "CliRep ON CCB.ContactGuid = (CASE WHEN CLIREP.CcxGuid IS NULL THEN CliRep.Guid ELSE CliRep.CcxGuid END) " _
                     & "WHERE CCB.EMP = 1 AND CCB.YEA = " & Today.Year & " AND CCB.CTA LIKE '" & oCta.Id & "' " _
                     & "GROUP BY CCB.ContactGuid, CLIREP.abr, CCB.PGCPLAN " _
                     & "ORDER BY CLIREP.abr"
                Case NewTransferCods.Staff
                    Dim oCta As PgcCta = mPlan.Cta(DTOPgcPlan.ctas.PagasTreballadors)
                    SQL = "SELECT CCB.ContactGuid, " _
                    & "SUM(CASE WHEN dh = 2 THEN eur ELSE - eur END) AS SDO, " _
                    & "'' as WARNSDO, " _
                    & "CLISTAFF.abr, " _
    ...
    Private Function CheckCuadra(ByVal oRow As DataRow) As Boolean
        ...
        Select Case mNewTransferCod
            Case NewTransferCods.Reps
                ...
                Dim oCta430 As PgcCta = mPlan.Cta(DTOPgcPlan.ctas.clients)
                Dim oRep As New Rep(CType(oRow(Cols.Contact), Guid))
                retval = (oRep.LastLiqEur = oRow(Cols.Eur))
                oCcd = New Ccd(oRep, iYea, oCta430)
                If oCcd.Saldo(Today).Eur <> 0 Then CheckCuadra = False
            Case NewTransferCods.Staff
                Dim oStaff As New Staff(CType(oRow(Cols.Contact), Guid))
                Dim oLiqStored As MaxiSrvr.Amt = oStaff.Liquid
    ...
    Private Sub LoadBancs()
        ...
        Dim oDrd As SqlClient.SqlDataReader = MaxiSrvr.GetDataReader(SQL, MaxiSrvr.Databases.Maxi, "@Emp", App.Current.Emp.Id)
        Dim i As Integer = -1
        ...
        With mBancTransfer
            ...
            For Each oRow In oTb.Rows
                If oRow(Cols.Eur) > 0 Then
                    ...
                    oCta = MaxiSrvr.PgcCta.FromNum(mPlan, oRow(Cols.CtaId))
                    oIban = BLL.BLLIban.FromDigits(oRow(Cols.Ccc).ToString)
                    oAmt = New MaxiSrvr.Amt(oRow(Cols.Eur), MaxiSrvr.Cur.Eur, oRow(Cols.Eur))
        ...
        If mDirtyCell Then
            ...
            Select Case e.ColumnIndex
                Case Cols.CtaNom
                    ...
                    Dim oCta As PgcCta = Finder.FindCta(mPlan, sKey)
                    If oCta IsNot Nothing Then
                        oRow.Cells(Cols.CtaId).Value = oCta.Id
                        oRow.Cells(Cols.CtaNom).Value = oCta.FullNom
                    End If
