...
Public Class Frm_NominasFactory
    ...
    Private _Emp = New DTOEmp(DTO.DTOEmp.Ids.MatiasMasso)
    Private _user = DTO.DTOUser.Wellknown(DTOUser.Wellknowns.matias)
    ...
    Private Async Sub Frm_NominasFactory_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If Not String.IsNullOrEmpty(_Filename) Then
            _user = Await FEB.User.Find(DTO.DTOUser.Wellknown(DTOUser.Wellknowns.matias).Guid, exs)
            _Emp = Await FEB.Emp.Find(DTO.DTOEmp.Ids.MatiasMasso, exs)
            If exs.Count > 0 Then
    ...
    Private Async Function refresca() As Task
        ...
        Dim nif As String = ""
        If NominaEscuraHelper.CheckFile(exs, _Filename, _user, fch) Then
            LabelInfo.Text = String.Format("nomines {0} del {1:dd/MM/yy}", _Emp.Org.nom, fch)
            Dim oCert = Await FEB.Cert.Find(_Emp.Org, exs)
            Dim oExercici As New DTOExercici(_Emp, DateTime.Now.Year)
            Dim oStaffs = Await FEB.Staffs.All(exs, oExercici)
            Dim oCtas = Await FEB.PgcCtas.All(exs, oExercici.Year)

            _Nominas = Await NominaEscuraHelper.nominas(exs, _Filename, _user, oCert, oStaffs, oCtas, AddressOf ShowProgress)
            ...
            End If
        Else
            exs.Add(New Exception("les nomines no corresponen a aquesta empresa"))
            UIHelper.ToggleProggressBar(Panel1, False)
            UIHelper.WarnError(exs)
            Me.Close()
        End If
