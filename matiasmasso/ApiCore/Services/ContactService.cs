    ...
    public class ContactService
    {
        public static ContactModel? GetValue(Guid guid)
        {
            using (var db = new Entities.MaxiContext())
            {
                var retval = db.CliGrals
                    ...
                    .Include(x => x.EmailClis)
                    .ThenInclude(x => x.Email)
                    .Include(x => x.CliTels)
                    .ThenInclude(x=>x.CountryNavigation)
                    .AsNoTracking()
                    .Where(x => x.Guid == guid)
                    .Select(x => new ContactModel(guid)
                    {
                        ...
                        Telefons = x.CliTels.Select(y => new ContactModel.Telefon(y.Guid)
                        {
                            Country = y.CountryNavigation == null ? null : new CountryModel(y.CountryNavigation.Guid)
                            {
                                ISO = y.CountryNavigation.Iso,
                                PrefixeTelefonic = y.CountryNavigation.PrefixeTelefonic,
                            },
                            Number = y.Num,
                            Obs = y.Obs
                        }).ToList(),
                        Emails = x.EmailClis.Select(y => new ContactModel.Email(y.EmailGuid)
                        {
                            EmailAddress = y.Email.Adr,
                            Rol = (UserModel.Rols) y.Email.Rol,
                            Obs = y.Email.Obs,
                            Obsoleto = y.Email.Obsoleto
                        }).ToList(),
                        Obsoleto = x.Obsoleto
                    })
                 .FirstOrDefault();
        ...
        public static bool Update(ContactModel value)
        {
            using (var db = new Entities.MaxiContext())
            {
                ...
                {
                    var telEntity = db.CliTels.FirstOrDefault(x => x.Guid == tel.Guid);
                    if (telEntity == null) telEntity = new Entities.CliTel();
                    telEntity.Num = tel.Number ?? "";
                    telEntity.Country = tel.Country?.Guid ?? CountryModel.Default().Guid;
                    telEntity.Obs = tel.Obs;
                    entity.CliTels.Add(telEntity!);
                }
                foreach (var email in value.Emails)
                {
                    var emailEntity = db.EmailClis.FirstOrDefault(x => x.EmailGuid == email.Guid);
                    if(emailEntity == null)
                    {
                        emailEntity = new Entities.EmailCli();
                        emailEntity.EmailGuid = email.Guid;
                        emailEntity.ContactGuid = value.Guid;
                        emailEntity.Ord = (short)(entity.EmailClis.Count + 1);
                        db.EmailClis.Add(emailEntity);
                        emailEntity.Email = new Entities.Email();
                        emailEntity.Email.Guid = email.Guid;
                        db.Emails.Add(emailEntity.Email);
                    }
                    if ( emailEntity.Email != null)
                    {
                        emailEntity.Email.Adr = email.EmailAddress ?? "";
                        emailEntity.Email.Rol = (short)(int)(email.Rol ?? UserModel.Rols.unregistered);
                        emailEntity.Email.Obs = email.Obs;
                        emailEntity.Email.Obsoleto = email.Obsoleto;
                    }
                }
        ...
        public static bool Delete(Guid guid)
        {
            using (var db = new Entities.MaxiContext())
            {
                ...
                db.Clls.RemoveRange(db.Clls.Where(x => x.ContactGuid == guid));
