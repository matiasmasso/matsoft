...
namespace Api.Services
{
    public class StoreLocatorService
    {
        ...
        public static StoreLocatorDTO Factory(ProductModel product, LangDTO lang)
        {
            using (var db = new Entities.MaxiContext())
            {
                return Factory(db,product,lang);
            }
        }
                ...
                switch (product.Src)
                {
                    ...
                    default:
                        ...
                        if(product is ProductSkuModel) 
                            categoryGuid = ((ProductSkuModel)product).Category;
                        else
                            categoryGuid = db.Arts.FirstOrDefault(x => x.Guid == product.Guid)?.Category;
                        //sku: get category distributors
                        retval = query!.
                            Where(x => x.Category.Equals(categoryGuid)).
                            GroupBy(y => new { y.Country, y.CountryEsp, y.CountryCat, y.CountryEng, y.CountryPor, y.AreaNom, y.Cit, y.Client, y.Nom, y.Adr }).
                            Select(z => new StoreLocatorDTO.FlatDistributor
                            {
                                CountryGuid = (Guid)z.Key.Country!,
