...
namespace Api.Services
{
    ...
    {
        public static DocfileModel? FindBySha256(string sha256)
        {
        ...
        public static DocfileModel? GetValue(string asin)
        {
            using (var db = new Entities.MaxiContext())
            {
                var retval = db.DocFiles
                              .Where(x=> x.Hash == asin)
                              .Select(x=> new DocfileModel(asin)
                              {
                                  Sha256 = x.Sha256,
                                  Nom = x.Nom,
                                  Fch = x.Fch,
        ...
        //for backwards compatibilty using deprecated MD5 hash
        //new method using Asin available at DocService
        public static DocfileModel? Media(string base64Hash)
        {
            var md5 = DTO.Helpers.CryptoHelper.FromUrFriendlyBase64(base64Hash);
            using (var db = new Entities.MaxiContext())
            {
        ...
        public static DocfileModel? Fetch(string asin)
        {
            using (var db = new Entities.MaxiContext())
            {
                var retval = (from x in db.DocFiles
                              where x.Hash == asin
                              select new DocfileModel(asin)
                              {
                                  Sha256 = x.Sha256,
                                  Nom = x.Nom,
                                  Fch = x.Fch,
        ...
        {
            bool retval = false;
            if (value.Sha256 != null) //if (value.Hash != null)
            {
        ...
        public static bool Delete(String asin)
        {
            using (var db = new Entities.MaxiContext())
            {
                var entity = db.DocFiles.Remove(db.DocFiles.Single(x => x.Hash.Equals(asin)));
                db.SaveChanges();
            }
            return true;
