...
namespace Api.Services.Integracions.Vivace
{
    public class Tracking
    {

        public async static Task<DTO.Integracions.Vivace.Trace> Trace(DeliveryModel? delivery)
        {
            var retval = new DTO.Integracions.Vivace.Trace();
            if (delivery != null)
            {
                if (delivery.Id == 0) delivery = DeliveryService.Fetch(delivery.Guid);
                ...
                if (string.IsNullOrEmpty(span))
                    throw new System.Exception("missing lbValHistorico id span on Vivace tracking page");
                else
                {
