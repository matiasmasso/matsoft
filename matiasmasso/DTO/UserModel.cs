...
namespace DTO
{
    public class UserModel : BaseGuid, IModel
    {
        ...
        {
            var retval = EmailAddress ?? Nickname ?? "{UserModel}";
            return retval;
        }
        ...
        public static short[] NonProfessionals = new short[] { (short)Rols.denied, (short)Rols.guest, (short)Rols.lead, (short)Rols.unregistered };

        public bool isProfesional()=>IsProfessional(Rol ?? Rols.notSet);

        public static bool IsProfessional(Rols rol = Rols.notSet)
        {
            bool retval = false;
            switch (rol)
            {
