...
namespace DTO
{
    ...
    public class CcaModel : BaseGuid, IModel
    {
        ...
        public string? DownloadUrl() => Docfile?.Hash == null ? null : Globals.ApiUrl("cca/doc", $"{Guid.ToString().Replace("-", "").ToUpper()}.pdf");
        ...
        public string? FraNum()
        {
            string? retval = null;
            Match match = Regex.Match(Concept ?? "", @"(?<=Fra\.)\S+");
            if (match.Success) retval = match.Value;
            return retval;
        }
        ...
        public bool Matches(decimal? searchterm)
        {
            ...
            if (searchterm == null)
                ...
                retval = Items.Any(x => x.Eur == searchterm);
            return retval;
        }
